inherited FormConsolidacionAcopioRemitos: TFormConsolidacionAcopioRemitos
  Left = 135
  Top = 113
  Caption = 'Consolidaci'#243'n de Acopios - Entregas'
  ClientHeight = 699
  ClientWidth = 974
  StyleElements = [seFont, seClient, seBorder]
  OnResize = FormResize
  ExplicitWidth = 992
  ExplicitHeight = 740
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 974
    Height = 646
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 974
    ExplicitHeight = 646
    object pcDatos: TPageControl
      Left = 0
      Top = 68
      Width = 974
      Height = 578
      ActivePage = pag1
      Align = alClient
      TabOrder = 0
      object pag1: TTabSheet
        Caption = 'Consolidado'
        DesignSize = (
          966
          550)
        object Label2: TLabel
          Left = 356
          Top = 530
          Width = 99
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Compras Anticipadas'
          ExplicitTop = 508
        end
        object Label3: TLabel
          Left = 578
          Top = 530
          Width = 85
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Entregas a Cobrar'
          ExplicitTop = 508
        end
        object Label4: TLabel
          Left = 3
          Top = 530
          Width = 65
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Devoluciones'
          ExplicitTop = 508
        end
        object Label6: TLabel
          Left = 910
          Top = 511
          Width = 32
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Parcial'
          ExplicitTop = 489
        end
        object edTotal: TJvCalcEdit
          Left = 669
          Top = 526
          Width = 115
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 0
          DisabledTextColor = clBlue
          DecimalPlacesAlwaysShown = False
        end
        object edCompraAnticipada: TJvCalcEdit
          Left = 461
          Top = 526
          Width = 115
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 1
          DisabledTextColor = clRed
          DecimalPlacesAlwaysShown = False
        end
        object edDevoluciones: TJvCalcEdit
          Left = 75
          Top = 526
          Width = 115
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 2
          DisabledTextColor = clNavy
          DecimalPlacesAlwaysShown = False
        end
        object edParcial: TJvCalcEdit
          Left = 842
          Top = 526
          Width = 115
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 3
          DisabledTextColor = clBlue
          DecimalPlacesAlwaysShown = False
        end
        object Panel2: TPanel
          Left = 0
          Top = 0
          Width = 966
          Height = 504
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          Caption = 'Panel2'
          TabOrder = 4
          object cxGrid1: TcxGrid
            Left = 1
            Top = 1
            Width = 964
            Height = 502
            Align = alClient
            PopupMenu = PopupMenu1
            TabOrder = 0
            LookAndFeel.Kind = lfOffice11
            LookAndFeel.NativeStyle = False
            object cxGrid1DBTableView1: TcxGridDBTableView
              OnCustomDrawCell = cxGrid1DBTableView1CustomDrawCell
              DataController.DataModeController.GridMode = True
              DataController.DataSource = DSDatos
              DataController.Options = [dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding, dcoImmediatePost]
              DataController.Summary.DefaultGroupSummaryItems = <
                item
                  Kind = skSum
                  Position = spFooter
                  Column = cxGrid1DBTableView1TOTAL
                end>
              DataController.Summary.FooterSummaryItems = <
                item
                  Kind = skSum
                  FieldName = 'TOTAL'
                  Column = cxGrid1DBTableView1TOTAL
                end>
              OptionsData.Deleting = False
              OptionsData.DeletingConfirmation = False
              OptionsData.Inserting = False
              OptionsView.GroupByBox = False
              object cxGrid1DBTableView1CODIGO: TcxGridDBColumn
                DataBinding.FieldName = 'CODIGO'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 90
              end
              object cxGrid1DBTableView1DETALLE: TcxGridDBColumn
                DataBinding.FieldName = 'DETALLE'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 330
              end
              object cxGrid1DBTableView1ACOPIO: TcxGridDBColumn
                DataBinding.FieldName = 'ACOPIO'
                PropertiesClassName = 'TcxCalcEditProperties'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 81
              end
              object cxGrid1DBTableView1REMITO: TcxGridDBColumn
                DataBinding.FieldName = 'REMITO'
                PropertiesClassName = 'TcxCalcEditProperties'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 79
              end
              object cxGrid1DBTableView1FACTURAR: TcxGridDBColumn
                DataBinding.FieldName = 'FACTURAR'
                PropertiesClassName = 'TcxCalcEditProperties'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 80
              end
              object cxGrid1DBTableView1PRECIO: TcxGridDBColumn
                DataBinding.FieldName = 'PRECIO'
                PropertiesClassName = 'TcxCalcEditProperties'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Options.Sorting = False
                Width = 86
              end
              object cxGrid1DBTableView1DEVOLUCIONES: TcxGridDBColumn
                DataBinding.FieldName = 'DEVOLUCIONES'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Width = 82
              end
              object cxGrid1DBTableView1TOTAL: TcxGridDBColumn
                DataBinding.FieldName = 'TOTAL'
                PropertiesClassName = 'TcxCalcEditProperties'
                HeaderAlignmentHorz = taCenter
                Options.Editing = False
                Options.Filtering = False
                Options.Focusing = False
                Options.Moving = False
                Options.Sorting = False
                Width = 73
              end
              object cxGrid1DBTableView1SELECCCION: TcxGridDBColumn
                DataBinding.FieldName = 'SELECCCION'
                PropertiesClassName = 'TcxCheckBoxProperties'
                Properties.ClearKey = 46
                Properties.ImmediatePost = True
                HeaderAlignmentHorz = taCenter
                Options.Filtering = False
              end
            end
            object cxGrid1Level1: TcxGridLevel
              GridView = cxGrid1DBTableView1
            end
          end
        end
      end
      object pag2: TTabSheet
        Caption = 'Acopios'
        ImageIndex = 1
        DesignSize = (
          966
          550)
        object dbgAcopios: TJvDBGrid
          Left = 3
          Top = 0
          Width = 782
          Height = 527
          Anchors = [akLeft, akTop, akBottom]
          DataSource = DSAcopios
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDrawColumnCell = dbgAcopiosDrawColumnCell
          OnDblClick = dbgAcopiosDblClick
          ScrollBars = ssVertical
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Expanded = False
              FieldName = 'CODIGO'
              Title.Alignment = taCenter
              Width = 75
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DETALLE'
              Title.Alignment = taCenter
              Width = 319
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NROACOPIO'
              Title.Alignment = taCenter
              Width = 127
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHA'
              Title.Alignment = taCenter
              Width = 107
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'SALDO'
              Title.Alignment = taCenter
              Width = 112
              Visible = True
            end>
        end
      end
      object pag3: TTabSheet
        Caption = 'Remitos'
        ImageIndex = 2
        DesignSize = (
          966
          550)
        object Label5: TLabel
          Left = 10
          Top = 534
          Width = 589
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 
            'Para Consolidar las Devoluciones elegir el Item marcado como Dev' +
            '. y Hacer Doble CLick para generar la NC. Corresponiente'
          ExplicitTop = 512
        end
        object Panel3: TPanel
          Left = 0
          Top = 0
          Width = 966
          Height = 523
          Align = alTop
          Anchors = [akLeft, akTop, akBottom]
          Caption = 'Panel3'
          TabOrder = 0
          object dbgRemitos: TJvDBGrid
            Left = 1
            Top = 1
            Width = 964
            Height = 521
            Align = alClient
            DataSource = DSRtos
            Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -11
            TitleFont.Name = 'MS Sans Serif'
            TitleFont.Style = []
            OnDrawColumnCell = dbgRemitosDrawColumnCell
            OnDblClick = dbgRemitosDblClick
            ScrollBars = ssVertical
            SelectColumnsDialogStrings.Caption = 'Select columns'
            SelectColumnsDialogStrings.OK = '&OK'
            SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
            EditControls = <>
            RowsHeight = 17
            TitleRowHeight = 17
            Columns = <
              item
                Expanded = False
                FieldName = 'FECHAVTA'
                Title.Alignment = taCenter
                Width = 102
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'NROCPBTE'
                Title.Alignment = taCenter
                Width = 98
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CLASECPBTE'
                Title.Alignment = taCenter
                Width = 35
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CODIGOARTICULO'
                Title.Alignment = taCenter
                Width = 75
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'DETALLE'
                Title.Alignment = taCenter
                Width = 326
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CANTIDAD'
                Title.Alignment = taCenter
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'CANT_FACTURA'
                Title.Alignment = taCenter
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'DEVOLUCION'
                Width = 71
                Visible = True
              end>
          end
        end
      end
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 974
      Height = 68
      Align = alTop
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 1
      object Label1: TLabel
        Left = 14
        Top = 6
        Width = 32
        Height = 13
        Caption = 'Cliente'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object SpeedButton1: TSpeedButton
        Left = 352
        Top = 25
        Width = 25
        Height = 22
        Hint = 'Buscar Acopio'
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFF625E5C9C9997726E6E53514EFFFFFFFFFFFFFFFFFF}
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton1Click
      end
      object lbCondicionIva: TLabel
        Left = 452
        Top = 49
        Width = 6
        Height = 13
        Caption = '--'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clYellow
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object lbCuit: TLabel
        Left = 557
        Top = 49
        Width = 18
        Height = 13
        Caption = 'Cuit'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clYellow
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object lbDireccion: TLabel
        Left = 452
        Top = 30
        Width = 6
        Height = 13
        Caption = '--'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clYellow
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object lbDiscrimina: TLabel
        Left = 452
        Top = 12
        Width = 6
        Height = 13
        Caption = '--'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clYellow
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label7: TLabel
        Left = 383
        Top = 11
        Width = 48
        Height = 13
        Caption = 'Discrimina'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label8: TLabel
        Left = 383
        Top = 30
        Width = 45
        Height = 13
        Caption = 'Direccion'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label9: TLabel
        Left = 383
        Top = 49
        Width = 65
        Height = 13
        Caption = 'Condicion Iva'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object ceCliente: TJvComboEdit
        Left = 10
        Top = 25
        Width = 73
        Height = 21
        AlwaysEnableButton = True
        ClickKey = 16397
        DisabledTextColor = clBlack
        Glyph.Data = {
          4E000000424D4E000000000000003E000000280000000A000000040000000100
          010000000000100000000000000000000000020000000200000000000000FFFF
          FF00FFC000003300000033000000FFC00000}
        MaxLength = 6
        TabOrder = 0
        Text = ''
        OnButtonClick = ceClienteButtonClick
        OnKeyPress = ceClienteKeyPress
      end
      object edNombreCliente: TEdit
        Left = 89
        Top = 25
        Width = 257
        Height = 21
        Color = clInfoBk
        Enabled = False
        TabOrder = 1
      end
      object gbLista: TGroupBox
        Left = 673
        Top = 9
        Width = 288
        Height = 50
        Caption = 'Lista'
        Color = clInactiveCaptionText
        DefaultHeaderFont = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        HeaderFont.Charset = DEFAULT_CHARSET
        HeaderFont.Color = clWhite
        HeaderFont.Height = -11
        HeaderFont.Name = 'MS Sans Serif'
        HeaderFont.Style = [fsBold]
        ParentColor = False
        ParentFont = False
        TabOrder = 2
        object dbcLista: TDBLookupComboBox
          Left = 9
          Top = 17
          Width = 264
          Height = 21
          DropDownAlign = daRight
          DropDownWidth = 300
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          KeyField = 'ID'
          ListField = 'NOMBRE'
          ListSource = DSListaPrecios
          ParentFont = False
          TabOrder = 0
          OnClick = dbcListaClick
        end
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 646
    Width = 974
    Align = alBottom
    ExplicitTop = 646
    ExplicitWidth = 974
    inherited btConfirma: TBitBtn
      Visible = False
    end
    inherited btNuevo: TBitBtn
      Visible = False
    end
    inherited btCancelar: TBitBtn
      Visible = False
    end
    inherited btModificar: TBitBtn
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
    inherited btBorrar: TBitBtn
      Visible = False
    end
    object ToolButton1: TToolButton
      Left = 557
      Top = 0
      Width = 28
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object btFacturar: TBitBtn
      Left = 585
      Top = 0
      Width = 88
      Height = 29
      Caption = 'Fc.Cta.Cte'
      Glyph.Data = {
        
        33333333333333333333}
      NumGlyphs = 2
      TabOrder = 7
      OnClick = btFacturarClick
    end
    object btFcCtod: TBitBtn
      Left = 673
      Top = 0
      Width = 74
      Height = 29
      Caption = 'Fc.Ctdo'
      Glyph.Data = {
        
        AF432CAC451E9E330B8516FF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 9
      OnClick = btFcCtodClick
    end
    object btImprimir: TBitBtn
      Left = 747
      Top = 0
      Width = 75
      Height = 29
      Caption = '&Imprimir'
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
      TabOrder = 8
      OnClick = btImprimirClick
    end
  end
  inherited Panel1: TPanel
    Top = 676
    Width = 974
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 676
    ExplicitWidth = 974
    inherited sbEstado: TStatusBar
      Width = 885
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 885
    end
  end
  inherited ActionList1: TActionList
    Left = 832
    Top = 80
    object Facturar: TAction
      Caption = 'Facturar'
      OnExecute = FacturarExecute
    end
    object FacturarCtdo: TAction
      Caption = 'FacturarCtdo'
      OnExecute = FacturarCtdoExecute
    end
    object ConsolidaDev: TAction
      Caption = 'Consolida Devolucion'
      Enabled = False
      Visible = False
      OnExecute = ConsolidaDevExecute
    end
    object Remitar: TAction
      Caption = 'Remitar el Pendiente de entrega de lo  Anticipado'
      OnExecute = RemitarExecute
    end
    object ExportarXLS: TAction
      Caption = 'Exportar XLS'
      OnExecute = ExportarXLSExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 176
    Top = 96
  end
  inherited ImageList1: TImageList
    Left = 112
    Top = 200
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 65520
    Top = 624
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 280
    Top = 264
  end
  inherited ComBuscadorBase: TComBuscador
    Left = 304
    Top = 144
  end
  inherited QBrowse2: TFDQuery
    Left = 192
    Top = 560
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 112
    Top = 560
  end
  inherited ImageListDos: TImageList
    Left = 112
    Top = 160
  end
  object CDSAcopios: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPAcopios'
    Left = 576
    Top = 128
    object CDSAcopiosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSAcopiosID_CAB: TIntegerField
      FieldName = 'ID_CAB'
      Origin = 'ID_CAB'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSAcopiosCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = []
      Size = 8
    end
    object CDSAcopiosDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 45
    end
    object CDSAcopiosFECHA_ULTIMAENTREGA: TSQLTimeStampField
      DisplayLabel = 'Ult.Entrega'
      FieldName = 'FECHA_ULTIMAENTREGA'
      Origin = 'FECHA_ULTIMAENTREGA'
    end
    object CDSAcopiosCUMPLIDO: TStringField
      DisplayLabel = 'Cumplido'
      FieldName = 'CUMPLIDO'
      Origin = 'CUMPLIDO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSAcopiosFECHA: TSQLTimeStampField
      Alignment = taCenter
      DisplayLabel = 'Fecha Ac'
      FieldName = 'FECHA'
      Origin = 'FECHA'
      ProviderFlags = []
    end
    object CDSAcopiosNROACOPIO: TStringField
      Alignment = taCenter
      DisplayLabel = 'Nro.Acopio'
      FieldName = 'NROACOPIO'
      Origin = 'NROCPBTE'
      ProviderFlags = []
      EditMask = 'a-0000-00000000;0;_'
      Size = 13
    end
    object CDSAcopiosID_FACTURA: TIntegerField
      FieldName = 'ID_FACTURA'
      Origin = 'ID_FACTURA'
      ProviderFlags = []
    end
    object CDSAcopiosFECHAFC: TSQLTimeStampField
      DisplayLabel = 'Fecha Fc'
      FieldName = 'FECHAFC'
      Origin = 'FECHAVTA'
      ProviderFlags = []
    end
    object CDSAcopiosTIPOFC: TStringField
      FieldName = 'TIPOFC'
      Origin = 'TIPOCPBTE'
      ProviderFlags = []
      Size = 2
    end
    object CDSAcopiosNROFC: TStringField
      DisplayLabel = 'Nro.Fc'
      FieldName = 'NROFC'
      Origin = 'NROCPBTE'
      ProviderFlags = []
      Size = 13
    end
    object CDSAcopiosCANTIDAD: TFloatField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.000'
    end
    object CDSAcopiosENTREGADO: TFloatField
      DisplayLabel = 'Entregado'
      FieldName = 'ENTREGADO'
      Origin = 'ENTREGADO'
      DisplayFormat = ',0.000'
    end
    object CDSAcopiosSALDO: TFloatField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      Origin = 'SALDO'
      DisplayFormat = ',0.000'
    end
    object CDSAcopiosCODIGOCLIENTE: TStringField
      FieldName = 'CODIGOCLIENTE'
      ReadOnly = True
      Size = 6
    end
    object CDSAcopiosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      ReadOnly = True
      Size = 35
    end
    object CDSAcopiosRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      ReadOnly = True
      Size = 35
    end
    object CDSAcopiosDIRECCION_COMERCIAL: TStringField
      FieldName = 'DIRECCION_COMERCIAL'
      ReadOnly = True
      Size = 35
    end
  end
  object DSPAcopios: TDataSetProvider
    DataSet = QAcopios
    Left = 496
    Top = 128
  end
  object DSAcopios: TDataSource
    DataSet = CDSAcopios
    Left = 648
    Top = 120
  end
  object DSPRtos: TDataSetProvider
    DataSet = QRtos
    Left = 496
    Top = 184
  end
  object CDSRtos: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPRtos'
    Left = 584
    Top = 184
    object CDSRtosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSRtosID_CABRTO: TIntegerField
      FieldName = 'ID_CABRTO'
      Origin = 'ID_CABRTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object CDSRtosTIPOCPBTE: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      ProviderFlags = []
      Size = 2
    end
    object CDSRtosCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      ProviderFlags = []
      Size = 2
    end
    object CDSRtosNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      ProviderFlags = []
      Size = 13
    end
    object CDSRtosFECHAVTA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHAVTA'
      Origin = 'FECHAVTA'
      ProviderFlags = []
    end
    object CDSRtosCODIGOARTICULO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGOARTICULO'
      Origin = 'CODIGOARTICULO'
      ProviderFlags = []
      Size = 8
    end
    object CDSRtosDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 45
    end
    object CDSRtosUNIDAD: TStringField
      FieldName = 'UNIDAD'
      Origin = 'UNIDAD'
      ProviderFlags = []
      Size = 3
    end
    object CDSRtosCOSTO_GRAVADO: TFloatField
      FieldName = 'COSTO_GRAVADO'
      Origin = 'COSTO_GRAVADO'
      ProviderFlags = []
    end
    object CDSRtosCOSTO_EXENTO: TFloatField
      FieldName = 'COSTO_EXENTO'
      Origin = 'COSTO_EXENTO'
      ProviderFlags = []
    end
    object CDSRtosCOSTO_TOTAL: TFloatField
      FieldName = 'COSTO_TOTAL'
      Origin = 'COSTO_TOTAL'
      ProviderFlags = []
    end
    object CDSRtosUNITARIO_GRAVADO: TFloatField
      FieldName = 'UNITARIO_GRAVADO'
      Origin = 'UNITARIO_GRAVADO'
      ProviderFlags = []
    end
    object CDSRtosIVA_UNITARIO: TFloatField
      FieldName = 'IVA_UNITARIO'
      Origin = 'IVA_UNITARIO'
      ProviderFlags = []
      Required = True
    end
    object CDSRtosUNITARIO_EXENTO: TFloatField
      FieldName = 'UNITARIO_EXENTO'
      Origin = 'UNITARIO_EXENTO'
      ProviderFlags = []
    end
    object CDSRtosUNITARIO_TOTAL: TFloatField
      FieldName = 'UNITARIO_TOTAL'
      Origin = 'UNITARIO_TOTAL'
      ProviderFlags = []
    end
    object CDSRtosDESGLOZAIVA: TStringField
      FieldName = 'DESGLOZAIVA'
      Origin = 'DESGLOZAIVA'
      ProviderFlags = []
      Size = 1
    end
    object CDSRtosMODO_GRAVAMEN: TStringField
      FieldName = 'MODO_GRAVAMEN'
      Origin = 'MODO_GRAVAMEN'
      ProviderFlags = []
      Size = 1
    end
    object CDSRtosTIPOIVA_TASA: TIntegerField
      FieldName = 'TIPOIVA_TASA'
      Origin = 'TIPOIVA_TASA'
      ProviderFlags = []
    end
    object CDSRtosTIPOIVA_SOBRETASA: TIntegerField
      FieldName = 'TIPOIVA_SOBRETASA'
      Origin = 'TIPOIVA_SOBRETASA'
      ProviderFlags = []
    end
    object CDSRtosDEPOSITO: TIntegerField
      FieldName = 'DEPOSITO'
      Origin = 'DEPOSITO'
      ProviderFlags = []
    end
    object CDSRtosAFECTA_STOCK: TStringField
      FieldName = 'AFECTA_STOCK'
      Origin = 'AFECTA_STOCK'
      ProviderFlags = []
      Size = 1
    end
    object CDSRtosCALCULA_SOBRETASA: TStringField
      FieldName = 'CALCULA_SOBRETASA'
      Origin = 'CALCULA_SOBRETASA'
      ProviderFlags = []
      Size = 1
    end
    object CDSRtosGRAVADO_IB: TStringField
      FieldName = 'GRAVADO_IB'
      Origin = 'GRAVADO_IB'
      ProviderFlags = []
      FixedChar = True
      Size = 1
    end
    object CDSRtosID_MONEDA: TIntegerField
      FieldName = 'ID_MONEDA'
      Origin = 'ID_MONEDA'
      ProviderFlags = []
      Required = True
    end
    object CDSRtosLOTE: TIntegerField
      FieldName = 'LOTE'
      Origin = 'LOTE'
      ProviderFlags = []
      Required = True
    end
    object CDSRtosOBSERVACION: TStringField
      FieldName = 'OBSERVACION'
      Origin = 'OBSERVACION'
      ProviderFlags = []
    end
    object CDSRtosNOTAVENTA_ID: TIntegerField
      FieldName = 'NOTAVENTA_ID'
      Origin = 'NOTAVENTA_ID'
      ProviderFlags = []
    end
    object CDSRtosNOTAVENTA_ID_DET: TIntegerField
      FieldName = 'NOTAVENTA_ID_DET'
      Origin = 'NOTAVENTA_ID_DET'
      ProviderFlags = []
    end
    object CDSRtosCODIGOBARRA: TStringField
      FieldName = 'CODIGOBARRA'
      Origin = 'CODIGOBARRA'
      ProviderFlags = []
      Size = 50
    end
    object CDSRtosPRECIO_EDITABLE: TStringField
      FieldName = 'PRECIO_EDITABLE'
      Origin = 'PRECIO_EDITABLE'
      ProviderFlags = []
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSRtosDEVOLUCION: TStringField
      Alignment = taCenter
      DisplayLabel = 'Tipo Entrega'
      FieldName = 'DEVOLUCION'
      Origin = 'DEVOLUCION'
      ProviderFlags = []
      OnGetText = CDSRtosDEVOLUCIONGetText
      FixedChar = True
      Size = 1
    end
    object CDSRtosRENGLON: TFloatField
      DisplayLabel = 'Renglon'
      FieldName = 'RENGLON'
      Origin = 'RENGLON'
    end
    object CDSRtosCANTIDAD: TFloatField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.000'
    end
    object CDSRtosTOTAL_GRAVADO: TFloatField
      FieldName = 'TOTAL_GRAVADO'
      Origin = 'TOTAL_GRAVADO'
      Required = True
    end
    object CDSRtosIVA_TOTAL: TFloatField
      FieldName = 'IVA_TOTAL'
      Origin = 'IVA_TOTAL'
      Required = True
    end
    object CDSRtosTOTAL_EXENTO: TFloatField
      FieldName = 'TOTAL_EXENTO'
      Origin = 'TOTAL_EXENTO'
      Required = True
    end
    object CDSRtosTOTAL: TFloatField
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
    end
    object CDSRtosIVA_TASA: TFloatField
      FieldName = 'IVA_TASA'
      Origin = 'IVA_TASA'
    end
    object CDSRtosIVA_SOBRETASA: TFloatField
      FieldName = 'IVA_SOBRETASA'
      Origin = 'IVA_SOBRETASA'
    end
    object CDSRtosDESCUENTO: TFloatField
      FieldName = 'DESCUENTO'
      Origin = 'DESCUENTO'
    end
    object CDSRtosMARGEN: TFloatField
      FieldName = 'MARGEN'
      Origin = 'MARGEN'
    end
    object CDSRtosCOTIZACION: TFloatField
      FieldName = 'COTIZACION'
      Origin = 'COTIZACION'
      Required = True
    end
    object CDSRtosCANT_FACTURA: TFloatField
      DisplayLabel = 'A Facturar'
      FieldName = 'CANT_FACTURA'
      Origin = 'CANT_FACTURA'
      Required = True
      DisplayFormat = ',0.000'
    end
  end
  object DSRtos: TDataSource
    DataSet = CDSRtos
    Left = 648
    Top = 184
  end
  object MDatos: TJvMemoryData
    FieldDefs = <
      item
        Name = 'CODIGO'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DETALLE'
        DataType = ftString
        Size = 45
      end
      item
        Name = 'ACOPIO'
        DataType = ftFloat
      end
      item
        Name = 'REMITO'
        DataType = ftFloat
      end
      item
        Name = 'FACTURAR'
        DataType = ftFloat
      end
      item
        Name = 'PRECIO'
        DataType = ftFloat
      end
      item
        Name = 'TOTAL'
        DataType = ftFloat
      end
      item
        Name = 'SELECCCION'
        DataType = ftBoolean
      end>
    AfterPost = MDatosAfterPost
    AfterScroll = MDatosAfterScroll
    OnNewRecord = MDatosNewRecord
    Left = 456
    Top = 400
    object MDatosCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
    end
    object MDatosDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Size = 45
    end
    object MDatosACOPIO: TFloatField
      DisplayLabel = 'Acopiado'
      FieldName = 'ACOPIO'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosREMITO: TFloatField
      DisplayLabel = 'Ent.x Remito'
      FieldName = 'REMITO'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosFACTURAR: TFloatField
      DisplayLabel = 'A Factu.'
      FieldName = 'FACTURAR'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosPRECIO: TFloatField
      DisplayLabel = 'Precio'
      FieldName = 'PRECIO'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosTOTAL: TFloatField
      DisplayLabel = 'Total'
      FieldName = 'TOTAL'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosDEVOLUCIONES: TFloatField
      DisplayLabel = 'Devoluciones'
      FieldName = 'DEVOLUCIONES'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00;-,0.00;-'
    end
    object MDatosSELECCCION: TBooleanField
      DisplayLabel = 'Sel.'
      FieldName = 'SELECCCION'
    end
  end
  object DSDatos: TDataSource
    DataSet = MDatos
    Left = 528
    Top = 400
  end
  object DSListaPrecios: TDataSource
    DataSet = QListaPrecios
    Left = 536
    Top = 328
  end
  object frxReporte: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 42488.601807604170000000
    ReportOptions.LastChange = 42488.601807604170000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 772
    Top = 276
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frxDBDatos: TfrxDBDataset
    UserName = 'frxDBDatos'
    CloseDataSource = False
    DataSource = DSDatos
    BCDToCurrency = False
    DataSetOptions = []
    Left = 772
    Top = 348
    FieldDefs = <
      item
        FieldName = 'CODIGO'
        FieldType = fftString
        Size = 20
      end
      item
        FieldName = 'DETALLE'
        FieldType = fftString
        Size = 45
      end
      item
        FieldName = 'ACOPIO'
      end
      item
        FieldName = 'REMITO'
      end
      item
        FieldName = 'FACTURAR'
      end
      item
        FieldName = 'PRECIO'
      end
      item
        FieldName = 'TOTAL'
      end
      item
        FieldName = 'DEVOLUCIONES'
      end
      item
        FieldName = 'SELECCCION'
        FieldType = fftBoolean
      end>
  end
  object frxDBRemitos: TfrxDBDataset
    UserName = 'frxDBRemitos'
    CloseDataSource = False
    DataSource = DSRtos
    BCDToCurrency = False
    DataSetOptions = []
    Left = 868
    Top = 228
    FieldDefs = <
      item
        FieldName = 'ID'
      end
      item
        FieldName = 'ID_CABRTO'
      end
      item
        FieldName = 'TIPOCPBTE'
        FieldType = fftString
      end
      item
        FieldName = 'CLASECPBTE'
        FieldType = fftString
      end
      item
        FieldName = 'NROCPBTE'
        FieldType = fftString
        Size = 13
      end
      item
        FieldName = 'FECHAVTA'
      end
      item
        FieldName = 'CODIGOARTICULO'
        FieldType = fftString
      end
      item
        FieldName = 'DETALLE'
        FieldType = fftString
        Size = 45
      end
      item
        FieldName = 'UNIDAD'
        FieldType = fftString
      end
      item
        FieldName = 'COSTO_GRAVADO'
      end
      item
        FieldName = 'COSTO_EXENTO'
      end
      item
        FieldName = 'COSTO_TOTAL'
      end
      item
        FieldName = 'UNITARIO_GRAVADO'
      end
      item
        FieldName = 'IVA_UNITARIO'
      end
      item
        FieldName = 'UNITARIO_EXENTO'
      end
      item
        FieldName = 'UNITARIO_TOTAL'
      end
      item
        FieldName = 'DESGLOZAIVA'
        FieldType = fftString
      end
      item
        FieldName = 'MODO_GRAVAMEN'
        FieldType = fftString
      end
      item
        FieldName = 'TIPOIVA_TASA'
      end
      item
        FieldName = 'TIPOIVA_SOBRETASA'
      end
      item
        FieldName = 'DEPOSITO'
      end
      item
        FieldName = 'AFECTA_STOCK'
        FieldType = fftString
      end
      item
        FieldName = 'CALCULA_SOBRETASA'
        FieldType = fftString
      end
      item
        FieldName = 'GRAVADO_IB'
        FieldType = fftString
      end
      item
        FieldName = 'ID_MONEDA'
      end
      item
        FieldName = 'LOTE'
      end
      item
        FieldName = 'OBSERVACION'
        FieldType = fftString
        Size = 20
      end
      item
        FieldName = 'NOTAVENTA_ID'
      end
      item
        FieldName = 'NOTAVENTA_ID_DET'
      end
      item
        FieldName = 'CODIGOBARRA'
        FieldType = fftString
        Size = 50
      end
      item
        FieldName = 'PRECIO_EDITABLE'
        FieldType = fftString
      end
      item
        FieldName = 'DEVOLUCION'
        FieldType = fftString
      end
      item
        FieldName = 'RENGLON'
      end
      item
        FieldName = 'CANTIDAD'
      end
      item
        FieldName = 'TOTAL_GRAVADO'
      end
      item
        FieldName = 'IVA_TOTAL'
      end
      item
        FieldName = 'TOTAL_EXENTO'
      end
      item
        FieldName = 'TOTAL'
      end
      item
        FieldName = 'IVA_TASA'
      end
      item
        FieldName = 'IVA_SOBRETASA'
      end
      item
        FieldName = 'DESCUENTO'
      end
      item
        FieldName = 'MARGEN'
      end
      item
        FieldName = 'COTIZACION'
      end
      item
        FieldName = 'CANT_FACTURA'
      end>
  end
  object PopupMenu1: TPopupMenu
    Left = 820
    Top = 164
    object ConsolidaDev1: TMenuItem
      Caption = 'Cerrar Entrega y Finalizar'
      OnClick = ConsolidaDev1Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object Remitar1: TMenuItem
      Action = Remitar
    end
    object ConsolidaDevolucion1: TMenuItem
      Action = ConsolidaDev
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object ExportarXLS1: TMenuItem
      Action = ExportarXLS
    end
  end
  object frxDBAcopio: TfrxDBDataset
    UserName = 'frxDBAcopio'
    CloseDataSource = False
    DataSource = DSAcopios
    BCDToCurrency = False
    DataSetOptions = []
    Left = 876
    Top = 292
    FieldDefs = <
      item
        FieldName = 'ID'
        FieldAlias = 'ID'
      end
      item
        FieldName = 'ID_CABRTO'
        FieldAlias = 'ID_CABRTO'
      end
      item
        FieldName = 'TIPOCPBTE'
        FieldAlias = 'TIPOCPBTE'
      end
      item
        FieldName = 'CLASECPBTE'
        FieldAlias = 'CLASECPBTE'
      end
      item
        FieldName = 'NROCPBTE'
        FieldAlias = 'NROCPBTE'
      end
      item
        FieldName = 'RENGLON'
        FieldAlias = 'RENGLON'
      end
      item
        FieldName = 'FECHAVTA'
        FieldAlias = 'FECHAVTA'
      end
      item
        FieldName = 'CODIGOARTICULO'
        FieldAlias = 'CODIGOARTICULO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
        FieldType = fftString
        Size = 45
      end
      item
        FieldName = 'UNIDAD'
        FieldAlias = 'UNIDAD'
      end
      item
        FieldName = 'CANTIDAD'
        FieldAlias = 'CANTIDAD'
      end
      item
        FieldName = 'COSTO_GRAVADO'
        FieldAlias = 'COSTO_GRAVADO'
      end
      item
        FieldName = 'COSTO_EXENTO'
        FieldAlias = 'COSTO_EXENTO'
      end
      item
        FieldName = 'COSTO_TOTAL'
        FieldAlias = 'COSTO_TOTAL'
      end
      item
        FieldName = 'UNITARIO_GRAVADO'
        FieldAlias = 'UNITARIO_GRAVADO'
      end
      item
        FieldName = 'IVA_UNITARIO'
        FieldAlias = 'IVA_UNITARIO'
      end
      item
        FieldName = 'UNITARIO_EXENTO'
        FieldAlias = 'UNITARIO_EXENTO'
      end
      item
        FieldName = 'UNITARIO_TOTAL'
        FieldAlias = 'UNITARIO_TOTAL'
      end
      item
        FieldName = 'TOTAL_GRAVADO'
        FieldAlias = 'TOTAL_GRAVADO'
      end
      item
        FieldName = 'IVA_TOTAL'
        FieldAlias = 'IVA_TOTAL'
      end
      item
        FieldName = 'TOTAL_EXENTO'
        FieldAlias = 'TOTAL_EXENTO'
      end
      item
        FieldName = 'TOTAL'
        FieldAlias = 'TOTAL'
      end
      item
        FieldName = 'DESGLOZAIVA'
        FieldAlias = 'DESGLOZAIVA'
      end
      item
        FieldName = 'MODO_GRAVAMEN'
        FieldAlias = 'MODO_GRAVAMEN'
      end
      item
        FieldName = 'IVA_TASA'
        FieldAlias = 'IVA_TASA'
      end
      item
        FieldName = 'IVA_SOBRETASA'
        FieldAlias = 'IVA_SOBRETASA'
      end
      item
        FieldName = 'TIPOIVA_TASA'
        FieldAlias = 'TIPOIVA_TASA'
      end
      item
        FieldName = 'TIPOIVA_SOBRETASA'
        FieldAlias = 'TIPOIVA_SOBRETASA'
      end
      item
        FieldName = 'DESCUENTO'
        FieldAlias = 'DESCUENTO'
      end
      item
        FieldName = 'DEPOSITO'
        FieldAlias = 'DEPOSITO'
      end
      item
        FieldName = 'MARGEN'
        FieldAlias = 'MARGEN'
      end
      item
        FieldName = 'AFECTA_STOCK'
        FieldAlias = 'AFECTA_STOCK'
      end
      item
        FieldName = 'CALCULA_SOBRETASA'
        FieldAlias = 'CALCULA_SOBRETASA'
      end
      item
        FieldName = 'GRAVADO_IB'
        FieldAlias = 'GRAVADO_IB'
      end
      item
        FieldName = 'ID_MONEDA'
        FieldAlias = 'ID_MONEDA'
      end
      item
        FieldName = 'COTIZACION'
        FieldAlias = 'COTIZACION'
      end
      item
        FieldName = 'LOTE'
        FieldAlias = 'LOTE'
      end
      item
        FieldName = 'OBSERVACION'
        FieldAlias = 'OBSERVACION'
      end
      item
        FieldName = 'NOTAVENTA_ID'
        FieldAlias = 'NOTAVENTA_ID'
      end
      item
        FieldName = 'NOTAVENTA_ID_DET'
        FieldAlias = 'NOTAVENTA_ID_DET'
      end
      item
        FieldName = 'CODIGOBARRA'
        FieldAlias = 'CODIGOBARRA'
      end
      item
        FieldName = 'PRECIO_EDITABLE'
        FieldAlias = 'PRECIO_EDITABLE'
      end
      item
        FieldName = 'CANT_FACTURA'
        FieldAlias = 'CANT_FACTURA'
      end
      item
        FieldName = 'ID_CAB'
      end
      item
        FieldName = 'CODIGO'
        FieldType = fftString
      end
      item
        FieldName = 'FECHA_ULTIMAENTREGA'
      end
      item
        FieldName = 'CUMPLIDO'
        FieldType = fftString
      end
      item
        FieldName = 'FECHA'
      end
      item
        FieldName = 'NROACOPIO'
        FieldType = fftString
        Size = 13
      end
      item
        FieldName = 'ID_FACTURA'
      end
      item
        FieldName = 'FECHAFC'
      end
      item
        FieldName = 'TIPOFC'
        FieldType = fftString
      end
      item
        FieldName = 'NROFC'
        FieldType = fftString
        Size = 13
      end
      item
        FieldName = 'ENTREGADO'
      end
      item
        FieldName = 'SALDO'
      end
      item
        FieldName = 'CODIGOCLIENTE'
        FieldType = fftString
      end
      item
        FieldName = 'NOMBRE'
        FieldType = fftString
        Size = 35
      end
      item
        FieldName = 'RAZON_SOCIAL'
        FieldType = fftString
        Size = 35
      end
      item
        FieldName = 'DIRECCION_COMERCIAL'
        FieldType = fftString
        Size = 35
      end>
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    EmbedFontsIfProtected = False
    InteractiveFormsFontSubset = 'A-Z,a-z,0-9,#43-#47 '
    OpenAfterExport = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Transparency = False
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    Creator = 'FastReport'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    PdfA = False
    PDFStandard = psNone
    PDFVersion = pv17
    Left = 192
    Top = 344
  end
  object dskAviso: TJvDesktopAlert
    HelpContext = 872
    Colors.WindowFrom = clMoneyGreen
    Colors.WindowTo = clTeal
    Location.Position = dapActiveFormCenter
    Location.Top = 90
    Location.Left = 0
    Location.Width = 300
    Location.Height = 80
    Location.AlwaysResetPosition = False
    StyleOptions.DisplayDuration = 90000
    HeaderText = 'Aviso'
    MessageText = 'Aviso'
    HeaderFont.Charset = DEFAULT_CHARSET
    HeaderFont.Color = clWindowText
    HeaderFont.Height = -11
    HeaderFont.Name = 'Tahoma'
    HeaderFont.Style = [fsBold]
    ShowHint = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    Buttons = <>
    Image.Data = {
      
      00000000}
    Left = 280
    Top = 448
  end
  object QClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'SELECT C.*,i.discriminaiva,i.detalle  FROM Clientes C'
      'left join inscripcion i on i.codigo=c.condicion_iva'
      'where (c.codigo=:codigo)')
    Left = 44
    Top = 260
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 6
        Value = Null
      end>
    object QClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 6
    end
    object QClientesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Required = True
      Size = 35
    end
    object QClientesNOMBRE_COMPLETO: TStringField
      FieldName = 'NOMBRE_COMPLETO'
      Origin = 'NOMBRE_COMPLETO'
      Size = 100
    end
    object QClientesRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      Origin = 'RAZON_SOCIAL'
      Size = 35
    end
    object QClientesDIRECCION_PARTICULAR: TStringField
      FieldName = 'DIRECCION_PARTICULAR'
      Origin = 'DIRECCION_PARTICULAR'
      Size = 35
    end
    object QClientesDIRECCION_COMERCIAL: TStringField
      FieldName = 'DIRECCION_COMERCIAL'
      Origin = 'DIRECCION_COMERCIAL'
      Size = 35
    end
    object QClientesTELEFONO_PARTICULAR: TStringField
      FieldName = 'TELEFONO_PARTICULAR'
      Origin = 'TELEFONO_PARTICULAR'
      Size = 10
    end
    object QClientesTELEFONO_COMERCIAL_1: TStringField
      FieldName = 'TELEFONO_COMERCIAL_1'
      Origin = 'TELEFONO_COMERCIAL_1'
      Size = 10
    end
    object QClientesTELEFONO_COMERCIAL_2: TStringField
      FieldName = 'TELEFONO_COMERCIAL_2'
      Origin = 'TELEFONO_COMERCIAL_2'
      Size = 10
    end
    object QClientesTELEFONO_CELULAR: TStringField
      FieldName = 'TELEFONO_CELULAR'
      Origin = 'TELEFONO_CELULAR'
      Size = 11
    end
    object QClientesID_COD_POSTAL: TIntegerField
      FieldName = 'ID_COD_POSTAL'
      Origin = 'ID_COD_POSTAL'
    end
    object QClientesCPOSTAL: TStringField
      FieldName = 'CPOSTAL'
      Origin = 'CPOSTAL'
      Size = 8
    end
    object QClientesLOCALIDAD: TStringField
      FieldName = 'LOCALIDAD'
      Origin = 'LOCALIDAD'
      Size = 15
    end
    object QClientesCONDICION_IVA: TIntegerField
      FieldName = 'CONDICION_IVA'
      Origin = 'CONDICION_IVA'
      Required = True
    end
    object QClientesEMITE_REMITO: TStringField
      FieldName = 'EMITE_REMITO'
      Origin = 'EMITE_REMITO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object QClientesEMITE_FACTURA: TStringField
      FieldName = 'EMITE_FACTURA'
      Origin = 'EMITE_FACTURA'
      Required = True
      FixedChar = True
      Size = 1
    end
    object QClientesNRO_DE_CUIT: TStringField
      FieldName = 'NRO_DE_CUIT'
      Origin = 'NRO_DE_CUIT'
      Size = 13
    end
    object QClientesACTIVO: TStringField
      FieldName = 'ACTIVO'
      Origin = 'ACTIVO'
      FixedChar = True
      Size = 1
    end
    object QClientesLIMITE_CREDITO: TFloatField
      FieldName = 'LIMITE_CREDITO'
      Origin = 'LIMITE_CREDITO'
    end
    object QClientesSALDOINICIAL: TFloatField
      FieldName = 'SALDOINICIAL'
      Origin = 'SALDOINICIAL'
    end
    object QClientesOBSERVACIONES: TMemoField
      FieldName = 'OBSERVACIONES'
      Origin = 'OBSERVACIONES'
      BlobType = ftMemo
    end
    object QClientesFECHAULTIMACOMPRA: TSQLTimeStampField
      FieldName = 'FECHAULTIMACOMPRA'
      Origin = 'FECHAULTIMACOMPRA'
    end
    object QClientesFECHAALTA: TSQLTimeStampField
      FieldName = 'FECHAALTA'
      Origin = 'FECHAALTA'
    end
    object QClientesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object QClientesZONA: TIntegerField
      FieldName = 'ZONA'
      Origin = 'ZONA'
    end
    object QClientesDISCRIMINAIVA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'DISCRIMINAIVA'
      Origin = 'DISCRIMINAIVA'
      ProviderFlags = []
      ReadOnly = True
      FixedChar = True
      Size = 1
    end
    object QClientesDETALLE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      ReadOnly = True
      Size = 15
    end
    object QClientesAVISO: TStringField
      FieldName = 'AVISO'
      Origin = 'AVISO'
      Size = 250
    end
  end
  object QArt: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select s.codigo_stk from stock s where s.codigo_stk=:codigo'
      'and s.clase_articulo<>0')
    Left = 44
    Top = 308
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 8
        Value = Null
      end>
    object QArtCODIGO_STK: TStringField
      FieldName = 'CODIGO_STK'
      Origin = 'CODIGO_STK'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 8
    end
  end
  object QStockPrecio_Vta: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from PRECIO_VENTA(:codigo,:lista,:deposito,:sucursal)')
    Left = 44
    Top = 364
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 8
        Value = Null
      end
      item
        Name = 'LISTA'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'DEPOSITO'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'SUCURSAL'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QStockPrecio_VtaDESCUENTO: TFloatField
      FieldName = 'DESCUENTO'
      Origin = 'DESCUENTO'
    end
    object QStockPrecio_VtaCOSTO_TOTAL: TFloatField
      FieldName = 'COSTO_TOTAL'
      Origin = 'COSTO_TOTAL'
    end
    object QStockPrecio_VtaPRECIO: TFloatField
      FieldName = 'PRECIO'
      Origin = 'PRECIO'
    end
    object QStockPrecio_VtaCOSTO_EXENTO: TFloatField
      FieldName = 'COSTO_EXENTO'
      Origin = 'COSTO_EXENTO'
    end
    object QStockPrecio_VtaCOSTO_GRAVADO: TFloatField
      FieldName = 'COSTO_GRAVADO'
      Origin = 'COSTO_GRAVADO'
    end
  end
  object QAcopios: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select md.*,mc.fecha,mc.nrocpbte as NroAcopio,mc.id_factura,'
      
        '       fc.fechavta as FechaFc,fc.tipocpbte as TipoFc,fc.nrocpbte' +
        ' as NroFc,'
      
        '       c.codigo as CodigoCliente,c.nombre,c.razon_social,c.direc' +
        'cion_comercial from merca_enacopio_det md'
      'left join merca_enacopio_cab mc on mc.id=md.id_cab'
      'left join fcvtacab fc on fc.id_fc=mc.id_factura'
      'left join clientes c on c.codigo=fc.codigo'
      'where mc.cumplido<>'#39'S'#39' and mc.cliente = :codigo'
      'order by mc.fecha asc')
    Left = 428
    Top = 124
    ParamData = <
      item
        Position = 1
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QRtos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select rd.*,rc.devolucion from rtodet rd'
      '  left join rtocab rc on rc.id_rto=rd.id_cabrto'
      'where rc.anulado='#39'N'#39' and rc.codigo= :codigo and'
      '      ((rc.idfactura is null) or (rc.idfactura<=0)) and'
      '      ( rd.CANT_FACTURA>0 or  rd.CANT_FACTURA<0)'
      'order by rc.devolucion,rc.nrocpbte')
    Left = 428
    Top = 180
    ParamData = <
      item
        Position = 1
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
        Value = Null
      end>
  end
  object QListaPrecios: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select id,nombre,f_pago from  listaprecioespecialcab'
      ' where uso_administrador='#39'N'#39)
    Left = 444
    Top = 324
    object QListaPreciosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QListaPreciosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 50
    end
    object QListaPreciosF_PAGO: TIntegerField
      FieldName = 'F_PAGO'
      Origin = 'F_PAGO'
    end
  end
  object QPrecio: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select la.precio,s.contenido_compra from listaprecioespecialart ' +
        'la'
      'left join stock s on s.codigo_stk=la.codigoarticulo'
      'where la.codigoarticulo=:articulo and la.id_cab=:lista')
    Left = 644
    Top = 388
    ParamData = <
      item
        Name = 'ARTICULO'
        DataType = ftString
        ParamType = ptInput
        Size = 8
        Value = Null
      end
      item
        Name = 'LISTA'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QPrecioPRECIO: TFloatField
      FieldName = 'PRECIO'
      Origin = 'PRECIO'
    end
    object QPrecioCONTENIDO_COMPRA: TFloatField
      AutoGenerateValue = arDefault
      FieldName = 'CONTENIDO_COMPRA'
      Origin = 'CONTENIDO_COMPRA'
      ProviderFlags = []
      ReadOnly = True
    end
  end
  object QMarcarAcopios: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update merca_enacopio_det md set'
      '   md.SALDO = 0,'
      '   md.ENTREGADO = md.CANTIDAD ,'
      '   md.FECHA_ULTIMAENTREGA = :fecha ,'
      '   md.cumplido='#39'S'#39' '
      'where md.id = :id')
    Left = 644
    Top = 252
    ParamData = <
      item
        Name = 'FECHA'
        DataType = ftDate
        ParamType = ptInput
        Value = Null
      end
      item
        Name = 'ID'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object spCerrarAcopios: TFDStoredProc
    Connection = DMMain_FD.fdcGestion
    StoredProcName = 'CONSOLIDAR_ACOPIO'
    Left = 808
    Top = 527
    ParamData = <
      item
        Position = 1
        Name = 'ID_ACOPIO'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'DEPOSITO'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'USUARIO'
        DataType = ftString
        ParamType = ptInput
        Size = 50
      end>
  end
  object QMarcarEntregado: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update rtocab r set r.idfactura=1 ,  r.nrofact='#39'S/N'#39' '
      'where r.id_rto = :id')
    Left = 620
    Top = 484
    ParamData = <
      item
        Name = 'ID'
        ParamType = ptInput
      end>
  end
  object QMarcarDetRtos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update rtodet rd set rd.cant_factura=0 where rd.id = :id')
    Left = 468
    Top = 484
    ParamData = <
      item
        Name = 'ID'
        ParamType = ptInput
      end>
  end
  object spCerrarRemtios: TFDStoredProc
    Connection = DMMain_FD.fdcGestion
    StoredProcName = 'MARCAR_RTOS_DE_ACOPIO'
    Left = 804
    Top = 460
    ParamData = <
      item
        Position = 1
        Name = 'ID_RTO'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object SaveDialog1: TSaveDialog
    Left = 700
    Top = 324
  end
end
