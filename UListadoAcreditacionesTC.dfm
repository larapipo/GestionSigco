inherited FormListadoAcreditacionesTC: TFormListadoAcreditacionesTC
  Left = 276
  Top = 110
  Caption = 'Listado Acreditaciones de TC'
  ClientHeight = 653
  ClientWidth = 951
  StyleElements = [seFont, seClient, seBorder]
  OnResize = FormResize
  ExplicitWidth = 969
  ExplicitHeight = 694
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 951
    Height = 600
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 943
    ExplicitHeight = 600
    object RxLabel6: TJvLabel
      Left = 5
      Top = 430
      Width = 17
      Height = 13
      Caption = 'Iva'
      Anchors = [akLeft, akBottom]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel7: TJvLabel
      Left = 5
      Top = 510
      Width = 90
      Height = 13
      Caption = 'Percepciones IIBB'
      Anchors = [akLeft, akBottom]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel9: TJvLabel
      Left = 395
      Top = 427
      Width = 62
      Height = 13
      Caption = 'Retenciones'
      Anchors = [akLeft, akBottom]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object dbgDebitos: TJvDBGrid
      Left = 0
      Top = 54
      Width = 951
      Height = 372
      Align = alTop
      Anchors = [akLeft, akTop, akRight, akBottom]
      DataSource = DSLiquidaciones
      DrawingStyle = gdsGradient
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDblClick = dbgDebitosDblClick
      TitleButtons = True
      OnTitleBtnClick = dbgDebitosTitleBtnClick
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'FECHA'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 79
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NUMEROCPBTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 94
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NOMBRE_CTABCO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRESENTACION_ADICIONAL'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 92
          Visible = True
        end
        item
          Color = clAqua
          Expanded = False
          FieldName = 'TOTAL_PRESENTACION'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 74
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ARANCEL'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 77
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'COSTO_FINANCIERO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IVA'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 55
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IIBB'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 53
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GANANCIAS'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 61
          Visible = True
        end
        item
          Color = clMoneyGreen
          Expanded = False
          FieldName = 'TOTAL_ACREDITADO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 78
          Visible = True
        end>
    end
    object dbgIva: TDBGrid
      Left = 5
      Top = 446
      Width = 384
      Height = 61
      Anchors = [akLeft, akBottom]
      DataSource = DSMovIva
      Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          ButtonStyle = cbsEllipsis
          Expanded = False
          FieldName = 'CODIGO_GRAVAMEN'
          Title.Alignment = taCenter
          Title.Caption = 'Codigo'
          Width = 40
          Visible = True
        end
        item
          Color = clAqua
          Expanded = False
          FieldName = 'DETALLE'
          Title.Alignment = taCenter
          Width = 119
          Visible = True
        end
        item
          Color = clAqua
          Expanded = False
          FieldName = 'TASA'
          Title.Alignment = taCenter
          Width = 58
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NETO'
          Title.Alignment = taCenter
          Title.Caption = 'Neto Grav.'
          Width = 71
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IMPORTE'
          Title.Alignment = taCenter
          Width = 67
          Visible = True
        end>
    end
    object dbgPercepcionIB: TDBGrid
      Left = 2
      Top = 529
      Width = 384
      Height = 65
      Anchors = [akLeft, akBottom]
      DataSource = DSMovIIBB
      Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          ButtonStyle = cbsEllipsis
          Expanded = False
          FieldName = 'CODIGO_PERCEPCION'
          Title.Alignment = taCenter
          Title.Caption = 'Codigo'
          Width = 42
          Visible = True
        end
        item
          Color = clAqua
          Expanded = False
          FieldName = 'DETALLE'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Color = clAqua
          Expanded = False
          FieldName = 'TASA'
          Title.Alignment = taCenter
          Width = 56
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NETO'
          Title.Alignment = taCenter
          Title.Caption = 'Neto.Grav'
          Width = 66
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IMPORTE'
          Title.Alignment = taCenter
          Width = 68
          Visible = True
        end>
    end
    object dbg1: TDBGrid
      Left = 395
      Top = 446
      Width = 414
      Height = 61
      Anchors = [akLeft, akBottom]
      DataSource = DSRetenciones
      Options = [dgEditing, dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 3
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          ButtonStyle = cbsEllipsis
          Expanded = False
          FieldName = 'ID_RETENCION'
          Title.Alignment = taCenter
          Width = 24
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DETALLE'
          Title.Alignment = taCenter
          Width = 108
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FECHA_RETENCION'
          Title.Alignment = taCenter
          Width = 75
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NRO_RETENCION'
          Title.Alignment = taCenter
          Width = 84
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IMPORTE'
          Title.Alignment = taCenter
          Width = 92
          Visible = True
        end>
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 951
      Height = 54
      Align = alTop
      BevelOuter = bvNone
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 4
      ExplicitWidth = 943
      object RxLabel1: TJvLabel
        Left = 681
        Top = 4
        Width = 33
        Height = 13
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel3: TJvLabel
        Left = 784
        Top = 4
        Width = 30
        Height = 13
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object JvLabel1: TJvLabel
        Left = 13
        Top = 13
        Width = 356
        Height = 29
        Caption = 'Listado de Acreditacion de TC'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -24
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -24
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object Desde: TJvDateEdit
        Left = 681
        Top = 20
        Width = 97
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 0
      end
      object Hasta: TJvDateEdit
        Left = 785
        Top = 20
        Width = 97
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 1
      end
      object ud1: TUpDown
        Left = 887
        Top = 21
        Width = 16
        Height = 20
        Hint = 'Cambia el Per'#237'odo'
        Max = 10000
        ParentShowHint = False
        Position = 100
        ShowHint = True
        TabOrder = 3
        OnClick = ud1Click
      end
      object rgModo: TAdvOfficeRadioGroup
        Left = 407
        Top = 2
        Width = 256
        Height = 39
        CaptionFont.Charset = DEFAULT_CHARSET
        CaptionFont.Color = clWindowText
        CaptionFont.Height = -12
        CaptionFont.Name = 'Segoe UI'
        CaptionFont.Style = []
        Version = '1.8.4.2'
        Caption = 'Modo de Consulta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBackground = False
        ParentCtl3D = True
        ParentFont = False
        TabOrder = 2
        OnClick = rgModoClick
        Columns = 2
        Items.Strings = (
          'Fecha de Carga'
          'Fecha Fiscal')
        Themed = True
        Ellipsis = False
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 600
    Width = 951
    Align = alBottom
    ButtonHeight = 28
    ExplicitTop = 600
    ExplicitWidth = 943
    inherited btConfirma: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btNuevo: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btCancelar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btModificar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited Pr: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBuscar: TBitBtn
      Height = 28
      DoubleBuffered = True
      ExplicitHeight = 28
    end
    inherited Ne: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBorrar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btSalir: TBitBtn
      Height = 28
      DoubleBuffered = True
      ExplicitHeight = 28
    end
    object btn1: TToolButton
      Left = 557
      Top = 0
      Width = 14
      Caption = 'btn1'
      Style = tbsSeparator
    end
    object btImprimir: TBitBtn
      Left = 571
      Top = 0
      Width = 75
      Height = 28
      Action = Imprimir
      Caption = 'Imprimir'
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      NumGlyphs = 2
      TabOrder = 7
    end
  end
  inherited Panel1: TPanel
    Top = 630
    Width = 951
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 630
    ExplicitWidth = 943
    inherited sbEstado: TStatusBar
      Width = 862
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 854
    end
  end
  inherited ActionList1: TActionList
    Left = 448
    Top = 183
    object Imprimir: TAction
      Caption = 'Imprimir'
      OnExecute = ImprimirExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 487
    Top = 51
  end
  inherited ImageList1: TImageList
    Top = 126
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 120
    Top = 216
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Top = 352
  end
  inherited ComBuscadorBase: TComBuscador
    Top = 248
  end
  inherited QBrowse2: TFDQuery
    Left = 80
    Top = 184
  end
  object CDSLiquidaciones: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPLiquidaciones'
    Left = 356
    Top = 181
    object CDSLiquidacionesID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSLiquidacionesFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
      Required = True
    end
    object CDSLiquidacionesFECHA_FISCAL: TSQLTimeStampField
      DisplayLabel = 'F.Fiscal'
      FieldName = 'FECHA_FISCAL'
      Origin = 'FECHA_FISCAL'
      Required = True
    end
    object CDSLiquidacionesPERIODO_LIQUIDADO: TStringField
      DisplayLabel = 'Periodo'
      FieldName = 'PERIODO_LIQUIDADO'
      Origin = 'PERIODO_LIQUIDADO'
      Size = 6
    end
    object CDSLiquidacionesNRO_LIQUIDACION: TStringField
      DisplayLabel = 'Nro.Liq.'
      FieldName = 'NRO_LIQUIDACION'
      Origin = 'NRO_LIQUIDACION'
      Size = 8
    end
    object CDSLiquidacionesNUMEROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NUMEROCPBTE'
      Origin = 'NUMEROCPBTE'
      EditMask = 'a-####-########;0'
      Size = 13
    end
    object CDSLiquidacionesNOMBRE_CTABCO: TStringField
      DisplayLabel = 'Banco'
      FieldName = 'NOMBRE_CTABCO'
      Origin = 'NOMBRE_CTABCO'
      Size = 25
    end
    object CDSLiquidacionesARANCEL: TFloatField
      DisplayLabel = 'Arancel'
      FieldName = 'ARANCEL'
      Origin = 'ARANCEL'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesCOSTO_FINANCIERO: TFloatField
      DisplayLabel = 'Cst.Financiero'
      FieldName = 'COSTO_FINANCIERO'
      Origin = 'COSTO_FINANCIERO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesGANANCIAS: TFloatField
      DisplayLabel = 'Ganancia'
      FieldName = 'GANANCIAS'
      Origin = 'GANANCIAS'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesIIBB: TFloatField
      FieldName = 'IIBB'
      Origin = 'IIBB'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesIVA: TFloatField
      DisplayLabel = 'Iva'
      FieldName = 'IVA'
      Origin = 'IVA'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesPRESENTACION_ADICIONAL: TFloatField
      DisplayLabel = 'Pres.Adicional'
      FieldName = 'PRESENTACION_ADICIONAL'
      Origin = 'PRESENTACION_ADICIONAL'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesTOTAL_PRESENTACION: TFloatField
      DisplayLabel = 'T.Presen.'
      FieldName = 'TOTAL_PRESENTACION'
      Origin = 'TOTAL_PRESENTACION'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSLiquidacionesTOTAL_ACREDITADO: TFloatField
      DisplayLabel = 'Acreditado'
      FieldName = 'TOTAL_ACREDITADO'
      Origin = 'TOTAL_ACREDITADO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object DSLiquidaciones: TDataSource
    DataSet = CDSLiquidaciones
    Left = 524
    Top = 185
  end
  object DSPLiquidaciones: TDataSetProvider
    DataSet = QLiquidaciones
    Options = [poAllowCommandText, poUseQuoteChar]
    Left = 261
    Top = 181
  end
  object DSMovIva: TDataSource
    DataSet = QTotalIva
    Left = 200
    Top = 400
  end
  object DSMovIIBB: TDataSource
    DataSet = QTotalIIB
    Left = 200
    Top = 512
  end
  object DSRetenciones: TDataSource
    DataSet = QRetenciones
    Left = 199
    Top = 577
  end
  object frDBIva: TfrxDBDataset
    UserName = 'frDBIva'
    CloseDataSource = False
    DataSource = DSMovIva
    BCDToCurrency = False
    DataSetOptions = []
    Left = 416
    Top = 464
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'FECHA_FISCAL'
        FieldAlias = 'FECHA_FISCAL'
      end
      item
        FieldName = 'PERIODO_LIQUIDADO'
        FieldAlias = 'PERIODO_LIQUIDADO'
      end
      item
        FieldName = 'NUMEROCPBTE'
        FieldAlias = 'NUMEROCPBTE'
      end
      item
        FieldName = 'CODIGO_GRAVAMEN'
        FieldAlias = 'CODIGO_GRAVAMEN'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'TASA'
        FieldAlias = 'TASA'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end
      item
        FieldName = 'NETO'
        FieldAlias = 'NETO'
      end>
  end
  object frDBIIBB: TfrxDBDataset
    UserName = 'frDBIIBB'
    CloseDataSource = False
    DataSource = DSMovIIBB
    BCDToCurrency = False
    DataSetOptions = []
    Left = 424
    Top = 520
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'FECHA_FISCAL'
        FieldAlias = 'FECHA_FISCAL'
      end
      item
        FieldName = 'PERIODO_LIQUIDADO'
        FieldAlias = 'PERIODO_LIQUIDADO'
      end
      item
        FieldName = 'NUMEROCPBTE'
        FieldAlias = 'NUMEROCPBTE'
      end
      item
        FieldName = 'CODIGO_PERCEPCION'
        FieldAlias = 'CODIGO_PERCEPCION'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'TASA'
        FieldAlias = 'TASA'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end
      item
        FieldName = 'NETO'
        FieldAlias = 'NETO'
      end>
  end
  object frDBRetenciones: TfrxDBDataset
    UserName = 'frDBRetenciones'
    CloseDataSource = False
    DataSource = DSRetenciones
    BCDToCurrency = False
    DataSetOptions = []
    Left = 424
    Top = 576
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'FECHA_FISCAL'
        FieldAlias = 'FECHA_FISCAL'
      end
      item
        FieldName = 'PERIODO_LIQUIDADO'
        FieldAlias = 'PERIODO_LIQUIDADO'
      end
      item
        FieldName = 'NUMEROCPBTE'
        FieldAlias = 'NUMEROCPBTE'
      end
      item
        FieldName = 'FECHA_RETENCION'
        FieldAlias = 'FECHA_RETENCION'
      end
      item
        FieldName = 'NRO_RETENCION'
        FieldAlias = 'NRO_RETENCION'
      end
      item
        FieldName = 'ID_RETENCION'
        FieldAlias = 'ID_RETENCION'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end>
  end
  object frResumen: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39638.831592338000000000
    ReportOptions.LastChange = 40267.684181666660000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 528
    Top = 376
    Datasets = <
      item
        DataSet = frDBIIBB
        DataSetName = 'frDBIIBB'
      end
      item
        DataSet = frDBIva
        DataSetName = 'frDBIva'
      end
      item
        DataSet = frDBRetenciones
        DataSetName = 'frDBRetenciones'
      end
      item
        DataSet = frDSLiquidaciones
        DataSetName = 'frDSLiquidaciones'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 215.900000000000000000
      PaperHeight = 355.600000000000000000
      PaperSize = 5
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object PageHeader1: TfrxPageHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        Height = 71.811070000000000000
        ParentFont = False
        Top = 22.677180000000000000
        Width = 740.409927000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Top = 3.779530000000000000
          Width = 430.866420000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Listado de Acreditaciones de tarjeta de Credito')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          AllowVectorExport = True
          Left = 540.472790000000000000
          Top = 3.779530000000000000
          Width = 196.535560000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            'Emitido el:[Date]:[Time]')
          ParentFont = False
        end
        object Memo38: TfrxMemoView
          AllowVectorExport = True
          Left = 15.118120000000000000
          Top = 41.574830000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Periodo del')
          ParentFont = False
        end
        object Memo39: TfrxMemoView
          AllowVectorExport = True
          Left = 102.047310000000000000
          Top = 41.574830000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsItalic]
          Frame.Typ = []
          ParentFont = False
        end
        object Memo40: TfrxMemoView
          AllowVectorExport = True
          Left = 204.094620000000000000
          Top = 41.574830000000000000
          Width = 18.897650000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'al')
          ParentFont = False
        end
        object Memo41: TfrxMemoView
          AllowVectorExport = True
          Left = 226.771800000000000000
          Top = 41.574830000000000000
          Width = 94.488250000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsItalic]
          Frame.Typ = []
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 15.118120000000000000
        Top = 192.756030000000000000
        Width = 740.409927000000000000
        DataSet = frDSLiquidaciones
        DataSetName = 'frDSLiquidaciones'
        RowCount = 0
        object Memo3: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Width = 68.031540000000000000
          Height = 15.118120000000000000
          DataField = 'FECHA'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."FECHA"]')
          ParentFont = False
        end
        object Memo5: TfrxMemoView
          AllowVectorExport = True
          Left = 75.590600000000000000
          Width = 68.031540000000000000
          Height = 15.118120000000000000
          DataField = 'NRO_LIQUIDACION'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."NRO_LIQUIDACION"]')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          AllowVectorExport = True
          Left = 151.181200000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          DataField = 'NOMBRE_CTABCO'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."NOMBRE_CTABCO"]')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          AllowVectorExport = True
          Left = 253.228510000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          DataField = 'TOTAL_PRESENTACION'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."TOTAL_PRESENTACION"]')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          AllowVectorExport = True
          Left = 336.378170000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          DataField = 'ARANCEL'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."ARANCEL"]')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          AllowVectorExport = True
          Left = 415.748300000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataField = 'COSTO_FINANCIERO'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."COSTO_FINANCIERO"]')
          ParentFont = False
        end
        object Memo10: TfrxMemoView
          AllowVectorExport = True
          Left = 491.338900000000000000
          Width = 64.252010000000000000
          Height = 15.118120000000000000
          DataField = 'GANANCIAS'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."GANANCIAS"]')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          AllowVectorExport = True
          Left = 672.756340000000000000
          Width = 64.252010000000000000
          Height = 15.118120000000000000
          DataField = 'TOTAL_ACREDITADO'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."TOTAL_ACREDITADO"]')
          ParentFont = False
        end
        object Memo20: TfrxMemoView
          AllowVectorExport = True
          Left = 563.149970000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          DataField = 'IIBB'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."IIBB"]')
          ParentFont = False
        end
        object Memo21: TfrxMemoView
          AllowVectorExport = True
          Left = 616.063390000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          DataField = 'IVA'
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDSLiquidaciones."IVA"]')
          ParentFont = False
        end
      end
      object MasterData2: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 359.055350000000000000
        Width = 740.409927000000000000
        DataSet = frDBIva
        DataSetName = 'frDBIva'
        RowCount = 0
        object Memo4: TfrxMemoView
          AllowVectorExport = True
          Left = 3.779530000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataField = 'FECHA'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."FECHA"]')
          ParentFont = False
        end
        object Memo31: TfrxMemoView
          AllowVectorExport = True
          Left = 83.149660000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataField = 'FECHA_FISCAL'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."FECHA_FISCAL"]')
          ParentFont = False
        end
        object Memo32: TfrxMemoView
          AllowVectorExport = True
          Left = 158.740260000000000000
          Top = 3.779530000000000000
          Width = 94.488250000000000000
          Height = 15.118120000000000000
          DataField = 'NRO_LIQUIDACION'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."NRO_LIQUIDACION"]')
          ParentFont = False
        end
        object Memo33: TfrxMemoView
          AllowVectorExport = True
          Left = 257.008040000000000000
          Top = 3.779530000000000000
          Width = 102.047310000000000000
          Height = 15.118120000000000000
          DataField = 'DETALLE'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."DETALLE"]')
          ParentFont = False
        end
        object Memo34: TfrxMemoView
          AllowVectorExport = True
          Left = 366.614410000000000000
          Top = 3.779530000000000000
          Width = 60.472480000000000000
          Height = 15.118120000000000000
          DataField = 'TASA'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."TASA"]')
          ParentFont = False
        end
        object Memo35: TfrxMemoView
          AllowVectorExport = True
          Left = 434.645950000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataField = 'IMPORTE'
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[frDBIva."IMPORTE"]')
          ParentFont = False
        end
      end
      object Header1: TfrxHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 136.063080000000000000
        Width = 740.409927000000000000
        object Memo12: TfrxMemoView
          AllowVectorExport = True
          Left = 15.118120000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Fecha')
          ParentFont = False
        end
        object Memo13: TfrxMemoView
          AllowVectorExport = True
          Left = 86.929190000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Nro.Liq')
          ParentFont = False
        end
        object Memo14: TfrxMemoView
          AllowVectorExport = True
          Left = 181.417440000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Entidad')
          ParentFont = False
        end
        object Memo15: TfrxMemoView
          AllowVectorExport = True
          Left = 279.685220000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'T.Present.')
          ParentFont = False
        end
        object Memo16: TfrxMemoView
          AllowVectorExport = True
          Left = 355.275820000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Arancel')
          ParentFont = False
        end
        object Memo17: TfrxMemoView
          AllowVectorExport = True
          Left = 430.866420000000000000
          Top = 3.779530000000000000
          Width = 52.913420000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'C.Financ.')
          ParentFont = False
        end
        object Memo18: TfrxMemoView
          AllowVectorExport = True
          Left = 502.677490000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Ganac.')
          ParentFont = False
        end
        object Memo19: TfrxMemoView
          AllowVectorExport = True
          Left = 672.756340000000000000
          Top = 3.779530000000000000
          Width = 60.472480000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Acreditado')
          ParentFont = False
        end
        object Memo22: TfrxMemoView
          AllowVectorExport = True
          Left = 612.283860000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'Iva')
          ParentFont = False
        end
        object Memo23: TfrxMemoView
          AllowVectorExport = True
          Left = 563.149970000000000000
          Top = 3.779530000000000000
          Width = 41.574830000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haCenter
          Memo.UTF8W = (
            'IIBB')
          ParentFont = False
        end
      end
      object Footer1: TfrxFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 37.795300000000000000
        Top = 238.110390000000000000
        Width = 740.409927000000000000
        object Memo24: TfrxMemoView
          AllowVectorExport = True
          Left = 249.448980000000000000
          Top = 3.779530000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."TOTAL_PRESENTACION">)]')
          ParentFont = False
        end
        object Memo25: TfrxMemoView
          AllowVectorExport = True
          Left = 332.598640000000000000
          Top = 3.779530000000000000
          Width = 75.590600000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."ARANCEL">)]')
          ParentFont = False
        end
        object Memo26: TfrxMemoView
          AllowVectorExport = True
          Left = 411.968770000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."COSTO_FINANCIERO">)]')
          ParentFont = False
        end
        object Memo27: TfrxMemoView
          AllowVectorExport = True
          Left = 487.559370000000000000
          Top = 3.779530000000000000
          Width = 64.252010000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."GANANCIAS">)]')
          ParentFont = False
        end
        object Memo28: TfrxMemoView
          AllowVectorExport = True
          Left = 668.976810000000000000
          Top = 3.779530000000000000
          Width = 64.252010000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."TOTAL_ACREDITADO">)]')
          ParentFont = False
        end
        object Memo29: TfrxMemoView
          AllowVectorExport = True
          Left = 559.370440000000000000
          Top = 3.779530000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."IIBB">)]')
          ParentFont = False
        end
        object Memo30: TfrxMemoView
          AllowVectorExport = True
          Left = 612.283860000000000000
          Top = 3.779530000000000000
          Width = 45.354360000000000000
          Height = 15.118120000000000000
          DataSet = frDSLiquidaciones
          DataSetName = 'frDSLiquidaciones'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDSLiquidaciones."IVA">)]')
          ParentFont = False
        end
        object Line1: TfrxLineView
          AllowVectorExport = True
          Left = 222.992270000000000000
          Width = 517.795610000000000000
          Color = clBlack
          Frame.Typ = [ftTop]
        end
      end
      object Header2: TfrxHeader
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 309.921460000000000000
        Width = 740.409927000000000000
        object Memo36: TfrxMemoView
          AllowVectorExport = True
          Left = 7.559060000000000000
          Width = 83.149660000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          Memo.UTF8W = (
            'Detalle Iva')
          ParentFont = False
        end
      end
      object Footer2: TfrxFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 404.409710000000000000
        Width = 740.409927000000000000
        object Memo37: TfrxMemoView
          AllowVectorExport = True
          Left = 430.866420000000000000
          Top = 3.779530000000000000
          Width = 71.811070000000000000
          Height = 15.118120000000000000
          DataSet = frDBIva
          DataSetName = 'frDBIva'
          DisplayFormat.FormatStr = '%2.2n'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[SUM(<frDBIva."IMPORTE">)]')
          ParentFont = False
        end
      end
    end
  end
  object frDSLiquidaciones: TfrxDBDataset
    UserName = 'frDSLiquidaciones'
    CloseDataSource = False
    DataSet = CDSLiquidaciones
    BCDToCurrency = False
    DataSetOptions = []
    Left = 622
    Top = 413
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'FECHA_FISCAL'
        FieldAlias = 'FECHA_FISCAL'
      end
      item
        FieldName = 'PERIODO_LIQUIDADO'
        FieldAlias = 'PERIODO_LIQUIDADO'
      end
      item
        FieldName = 'NRO_LIQUIDACION'
        FieldAlias = 'NRO_LIQUIDACION'
      end
      item
        FieldName = 'NUMEROCPBTE'
        FieldAlias = 'NUMEROCPBTE'
      end
      item
        FieldName = 'NOMBRE_CTABCO'
        FieldAlias = 'NOMBRE_CTABCO'
      end
      item
        FieldName = 'ARANCEL'
        FieldAlias = 'ARANCEL'
      end
      item
        FieldName = 'COSTO_FINANCIERO'
        FieldAlias = 'COSTO_FINANCIERO'
      end
      item
        FieldName = 'GANANCIAS'
        FieldAlias = 'GANANCIAS'
      end
      item
        FieldName = 'IIBB'
        FieldAlias = 'IIBB'
      end
      item
        FieldName = 'IVA'
        FieldAlias = 'IVA'
      end
      item
        FieldName = 'PRESENTACION_ADICIONAL'
        FieldAlias = 'PRESENTACION_ADICIONAL'
      end
      item
        FieldName = 'TOTAL_PRESENTACION'
        FieldAlias = 'TOTAL_PRESENTACION'
      end
      item
        FieldName = 'TOTAL_ACREDITADO'
        FieldAlias = 'TOTAL_ACREDITADO'
      end>
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    EmbedFontsIfProtected = False
    InteractiveFormsFontSubset = 'A-Z,a-z,0-9,#43-#47 '
    OpenAfterExport = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    Creator = 'FastReport'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    PdfA = False
    PDFStandard = psNone
    PDFVersion = pv17
    Left = 736
    Top = 536
  end
  object QLiquidaciones: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select cab.id, cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado' +
        ',cab.nro_liquidacion,cab.numerocpbte,cab.nombre_ctabco,'
      
        ' cab.arancel,cab.costo_financiero,cab.ganancias,cab.iibb,cab.iva' +
        ',cab.presentacion_adicional,cab.total_presentacion,cab.total_acr' +
        'editado from liquidacion_tc_cab cab'
      
        'where cab.fecha between :desde and :hasta order by cab.nro_liqui' +
        'dacion')
    Left = 184
    Top = 184
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
  end
  object QTotalIva: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.num' +
        'erocpbte,iva.neto,'
      
        'iva.codigo_gravamen,iva.detalle,iva.tasa, sum(iva.importe) as im' +
        'porte  from liquidacion_tc_iva iva'
      'left join liquidacion_tc_cab cab on cab.id=iva.id_liq'
      'where cab.fecha between :desde and :hasta'
      
        'group by cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.n' +
        'umerocpbte,iva.neto,'
      'iva.codigo_gravamen,iva.detalle,iva.tasa')
    Left = 136
    Top = 400
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QTotalIvaFECHA: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA'
      Origin = 'FECHA'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIvaFECHA_FISCAL: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA_FISCAL'
      Origin = 'FECHA_FISCAL'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIvaPERIODO_LIQUIDADO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'PERIODO_LIQUIDADO'
      Origin = 'PERIODO_LIQUIDADO'
      ProviderFlags = []
      ReadOnly = True
      Size = 6
    end
    object QTotalIvaNUMEROCPBTE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NUMEROCPBTE'
      Origin = 'NUMEROCPBTE'
      ProviderFlags = []
      ReadOnly = True
      Size = 13
    end
    object QTotalIvaNETO: TFloatField
      AutoGenerateValue = arDefault
      FieldName = 'NETO'
      Origin = 'NETO'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00'
    end
    object QTotalIvaCODIGO_GRAVAMEN: TIntegerField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Cod.'
      FieldName = 'CODIGO_GRAVAMEN'
      Origin = 'CODIGO_GRAVAMEN'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIvaDETALLE: TStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIvaTASA: TFloatField
      AutoGenerateValue = arDefault
      DisplayLabel = '%'
      FieldName = 'TASA'
      Origin = 'TASA'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00'
    end
    object QTotalIvaIMPORTE: TFloatField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object QRetenciones: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.num' +
        'erocpbte,'
      
        'm.fecha_retencion,m.nro_retencion, m.id_retencion,m.detalle,sum(' +
        'm.importe) as Importe from mov_retenciones m'
      
        'left join liquidacion_tc_cab cab on cab.id=m.id_cpbte and m.tipo' +
        'cpbte='#39'LQ'#39
      'where cab.fecha between :desde and :hasta'
      
        'group by cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.n' +
        'umerocpbte,m.fecha_retencion,m.nro_retencion, m.id_retencion,m.d' +
        'etalle')
    Left = 120
    Top = 576
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QRetencionesFECHA: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA'
      Origin = 'FECHA'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesFECHA_FISCAL: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA_FISCAL'
      Origin = 'FECHA_FISCAL'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesPERIODO_LIQUIDADO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'PERIODO_LIQUIDADO'
      Origin = 'PERIODO_LIQUIDADO'
      ProviderFlags = []
      ReadOnly = True
      Size = 6
    end
    object QRetencionesNUMEROCPBTE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NUMEROCPBTE'
      Origin = 'NUMEROCPBTE'
      ProviderFlags = []
      ReadOnly = True
      Size = 13
    end
    object QRetencionesFECHA_RETENCION: TSQLTimeStampField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Fecha Ret.'
      FieldName = 'FECHA_RETENCION'
      Origin = 'FECHA_RETENCION'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesNRO_RETENCION: TStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Nro.Ret'
      FieldName = 'NRO_RETENCION'
      Origin = 'NRO_RETENCION'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesID_RETENCION: TIntegerField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Id'
      FieldName = 'ID_RETENCION'
      Origin = 'ID_RETENCION'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesDETALLE: TStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      ReadOnly = True
    end
    object QRetencionesIMPORTE: TFloatField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00'
    end
  end
  object QTotalIIB: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.num' +
        'erocpbte,iibb.neto,'
      
        'iibb.codigo_percepcion,iibb.detalle,iibb.tasa, sum(iibb.importe)' +
        ' as importe  from liquidacion_tc_iibb iibb'
      'left join liquidacion_tc_cab cab on cab.id=iibb.id_liq'
      'where cab.fecha between :desde and :hasta'
      
        'group by cab.fecha,cab.fecha_fiscal, cab.periodo_liquidado,cab.n' +
        'umerocpbte,iibb.neto,'
      'iibb.codigo_percepcion,iibb.detalle,iibb.tasa')
    Left = 128
    Top = 512
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QTotalIIBFECHA: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA'
      Origin = 'FECHA'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIIBFECHA_FISCAL: TSQLTimeStampField
      AutoGenerateValue = arDefault
      FieldName = 'FECHA_FISCAL'
      Origin = 'FECHA_FISCAL'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIIBPERIODO_LIQUIDADO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'PERIODO_LIQUIDADO'
      Origin = 'PERIODO_LIQUIDADO'
      ProviderFlags = []
      ReadOnly = True
      Size = 6
    end
    object QTotalIIBNUMEROCPBTE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NUMEROCPBTE'
      Origin = 'NUMEROCPBTE'
      ProviderFlags = []
      ReadOnly = True
      Size = 13
    end
    object QTotalIIBNETO: TFloatField
      AutoGenerateValue = arDefault
      FieldName = 'NETO'
      Origin = 'NETO'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00'
    end
    object QTotalIIBCODIGO_PERCEPCION: TIntegerField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Cod.'
      FieldName = 'CODIGO_PERCEPCION'
      Origin = 'CODIGO_PERCEPCION'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIIBDETALLE: TStringField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      ReadOnly = True
    end
    object QTotalIIBTASA: TFloatField
      AutoGenerateValue = arDefault
      DisplayLabel = '%'
      FieldName = 'TASA'
      Origin = 'TASA'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = '0.00'
    end
    object QTotalIIBIMPORTE: TFloatField
      AutoGenerateValue = arDefault
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
end
