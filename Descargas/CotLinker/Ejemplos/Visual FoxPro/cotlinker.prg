...
cliente = CreateObject("CotLinker.ClienteRE")

    cliente.CUIT = 20939802593
    cliente.ModoProduccion = .F. && Cambiar a true para trabajar en producción
    cliente.Password = "clave" && Obtenerla segun las instrucciones en www.bitingenieria.com.ar/cotlinker
    cliente.NroPlanta = 0
    cliente.NroPuerta = 1
    cliente.Fecha = "20190326"
    cliente.NroSecuencial = 10
    cliente.UsarClienteNet = .F.

    cliente.Inicializar

cab = CreateObject("CotLinker.remito")
    ...
    cab.FECHA_EMISION = "20190326"
    cab.FECHA_SALIDA_TRANSPORTE = "20190326"
    ...
    cab.ORIGEN_DOMICILIO_NUMERO = 1217
...
det = CreateObject("CotLinker.producto")
    ...
    If Cliente.Resultado.TransactionOk Then
      MessageBox("Felicitaciones! COT obtenido: " + str(Cliente.Resultado.COT))
    Else
      MessageBox(Cliente.Resultado.Errores)
    EndIf
