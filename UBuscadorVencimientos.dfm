object FormBuscadorVencimientos: TFormBuscadorVencimientos
  Left = 388
  Top = 133
  Caption = 'Buscador de Vencimientos'
  ClientHeight = 579
  ClientWidth = 790
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  PopupMode = pmAuto
  Position = poOwnerFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  TextHeight = 13
  object Panel1: TPanel
    Left = 8
    Top = 8
    Width = 777
    Height = 489
    Color = clGradientInactiveCaption
    ParentBackground = False
    TabOrder = 0
    DesignSize = (
      777
      489)
    object Label1: TLabel
      Left = 24
      Top = 16
      Width = 223
      Height = 23
      Caption = 'Buscador Vencimientos'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -19
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object rgTipo: TRadioGroup
      Left = 461
      Top = 0
      Width = 218
      Height = 56
      Caption = 'Movimientos'
      Columns = 3
      ItemIndex = 1
      Items.Strings = (
        'Todos'
        'Sin Cumplir'
        'Cumplidos')
      TabOrder = 1
      OnClick = rgTipoClick
    end
    object btCancelar: TBitBtn
      Left = 604
      Top = 451
      Width = 75
      Height = 25
      Anchors = [akLeft, akBottom]
      Cancel = True
      Caption = 'Cancelar'
      Glyph.Data = {
        
        0000}
      ModalResult = 2
      NumGlyphs = 2
      TabOrder = 2
      OnClick = btCancelarClick
      ExplicitTop = 379
    end
    object btAceptar: TBitBtn
      Left = 685
      Top = 451
      Width = 75
      Height = 25
      Anchors = [akLeft, akBottom]
      Caption = 'Aceptar'
      Default = True
      Glyph.Data = {
        
        0000}
      ModalResult = 1
      NumGlyphs = 2
      TabOrder = 3
      OnClick = btAceptarClick
      ExplicitTop = 379
    end
    object dbgComprobantes: TJvDBGrid
      Left = 5
      Top = 63
      Width = 756
      Height = 376
      Anchors = [akLeft, akTop, akBottom]
      DataSource = DSBuscador
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnDblClick = dbgComprobantesDblClick
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'ID'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 39
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FECHA_INCIO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 82
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ASUNTO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 266
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CUOTA_NRO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 68
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IMPORTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 87
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FECHA_CUMPLIDO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 86
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'NROCPBTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 90
          Visible = True
        end>
    end
  end
  object DSPBuscador: TDataSetProvider
    DataSet = QBuscador
    Left = 384
    Top = 224
  end
  object CDSBUscador: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftFixedChar
        Name = 'finalizado'
        ParamType = ptInput
      end
      item
        DataType = ftFixedChar
        Name = 'finalizado'
        ParamType = ptInput
      end>
    ProviderName = 'DSPBuscador'
    Left = 456
    Top = 208
    object CDSBUscadorID: TIntegerField
      DisplayLabel = 'Id'
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSBUscadorFECHA_INCIO: TSQLTimeStampField
      Alignment = taCenter
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA_INCIO'
      Origin = 'FECHA_INCIO'
      Required = True
      DisplayFormat = 'dd/mm/yyyy'
    end
    object CDSBUscadorASUNTO: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'ASUNTO'
      Origin = 'ASUNTO'
      Size = 200
    end
    object CDSBUscadorNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      EditMask = 'a-0000-00000000;0'
      Size = 13
    end
    object CDSBUscadorFINALIZADO: TStringField
      Alignment = taCenter
      DisplayLabel = 'Final.'
      FieldName = 'FINALIZADO'
      Origin = 'FINALIZADO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSBUscadorFECHA_CUMPLIDO: TSQLTimeStampField
      Alignment = taCenter
      DisplayLabel = 'Cumplido'
      FieldName = 'FECHA_CUMPLIDO'
      Origin = 'FECHA_CUMPLIDO'
    end
    object CDSBUscadorCUOTA_NRO: TSmallintField
      Alignment = taCenter
      DisplayLabel = 'Nro.Cuota'
      FieldName = 'CUOTA_NRO'
      Origin = 'CUOTA_NRO'
      Required = True
    end
    object CDSBUscadorLOTE: TIntegerField
      DisplayLabel = 'Lote'
      FieldName = 'LOTE'
      Origin = 'LOTE'
      Required = True
    end
    object CDSBUscadorIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
  end
  object DSBuscador: TDataSource
    DataSet = CDSBUscador
    Left = 192
    Top = 192
  end
  object QBuscador: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select * from vencimientos where finalizado = :finalizado or :fi' +
        'nalizado = '#39'*'#39
      'order by  fecha_incio desc')
    Left = 288
    Top = 200
    ParamData = <
      item
        Position = 1
        Name = 'finalizado'
        DataType = ftFixedChar
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'finalizado'
        DataType = ftFixedChar
        ParamType = ptInput
      end>
  end
end
