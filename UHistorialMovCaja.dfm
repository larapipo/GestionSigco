inherited FormHistorialMovCaja: TFormHistorialMovCaja
  Caption = 'Historial Mov. de Cajas'
  ClientHeight = 719
  ClientWidth = 1014
  Position = poOwnerFormCenter
  StyleElements = [seFont, seClient, seBorder]
  OnResize = FormResize
  ExplicitWidth = 1032
  ExplicitHeight = 760
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1014
    Height = 666
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1014
    ExplicitHeight = 666
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 1014
      Height = 57
      Align = alTop
      BevelOuter = bvNone
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 0
      object Label2: TLabel
        Left = 7
        Top = 5
        Width = 60
        Height = 16
        Caption = 'Cta.Caja'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label1: TLabel
        Left = 227
        Top = 5
        Width = 47
        Height = 16
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label3: TLabel
        Left = 482
        Top = 5
        Width = 42
        Height = 16
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object dbcCtaCaja: TJvDBLookupCombo
        Left = 7
        Top = 24
        Width = 193
        Height = 21
        DisplayEmpty = 'Todas las Cajas'
        EmptyValue = '-1'
        LookupField = 'ID_CUENTA'
        LookupDisplay = 'NOMBRE'
        LookupSource = DSCtaCaja
        TabOrder = 0
        OnChange = dbcCtaCajaChange
      end
      object dbcDesde: TJvDBLookupCombo
        Left = 227
        Top = 24
        Width = 235
        Height = 21
        DropDownAlign = daRight
        DropDownWidth = 250
        DisplayAllFields = True
        LookupField = 'ID_CAJA'
        LookupDisplay = 'NUMEROCAJA;Fecha_INCIO;Fecha_Cierre'
        LookupSource = DSCajaDesde
        TabOrder = 1
      end
      object dbcHasta: TJvDBLookupCombo
        Left = 482
        Top = 24
        Width = 235
        Height = 21
        DropDownWidth = 250
        DisplayAllFields = True
        LookupField = 'ID_CAJA'
        LookupDisplay = 'NUMEROCAJA;FECHA_INCIO;FECHA_CIERRE'
        LookupSource = DSCajaHasta
        TabOrder = 2
        OnChange = dbcHastaChange
      end
    end
    object AdvPageControl1: TAdvPageControl
      AlignWithMargins = True
      Left = 3
      Top = 60
      Width = 1008
      Height = 603
      ActivePage = pagListado
      ActiveFont.Charset = DEFAULT_CHARSET
      ActiveFont.Color = clWindowText
      ActiveFont.Height = -11
      ActiveFont.Name = 'Tahoma'
      ActiveFont.Style = [fsBold]
      Align = alClient
      DoubleBuffered = True
      HotTrack = True
      ActiveColor = clGradientInactiveCaption
      ActiveColorTo = clWhite
      TabBackGroundColor = clBtnFace
      TabMargin.RightMargin = 0
      TabOverlap = 0
      TabStyle = tsDotNet
      Version = '2.1.3.1'
      PersistPagesState.Location = plRegistry
      PersistPagesState.Enabled = False
      TabOrder = 1
      object pagListado: TAdvTabSheet
        Caption = 'Listado'
        Color = clBtnFace
        ColorTo = clNone
        TabColor = clBtnFace
        TabColorTo = clNone
        DesignSize = (
          1000
          575)
        object dbgCabeceras: TDBGrid
          Left = 335
          Top = 3
          Width = 336
          Height = 120
          DataSource = DSCajaSaldosInicial
          DrawingStyle = gdsGradient
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'DESCRIPCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 197
              Visible = True
            end
            item
              Alignment = taLeftJustify
              Expanded = False
              FieldName = 'IMPORTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
        object dbgCajas: TDBGrid
          Left = 3
          Top = 7
          Width = 326
          Height = 569
          Anchors = [akLeft, akTop, akBottom]
          DataSource = DSCajas
          DrawingStyle = gdsGradient
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'ID_CAJA'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 46
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NUMEROCAJA'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHA_INCIO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 83
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHA_CIERRE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 92
              Visible = True
            end>
        end
        object dbgMovimientos: TDBGrid
          Left = 335
          Top = 127
          Width = 642
          Height = 300
          Anchors = [akLeft, akTop, akBottom]
          DataSource = DSMovCaja
          DrawingStyle = gdsGradient
          TabOrder = 2
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'FECHA'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 79
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DESCRIPCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 192
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MUESTRAFPAGO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DEBE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'HABER'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
        object dbgPies: TDBGrid
          Left = 335
          Top = 435
          Width = 336
          Height = 137
          Anchors = [akLeft, akBottom]
          DataSource = DSCajaSaldosFinales
          DrawingStyle = gdsGradient
          TabOrder = 3
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'DESCRIPCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 204
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'IMPORTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 97
              Visible = True
            end>
        end
      end
      object pagExcel: TAdvTabSheet
        Caption = 'Excel'
        Color = clBtnFace
        ColorTo = clNone
        TabColor = clBtnFace
        TabColorTo = clNone
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        DesignSize = (
          1000
          575)
        object AdvSpreadGrid: TAdvSpreadGrid
          Left = 0
          Top = 0
          Width = 1000
          Height = 526
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          ColCount = 5
          DrawingStyle = gdsClassic
          FixedCols = 0
          RowCount = 10
          FixedRows = 0
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goRowSizing, goColSizing, goEditing]
          ParentFont = False
          TabOrder = 0
          ActiveCellFont.Charset = DEFAULT_CHARSET
          ActiveCellFont.Color = clWindowText
          ActiveCellFont.Height = -11
          ActiveCellFont.Name = 'Tahoma'
          ActiveCellFont.Style = [fsBold]
          ActiveCellColor = clCream
          ControlLook.FixedGradientHoverFrom = clGray
          ControlLook.FixedGradientHoverTo = clWhite
          ControlLook.FixedGradientDownFrom = clGray
          ControlLook.FixedGradientDownTo = clSilver
          ControlLook.DropDownHeader.Font.Charset = DEFAULT_CHARSET
          ControlLook.DropDownHeader.Font.Color = clWindowText
          ControlLook.DropDownHeader.Font.Height = -11
          ControlLook.DropDownHeader.Font.Name = 'Tahoma'
          ControlLook.DropDownHeader.Font.Style = []
          ControlLook.DropDownHeader.Visible = True
          ControlLook.DropDownHeader.Buttons = <>
          ControlLook.DropDownFooter.Font.Charset = DEFAULT_CHARSET
          ControlLook.DropDownFooter.Font.Color = clWindowText
          ControlLook.DropDownFooter.Font.Height = -11
          ControlLook.DropDownFooter.Font.Name = 'Tahoma'
          ControlLook.DropDownFooter.Font.Style = []
          ControlLook.DropDownFooter.Visible = True
          ControlLook.DropDownFooter.Buttons = <>
          ControlLook.ToggleSwitch.BackgroundBorderWidth = 1.000000000000000000
          ControlLook.ToggleSwitch.ButtonBorderWidth = 1.000000000000000000
          ControlLook.ToggleSwitch.CaptionFont.Charset = DEFAULT_CHARSET
          ControlLook.ToggleSwitch.CaptionFont.Color = clWindowText
          ControlLook.ToggleSwitch.CaptionFont.Height = -12
          ControlLook.ToggleSwitch.CaptionFont.Name = 'Segoe UI'
          ControlLook.ToggleSwitch.CaptionFont.Style = []
          ControlLook.ToggleSwitch.Shadow = False
          Filter = <>
          FilterDropDown.Font.Charset = DEFAULT_CHARSET
          FilterDropDown.Font.Color = clWindowText
          FilterDropDown.Font.Height = -11
          FilterDropDown.Font.Name = 'Tahoma'
          FilterDropDown.Font.Style = []
          FilterDropDown.TextChecked = 'Checked'
          FilterDropDown.TextUnChecked = 'Unchecked'
          FilterDropDownClear = '(All)'
          FilterEdit.TypeNames.Strings = (
            'Starts with'
            'Ends with'
            'Contains'
            'Not contains'
            'Equal'
            'Not equal'
            'Larger than'
            'Smaller than'
            'Clear')
          FixedRowHeight = 22
          FixedFont.Charset = DEFAULT_CHARSET
          FixedFont.Color = clWindowText
          FixedFont.Height = -11
          FixedFont.Name = 'Tahoma'
          FixedFont.Style = [fsBold]
          FloatFormat = '%.2f'
          HoverButtons.Buttons = <>
          HTMLSettings.ImageFolder = 'images'
          HTMLSettings.ImageBaseName = 'img'
          PrintSettings.DateFormat = 'dd/mm/yyyy'
          PrintSettings.Font.Charset = DEFAULT_CHARSET
          PrintSettings.Font.Color = clWindowText
          PrintSettings.Font.Height = -11
          PrintSettings.Font.Name = 'Tahoma'
          PrintSettings.Font.Style = []
          PrintSettings.FixedFont.Charset = DEFAULT_CHARSET
          PrintSettings.FixedFont.Color = clWindowText
          PrintSettings.FixedFont.Height = -11
          PrintSettings.FixedFont.Name = 'Tahoma'
          PrintSettings.FixedFont.Style = []
          PrintSettings.HeaderFont.Charset = DEFAULT_CHARSET
          PrintSettings.HeaderFont.Color = clWindowText
          PrintSettings.HeaderFont.Height = -11
          PrintSettings.HeaderFont.Name = 'Tahoma'
          PrintSettings.HeaderFont.Style = []
          PrintSettings.FooterFont.Charset = DEFAULT_CHARSET
          PrintSettings.FooterFont.Color = clWindowText
          PrintSettings.FooterFont.Height = -11
          PrintSettings.FooterFont.Name = 'Tahoma'
          PrintSettings.FooterFont.Style = []
          PrintSettings.PageNumSep = '/'
          SearchFooter.FindNextCaption = 'Find &next'
          SearchFooter.FindPrevCaption = 'Find &previous'
          SearchFooter.Font.Charset = DEFAULT_CHARSET
          SearchFooter.Font.Color = clWindowText
          SearchFooter.Font.Height = -11
          SearchFooter.Font.Name = 'Tahoma'
          SearchFooter.Font.Style = []
          SearchFooter.HighLightCaption = 'Highlight'
          SearchFooter.HintClose = 'Close'
          SearchFooter.HintFindNext = 'Find next occurrence'
          SearchFooter.HintFindPrev = 'Find previous occurrence'
          SearchFooter.HintHighlight = 'Highlight occurrences'
          SearchFooter.MatchCaseCaption = 'Match case'
          SearchFooter.ResultFormat = '(%d of %d)'
          Version = '2.3.3.1'
          ErrorText = 'Error'
          CellFormat = '%.2f'
          ColWidths = (
            64
            64
            64
            64
            64)
          RowHeights = (
            22
            22
            22
            22
            22
            22
            22
            22
            22
            22)
        end
        object btVer: TBitBtn
          Left = 888
          Top = 547
          Width = 75
          Height = 25
          Anchors = [akLeft, akBottom]
          Caption = 'Ver'
          TabOrder = 1
          OnClick = btVerClick
        end
        object btExportarXLS: TBitBtn
          Left = 760
          Top = 546
          Width = 107
          Height = 25
          Anchors = [akLeft, akBottom]
          Caption = 'Exportar a Excel'
          TabOrder = 2
          OnClick = btExportarXLSClick
        end
      end
      object pagDetalles: TAdvTabSheet
        Caption = 'Detalles'
        Color = clBtnFace
        ColorTo = clNone
        TabColor = clBtnFace
        TabColorTo = clNone
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object cxGrid1: TcxGrid
          Left = 0
          Top = 0
          Width = 1000
          Height = 575
          Align = alClient
          TabOrder = 0
          object cxGrid1DBTableView1: TcxGridDBTableView
            DataController.DataSource = DSMovCaja
            DataController.Summary.FooterSummaryItems = <
              item
                Format = ',0.00'
                Kind = skSum
                FieldName = 'DEBE'
                Column = cxGrid1DBTableView1DEBE
              end
              item
                Format = ',0.00'
                Kind = skSum
                FieldName = 'HABER'
                Column = cxGrid1DBTableView1HABER
              end>
            OptionsView.Footer = True
            OptionsView.GroupByBox = False
            object cxGrid1DBTableView1FECHA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA'
              HeaderAlignmentHorz = taCenter
              Width = 105
            end
            object cxGrid1DBTableView1CLASE_COMPRO: TcxGridDBColumn
              DataBinding.FieldName = 'CLASE_COMPRO'
              HeaderAlignmentHorz = taCenter
              Width = 40
            end
            object cxGrid1DBTableView1NROCOMPROB: TcxGridDBColumn
              DataBinding.FieldName = 'NROCOMPROB'
              HeaderAlignmentHorz = taCenter
            end
            object cxGrid1DBTableView1MUESTRAFPAGO: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRAFPAGO'
              HeaderAlignmentHorz = taCenter
            end
            object cxGrid1DBTableView1DESCRIPCION: TcxGridDBColumn
              DataBinding.FieldName = 'DESCRIPCION'
              HeaderAlignmentHorz = taCenter
              Width = 128
            end
            object cxGrid1DBTableView1NOMBRE: TcxGridDBColumn
              DataBinding.FieldName = 'NOMBRE'
              HeaderAlignmentHorz = taCenter
            end
            object cxGrid1DBTableView1DEBE: TcxGridDBColumn
              DataBinding.FieldName = 'DEBE'
              HeaderAlignmentHorz = taCenter
            end
            object cxGrid1DBTableView1HABER: TcxGridDBColumn
              DataBinding.FieldName = 'HABER'
              HeaderAlignmentHorz = taCenter
            end
          end
          object cxGrid1Level1: TcxGridLevel
            GridView = cxGrid1DBTableView1
          end
        end
      end
      object pagGrupo: TAdvTabSheet
        Caption = 'Agrupado'
        Color = clBtnFace
        ColorTo = clNone
        TabColor = clBtnFace
        TabColorTo = clNone
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object cxDBPivotGrid1: TcxDBPivotGrid
          Left = 0
          Top = 0
          Width = 1000
          Height = 575
          Align = alClient
          DataSource = DSMovCaja
          Groups = <>
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          TabOrder = 0
          object cxDBPivotGrid1NOMBRECTACAJA: TcxDBPivotGridField
            Area = faColumn
            AreaIndex = 0
            DataBinding.FieldName = 'NOMBRECTACAJA'
            Visible = True
            UniqueName = 'Nombre Cta de Caja'
          end
          object cxDBPivotGrid1NROCAJA: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'NROCAJA'
            Visible = True
            UniqueName = 'Nro Caja'
          end
          object cxDBPivotGrid1FECHAINICIO: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'FECHAINICIO'
            Visible = True
            UniqueName = 'Fecha Incio'
          end
          object cxDBPivotGrid1FECHACIERRE: TcxDBPivotGridField
            AreaIndex = 2
            DataBinding.FieldName = 'FECHACIERRE'
            Visible = True
            UniqueName = 'Fecha Cierre'
          end
          object cxDBPivotGrid1FECHA: TcxDBPivotGridField
            AreaIndex = 3
            DataBinding.FieldName = 'FECHA'
            Visible = True
            UniqueName = 'Fecha'
          end
          object cxDBPivotGrid1TIPO_COMPROB: TcxDBPivotGridField
            AreaIndex = 4
            DataBinding.FieldName = 'TIPO_COMPROB'
            Visible = True
            UniqueName = 'Tp'
          end
          object cxDBPivotGrid1CLASE_COMPRO: TcxDBPivotGridField
            AreaIndex = 5
            DataBinding.FieldName = 'CLASE_COMPRO'
            Visible = True
            UniqueName = 'Cl'
          end
          object cxDBPivotGrid1NROCOMPROB: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 1
            DataBinding.FieldName = 'NROCOMPROB'
            Visible = True
            UniqueName = 'Nro.Comprob.'
          end
          object cxDBPivotGrid1DEBE: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'DEBE'
            Visible = True
            UniqueName = 'Debe'
          end
          object cxDBPivotGrid1HABER: TcxDBPivotGridField
            Area = faData
            AreaIndex = 1
            DataBinding.FieldName = 'HABER'
            Visible = True
            UniqueName = 'Haber'
          end
          object cxDBPivotGrid1MUESTRAFPAGO: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'MUESTRAFPAGO'
            Visible = True
            UniqueName = 'Forma Pago'
          end
          object cxDBPivotGrid1COTIZACION: TcxDBPivotGridField
            AreaIndex = 6
            DataBinding.FieldName = 'COTIZACION'
            Visible = True
            UniqueName = 'COTIZACION'
          end
          object cxDBPivotGrid1DESCRIPCION: TcxDBPivotGridField
            AreaIndex = 7
            DataBinding.FieldName = 'DESCRIPCION'
            Visible = True
            UniqueName = 'Descripcion'
          end
          object cxDBPivotGrid1NOMBRE: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 2
            DataBinding.FieldName = 'NOMBRE'
            Visible = True
            UniqueName = 'Nombre'
          end
          object cxDBPivotGrid1SIGNO: TcxDBPivotGridField
            AreaIndex = 8
            DataBinding.FieldName = 'SIGNO'
            Visible = True
            UniqueName = 'Sig'
          end
        end
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 666
    Width = 1014
    Align = alBottom
    ButtonHeight = 28
    ExplicitTop = 666
    ExplicitWidth = 1014
    inherited btConfirma: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btNuevo: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btCancelar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btModificar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited Pr: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBuscar: TBitBtn
      Height = 28
      ExplicitHeight = 28
    end
    inherited Ne: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBorrar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btSalir: TBitBtn
      Height = 28
      ExplicitHeight = 28
    end
    object ToolButton1: TToolButton
      Left = 557
      Top = 0
      Width = 28
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object chExcel: TCheckBox
      Left = 585
      Top = 0
      Width = 169
      Height = 28
      Caption = 'Armar Cuadro Excel al Buscar'
      TabOrder = 7
    end
  end
  inherited Panel1: TPanel
    Top = 696
    Width = 1014
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 696
    ExplicitWidth = 1014
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
    end
    inherited sbEstado: TStatusBar
      Width = 925
      ExplicitWidth = 925
    end
  end
  inherited ActionList1: TActionList
    Left = 224
    Top = 48
  end
  inherited DSBase: TDataSource
    Left = 384
    Top = 160
  end
  inherited ImageList1: TImageList
    Top = 16
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Top = 336
  end
  inherited QUltimoCodigo: TSQLQuery
    Top = 576
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Top = 320
  end
  inherited ComBuscadorBase: TComBuscador
    Left = 488
    Top = 248
  end
  object DSPCtaCaja: TDataSetProvider
    DataSet = QCtaCaja
    Left = 128
    Top = 176
  end
  object CDSCtaCaja: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPCtaCaja'
    Left = 192
    Top = 160
    object CDSCtaCajaID_CUENTA: TIntegerField
      FieldName = 'ID_CUENTA'
      Origin = 'ID_CUENTA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSCtaCajaID_TIPO_CTA: TIntegerField
      FieldName = 'ID_TIPO_CTA'
      Origin = 'ID_TIPO_CTA'
      Required = True
    end
    object CDSCtaCajaNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 25
    end
    object CDSCtaCajaNRO_CUENTA: TStringField
      FieldName = 'NRO_CUENTA'
      Origin = 'NRO_CUENTA'
      Size = 15
    end
    object CDSCtaCajaID_BANCO: TIntegerField
      FieldName = 'ID_BANCO'
      Origin = 'ID_BANCO'
    end
    object CDSCtaCajaCUIT: TStringField
      FieldName = 'CUIT'
      Origin = 'CUIT'
      Size = 13
    end
    object CDSCtaCajaRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      Origin = 'RAZONSOCIAL'
      Size = 35
    end
  end
  object DSCtaCaja: TDataSource
    DataSet = CDSCtaCaja
    Left = 264
    Top = 160
  end
  object DSPCajaSAldos: TDataSetProvider
    DataSet = QCajaSaldos
    Left = 616
    Top = 112
  end
  object CDSCajaSaldosInicial: TClientDataSet
    Aggregates = <>
    Filter = 'estado_caja=0'
    Filtered = True
    IndexFieldNames = 'ID_CAJA_CAB'
    Params = <
      item
        DataType = ftInteger
        Name = 'idcab'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCajaSAldos'
    Left = 752
    Top = 160
    object CDSCajaSaldosInicialID_CAJA_CAB: TIntegerField
      FieldName = 'ID_CAJA_CAB'
      Origin = 'ID_CAJA_CAB'
      Required = True
    end
    object CDSCajaSaldosInicialID_TPAGO: TIntegerField
      FieldName = 'ID_TPAGO'
      Origin = 'ID_TPAGO'
      Required = True
    end
    object CDSCajaSaldosInicialID_FPAGO: TIntegerField
      FieldName = 'ID_FPAGO'
      Origin = 'ID_FPAGO'
      Required = True
    end
    object CDSCajaSaldosInicialDESCRIPCION: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DESCRIPCION'
      Origin = 'DESCRIPCION'
      ProviderFlags = []
    end
    object CDSCajaSaldosInicialESTADO_CAJA: TSmallintField
      FieldName = 'ESTADO_CAJA'
      Origin = 'ESTADO_CAJA'
      Required = True
    end
    object CDSCajaSaldosInicialIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object CDSCajaSaldosFinales: TClientDataSet
    Aggregates = <>
    Filter = 'estado_caja=1'
    Filtered = True
    IndexFieldNames = 'ID_CAJA_CAB'
    Params = <
      item
        DataType = ftInteger
        Name = 'idcab'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCajaSAldos'
    Left = 888
    Top = 160
    object CDSCajaSaldosFinalesID_CAJA_CAB: TIntegerField
      FieldName = 'ID_CAJA_CAB'
      Origin = 'ID_CAJA_CAB'
      Required = True
    end
    object CDSCajaSaldosFinalesID_TPAGO: TIntegerField
      FieldName = 'ID_TPAGO'
      Origin = 'ID_TPAGO'
      Required = True
    end
    object CDSCajaSaldosFinalesID_FPAGO: TIntegerField
      FieldName = 'ID_FPAGO'
      Origin = 'ID_FPAGO'
      Required = True
    end
    object CDSCajaSaldosFinalesDESCRIPCION: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DESCRIPCION'
      Origin = 'DESCRIPCION'
      ProviderFlags = []
    end
    object CDSCajaSaldosFinalesESTADO_CAJA: TSmallintField
      FieldName = 'ESTADO_CAJA'
      Origin = 'ESTADO_CAJA'
      Required = True
    end
    object CDSCajaSaldosFinalesIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00;,0.00;-'
    end
  end
  object DSPCajas: TDataSetProvider
    DataSet = QCajas
    Left = 608
    Top = 32
  end
  object CDSCajas: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'idctacaja'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCajas'
    AfterScroll = CDSCajasAfterScroll
    Left = 696
    Top = 32
    object CDSCajasID_CAJA: TIntegerField
      DisplayLabel = 'Id'
      FieldName = 'ID_CAJA'
      Origin = 'ID_CAJA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSCajasID_CUENTA_CAJA: TIntegerField
      FieldName = 'ID_CUENTA_CAJA'
      Origin = 'ID_CUENTA_CAJA'
      Required = True
    end
    object CDSCajasFECHA_INCIO: TSQLTimeStampField
      DisplayLabel = 'F.Inicio'
      FieldName = 'FECHA_INCIO'
      Origin = 'FECHA_INCIO'
      Required = True
    end
    object CDSCajasFECHA_CIERRE: TSQLTimeStampField
      DisplayLabel = 'F.Cierre'
      FieldName = 'FECHA_CIERRE'
      Origin = 'FECHA_CIERRE'
    end
    object CDSCajasESTADO: TIntegerField
      FieldName = 'ESTADO'
      Origin = 'ESTADO'
      Required = True
    end
    object CDSCajasOBS: TMemoField
      FieldName = 'OBS'
      Origin = 'OBS'
      BlobType = ftMemo
    end
    object CDSCajasNUMEROCAJA: TIntegerField
      DisplayLabel = 'Nro.Caja'
      FieldName = 'NUMEROCAJA'
      Origin = 'NUMEROCAJA'
    end
    object CDSCajasSALDO_INCIAL: TFloatField
      FieldName = 'SALDO_INCIAL'
      Origin = 'SALDO_INCIAL'
    end
  end
  object DSPCajaDesde: TDataSetProvider
    DataSet = QCajaDesde
    Left = 104
    Top = 408
  end
  object CDSCajaDesde: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'cta_caja'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCajaDesde'
    Left = 160
    Top = 408
    object CDSCajaDesdeID_CAJA: TIntegerField
      FieldName = 'ID_CAJA'
      Origin = 'ID_CAJA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSCajaDesdeID_CUENTA_CAJA: TIntegerField
      FieldName = 'ID_CUENTA_CAJA'
      Origin = 'ID_CUENTA_CAJA'
      Required = True
    end
    object CDSCajaDesdeFECHA_INCIO: TSQLTimeStampField
      Alignment = taCenter
      DisplayWidth = 8
      FieldName = 'FECHA_INCIO'
      Origin = 'FECHA_INCIO'
      Required = True
    end
    object CDSCajaDesdeFECHA_CIERRE: TSQLTimeStampField
      Alignment = taCenter
      DisplayWidth = 8
      FieldName = 'FECHA_CIERRE'
      Origin = 'FECHA_CIERRE'
    end
    object CDSCajaDesdeESTADO: TIntegerField
      FieldName = 'ESTADO'
      Origin = 'ESTADO'
      Required = True
    end
    object CDSCajaDesdeOBS: TMemoField
      FieldName = 'OBS'
      Origin = 'OBS'
      BlobType = ftMemo
    end
    object CDSCajaDesdeNUMEROCAJA: TIntegerField
      DisplayWidth = 5
      FieldName = 'NUMEROCAJA'
      Origin = 'NUMEROCAJA'
    end
    object CDSCajaDesdeSALDO_INCIAL: TFloatField
      FieldName = 'SALDO_INCIAL'
      Origin = 'SALDO_INCIAL'
    end
  end
  object DSCajaDesde: TDataSource
    DataSet = CDSCajaDesde
    Left = 240
    Top = 416
  end
  object DSPCajaHasta: TDataSetProvider
    DataSet = QCajaHasta
    Left = 96
    Top = 464
  end
  object CDSCajaHasta: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'cta_caja'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCajaHasta'
    Left = 160
    Top = 472
    object CDSCajaHastaID_CAJA: TIntegerField
      FieldName = 'ID_CAJA'
      Origin = 'ID_CAJA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSCajaHastaID_CUENTA_CAJA: TIntegerField
      FieldName = 'ID_CUENTA_CAJA'
      Origin = 'ID_CUENTA_CAJA'
      Required = True
    end
    object CDSCajaHastaFECHA_INCIO: TSQLTimeStampField
      Alignment = taCenter
      DisplayWidth = 8
      FieldName = 'FECHA_INCIO'
      Origin = 'FECHA_INCIO'
      Required = True
    end
    object CDSCajaHastaFECHA_CIERRE: TSQLTimeStampField
      Alignment = taCenter
      DisplayWidth = 8
      FieldName = 'FECHA_CIERRE'
      Origin = 'FECHA_CIERRE'
    end
    object CDSCajaHastaESTADO: TIntegerField
      FieldName = 'ESTADO'
      Origin = 'ESTADO'
      Required = True
    end
    object CDSCajaHastaOBS: TMemoField
      FieldName = 'OBS'
      Origin = 'OBS'
      BlobType = ftMemo
    end
    object CDSCajaHastaNUMEROCAJA: TIntegerField
      DisplayWidth = 5
      FieldName = 'NUMEROCAJA'
      Origin = 'NUMEROCAJA'
    end
    object CDSCajaHastaSALDO_INCIAL: TFloatField
      FieldName = 'SALDO_INCIAL'
      Origin = 'SALDO_INCIAL'
    end
  end
  object DSCajaHasta: TDataSource
    DataSet = CDSCajaHasta
    Left = 240
    Top = 472
  end
  object DSPMovCaja: TDataSetProvider
    DataSet = QMovCaja
    Left = 568
    Top = 160
  end
  object CDSMovCaja: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'idcta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPMovCaja'
    Left = 640
    Top = 160
    object CDSMovCajaIDCAJA: TIntegerField
      FieldName = 'IDCAJA'
      Origin = 'IDCAJA'
    end
    object CDSMovCajaID_CTA_CAJA: TIntegerField
      FieldName = 'ID_CTA_CAJA'
      Origin = 'ID_CTA_CAJA'
    end
    object CDSMovCajaNOMBRECTACAJA: TStringField
      DisplayLabel = 'Nombre Cta de Caja'
      FieldName = 'NOMBRECTACAJA'
      Origin = 'NOMBRECTACAJA'
      Size = 25
    end
    object CDSMovCajaNROCAJA: TIntegerField
      DisplayLabel = 'Nro Caja'
      FieldName = 'NROCAJA'
      Origin = 'NROCAJA'
    end
    object CDSMovCajaFECHAINICIO: TSQLTimeStampField
      DisplayLabel = 'Fecha Incio'
      FieldName = 'FECHAINICIO'
      Origin = 'FECHAINICIO'
    end
    object CDSMovCajaFECHACIERRE: TSQLTimeStampField
      DisplayLabel = 'Fecha Cierre'
      FieldName = 'FECHACIERRE'
      Origin = 'FECHACIERRE'
    end
    object CDSMovCajaID_MOVIMIENTO: TIntegerField
      FieldName = 'ID_MOVIMIENTO'
      Origin = 'ID_MOVIMIENTO'
    end
    object CDSMovCajaFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSMovCajaTIPO_COMPROB: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPO_COMPROB'
      Origin = 'TIPO_COMPROB'
      Size = 2
    end
    object CDSMovCajaCLASE_COMPRO: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASE_COMPRO'
      Origin = 'CLASE_COMPRO'
      Size = 2
    end
    object CDSMovCajaNROCOMPROB: TStringField
      DisplayLabel = 'Nro.Comprob.'
      FieldName = 'NROCOMPROB'
      Origin = 'NROCOMPROB'
      Size = 13
    end
    object CDSMovCajaID_TPAGO: TIntegerField
      FieldName = 'ID_TPAGO'
      Origin = 'ID_TPAGO'
    end
    object CDSMovCajaID_FPAGO: TIntegerField
      FieldName = 'ID_FPAGO'
      Origin = 'ID_FPAGO'
    end
    object CDSMovCajaMUESTRAFPAGO: TStringField
      DisplayLabel = 'Forma Pago'
      FieldName = 'MUESTRAFPAGO'
      Origin = 'MUESTRAFPAGO'
    end
    object CDSMovCajaDESCRIPCION: TStringField
      DisplayLabel = 'Descripcion'
      FieldName = 'DESCRIPCION'
      Origin = 'DESCRIPCION'
      Size = 15
    end
    object CDSMovCajaNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 100
    end
    object CDSMovCajaSIGNO: TStringField
      DisplayLabel = 'Sig'
      FieldName = 'SIGNO'
      Origin = 'SIGNO'
      Size = 5
    end
    object CDSMovCajaDEBE: TFloatField
      DisplayLabel = 'Debe'
      FieldName = 'DEBE'
      Origin = 'DEBE'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSMovCajaHABER: TFloatField
      DisplayLabel = 'Haber'
      FieldName = 'HABER'
      Origin = 'HABER'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSMovCajaCOTIZACION: TFloatField
      DisplayLabel = 'Cotiz.'
      FieldName = 'COTIZACION'
      Origin = 'COTIZACION'
      DisplayFormat = ',0.000;-,0.000;-'
    end
    object CDSMovCajaUNIDADES: TFloatField
      FieldName = 'UNIDADES'
      Origin = 'UNIDADES'
    end
  end
  object DSCajas: TDataSource
    DataSet = CDSCajas
    Left = 776
    Top = 40
  end
  object DSCajaSaldosInicial: TDataSource
    DataSet = CDSCajaSaldosInicial
    Left = 760
    Top = 120
  end
  object DSCajaSaldosFinales: TDataSource
    DataSet = CDSCajaSaldosFinales
    Left = 888
    Top = 112
  end
  object DSMovCaja: TDataSource
    DataSet = CDSMovCaja
    Left = 768
    Top = 264
  end
  object SaveDialog: TSaveDialog
    Left = 580
    Top = 256
  end
  object QCajaDesde: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select cc.* from Caja_cab cc'
      'where cc.id_cuenta_caja=:cta_caja'
      'order by '
      'cc.id_caja,cc.numerocaja ')
    Left = 47
    Top = 404
    ParamData = <
      item
        Position = 1
        Name = 'cta_caja'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QCajaHasta: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select cc.* from Caja_cab cc'
      'where cc.id_cuenta_caja=:cta_caja'
      'order by'
      'cc.id_caja,cc.numerocaja ')
    Left = 39
    Top = 460
    ParamData = <
      item
        Position = 1
        Name = 'cta_caja'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QCtaCaja: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from cuenta_caja where id_tipo_cta=1 order by nombre')
    Left = 63
    Top = 172
  end
  object QCajas: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select c.* from caja_cab c '
      'where c.id_cuenta_caja=:idctacaja and '
      
        '         ((c.id_caja between :desde and :hasta) or (:desde = -1 ' +
        'and :hasta = -1 ))')
    Left = 528
    Top = 32
    ParamData = <
      item
        Position = 1
        Name = 'idctacaja'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'desde'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'hasta'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'desde'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'hasta'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QCajaSaldos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select cs.id_caja_cab,cs.id_tpago,cs.id_fpago,fp.descripcion,'
      '       cs.importe,cs.estado_caja from caja_saldos cs'
      '  left join tipopago tp on tp.id_tpago=cs.id_tpago'
      '  left join formapago fp on fp.id_fpago=cs.id_fpago'
      'where cs.id_caja_cab = :idcab')
    Left = 551
    Top = 108
    ParamData = <
      item
        Position = 1
        Name = 'idcab'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QMovCaja: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select * from detalle_mov_val_x_ctacaja (:idcta,:desde,:hasta,-1' +
        ')')
    Left = 511
    Top = 156
    ParamData = <
      item
        Position = 1
        Name = 'idcta'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'desde'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'hasta'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
end
