inherited FormListadoEntregasAcopios: TFormListadoEntregasAcopios
  Left = 98
  Top = 74
  Caption = 'Listado Entregas de Acopios'
  ClientHeight = 660
  ClientWidth = 1103
  ExplicitWidth = 1121
  ExplicitHeight = 701
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1103
    Height = 607
    Align = alClient
    ExplicitLeft = 0
    ExplicitTop = 0
    ExplicitWidth = 1103
    ExplicitHeight = 607
    object PageControl1: TPageControl
      Left = 0
      Top = 0
      Width = 1103
      Height = 607
      ActivePage = TabSheet2
      Align = alClient
      TabOrder = 0
      object TabSheet1: TTabSheet
        Caption = 'Listado'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 1093
        ExplicitHeight = 497
        DesignSize = (
          1095
          579)
        object cxGrid1: TcxGrid
          Left = 0
          Top = 0
          Width = 1095
          Height = 513
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 0
          ExplicitWidth = 1093
          ExplicitHeight = 503
          object cxGrid1DBTableView1: TcxGridDBTableView
            OnDblClick = cxGrid1DBTableView1DblClick
            Navigator.Buttons.CustomButtons = <>
            Navigator.Buttons.First.Visible = True
            Navigator.Buttons.PriorPage.Visible = True
            Navigator.Buttons.Prior.Visible = True
            Navigator.Buttons.Next.Visible = True
            Navigator.Buttons.NextPage.Visible = True
            Navigator.Buttons.Last.Visible = True
            Navigator.Buttons.Insert.Visible = True
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Visible = True
            Navigator.Buttons.Edit.Visible = True
            Navigator.Buttons.Post.Visible = True
            Navigator.Buttons.Cancel.Visible = True
            Navigator.Buttons.Refresh.Visible = True
            Navigator.Buttons.SaveBookmark.Visible = True
            Navigator.Buttons.GotoBookmark.Visible = True
            Navigator.Buttons.Filter.Visible = True
            ScrollbarAnnotations.CustomAnnotations = <>
            DataController.DataModeController.GridMode = True
            DataController.DataSource = DSAcopioCab
            DataController.Summary.DefaultGroupSummaryItems = <>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            OptionsData.Deleting = False
            OptionsData.Editing = False
            OptionsData.Inserting = False
            OptionsView.GroupByBox = False
            OptionsView.Indicator = True
            object cxGrid1DBTableView1FECHA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 65
            end
            object cxGrid1DBTableView1CLIENTE: TcxGridDBColumn
              DataBinding.FieldName = 'CLIENTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 75
            end
            object cxGrid1DBTableView1NOMBRE: TcxGridDBColumn
              DataBinding.FieldName = 'NOMBRE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 260
            end
            object cxGrid1DBTableView1NROCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'NROCPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 100
            end
          end
          object cxGrid1DBTableView2: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            Navigator.Buttons.First.Visible = True
            Navigator.Buttons.PriorPage.Visible = True
            Navigator.Buttons.Prior.Visible = True
            Navigator.Buttons.Next.Visible = True
            Navigator.Buttons.NextPage.Visible = True
            Navigator.Buttons.Last.Visible = True
            Navigator.Buttons.Insert.Visible = True
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Visible = True
            Navigator.Buttons.Edit.Visible = True
            Navigator.Buttons.Post.Visible = True
            Navigator.Buttons.Cancel.Visible = True
            Navigator.Buttons.Refresh.Visible = True
            Navigator.Buttons.SaveBookmark.Visible = True
            Navigator.Buttons.GotoBookmark.Visible = True
            Navigator.Buttons.Filter.Visible = True
            ScrollbarAnnotations.CustomAnnotations = <>
            DataController.DataModeController.GridMode = True
            DataController.DataSource = DSAcopioDet
            DataController.DetailKeyFieldNames = 'ID_CAB'
            DataController.KeyFieldNames = 'ID_CAB'
            DataController.MasterKeyFieldNames = 'ID'
            DataController.Summary.DefaultGroupSummaryItems = <>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            OptionsData.Deleting = False
            OptionsData.Editing = False
            OptionsData.Inserting = False
            OptionsView.Footer = True
            OptionsView.GroupByBox = False
            OptionsView.Indicator = True
            object cxGrid1DBTableView2CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 60
            end
            object cxGrid1DBTableView2DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 250
            end
            object cxGrid1DBTableView2CANTIDAD: TcxGridDBColumn
              DataBinding.FieldName = 'CANTIDAD'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 80
            end
            object cxGrid1DBTableView2ENTREGADO: TcxGridDBColumn
              DataBinding.FieldName = 'ENTREGADO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 80
            end
            object cxGrid1DBTableView2SALDO: TcxGridDBColumn
              DataBinding.FieldName = 'SALDO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 80
            end
          end
          object cxGrid1DBTableView3: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            Navigator.Buttons.First.Visible = True
            Navigator.Buttons.PriorPage.Visible = True
            Navigator.Buttons.Prior.Visible = True
            Navigator.Buttons.Next.Visible = True
            Navigator.Buttons.NextPage.Visible = True
            Navigator.Buttons.Last.Visible = True
            Navigator.Buttons.Insert.Visible = True
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Visible = True
            Navigator.Buttons.Edit.Visible = True
            Navigator.Buttons.Post.Visible = True
            Navigator.Buttons.Cancel.Visible = True
            Navigator.Buttons.Refresh.Visible = True
            Navigator.Buttons.SaveBookmark.Visible = True
            Navigator.Buttons.GotoBookmark.Visible = True
            Navigator.Buttons.Filter.Visible = True
            ScrollbarAnnotations.CustomAnnotations = <>
            DataController.DataSource = DSEntregas
            DataController.DetailKeyFieldNames = 'ID_MERCA_ACOPIO_DET'
            DataController.KeyFieldNames = 'ID_MERCA_ACOPIO_DET'
            DataController.MasterKeyFieldNames = 'ID'
            DataController.Summary.DefaultGroupSummaryItems = <>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            OptionsData.Deleting = False
            OptionsData.Editing = False
            OptionsData.Inserting = False
            OptionsView.Footer = True
            OptionsView.GroupByBox = False
            OptionsView.Indicator = True
            object cxGrid1DBTableView3FECHA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 92
            end
            object cxGrid1DBTableView3TIPOCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'TIPOCPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 37
            end
            object cxGrid1DBTableView3CLASECPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'CLASECPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 36
            end
            object cxGrid1DBTableView3NROCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'NROCPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Width = 82
            end
            object cxGrid1DBTableView3CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
            end
            object cxGrid1DBTableView3DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
            end
            object cxGrid1DBTableView3CANTIDAD: TcxGridDBColumn
              DataBinding.FieldName = 'CANTIDAD'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
            end
          end
          object cxGrid1Level1: TcxGridLevel
            GridView = cxGrid1DBTableView1
            object cxGrid1Level2: TcxGridLevel
              GridView = cxGrid1DBTableView2
              object cxGrid1Level3: TcxGridLevel
                GridView = cxGrid1DBTableView3
              end
            end
          end
        end
        object rgEstado: TRadioGroup
          Left = 655
          Top = 518
          Width = 159
          Height = 54
          Anchors = [akRight, akBottom]
          Caption = 'Estado'
          ItemIndex = 0
          Items.Strings = (
            'Los Finalizados'
            'Los Activos'
            'Todos')
          TabOrder = 1
          OnClick = BuscarExecute
          ExplicitLeft = 658
          ExplicitTop = 509
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Det.Entregas'
        ImageIndex = 1
        DesignSize = (
          1095
          579)
        object Label2: TLabel
          Left = 802
          Top = 555
          Width = 76
          Height = 13
          Cursor = crHandPoint
          Anchors = [akRight, akBottom]
          Caption = 'Mostrar Campos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsUnderline]
          ParentFont = False
          OnClick = Label2Click
          ExplicitLeft = 800
          ExplicitTop = 542
        end
        object Label3: TLabel
          Left = 897
          Top = 555
          Width = 76
          Height = 13
          Cursor = crHandPoint
          Anchors = [akRight, akBottom]
          Caption = 'Muestra Totales'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsUnderline]
          ParentFont = False
          OnClick = Label3Click
          ExplicitLeft = 895
          ExplicitTop = 542
        end
        object cxDBPivotGrid1: TcxDBPivotGrid
          Left = 0
          Top = 0
          Width = 1095
          Height = 542
          Customization.AvailableFieldsSorted = True
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          DataSource = DSDetEntrega
          Groups = <>
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          OptionsView.ColumnGrandTotalText = 'Totales'
          PopupMenu = PopupMenu1
          TabOrder = 0
          object cxDBPivotGrid1Field2: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'CLIENTE'
            Visible = True
            UniqueName = 'Cliente'
          end
          object cxDBPivotGrid1Field3: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'NOMBRE'
            Visible = True
            UniqueName = 'Nombre'
          end
          object cxDBPivotGrid1Field5: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 3
            DataBinding.FieldName = 'NROCPBTE'
            DisplayFormat = 'a-0000-00000000;0'
            PropertiesClassName = 'TcxMaskEditProperties'
            Properties.Alignment.Horz = taCenter
            Properties.EditMask = 'a-0000-00000000;0'
            Properties.ReadOnly = False
            MinWidth = 18
            Visible = True
            UniqueName = 'Cpbte Acopio'
          end
          object cxDBPivotGrid1Field8: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'ID_CAB'
            Visible = True
            UniqueName = 'Id RtoAcop.'
          end
          object cxDBPivotGrid1Field9: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 1
            DataBinding.FieldName = 'CODIGO'
            TotalsVisibility = tvNone
            Visible = True
            UniqueName = 'Codigo'
          end
          object cxDBPivotGrid1Field10: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 2
            DataBinding.FieldName = 'DETALLE'
            TotalsVisibility = tvNone
            Visible = True
            UniqueName = 'Detalle'
          end
          object cxDBPivotGrid1Field11: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 4
            DataBinding.FieldName = 'CANTIDAD'
            PropertiesClassName = 'TcxCurrencyEditProperties'
            Properties.Alignment.Horz = taRightJustify
            SummaryType = stMax
            TotalsVisibility = tvNone
            Visible = True
            Width = 87
            UniqueName = 'Cant.Acop.'
          end
          object cxDBPivotGrid1Field12: TcxDBPivotGridField
            AreaIndex = 2
            DataBinding.FieldName = 'FECHAENTREGA'
            Visible = True
            UniqueName = 'F.Entrega'
          end
          object cxDBPivotGrid1Field13: TcxDBPivotGridField
            AreaIndex = 3
            DataBinding.FieldName = 'TPCPBTENTREGA'
            Visible = True
            UniqueName = 'Tp.Entr.'
          end
          object cxDBPivotGrid1Field14: TcxDBPivotGridField
            AreaIndex = 4
            DataBinding.FieldName = 'CLCPBTEENTREGA'
            Visible = True
            UniqueName = 'Cl.Entre.'
          end
          object cxDBPivotGrid1Field15: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 5
            DataBinding.FieldName = 'NROCPBTEENTREGA'
            MinWidth = 10
            Visible = True
            UniqueName = 'Nro.Entrega'
          end
          object cxDBPivotGrid1Field16: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'ENTREGADO'
            PropertiesClassName = 'TcxCalcEditProperties'
            Visible = True
            Width = 85
            UniqueName = 'Entregado'
          end
          object cxDBPivotGrid1Field4: TcxDBPivotGridField
            Area = faData
            AreaIndex = 1
            DataBinding.FieldName = 'SALDO'
            PropertiesClassName = 'TcxCalcEditProperties'
            Visible = True
            Width = 94
            UniqueName = 'Saldo'
          end
        end
        object pnTotales: TJvPanel
          Left = 25
          Top = 324
          Width = 320
          Height = 127
          Color = clTeal
          Padding.Left = 10
          Padding.Top = 10
          Padding.Right = 10
          Padding.Bottom = 10
          ParentBackground = False
          TabOrder = 1
          Visible = False
          object Label1: TLabel
            Left = 11
            Top = 9
            Width = 113
            Height = 13
            Caption = 'Mostrar Los Totales'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clYellow
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object SpeedButton1: TSpeedButton
            Left = 288
            Top = 5
            Width = 23
            Height = 22
            Hint = 'Cerrar'
            Caption = 'X'
            Flat = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -16
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
            ParentShowHint = False
            ShowHint = True
            OnClick = SpeedButton1Click
          end
          object chOpSumas: TCheckListBox
            Left = 11
            Top = 28
            Width = 300
            Height = 77
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -13
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            Items.Strings = (
              'Total General Columnas'
              'Totales por Columnas'
              'Total General Linea'
              'Totales Por Linea')
            ParentFont = False
            TabOrder = 0
            OnClick = chOpSumasClick
          end
        end
        object BitBtn2: TBitBtn
          Left = 993
          Top = 548
          Width = 75
          Height = 28
          Anchors = [akRight, akBottom]
          Caption = 'Imprimir'
          Glyph.Data = {
            
            FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
          TabOrder = 2
          OnClick = BitBtn2Click
        end
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 607
    Width = 1103
    Align = alBottom
    ExplicitTop = 607
    ExplicitWidth = 1103
    inherited btConfirma: TBitBtn
      Visible = False
    end
    inherited btNuevo: TBitBtn
      Visible = False
    end
    inherited btCancelar: TBitBtn
      Visible = False
    end
    inherited btModificar: TBitBtn
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
    inherited btBorrar: TBitBtn
      Visible = False
    end
    object BitBtn1: TBitBtn
      Left = 557
      Top = 0
      Width = 75
      Height = 29
      Action = Imprimir
      Caption = 'Imprimir'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      TabOrder = 7
    end
  end
  inherited Panel1: TPanel
    Top = 637
    Width = 1103
    Align = alBottom
    ExplicitTop = 637
    ExplicitWidth = 1103
    inherited sbEstado: TStatusBar
      Width = 941
      OnClick = sbEstadoClick
      ExplicitWidth = 941
    end
  end
  inherited ActionList1: TActionList
    Left = 384
    Top = 80
    object Imprimir: TAction
      Caption = 'Imprimir'
      OnExecute = ImprimirExecute
    end
    object LimpiarFiltros: TAction
      Caption = 'Limpiar Filtros'
      OnExecute = LimpiarFiltrosExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 504
  end
  inherited ImageList1: TImageList
    Left = 248
    Top = 128
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 320
    Top = 328
  end
  inherited ComBuscadorBase: TComBuscador
    Top = 360
  end
  object DSPAcopiosCab: TDataSetProvider
    DataSet = QAcopiosCab
    Left = 536
    Top = 160
  end
  object DSPAcopiosDet: TDataSetProvider
    DataSet = QAcopiosDet
    Left = 544
    Top = 232
  end
  object DSPDetalleEntregas: TDataSetProvider
    DataSet = QDetalleEntregas
    Left = 544
    Top = 288
  end
  object CDSDetalleEntregas: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_MERCA_ACOPIO_DET'
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPDetalleEntregas'
    Left = 616
    Top = 288
    object CDSDetalleEntregasID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSDetalleEntregasID_CAB: TIntegerField
      FieldName = 'ID_CAB'
      Origin = 'ID_CAB'
    end
    object CDSDetalleEntregasFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSDetalleEntregasTIPOCPBTE: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSDetalleEntregasCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Required = True
      Size = 2
    end
    object CDSDetalleEntregasDEPOSITO: TIntegerField
      DisplayLabel = 'Depo.'
      FieldName = 'DEPOSITO'
      Origin = 'DEPOSITO'
      Required = True
    end
    object CDSDetalleEntregasCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 8
    end
    object CDSDetalleEntregasDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSDetalleEntregasID_MERCA_ACOPIO_DET: TIntegerField
      FieldName = 'ID_MERCA_ACOPIO_DET'
      Origin = 'ID_MERCA_ACOPIO_DET'
    end
    object CDSDetalleEntregasNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 8
    end
    object CDSDetalleEntregasCANTIDAD: TFloatField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object CDSAcopioCab: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'estado'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'estado'
        ParamType = ptInput
      end>
    ProviderName = 'DSPAcopiosCab'
    Left = 584
    Top = 160
    object CDSAcopioCabID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSAcopioCabFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSAcopioCabCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      Origin = 'CLIENTE'
      Required = True
      Size = 6
    end
    object CDSAcopioCabNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 35
    end
    object CDSAcopioCabDIRECCION: TStringField
      FieldName = 'DIRECCION'
      Origin = 'DIRECCION'
      Size = 35
    end
    object CDSAcopioCabCPOSTAL: TStringField
      FieldName = 'CPOSTAL'
      Origin = 'CPOSTAL'
      Size = 8
    end
    object CDSAcopioCabLOCALIDAD: TStringField
      FieldName = 'LOCALIDAD'
      Origin = 'LOCALIDAD'
      Size = 15
    end
    object CDSAcopioCabNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      EditMask = 'a-0000-00000000;0;_'
      Size = 13
    end
    object CDSAcopioCabID_FACTURA: TIntegerField
      FieldName = 'ID_FACTURA'
      Origin = 'ID_FACTURA'
      Required = True
    end
    object CDSAcopioCabCUMPLIDO: TStringField
      FieldName = 'CUMPLIDO'
      Origin = 'CUMPLIDO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object CDSAcopioDet: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_CAB'
    MasterFields = 'ID'
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPAcopiosDet'
    Left = 600
    Top = 232
    object CDSAcopioDetID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSAcopioDetID_CAB: TIntegerField
      FieldName = 'ID_CAB'
      Origin = 'ID_CAB'
      Required = True
    end
    object CDSAcopioDetCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 8
    end
    object CDSAcopioDetDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSAcopioDetFECHA_ULTIMAENTREGA: TSQLTimeStampField
      DisplayLabel = 'Ult.Entrega'
      FieldName = 'FECHA_ULTIMAENTREGA'
      Origin = 'FECHA_ULTIMAENTREGA'
    end
    object CDSAcopioDetCUMPLIDO: TStringField
      FieldName = 'CUMPLIDO'
      Origin = 'CUMPLIDO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSAcopioDetCANTIDAD: TFloatField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSAcopioDetID_FCDET: TIntegerField
      FieldName = 'ID_FCDET'
      Origin = 'ID_FCDET'
    end
    object CDSAcopioDetENTREGADO: TFloatField
      DisplayLabel = 'Entregado'
      FieldName = 'ENTREGADO'
      Origin = 'ENTREGADO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSAcopioDetSALDO: TFloatField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      Origin = 'SALDO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object DSAcopioCab: TDataSource
    DataSet = CDSAcopioCab
    Left = 656
    Top = 168
  end
  object DSAcopioDet: TDataSource
    DataSet = CDSAcopioDet
    Left = 680
    Top = 232
  end
  object DSEntregas: TDataSource
    DataSet = CDSDetalleEntregas
    Left = 704
    Top = 296
  end
  object frxReporte: TfrxReport
    Version = '6.8.5'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 42113.820337974540000000
    ReportOptions.LastChange = 42113.820337974540000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 400
    Top = 376
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frxDBCab: TfrxDBDataset
    UserName = 'frxDBCab'
    CloseDataSource = False
    FieldAliases.Strings = (
      'ID=ID'
      'FECHA=FECHA'
      'CLIENTE=CLIENTE'
      'NOMBRE=NOMBRE'
      'DIRECCION=DIRECCION'
      'CPOSTAL=CPOSTAL'
      'LOCALIDAD=LOCALIDAD'
      'NROCPBTE=NROCPBTE'
      'ID_FACTURA=ID_FACTURA'
      'CUMPLIDO=CUMPLIDO')
    DataSet = CDSAcopioCab
    BCDToCurrency = False
    Left = 496
    Top = 448
  end
  object frxDBDet: TfrxDBDataset
    UserName = 'frxDBDet'
    CloseDataSource = False
    FieldAliases.Strings = (
      'ID=ID'
      'ID_CAB=ID_CAB'
      'CODIGO=CODIGO'
      'DETALLE=DETALLE'
      'CANTIDAD=CANTIDAD'
      'ENTREGADO=ENTREGADO'
      'SALDO=SALDO'
      'FECHA_ULTIMAENTREGA=FECHA_ULTIMAENTREGA'
      'CUMPLIDO=CUMPLIDO')
    DataSource = DSAcopioDet
    BCDToCurrency = False
    Left = 352
    Top = 392
  end
  object frxDBEntregas: TfrxDBDataset
    UserName = 'frxDBEntregas'
    CloseDataSource = False
    FieldAliases.Strings = (
      'ID=ID'
      'ID_CAB=ID_CAB'
      'FECHA=FECHA'
      'TIPOCPBTE=TIPOCPBTE'
      'CLASECPBTE=CLASECPBTE'
      'DEPOSITO=DEPOSITO'
      'CODIGO=CODIGO'
      'DETALLE=DETALLE'
      'CANTIDAD=CANTIDAD'
      'ID_MERCA_ACOPIO_DET=ID_MERCA_ACOPIO_DET'
      'NROCPBTE=NROCPBTE')
    DataSource = DSEntregas
    BCDToCurrency = False
    Left = 312
    Top = 424
  end
  object DSPDetEntregas: TDataSetProvider
    DataSet = QDetEntregas
    Left = 424
    Top = 552
  end
  object CDSDetEntregas: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPDetEntregas'
    Left = 528
    Top = 548
    object CDSDetEntregasFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSDetEntregasCLIENTE: TStringField
      DisplayLabel = 'Cliente'
      FieldName = 'CLIENTE'
      Origin = 'CLIENTE'
      Size = 6
    end
    object CDSDetEntregasNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 35
    end
    object CDSDetEntregasDIRECCION: TStringField
      FieldName = 'DIRECCION'
      Origin = 'DIRECCION'
      Size = 35
    end
    object CDSDetEntregasNROCPBTE: TStringField
      DisplayLabel = 'Cpbte Acopio'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSDetEntregasID_FACTURA: TIntegerField
      Alignment = taLeftJustify
      FieldName = 'ID_FACTURA'
      Origin = 'ID_FACTURA'
    end
    object CDSDetEntregasID: TIntegerField
      Alignment = taLeftJustify
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSDetEntregasID_CAB: TIntegerField
      Alignment = taLeftJustify
      DisplayLabel = 'Id RtoAcop.'
      FieldName = 'ID_CAB'
      Origin = 'ID_CAB'
      Required = True
    end
    object CDSDetEntregasCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 8
    end
    object CDSDetEntregasDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSDetEntregasFECHAENTREGA: TSQLTimeStampField
      DisplayLabel = 'F.Entrega'
      FieldName = 'FECHAENTREGA'
      Origin = 'FECHAENTREGA'
    end
    object CDSDetEntregasTPCPBTENTREGA: TStringField
      DisplayLabel = 'Tp.Entr.'
      FieldName = 'TPCPBTENTREGA'
      Origin = 'TPCPBTENTREGA'
      Size = 2
    end
    object CDSDetEntregasCLCPBTEENTREGA: TStringField
      DisplayLabel = 'Cl.Entre.'
      FieldName = 'CLCPBTEENTREGA'
      Origin = 'CLCPBTEENTREGA'
      Size = 2
    end
    object CDSDetEntregasNROCPBTEENTREGA: TStringField
      DisplayLabel = 'Nro.Entrega'
      FieldName = 'NROCPBTEENTREGA'
      Origin = 'NROCPBTEENTREGA'
      Size = 8
    end
    object CDSDetEntregasCANTIDAD: TFloatField
      DisplayLabel = 'Cant.Acop.'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSDetEntregasENTREGADO: TFloatField
      DisplayLabel = 'Entregado'
      FieldName = 'ENTREGADO'
      Origin = 'ENTREGADO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSDetEntregasSALDO: TFloatField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      Origin = 'SALDO'
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object DSDetEntrega: TDataSource
    DataSet = CDSDetEntregas
    Left = 616
    Top = 553
  end
  object dxComponentPrinter1: TdxComponentPrinter
    CurrentLink = dxComponentPrinter1Link1
    Version = 0
    Left = 920
    Top = 576
    PixelsPerInch = 96
    object dxComponentPrinter1Link1: TcxPivotGridReportLink
      Active = True
      Component = cxDBPivotGrid1
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 5000
      PrinterPage.GrayShading = True
      PrinterPage.Header = 5000
      PrinterPage.Margins.Bottom = 5000
      PrinterPage.Margins.Left = 5000
      PrinterPage.Margins.Right = 5000
      PrinterPage.Margins.Top = 5000
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportDocument.CreationDate = 44647.579964143520000000
      OptionsOnEveryPage.FilterBar = False
      OptionsView.ColumnFields = False
      OptionsView.DataFields = False
      OptionsView.ExpandButtons = False
      OptionsView.Prefilter = pfvNever
      PixelsPerInch = 96
      BuiltInReportLink = True
    end
  end
  object QAcopiosCab: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select a.* from merca_enacopio_cab a where a.cumplido = :estado ' +
        'or :estado = '#39'*'#39
      '')
    Left = 468
    Top = 160
    ParamData = <
      item
        Position = 1
        Name = 'estado'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'estado'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QAcopiosDet: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select a.* from merca_enacopio_det a where a.id_cab=:id')
    Left = 476
    Top = 224
    ParamData = <
      item
        Position = 1
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QDetalleEntregas: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select d.* from rto_acopio_det d where d.id_merca_acopio_det=:id')
    Left = 476
    Top = 280
    ParamData = <
      item
        Position = 1
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QDetEntregas: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select c.fecha,c.cliente,c.nombre,c.direccion,c.nrocpbte,c.id_fa' +
        'ctura,'
      '       d.id,d.id_cab,d.codigo,d.detalle,d.cantidad,d.saldo,'
      '       rd.fecha as FechaEntrega,rd.tipocpbte as TpCpbtEntrega,'
      
        '       rd.clasecpbte as ClCpbteEntrega,rd.nrocpbte as NroCpbteEn' +
        'trega,'
      '       rd.cantidad as Entregado from merca_enacopio_det d'
      'left join merca_enacopio_cab c on c.id=d.id_cab'
      'left join rto_acopio_det rd on rd.id_merca_acopio_det=d.id')
    Left = 348
    Top = 552
  end
  object PopupMenu1: TPopupMenu
    Left = 788
    Top = 360
    object LimpiarFiltros1: TMenuItem
      Action = LimpiarFiltros
    end
  end
end
