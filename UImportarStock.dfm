inherited FormImportarStock: TFormImportarStock
  Left = 244
  Top = 95
  Caption = 'Importar Stock'
  ClientHeight = 686
  ClientWidth = 976
  StyleElements = [seFont, seClient, seBorder]
  OnResize = FormResize
  ExplicitWidth = 994
  ExplicitHeight = 727
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 976
    Height = 633
    Align = alClient
    Color = clMenuHighlight
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 976
    ExplicitHeight = 633
    object ScrollBox1: TScrollBox
      Left = 0
      Top = 0
      Width = 976
      Height = 568
      HorzScrollBar.ButtonSize = 20
      Align = alClient
      BevelInner = bvNone
      BevelOuter = bvNone
      Ctl3D = True
      ParentBackground = True
      ParentCtl3D = False
      TabOrder = 0
      UseWheelForScrolling = True
      object gbSubRubros: TGroupBox
        Left = 0
        Top = 106
        Width = 972
        Height = 53
        Align = alTop
        Caption = 'Sub Rubros de Art'#237'culos'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 0
        object Label6: TLabel
          Left = 143
          Top = 16
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label7: TLabel
          Left = 143
          Top = 32
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbSubRubroNuevos: TLabel
          Left = 281
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbSubRubroModificados: TLabel
          Left = 281
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label11: TLabel
          Left = 321
          Top = 24
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbSubRubroTotales: TLabel
          Left = 473
          Top = 24
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object BitBtn3: TBitBtn
          Left = 8
          Top = 15
          Width = 120
          Height = 33
          Action = ImportarSubRubros
          Caption = 'Importar '
          TabOrder = 0
        end
        object chbSubRubro: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbStock: TGroupBox
        Left = 0
        Top = 210
        Width = 972
        Height = 59
        Align = alTop
        Caption = 'Art'#237'culos'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 1
        object Label9: TLabel
          Left = 143
          Top = 14
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label10: TLabel
          Left = 143
          Top = 30
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbStockNuevos: TLabel
          Left = 281
          Top = 14
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbStockModificados: TLabel
          Left = 281
          Top = 30
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label14: TLabel
          Left = 321
          Top = 22
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbStockTotales: TLabel
          Left = 473
          Top = 24
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object btArticulos: TBitBtn
          Left = 8
          Top = 12
          Width = 120
          Height = 33
          Action = ImportarStock
          Caption = 'Importar '
          TabOrder = 0
        end
        object BitBtn8: TBitBtn
          Left = 662
          Top = 10
          Width = 120
          Height = 27
          Caption = 'Verifica Eliminados'
          TabOrder = 3
          OnClick = BitBtn8Click
        end
        object chSoloActualizarArticulos: TAdvOfficeCheckBox
          Left = 555
          Top = 11
          Width = 97
          Height = 17
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Solo Actualizar'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
        object chbXML: TAdvOfficeCheckBox
          Left = 555
          Top = 26
          Width = 97
          Height = 17
          TabOrder = 2
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Formato XML'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
        object chVerificaBorrados: TAdvOfficeCheckBox
          Left = 662
          Top = 37
          Width = 145
          Height = 17
          TabOrder = 4
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Verifica despues de Cargar'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
        object chbArticulos: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 5
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbRubro: TGroupBox
        Left = 0
        Top = 53
        Width = 972
        Height = 53
        Align = alTop
        Caption = 'Rubros de Art'#237'culos'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 2
        object Label4: TLabel
          Left = 143
          Top = 16
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label5: TLabel
          Left = 143
          Top = 32
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbRubroNuevos: TLabel
          Left = 283
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbRubroModificados: TLabel
          Left = 283
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label8: TLabel
          Left = 323
          Top = 24
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbRubroTotales: TLabel
          Left = 473
          Top = 24
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object BitBtn2: TBitBtn
          Left = 8
          Top = 14
          Width = 120
          Height = 33
          Action = ImportarRubros
          Caption = 'Importar '
          TabOrder = 0
        end
        object chbRubro: TAdvOfficeCheckBox
          Left = 832
          Top = 26
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbMarca: TGroupBox
        Left = 0
        Top = 159
        Width = 972
        Height = 51
        Align = alTop
        Caption = 'Marcas de Art'#237'culos'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 3
        object Label1: TLabel
          Left = 143
          Top = 15
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label2: TLabel
          Left = 143
          Top = 31
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbMarcaNuevos: TLabel
          Left = 286
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbMarcasModificados: TLabel
          Left = 286
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label3: TLabel
          Left = 326
          Top = 22
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbMarcasTotales: TLabel
          Left = 472
          Top = 22
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object BitBtn1: TBitBtn
          Left = 6
          Top = 12
          Width = 120
          Height = 33
          Action = ImportarMarca
          Caption = 'Importar '
          TabOrder = 0
        end
        object chbMarca: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbListaPrecio: TGroupBox
        Left = 0
        Top = 324
        Width = 972
        Height = 67
        Align = alTop
        Caption = 'Actualizacion precios'
        Color = clMenuHighlight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 4
        object lb1: TLabel
          Left = 143
          Top = 8
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lb2: TLabel
          Left = 143
          Top = 24
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbPreciosNuevos: TLabel
          Left = 314
          Top = 8
          Width = 7
          Height = 16
          Alignment = taRightJustify
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbPreciosModif: TLabel
          Left = 314
          Top = 27
          Width = 7
          Height = 16
          Alignment = taRightJustify
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lb3: TLabel
          Left = 140
          Top = 46
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbPreciosTotales: TLabel
          Left = 313
          Top = 47
          Width = 9
          Height = 16
          Alignment = taRightJustify
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label29: TLabel
          Left = 559
          Top = 12
          Width = 114
          Height = 13
          Caption = 'Lista de Precios Destino'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label23: TLabel
          Left = 345
          Top = 12
          Width = 109
          Height = 13
          Caption = 'Lista de Precios Origen'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn7: TBitBtn
          Left = 8
          Top = 15
          Width = 120
          Height = 33
          Action = ImportarPrecios
          Caption = 'ImportarPrecios'
          TabOrder = 0
        end
        object dbcListaPrecios: TJvDBLookupCombo
          Left = 557
          Top = 27
          Width = 196
          Height = 21
          DisplayAllFields = True
          DisplayEmpty = 'Sin Lista'
          EmptyValue = '-1'
          LookupField = 'ID'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSListaCab
          TabOrder = 1
        end
        object dbcListaOrigen: TJvDBLookupCombo
          Left = 344
          Top = 27
          Width = 186
          Height = 21
          DisplayAllFields = True
          DisplayEmpty = 'Sin Lista'
          EmptyValue = '-1'
          LookupField = 'ID'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSListaCabOrig
          TabOrder = 2
        end
        object chbLista: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbDescuentos: TGroupBox
        Left = 0
        Top = 445
        Width = 972
        Height = 55
        Align = alTop
        Caption = 'Actualizacion Descuentos'
        Color = clMenuHighlight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 5
        object Label26: TLabel
          Left = 145
          Top = 20
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbDescuentosNuevos: TLabel
          Left = 285
          Top = 22
          Width = 7
          Height = 16
          Alignment = taRightJustify
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn9: TBitBtn
          Left = 8
          Top = 13
          Width = 120
          Height = 33
          Action = ImportarDescuentos
          Caption = 'Importar Dstos.'
          TabOrder = 0
        end
        object chbDescuentos: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbDepositoStk: TGroupBox
        Left = 0
        Top = 269
        Width = 972
        Height = 55
        Align = alTop
        Caption = 'Depositos de Cada Art'#237'culo'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 6
        object Label12: TLabel
          Left = 143
          Top = 15
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label13: TLabel
          Left = 143
          Top = 32
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbDepNuevo: TLabel
          Left = 281
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbDepMod: TLabel
          Left = 281
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label17: TLabel
          Left = 321
          Top = 24
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbDepTotal: TLabel
          Left = 508
          Top = 24
          Width = 9
          Height = 16
          Alignment = taRightJustify
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label18: TLabel
          Left = 552
          Top = 9
          Width = 47
          Height = 13
          Caption = 'Depositos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object BitBtn5: TBitBtn
          Left = 8
          Top = 15
          Width = 120
          Height = 33
          Action = ImportarDep
          Caption = 'Importar'
          TabOrder = 0
        end
        object dbcDeposito: TJvDBLookupCombo
          Left = 552
          Top = 25
          Width = 180
          Height = 21
          DisplayEmpty = 'Todos '
          EmptyValue = '-1'
          LookupField = 'ID'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSListaDepositos
          TabOrder = 1
        end
        object chbDepStk: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbDepositos: TGroupBox
        Left = 0
        Top = 0
        Width = 972
        Height = 53
        Align = alTop
        Caption = 'Depositos de Mercaderias'
        Color = clMenuHighlight
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBackground = False
        ParentColor = False
        ParentFont = False
        TabOrder = 7
        object Label15: TLabel
          Left = 143
          Top = 16
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label16: TLabel
          Left = 143
          Top = 32
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbDepositosNuevos: TLabel
          Left = 286
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbDepositosModificados: TLabel
          Left = 286
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label20: TLabel
          Left = 326
          Top = 25
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbDepositosTotales: TLabel
          Left = 472
          Top = 25
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object BitBtn6: TBitBtn
          Left = 6
          Top = 15
          Width = 120
          Height = 33
          Action = ImportarDepositos
          Caption = 'Importar'
          TabOrder = 0
        end
        object chbDepositoMercaderia: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
      object gbCodigoBarra: TGroupBox
        Left = 0
        Top = 391
        Width = 972
        Height = 54
        Align = alTop
        Caption = 'Codigos de Barra'
        Color = clMenuHighlight
        ParentBackground = False
        ParentColor = False
        TabOrder = 8
        object Label19: TLabel
          Left = 143
          Top = 16
          Width = 76
          Height = 16
          Caption = 'Reg.Nuevos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label21: TLabel
          Left = 143
          Top = 32
          Width = 103
          Height = 16
          Caption = 'Reg.Modificados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbCodigosNuevos: TLabel
          Left = 286
          Top = 16
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lbCodigoModificados: TLabel
          Left = 286
          Top = 32
          Width = 7
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label24: TLabel
          Left = 326
          Top = 22
          Width = 125
          Height = 16
          Caption = 'Total Procesados'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clAqua
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object lbCodBarrasProc: TLabel
          Left = 508
          Top = 22
          Width = 9
          Height = 16
          Caption = '0'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object BitBtn4: TBitBtn
          Left = 8
          Top = 14
          Width = 120
          Height = 33
          Action = ImportarCodBarra
          Caption = 'Importar'
          TabOrder = 0
        end
        object chbCodigoBarra: TAdvOfficeCheckBox
          Left = 832
          Top = 18
          Width = 97
          Height = 17
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clYellow
          Font.Height = -13
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          TabStop = True
          Alignment = taRightJustify
          Caption = 'Habilitado'
          ReturnIsTab = False
          Version = '1.8.4.2'
        end
      end
    end
    object pnPie: TPanel
      Left = 0
      Top = 568
      Width = 976
      Height = 65
      Align = alBottom
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 1
      object Label25: TLabel
        Left = 503
        Top = 3
        Width = 68
        Height = 13
        Caption = 'Path Remoto..'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label22: TLabel
        Left = 7
        Top = 3
        Width = 111
        Height = 13
        Caption = 'Directorio de Busqueda'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object gEstado: TGauge
        Left = 1
        Top = 50
        Width = 974
        Height = 14
        Align = alBottom
        BorderStyle = bsNone
        ForeColor = clGreen
        Progress = 0
        ExplicitTop = 51
        ExplicitWidth = 990
      end
      object Button1: TButton
        Left = 877
        Top = 20
        Width = 75
        Height = 25
        Caption = 'Leer Datos'
        TabOrder = 0
        OnClick = Button1Click
      end
      object edDIrectorio: TJvDirectoryEdit
        Left = 11
        Top = 22
        Width = 479
        Height = 21
        TabOrder = 1
        Text = ''
      end
      object edRemoto: TEdit
        Left = 496
        Top = 22
        Width = 375
        Height = 21
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        TextHint = 'http://localhost:2001/tms/remotedb'
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 633
    Width = 976
    Align = alBottom
    ButtonHeight = 26
    ExplicitTop = 633
    ExplicitWidth = 976
    inherited btConfirma: TBitBtn
      Height = 26
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 26
    end
    inherited btNuevo: TBitBtn
      Height = 26
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 26
    end
    inherited btCancelar: TBitBtn
      Height = 26
      DoubleBuffered = True
      ExplicitHeight = 26
    end
    inherited btModificar: TBitBtn
      Height = 26
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 26
    end
    inherited Pr: TSpeedButton
      Height = 26
      Visible = False
      ExplicitHeight = 26
    end
    inherited btBuscar: TBitBtn
      Height = 26
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 26
    end
    inherited Ne: TSpeedButton
      Height = 26
      Visible = False
      ExplicitHeight = 26
    end
    inherited btBorrar: TBitBtn
      Height = 26
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 26
    end
    inherited btSalir: TBitBtn
      Height = 26
      DoubleBuffered = True
      ExplicitHeight = 26
    end
    object ToolButton1: TToolButton
      Left = 557
      Top = 0
      Width = 42
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object btNovedades: TButton
      Left = 599
      Top = 0
      Width = 116
      Height = 26
      Caption = 'Ver Novedades'
      TabOrder = 7
      OnClick = btNovedadesClick
    end
  end
  inherited Panel1: TPanel
    Top = 663
    Width = 976
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 663
    ExplicitWidth = 976
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
      Visible = False
    end
    inherited sbEstado: TStatusBar
      Width = 887
      SimplePanel = True
      ExplicitWidth = 887
    end
  end
  inherited ActionList1: TActionList
    Left = 672
    Top = 128
    object ImportarMarca: TAction
      Caption = 'Importar Marca'
      OnExecute = ImportarMarcaExecute
    end
    object ImportarRubros: TAction
      Caption = 'Importar Rubros'
      OnExecute = ImportarRubrosExecute
    end
    object ImportarSubRubros: TAction
      Caption = 'Importar Sub-Rubros'
      OnExecute = ImportarSubRubrosExecute
    end
    object ImportarStock: TAction
      Caption = 'Importar Stock'
      OnExecute = ImportarStockExecute
    end
    object ImportarDep: TAction
      Caption = 'Importar'
      OnExecute = ImportarDepExecute
    end
    object ImportarDepositos: TAction
      Caption = 'Importar'
      OnExecute = ImportarDepositosExecute
    end
    object ImportarCodBarra: TAction
      Caption = 'Importar'
      OnExecute = ImportarCodBarraExecute
    end
    object ImportarPrecios: TAction
      Caption = 'ImportarPrecios'
      OnExecute = ImportarPreciosExecute
    end
    object ImportarDescuentos: TAction
      Caption = 'Importar Dstos.'
      OnExecute = ImportarDescuentosExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 688
    Top = 32
  end
  inherited ImageList1: TImageList
    Top = 40
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Left = 736
    Top = 112
  end
  inherited QUltimoCodigo: TSQLQuery
    Top = 688
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 648
    Top = 24
  end
  inherited ComBuscadorBase: TComBuscador
    Left = 672
    Top = 96
  end
  inherited QBrowse2: TFDQuery
    Left = 24
    Top = 512
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 88
    Top = 544
  end
  object CDSDepXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 336
    Top = 129
  end
  object CDSMarcaXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 336
    Top = 62
  end
  object CDSRubroXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 328
    Top = 419
  end
  object CDSSubRubroXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 312
    Top = 192
  end
  object CDSStockXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 328
    Top = 304
  end
  object CDSDepositosXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 328
    Top = 361
  end
  object DSListaDepositos: TDataSource
    DataSet = QListaDepositos
    Left = 624
    Top = 632
  end
  object jvHiloArticulos: TJvThread
    Exclusive = False
    MaxCount = 0
    RunOnCreate = True
    FreeOnTerminate = True
    OnExecute = jvHiloArticulosExecute
    Left = 672
    Top = 312
  end
  object jvHiloDepositos: TJvThread
    Exclusive = False
    MaxCount = 0
    RunOnCreate = True
    FreeOnTerminate = True
    OnExecute = jvHiloDepositosExecute
    Left = 664
    Top = 368
  end
  object QListaDepositos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from depositos order by nombre')
    Left = 616
    Top = 688
    object QListaDepositosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QListaDepositosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 25
    end
    object QListaDepositosSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
      Required = True
    end
    object QListaDepositosCENTRALIZA_STOCK: TStringField
      FieldName = 'CENTRALIZA_STOCK'
      Origin = 'CENTRALIZA_STOCK'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object CDSCodgioBarraXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 520
    Top = 593
  end
  object CDSListaPreciosXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 336
    Top = 249
  end
  object DSListaCab: TDataSource
    DataSet = QListaCab
    Left = 720
    Top = 632
  end
  object QListaCab: TFDQuery
    CachedUpdates = True
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select l.* from listaprecioespecialcab l ORDER BY L.ID'
      ''
      '')
    Left = 713
    Top = 689
    object QListaCabID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QListaCabNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 50
    end
  end
  object CDSListasPrecioCabXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 376
    Top = 625
  end
  object DSListaCabOrig: TDataSource
    DataSet = CDSListasPrecioCabXML
    Left = 384
    Top = 688
  end
  object CDSDescuentoXML: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 616
    Top = 440
  end
  object DataSource1: TDataSource
    DataSet = DatosImportarStockRemoto.CDSDescuentos
    Left = 848
    Top = 664
  end
end
