inherited FormlistadoVtaPorVendedor: TFormlistadoVtaPorVendedor
  Left = 201
  Top = 76
  Caption = 'Listado de Vta Totales Por Vendedores'
  ClientHeight = 640
  ClientWidth = 970
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 988
  ExplicitHeight = 681
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 970
    Height = 587
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 970
    ExplicitHeight = 587
    object PageControl1: TPageControl
      Left = 0
      Top = 73
      Width = 970
      Height = 514
      ActivePage = TabSheet3
      Align = alClient
      TabOrder = 0
      object TabSheet1: TTabSheet
        Caption = 'General'
        object dbgMovimientos: TJvDBGrid
          Left = 0
          Top = 0
          Width = 962
          Height = 445
          Align = alClient
          DataSource = DSVtas
          FixedColor = clMoneyGreen
          Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDblClick = dbgMovimientosDblClick
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Expanded = False
              FieldName = 'FECHAVTA'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 100
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHAVTO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 99
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TIPOCPBTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 31
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NROCPBTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 103
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'RAZONSOCIAL'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 357
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TOTALNETOREC'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 101
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TOTALRECALCULADO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 101
              Visible = True
            end>
        end
        object pnPieGral: TPanel
          Left = 0
          Top = 445
          Width = 962
          Height = 41
          Align = alBottom
          Color = clMenuHighlight
          ParentBackground = False
          TabOrder = 1
          object JvDBStatusLabel1: TJvDBStatusLabel
            Left = 9
            Top = 10
            Width = 96
            Height = 13
            DataSetName = ' '
            DataSource = DSVtas
            Captions.Strings = (
              'Archivos Cerrados'
              'Browse'
              'Edicion'
              'Agregando')
            Style = lsRecordNo
            GlyphAlign = glGlyphRight
            Color = clBtnFace
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            ParentColor = False
            ParentFont = False
            ShadowColor = clInfoBk
            Transparent = True
            Visible = False
          end
          object edNeto: TJvCalcEdit
            Left = 649
            Top = 8
            Width = 105
            Height = 21
            DisplayFormat = ',0.00'
            ShowButton = False
            TabOrder = 0
            DecimalPlacesAlwaysShown = False
          end
          object edTotal: TJvCalcEdit
            Left = 760
            Top = 8
            Width = 105
            Height = 21
            DisplayFormat = ',0.00'
            ShowButton = False
            TabOrder = 1
            DecimalPlacesAlwaysShown = False
          end
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Cruzados'
        ImageIndex = 1
        object cxDBPivotGrid1: TcxDBPivotGrid
          Left = 0
          Top = 0
          Width = 962
          Height = 454
          Align = alClient
          DataSource = DSVtas
          Groups = <>
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          OptionsView.ColumnGrandTotalText = 'Total'
          OptionsView.RowGrandTotalText = 'Total'
          PopupMenu = PopupMenu1
          TabOrder = 0
          OnMouseDown = cxDBPivotGrid1MouseDown
          OnMouseMove = cxDBPivotGrid1MouseMove
          OnMouseUp = cxDBPivotGrid1MouseUp
          object cxDBPivotGrid1Field1: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'FECHAVTA'
            Visible = True
            UniqueName = 'F.Vta'
          end
          object cxDBPivotGrid1Field2: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'FECHAVTO'
            Visible = True
            UniqueName = 'F.Vto'
          end
          object cxDBPivotGrid1Field3: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'MUESTRAVENDEDOR'
            Visible = True
            UniqueName = 'Nomb.Vendedor'
          end
          object cxDBPivotGrid1Field4: TcxDBPivotGridField
            AreaIndex = 2
            DataBinding.FieldName = 'MUESTRASUCURSAL'
            Visible = True
            Width = 200
            UniqueName = 'Suc.'
          end
          object cxDBPivotGrid1Field5: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'TOTALNETOREC'
            DisplayFormat = ',0.00'
            Visible = True
            UniqueName = 'N.Sin Iva'
          end
          object cxDBPivotGrid1Field6: TcxDBPivotGridField
            AreaIndex = 3
            DataBinding.FieldName = 'TOTALRECALCULADO'
            DisplayFormat = ',0.00'
            Visible = True
            UniqueName = 'Total'
          end
          object cxDBPivotGrid1Field7: TcxDBPivotGridField
            AreaIndex = 5
            DataBinding.FieldName = 'RAZONSOCIAL'
            Visible = True
            UniqueName = 'Razon Social'
          end
          object cxDBPivotGrid1Field8: TcxDBPivotGridField
            AreaIndex = 8
            DataBinding.FieldName = 'TIPOCPBTE'
            Visible = True
            UniqueName = 'Tp'
          end
          object cxDBPivotGrid1Field9: TcxDBPivotGridField
            AreaIndex = 10
            DataBinding.FieldName = 'ZONA'
            Visible = True
            UniqueName = 'Zona'
          end
          object cxDBPivotGrid1Field10: TcxDBPivotGridField
            Area = faColumn
            AreaIndex = 0
            DataBinding.FieldName = 'MUESTRAMES'
            Visible = True
            Width = 150
            UniqueName = 'Mes'
          end
          object cxDBPivotGrid1Field11: TcxDBPivotGridField
            AreaIndex = 9
            DataBinding.FieldName = 'ANIO'
            Visible = True
            UniqueName = 'A'#241'o'
          end
          object cxDBPivotGrid1Field12: TcxDBPivotGridField
            AreaIndex = 7
            DataBinding.FieldName = 'CLASECPBTE'
            Visible = True
            UniqueName = 'Cl'
          end
          object cxDBPivotGrid1Field13: TcxDBPivotGridField
            AreaIndex = 6
            DataBinding.FieldName = 'MUESTRACATEGORIA'
            Visible = True
            UniqueName = 'Categoria'
          end
          object cxDBPivotGrid1Field14: TcxDBPivotGridField
            AreaIndex = 4
            DataBinding.FieldName = 'CODIGO'
            Visible = True
            UniqueName = 'Cod.Cli.'
          end
          object cxDBPivotGrid1Field15: TcxDBPivotGridField
            AreaIndex = 11
            DataBinding.FieldName = 'FECHA_HORA'
            Visible = True
            UniqueName = 'Fecha/Hora'
          end
          object cxDBPivotGrid1Field16: TcxDBPivotGridField
            AreaIndex = 12
            DataBinding.FieldName = 'MUESTRACOBRADOR'
            Visible = True
            UniqueName = 'Cobrador'
          end
          object cxDBPivotGrid1Field17: TcxDBPivotGridField
            AreaIndex = 13
            DataBinding.FieldName = 'NROCPBTE'
            Visible = True
            UniqueName = 'Nro.Cpbte'
          end
        end
        object pnPieCruzado: TPanel
          Left = 0
          Top = 454
          Width = 962
          Height = 32
          Align = alBottom
          TabOrder = 1
          DesignSize = (
            962
            32)
          object Label4: TLabel
            Left = 9
            Top = 13
            Width = 76
            Height = 13
            Cursor = crHandPoint
            Caption = 'Mostrar Campos'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsUnderline]
            ParentFont = False
            OnClick = Label4Click
          end
          object BitBtn2: TBitBtn
            Left = 818
            Top = 4
            Width = 70
            Height = 22
            Anchors = [akTop, akRight]
            Caption = 'Excel'
            Glyph.Data = {
              
              00FFFF00FFFF00FFFF00FF000000000000000000FF00FF000000}
            TabOrder = 0
            OnClick = BitBtn2Click
          end
        end
      end
      object TabSheet3: TTabSheet
        Caption = 'Detalles'
        ImageIndex = 2
        object cxDBPivotGrid2: TcxDBPivotGrid
          Left = 0
          Top = 0
          Width = 962
          Height = 453
          Align = alClient
          DataSource = DSVtasDet
          Groups = <>
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          OptionsView.ColumnGrandTotalText = 'Total'
          OptionsView.RowGrandTotalText = 'Total'
          OptionsView.RowGrandTotalWidth = 119
          PopupMenu = PopupMenu1
          TabOrder = 0
          object cxDBPivotGridField1: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'FECHAVTA'
            Visible = True
            UniqueName = 'Fecha Vta.'
          end
          object cxDBPivotGridField2: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'FECHAVTO'
            Visible = True
            UniqueName = 'Fecha Vto.'
          end
          object cxDBPivotGridField3: TcxDBPivotGridField
            AreaIndex = 14
            DataBinding.FieldName = 'MUESTRAVENDEDOR'
            Visible = True
            UniqueName = 'Vendedor'
          end
          object cxDBPivotGridField4: TcxDBPivotGridField
            AreaIndex = 2
            DataBinding.FieldName = 'MUESTRASUCURSAL'
            Visible = True
            Width = 200
            UniqueName = 'Sucursal'
          end
          object cxDBPivotGridField5: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'UNITARIOREC'
            Visible = True
            UniqueName = 'Unitario Neto'
          end
          object cxDBPivotGridField6: TcxDBPivotGridField
            AreaIndex = 3
            DataBinding.FieldName = 'TOTALRECALCULADO'
            Visible = True
            UniqueName = 'Total Neto'
          end
          object cxDBPivotGridField7: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'DETALLE'
            Visible = True
            UniqueName = 'Detalle'
          end
          object cxDBPivotGridField8: TcxDBPivotGridField
            AreaIndex = 7
            DataBinding.FieldName = 'TIPOCPBTE'
            Visible = True
            UniqueName = 'Tp.Cpbte'
          end
          object cxDBPivotGridField9: TcxDBPivotGridField
            AreaIndex = 9
            DataBinding.FieldName = 'ZONA'
            Visible = True
            UniqueName = 'Zona'
          end
          object cxDBPivotGridField10: TcxDBPivotGridField
            Area = faColumn
            AreaIndex = 0
            DataBinding.FieldName = 'MUESTRAMES'
            Visible = True
            Width = 150
            UniqueName = 'Mes'
          end
          object cxDBPivotGridField11: TcxDBPivotGridField
            AreaIndex = 8
            DataBinding.FieldName = 'ANIO'
            Visible = True
            UniqueName = 'A'#241'o'
          end
          object cxDBPivotGridField12: TcxDBPivotGridField
            AreaIndex = 6
            DataBinding.FieldName = 'CLASECPBTE'
            Visible = True
            UniqueName = 'Cl.Cpbte.'
          end
          object cxDBPivotGridField13: TcxDBPivotGridField
            AreaIndex = 5
            DataBinding.FieldName = 'MUESTRACATEGORIA'
            Visible = True
            UniqueName = 'Categoria'
          end
          object cxDBPivotGridField14: TcxDBPivotGridField
            AreaIndex = 4
            DataBinding.FieldName = 'CODIGOARTICULO'
            Visible = True
            UniqueName = 'Cod.Artic.'
          end
          object cxDBPivotGridField15: TcxDBPivotGridField
            AreaIndex = 10
            DataBinding.FieldName = 'FECHA_HORA'
            Visible = True
            UniqueName = 'Fec.Hora'
          end
          object cxDBPivotGrid2Field1: TcxDBPivotGridField
            AreaIndex = 11
            DataBinding.FieldName = 'UNITARIO_FINAL_REC'
            Visible = True
            UniqueName = 'Unitario Final'
          end
          object cxDBPivotGrid2Field2: TcxDBPivotGridField
            AreaIndex = 12
            DataBinding.FieldName = 'TOTALFINAL_REC'
            Visible = True
            UniqueName = 'Total Final'
          end
          object cxDBPivotGrid2Field3: TcxDBPivotGridField
            AreaIndex = 13
            DataBinding.FieldName = 'HORA'
            Visible = True
            UniqueName = 'Hora'
          end
          object cxDBPivotGrid2Field4: TcxDBPivotGridField
            AreaIndex = 15
            IsCaptionAssigned = True
            Caption = 'Costo Total'
            DataBinding.FieldName = 'COSTO_TOTAL_REC'
            Visible = True
            UniqueName = 'Costo Total'
          end
          object cxDBPivotGrid2Field5: TcxDBPivotGridField
            AreaIndex = 16
            DataBinding.FieldName = 'COSTO_UNITARIO_REC'
            Visible = True
            UniqueName = 'Costo Unitario'
          end
          object cxDBPivotGrid2Field6: TcxDBPivotGridField
            AreaIndex = 17
            DataBinding.FieldName = 'COSTO_UNI_MASIVA_REC'
            Visible = True
            UniqueName = 'Costo Unit.Final'
          end
          object cxDBPivotGrid2Field7: TcxDBPivotGridField
            AreaIndex = 18
            DataBinding.FieldName = 'COSTO_TOTAL_MAIVA_REC'
            Visible = True
            UniqueName = 'Costo Total Final'
          end
          object cxDBPivotGrid2Field8: TcxDBPivotGridField
            AreaIndex = 19
            DataBinding.FieldName = 'DIA'
            Visible = True
            UniqueName = 'Dia'
          end
          object cxDBPivotGrid2Field9: TcxDBPivotGridField
            AreaIndex = 20
            DataBinding.FieldName = 'MUESTRACOBRADOR'
            Visible = True
            UniqueName = 'Cobrador'
          end
          object cxDBPivotGrid2Field10: TcxDBPivotGridField
            AreaIndex = 21
            DataBinding.FieldName = 'TOTALCPBTE_REC'
            Visible = True
            UniqueName = 'Total  Cpbte '
          end
          object cxDBPivotGrid2Field11: TcxDBPivotGridField
            AreaIndex = 22
            DataBinding.FieldName = 'TOTALCPBTENETO_REC'
            Visible = True
            UniqueName = 'Total Neto Cpbte'
          end
        end
        object pnPieDetalles: TPanel
          Left = 0
          Top = 453
          Width = 962
          Height = 33
          Align = alBottom
          TabOrder = 1
          object Label5: TLabel
            Left = 12
            Top = 14
            Width = 76
            Height = 13
            Cursor = crHandPoint
            Caption = 'Mostrar Campos'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlue
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsUnderline]
            ParentFont = False
            OnClick = Label5Click
          end
          object BitBtn3: TBitBtn
            Left = 817
            Top = 5
            Width = 70
            Height = 22
            Caption = 'Excel'
            Glyph.Data = {
              
              00FFFF00FFFF00FFFF00FF000000000000000000FF00FF000000}
            TabOrder = 0
            OnClick = BitBtn3Click
          end
        end
      end
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 970
      Height = 73
      Align = alTop
      BevelOuter = bvNone
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 1
      object Label1: TLabel
        Left = 8
        Top = 10
        Width = 41
        Height = 13
        Caption = 'Sucursal'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label2: TLabel
        Left = 182
        Top = 10
        Width = 57
        Height = 13
        Caption = 'Vendedores'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label3: TLabel
        Left = 356
        Top = 10
        Width = 30
        Height = 13
        Caption = 'Zonas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label6: TLabel
        Left = 544
        Top = 10
        Width = 43
        Height = 13
        Caption = 'Cobrador'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object RxLabel1: TJvLabel
        Left = 710
        Top = 10
        Width = 33
        Height = 13
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -12
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel3: TJvLabel
        Left = 805
        Top = 10
        Width = 30
        Height = 13
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -12
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object dbcCobrador: TJvDBLookupCombo
        Left = 536
        Top = 28
        Width = 168
        Height = 21
        DisplayEmpty = 'Todos Los Cobradores'
        EmptyValue = '******'
        LookupField = 'CODIGO'
        LookupDisplay = 'NOMBRE'
        LookupSource = DSPersonal
        TabOrder = 0
        OnClick = BuscarExecute
      end
      object dbcSucursal: TJvDBLookupCombo
        Left = 8
        Top = 28
        Width = 168
        Height = 21
        DisplayEmpty = 'Todas las Sucursales'
        EmptyValue = '-1'
        LookupField = 'CODIGO'
        LookupDisplay = 'DETALLE'
        LookupSource = DSSucursales
        TabOrder = 1
        OnClick = BuscarExecute
      end
      object dbcVendedor: TJvDBLookupCombo
        Left = 182
        Top = 28
        Width = 168
        Height = 21
        DisplayEmpty = 'Todos Los Vendedores'
        EmptyValue = '******'
        LookupField = 'CODIGO'
        LookupDisplay = 'NOMBRE'
        LookupSource = DSPersonal
        TabOrder = 2
        OnClick = BuscarExecute
      end
      object dbcZona: TJvDBLookupCombo
        Left = 356
        Top = 28
        Width = 174
        Height = 21
        DisplayEmpty = 'Todas las Zonas'
        EmptyValue = '-1'
        LookupField = 'ID_ZONA'
        LookupDisplay = 'DETALLE'
        LookupSource = DSZona
        TabOrder = 3
        OnClick = BuscarExecute
      end
      object Desde: TJvDateEdit
        Left = 710
        Top = 28
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 4
      end
      object Hasta: TJvDateEdit
        Left = 805
        Top = 28
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 5
      end
      object UpDown1: TUpDown
        Left = 900
        Top = 28
        Width = 16
        Height = 20
        Hint = 'Cambia el Per'#237'odo'
        Max = 10000
        ParentShowHint = False
        Position = 100
        ShowHint = True
        TabOrder = 6
        OnClick = UpDown1Click
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 587
    Width = 970
    Align = alBottom
    ButtonHeight = 28
    ExplicitTop = 587
    ExplicitWidth = 970
    inherited btConfirma: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btNuevo: TBitBtn
      Width = 34
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitWidth = 34
      ExplicitHeight = 28
    end
    inherited btCancelar: TBitBtn
      Left = 111
      Width = 26
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 111
      ExplicitWidth = 26
      ExplicitHeight = 28
    end
    inherited btModificar: TBitBtn
      Left = 137
      Width = 21
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 137
      ExplicitWidth = 21
      ExplicitHeight = 28
    end
    inherited Pr: TSpeedButton
      Left = 158
      Height = 28
      Visible = False
      ExplicitLeft = 158
      ExplicitHeight = 28
    end
    inherited btBuscar: TBitBtn
      Left = 173
      Height = 28
      DoubleBuffered = True
      ExplicitLeft = 173
      ExplicitHeight = 28
    end
    inherited Ne: TSpeedButton
      Left = 248
      Height = 28
      Visible = False
      ExplicitLeft = 248
      ExplicitHeight = 28
    end
    inherited btBorrar: TBitBtn
      Left = 263
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 263
      ExplicitHeight = 28
    end
    inherited btSalir: TBitBtn
      Left = 338
      Height = 28
      DoubleBuffered = True
      ExplicitLeft = 338
      ExplicitHeight = 28
    end
    object ToolButton1: TToolButton
      Left = 413
      Top = 0
      Width = 12
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object BitBtn1: TBitBtn
      Left = 425
      Top = 0
      Width = 75
      Height = 28
      Action = Imprimir
      Caption = 'Imprimir'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      TabOrder = 7
    end
    object BitBtn4: TBitBtn
      Left = 500
      Top = 0
      Width = 96
      Height = 28
      Action = ExportarXLS
      Caption = 'Exportar XLS'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 8
    end
  end
  inherited Panel1: TPanel
    Top = 617
    Width = 970
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 617
    ExplicitWidth = 970
    inherited DBStatusLabel1: TJvDBStatusLabel
      Visible = False
    end
    inherited sbEstado: TStatusBar
      Width = 881
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 881
    end
  end
  inherited ActionList1: TActionList
    Left = 464
    Top = 192
    object Imprimir: TAction
      Caption = 'Imprimir'
      OnExecute = ImprimirExecute
    end
    object ExportarXLS: TAction
      Caption = 'Exportar XLS'
      OnExecute = ExportarXLSExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 520
    Top = 152
  end
  inherited ImageList1: TImageList
    Top = 136
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Left = 392
    Top = 264
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 80
    Top = 392
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 592
    Top = 328
  end
  inherited QBrowse2: TFDQuery
    Left = 104
    Top = 440
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 24
    Top = 432
  end
  object CDSVtas: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'zona'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'vendedor'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'cobrador'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPVtas'
    OnCalcFields = CDSVtasCalcFields
    Left = 256
    Top = 216
    object CDSVtasTIPOCPBTE: TStringField
      Alignment = taCenter
      DisplayLabel = 'Tp'
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      FixedChar = True
      Size = 2
    end
    object CDSVtasCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      FixedChar = True
      Size = 2
    end
    object CDSVtasLETRAFAC: TStringField
      DisplayLabel = 'Letra'
      FieldName = 'LETRAFAC'
      Origin = 'LETRAFAC'
      FixedChar = True
      Size = 1
    end
    object CDSVtasNROCPBTE: TStringField
      Alignment = taCenter
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      EditMask = 'a-####-########;0; '
      Size = 13
    end
    object CDSVtasCODIGO: TStringField
      DisplayLabel = 'Cod.Cli.'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object CDSVtasRAZONSOCIAL: TStringField
      DisplayLabel = 'Razon Social'
      FieldName = 'RAZONSOCIAL'
      Origin = 'RAZONSOCIAL'
      Size = 35
    end
    object CDSVtasVENDEDOR: TStringField
      DisplayLabel = 'Vendedor'
      FieldName = 'VENDEDOR'
      Origin = 'VENDEDOR'
      Size = 6
    end
    object CDSVtasZONA: TIntegerField
      DisplayLabel = 'Zona'
      FieldName = 'ZONA'
      Origin = 'ZONA'
    end
    object CDSVtasSUCURSAL: TIntegerField
      DisplayLabel = 'Suc.'
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSVtasMUESTRAVENDEDOR: TStringField
      DisplayLabel = 'Nomb.Vendedor'
      FieldName = 'MUESTRAVENDEDOR'
      Origin = 'NOMBRE'
      ProviderFlags = []
      Size = 50
    end
    object CDSVtasTOTALNETO: TFloatField
      FieldName = 'TOTALNETO'
      Origin = 'TOTALNETO'
      ProviderFlags = []
      Required = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSVtasTOTALNETOREC: TFloatField
      DisplayLabel = 'N.Sin Iva'
      FieldKind = fkInternalCalc
      FieldName = 'TOTALNETOREC'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00'
    end
    object CDSVtasTOTALRECALCULADO: TFloatField
      DisplayLabel = 'Total'
      FieldKind = fkInternalCalc
      FieldName = 'TOTALRECALCULADO'
      DisplayFormat = ',0.00;-,0.00;-'
      EditFormat = ',0.00'
    end
    object CDSVtasID_FC: TIntegerField
      FieldName = 'ID_FC'
      Origin = 'ID_FC'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSVtasCONDICIONVTA: TIntegerField
      FieldName = 'CONDICIONVTA'
      Origin = 'CONDICIONVTA'
      Required = True
    end
    object CDSVtasMUESTRASUCURSAL: TStringField
      DisplayLabel = 'Suc.'
      FieldName = 'MUESTRASUCURSAL'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 35
    end
    object CDSVtasANULADO: TStringField
      FieldName = 'ANULADO'
      Origin = 'ANULADO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSVtasANIO: TSmallintField
      DisplayLabel = 'A'#241'o'
      FieldName = 'ANIO'
      Origin = 'ANIO'
      ProviderFlags = []
      Required = True
    end
    object CDSVtasMES: TSmallintField
      FieldName = 'MES'
      Origin = 'MES'
      ProviderFlags = []
      Required = True
    end
    object CDSVtasMUESTRAMES: TStringField
      DisplayLabel = 'Mes'
      FieldKind = fkCalculated
      FieldName = 'MUESTRAMES'
      Calculated = True
    end
    object CDSVtasMUESTRACATEGORIA: TStringField
      DisplayLabel = 'Categoria'
      FieldName = 'MUESTRACATEGORIA'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 30
    end
    object CDSVtasMUESTRACOBRADOR: TStringField
      DisplayLabel = 'Cobrador'
      FieldName = 'MUESTRACOBRADOR'
      Origin = 'CODIGO'
      ProviderFlags = []
      Size = 6
    end
    object CDSVtasTOTAL: TFloatField
      DisplayLabel = 'Total'
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
      Required = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSVtasFECHAVTA: TSQLTimeStampField
      Alignment = taCenter
      DisplayLabel = 'Fecha Vta.'
      FieldName = 'FECHAVTA'
      Origin = 'FECHAVTA'
      Required = True
    end
    object CDSVtasFECHAVTO: TSQLTimeStampField
      Alignment = taCenter
      DisplayLabel = 'Fecha Vto.'
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object CDSVtasFECHA_HORA: TSQLTimeStampField
      DisplayLabel = 'Fecha/Hora'
      FieldName = 'FECHA_HORA'
      Origin = 'FECHA_HORA'
      DisplayFormat = 'HH:MM'
    end
  end
  object DSPVtas: TDataSetProvider
    DataSet = QVtas
    Left = 200
    Top = 216
  end
  object DSVtas: TDataSource
    DataSet = CDSVtas
    Left = 312
    Top = 216
  end
  object frxReport: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39785.472305046300000000
    ReportOptions.LastChange = 39785.472305046300000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 568
    Top = 224
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frVtas: TfrxDBDataset
    UserName = 'frVtas'
    CloseDataSource = False
    DataSet = CDSVtas
    BCDToCurrency = False
    DataSetOptions = []
    Left = 640
    Top = 392
    FieldDefs = <
      item
        FieldName = 'TIPOCPBTE'
        FieldAlias = 'TIPOCPBTE'
      end
      item
        FieldName = 'CLASECPBTE'
        FieldAlias = 'CLASECPBTE'
      end
      item
        FieldName = 'LETRAFAC'
        FieldAlias = 'LETRAFAC'
      end
      item
        FieldName = 'NROCPBTE'
        FieldAlias = 'NROCPBTE'
      end
      item
        FieldName = 'CODIGO'
        FieldAlias = 'CODIGO'
      end
      item
        FieldName = 'RAZONSOCIAL'
        FieldAlias = 'RAZONSOCIAL'
      end
      item
        FieldName = 'VENDEDOR'
        FieldAlias = 'VENDEDOR'
      end
      item
        FieldName = 'ZONA'
        FieldAlias = 'ZONA'
      end
      item
        FieldName = 'SUCURSAL'
        FieldAlias = 'SUCURSAL'
      end
      item
        FieldName = 'MUESTRAVENDEDOR'
        FieldAlias = 'MUESTRAVENDEDOR'
      end
      item
        FieldName = 'FECHAVTA'
        FieldAlias = 'FECHAVTA'
      end
      item
        FieldName = 'FECHAVTO'
        FieldAlias = 'FECHAVTO'
      end
      item
        FieldName = 'TOTAL'
        FieldAlias = 'TOTAL'
      end
      item
        FieldName = 'TOTALNETO'
        FieldAlias = 'TOTALNETO'
      end
      item
        FieldName = 'TOTALNETOREC'
        FieldAlias = 'TOTALNETOREC'
      end
      item
        FieldName = 'TOTALRECALCULADO'
        FieldAlias = 'TOTALRECALCULADO'
      end>
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    EmbedFontsIfProtected = False
    InteractiveFormsFontSubset = 'A-Z,a-z,0-9,#43-#47 '
    OpenAfterExport = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Transparency = False
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    Creator = 'FastReport'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    PdfA = False
    PDFStandard = psNone
    PDFVersion = pv17
    Left = 552
    Top = 440
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'txt'
    FileName = 'citi'
    InitialDir = 'c:\'
    Title = 'Archivo de Retenciones'
    Left = 360
    Top = 192
  end
  object DataToXLS: TDataToXLS
    DataSet = CDSVtas
    Columns = <
      item
        DataField = 'FECHAVTA'
        Title = 'F.de Vta'
        Alignment = taCenter
      end
      item
        DataField = 'FECHAVTO'
        Title = 'F.de Vto.'
        Alignment = taCenter
      end
      item
        DataField = 'TIPOCPBTE'
        Title = 'Tp'
      end
      item
        DataField = 'CLASECPBTE'
        Title = 'Cl'
      end
      item
        DataField = 'LETRAFAC'
        Title = 'Letra'
      end
      item
        DataField = 'NROCPBTE'
        Title = 'Nro.Cpbte'
      end
      item
        DataField = 'CODIGO'
        Title = 'Codigo'
      end
      item
        DataField = 'RAZONSOCIAL'
        Title = 'Razon Social'
      end
      item
        DataField = 'VENDEDOR'
        Title = 'Vendedor'
      end
      item
        DataField = 'ZONA'
        Title = 'Zona'
        Alignment = taRightJustify
        DataType = ctInteger
      end
      item
        DataField = 'SUCURSAL'
        Title = 'Suc.'
        Alignment = taRightJustify
        DataType = ctInteger
      end
      item
        DataField = 'MUESTRAVENDEDOR'
        Title = 'Nomb.Vendedor'
      end
      item
        DataField = 'TOTALRECALCULADO'
        Title = 'Total'
        Alignment = taRightJustify
        DataType = ctDouble
      end>
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -13
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Header.Font.Charset = DEFAULT_CHARSET
    Header.Font.Color = clWindowText
    Header.Font.Height = -13
    Header.Font.Name = 'Arial'
    Header.Font.Style = [fsBold]
    Left = 608
    Top = 160
  end
  object DSSucursales: TDataSource
    DataSet = CDSSucursal
    Left = 104
    Top = 328
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 160
    Top = 328
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 35
    end
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Left = 224
    Top = 328
  end
  object DSPPersonal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaPersonal
    Left = 224
    Top = 376
  end
  object CDSPersonal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPersonal'
    Left = 160
    Top = 376
    object CDSPersonalNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 50
    end
    object CDSPersonalCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 6
    end
  end
  object DSPersonal: TDataSource
    DataSet = CDSPersonal
    Left = 104
    Top = 376
  end
  object DSZona: TDataSource
    DataSet = CDSZona
    Left = 288
    Top = 336
  end
  object CDSZona: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPZona'
    Left = 344
    Top = 336
    object CDSZonaID_ZONA: TIntegerField
      FieldName = 'ID_ZONA'
      Required = True
    end
    object CDSZonaDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 25
    end
  end
  object DSPZona: TDataSetProvider
    DataSet = DMBuscadores.QBuscaZona
    Left = 408
    Top = 336
  end
  object PopupMenu1: TPopupMenu
    Left = 700
    Top = 221
    object CamposxColumnas1: TMenuItem
      Caption = 'Campos x Columnas'
      OnClick = CamposxColumnas1Click
    end
    object CamposdeDatos1: TMenuItem
      Caption = 'Campos de Datos'
      OnClick = CamposdeDatos1Click
    end
    object Filtros1: TMenuItem
      Caption = 'Filtros'
      OnClick = Filtros1Click
    end
    object CamposxRenglones1: TMenuItem
      Caption = 'Campos x Renglones'
      OnClick = CamposxRenglones1Click
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object LimpiarFiltros: TMenuItem
      Caption = 'Limpiar Filtros'
      OnClick = LimpiarFiltrosClick
    end
  end
  object DSPVtasDet: TDataSetProvider
    DataSet = QVtasDet
    Left = 204
    Top = 271
  end
  object CDSVtasDet: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'zona'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'vendedor'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'cobrador'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPVtasDet'
    OnCalcFields = CDSVtasDetCalcFields
    Left = 268
    Top = 271
    object CDSVtasDetTIPOCPBTE: TStringField
      DisplayLabel = 'Tp.Cpbte'
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSVtasDetCLASECPBTE: TStringField
      DisplayLabel = 'Cl.Cpbte.'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Required = True
      Size = 2
    end
    object CDSVtasDetLETRAFAC: TStringField
      DisplayLabel = 'Letra'
      FieldName = 'LETRAFAC'
      Origin = 'LETRAFAC'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSVtasDetNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      EditMask = 'a-0000-00000000;0'
      Size = 13
    end
    object CDSVtasDetID_FC: TIntegerField
      FieldName = 'ID_FC'
      Origin = 'ID_FC'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSVtasDetCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Required = True
      Size = 6
    end
    object CDSVtasDetRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      Origin = 'RAZONSOCIAL'
      Size = 35
    end
    object CDSVtasDetVENDEDOR: TStringField
      FieldName = 'VENDEDOR'
      Origin = 'VENDEDOR'
      Size = 6
    end
    object CDSVtasDetMUESTRAVENDEDOR: TStringField
      DisplayLabel = 'Vendedor'
      FieldName = 'MUESTRAVENDEDOR'
      Origin = 'NOMBRE'
      ProviderFlags = []
      Size = 50
    end
    object CDSVtasDetZONA: TIntegerField
      DisplayLabel = 'Zona'
      FieldName = 'ZONA'
      Origin = 'ZONA'
    end
    object CDSVtasDetSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
      Required = True
    end
    object CDSVtasDetCONDICIONVTA: TIntegerField
      FieldName = 'CONDICIONVTA'
      Origin = 'CONDICIONVTA'
      Required = True
    end
    object CDSVtasDetMUESTRASUCURSAL: TStringField
      DisplayLabel = 'Sucursal'
      FieldName = 'MUESTRASUCURSAL'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 35
    end
    object CDSVtasDetCATEGORIA: TIntegerField
      FieldName = 'CATEGORIA'
      Origin = 'CATEGORIA'
      ProviderFlags = []
    end
    object CDSVtasDetMUESTRACATEGORIA: TStringField
      DisplayLabel = 'Categoria'
      FieldName = 'MUESTRACATEGORIA'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 30
    end
    object CDSVtasDetANULADO: TStringField
      FieldName = 'ANULADO'
      Origin = 'ANULADO'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSVtasDetANIO: TSmallintField
      DisplayLabel = 'A'#241'o'
      FieldName = 'ANIO'
      Origin = 'ANIO'
      ProviderFlags = []
      Required = True
    end
    object CDSVtasDetMES: TSmallintField
      FieldName = 'MES'
      Origin = 'MES'
      ProviderFlags = []
      Required = True
    end
    object CDSVtasDetCODIGOARTICULO: TStringField
      DisplayLabel = 'Cod.Artic.'
      FieldName = 'CODIGOARTICULO'
      Origin = 'CODIGOARTICULO'
      ProviderFlags = []
      Size = 8
    end
    object CDSVtasDetDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 45
    end
    object CDSVtasDetUNITARIO_TOTAL: TFloatField
      FieldName = 'UNITARIO_TOTAL'
      Origin = 'UNITARIO_TOTAL'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasDetUNITARIO_MASIVA: TFloatField
      FieldName = 'UNITARIO_MASIVA'
      Origin = 'UNITARIO_MASIVA'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasDetTOTAL_MASIVA: TFloatField
      FieldName = 'TOTAL_MASIVA'
      Origin = 'TOTAL_MASIVA'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasDetMUESTRAMES: TStringField
      DisplayLabel = 'Mes'
      FieldKind = fkCalculated
      FieldName = 'MUESTRAMES'
      Calculated = True
    end
    object CDSVtasDetTOTALRECALCULADO: TFMTBCDField
      DisplayLabel = 'Total Neto'
      FieldKind = fkCalculated
      FieldName = 'TOTALRECALCULADO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetUNITARIOREC: TFMTBCDField
      DisplayLabel = 'Unitario Neto'
      FieldKind = fkCalculated
      FieldName = 'UNITARIOREC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetTOTALFINAL_REC: TFMTBCDField
      DisplayLabel = 'Total Final'
      FieldKind = fkCalculated
      FieldName = 'TOTALFINAL_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetUNITARIO_FINAL_REC: TFMTBCDField
      DisplayLabel = 'Unitario Final'
      FieldKind = fkCalculated
      FieldName = 'UNITARIO_FINAL_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetHORA: TStringField
      DisplayLabel = 'Hora'
      FieldKind = fkCalculated
      FieldName = 'HORA'
      Size = 5
      Calculated = True
    end
    object CDSVtasDetCOSTO_TOTAL: TFloatField
      DisplayLabel = 'Costo'
      FieldName = 'COSTO_TOTAL'
      Origin = 'COSTO_TOTAL'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasDetCOSTOUNITARIO: TFloatField
      DisplayLabel = 'Costo Unitario'
      FieldName = 'COSTOUNITARIO'
      Origin = 'COSTO_TOTAL'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasDetCOSTO_TOTAL_REC: TFMTBCDField
      DisplayLabel = 'Costo Total'
      FieldKind = fkCalculated
      FieldName = 'COSTO_TOTAL_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetCOSTO_UNITARIO_REC: TFMTBCDField
      DisplayLabel = 'Costo Unitario'
      FieldKind = fkCalculated
      FieldName = 'COSTO_UNITARIO_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetCOSTOUNITARIOMASIVA: TFloatField
      FieldName = 'COSTOUNITARIOMASIVA'
      Origin = 'COSTOUNITARIOMASIVA'
      ProviderFlags = []
    end
    object CDSVtasDetCOSTO_TOTALMASIVA: TFloatField
      FieldName = 'COSTO_TOTALMASIVA'
      Origin = 'COSTO_TOTALMASIVA'
      ProviderFlags = []
    end
    object CDSVtasDetCOSTO_UNI_MASIVA_REC: TFMTBCDField
      DisplayLabel = 'Costo Unit.Final'
      FieldKind = fkCalculated
      FieldName = 'COSTO_UNI_MASIVA_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetCOSTO_TOTAL_MAIVA_REC: TFMTBCDField
      DisplayLabel = 'Costo Total Final'
      FieldKind = fkCalculated
      FieldName = 'COSTO_TOTAL_MAIVA_REC'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Calculated = True
    end
    object CDSVtasDetDIA: TIntegerField
      DisplayLabel = 'Dia'
      FieldKind = fkCalculated
      FieldName = 'DIA'
      Calculated = True
    end
    object CDSVtasDetMUESTRACOBRADOR: TStringField
      DisplayLabel = 'Cobrador'
      FieldName = 'MUESTRACOBRADOR'
      Origin = 'CODIGO'
      ProviderFlags = []
      Size = 6
    end
    object CDSVtasDetCANTIDAD: TFloatField
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      ProviderFlags = []
      ReadOnly = True
    end
    object CDSVtasDetTOTAL: TFloatField
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
      ProviderFlags = []
      ReadOnly = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSVtasDetTOTALCPBTE: TFloatField
      FieldName = 'TOTALCPBTE'
      Origin = 'TOTAL'
      Required = True
    end
    object CDSVtasDetTOTALCPBTENETO: TFloatField
      FieldName = 'TOTALCPBTENETO'
      Origin = 'TOTALCPBTENETO'
      ProviderFlags = []
      ReadOnly = True
    end
    object CDSVtasDetTOTALCPBTE_REC: TFloatField
      DisplayLabel = 'Total  Cpbte '
      FieldKind = fkCalculated
      FieldName = 'TOTALCPBTE_REC'
      Calculated = True
    end
    object CDSVtasDetTOTALCPBTENETO_REC: TFloatField
      DisplayLabel = 'Total Neto Cpbte'
      FieldKind = fkCalculated
      FieldName = 'TOTALCPBTENETO_REC'
      Calculated = True
    end
    object CDSVtasDetFECHAVTA: TSQLTimeStampField
      DisplayLabel = 'Fecha Vta.'
      FieldName = 'FECHAVTA'
      Origin = 'FECHAVTA'
      Required = True
    end
    object CDSVtasDetFECHAVTO: TSQLTimeStampField
      DisplayLabel = 'Fecha Vto.'
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object CDSVtasDetFECHA_HORA: TSQLTimeStampField
      DisplayLabel = 'Fecha/Hora'
      FieldName = 'FECHA_HORA'
      Origin = 'FECHA_HORA'
      DisplayFormat = 'HH:HH'
    end
  end
  object DSVtasDet: TDataSource
    DataSet = CDSVtasDet
    Left = 332
    Top = 271
  end
  object CDSCobrador: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPersonal'
    Left = 160
    Top = 448
    object StringField1: TStringField
      FieldName = 'NOMBRE'
      Size = 50
    end
    object StringField2: TStringField
      FieldName = 'CODIGO'
      Size = 6
    end
  end
  object DSCobrador: TDataSource
    DataSet = CDSCobrador
    Left = 96
    Top = 440
  end
  object QVtas: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select f.fechavta,f.fechavto,f.tipocpbte,f.clasecpbte,f.letrafac' +
        ',f.nrocpbte,F.ID_FC,'
      
        '       f.codigo,f.razonsocial,f.vendedor,p.nombre as MuestraVend' +
        'edor,f.zona,f.sucursal,'
      
        '       f.condicionvta,suc.detalle as MuestraSucursal,cl.categori' +
        'a,cat.detalle as MuestraCategoria,'
      
        '       f.total,f.anulado,(f.netograv2+f.netoexen2) as TotalNeto,' +
        'Extract(Year from f.fechavta) as Anio,'
      
        '       extract(month from f.fechavta) as Mes,f.fecha_hora,cob.co' +
        'digo as muestracobrador from  fcvtacab f'
      'left join clientes cl on cl.codigo=f.codigo'
      'left join personal p on p.codigo=f.vendedor'
      'left join personal cob on cob.codigo=cl.cobrador'
      'left join sucursal suc on suc.codigo=f.sucursal'
      'left join clientes_categorias cat on cat.id=cl.categoria'
      'where'
      '(( f.zona = :zona ) or ( :zona = -1 )) AND'
      '(( f.vendedor = :vendedor ) or ( :vendedor = '#39'******'#39' )) and'
      '(( cob.codigo = :cobrador ) or ( :cobrador = '#39'******'#39' )) and'
      '(( f.fechavta between :desde and :hasta)) and'
      '(( f.sucursal = :sucursal ) or ( :sucursal = -1 )) and'
      '(f.anulado<>'#39'S'#39')'
      ''
      'order by f.vendedor,f.tipocpbte,f.nrocpbte')
    Left = 140
    Top = 216
    ParamData = <
      item
        Position = 1
        Name = 'zona'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'vendedor'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'cobrador'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 6
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QVtasDet: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select f.fechavta,f.fechavto,f.tipocpbte,f.clasecpbte,f.letrafac' +
        ',f.nrocpbte,F.id_fc,'
      
        '       f.codigo,f.razonsocial,f.vendedor,p.nombre as MuestraVend' +
        'edor,f.zona,f.sucursal,'
      
        '       f.condicionvta,suc.detalle as MuestraSucursal,cl.categori' +
        'a,cat.detalle as MuestraCategoria,'
      
        '       f.total as TotalCpbte,f.anulado,(f.netograv2+f.netoexen2)' +
        ' as TotalCpbteNeto,Extract(Year from f.fechavta) as Anio,'
      
        '       extract(month from f.fechavta) as Mes,f.fecha_hora,f.fech' +
        'a_hora as Hora,'
      
        '       fd.codigoarticulo,fd.detalle,fd.cantidad,fd.unitario_tota' +
        'l,'
      
        '       fd.unitario_total+fd.iva_unitario as Unitario_MasIva,fd.t' +
        'otal,fd.total+fd.iva_total as Total_masIva,'
      
        '       fd.costo_total as CostoUnitario,(fd.costo_total*fd.cantid' +
        'ad) as Costo_Total,'
      
        '       (fd.costo_total * (1+fd.iva_tasa*0.01)) as CostoUnitarioM' +
        'asIva,'
      
        '       ((fd.costo_total*(1+fd.iva_tasa*0.01))*fd.cantidad) as Co' +
        'sto_TotalMasIva,cob.codigo as muestracobrador from fcvtacab f'
      'left join fcvtadet fd on fd.id_cabfac=f.id_fc'
      'left join clientes cl on cl.codigo=f.codigo'
      'left join personal p on p.codigo=f.vendedor'
      'left join personal cob on cob.codigo=cl.cobrador'
      'left join sucursal suc on suc.codigo=f.sucursal'
      'left join clientes_categorias cat on cat.id=cl.categoria'
      'where'
      '(( f.zona = :zona ) or ( :zona = -1 )) AND'
      '(( f.vendedor = :vendedor ) or ( :vendedor = '#39'******'#39' )) and'
      '(( f.fechavta between :desde and :hasta)) and'
      '(( cob.codigo = :cobrador ) or ( :cobrador = '#39'******'#39' )) and'
      '(( f.sucursal = :sucursal ) or ( :sucursal = -1 )) and'
      '(f.anulado<>'#39'S'#39')'
      ''
      'order by f.vendedor,fd.codigoarticulo')
    Left = 132
    Top = 280
    ParamData = <
      item
        Position = 1
        Name = 'zona'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'vendedor'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'cobrador'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 6
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
end
