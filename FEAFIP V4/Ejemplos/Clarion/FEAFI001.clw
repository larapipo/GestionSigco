...
AppFrame             APPLICATION('Process Customer Orders'),AT(,,400,253),FONT('MS Sans Serif',8,COLOR:Black,),STATUS(-1,80,120,45),SYSTEM,MAX,RESIZE,IMM
                       TOOLBAR,AT(0,0,400,75)
                         ...
                         END
                         BUTTON('Consulta CUIT'),AT(12,33,57,20),USE(?Button2)
                         BUTTON('Obtener CAE'),AT(11,7,57,19),USE(?Button1)
                       END
                     END
    ...
    OF ?Button2
              ...
              If ?OLE{'login("certificado.crt", "clave.key")'} <> 0
                 ...
                 if ?OLE{'sfConsultar(30610171601)'} <> 0
                   ...
                   !thisform.lbNombre.caption = contribuyente.nombre
                   !       thisform.lbTipo.caption = contribuyente.tipoPersona
                   !       thisform.lbEstado.caption = contribuyente.estadoClave
                   !       domicilio = contribuyente.domicilioFiscal
                   !       thisform.lbDomicilio.caption = domicilio.direccion + ", " + domicilio.localidad + ", " + domicilio.provincia
                      ! Control de constancia de inscripción. Si hay observaciones el contribuyente tiene errores de constancia.
                    !      thisform.label7.caption = contribuyente.observaciones
                 else
    ...
    OF ?Button1
              ...
              If ?OLE{'login("certificado.crt", "clave.key","' & URLWSAA & '")'} <> 0
                  ...
                  End
                    nro = nro + 1
                    ?OLE{'Reset()'}
                    ?OLE{'AgregaFactura(1, 80, 30702637895,' & nro & ',' & nro & ',' & clip(fechacmp) &  ', 121, 0, 100, 0, "", "", "", "PES", 1)'}
