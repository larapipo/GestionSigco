...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, UABMBase, Data.FMTBcd, System.DateUtils,
  ...
  AdvSmoothButton,System.SysUtils;
...
type
  TFormImportarFcCompraAFIP = class(TFormABMBase)
    ...
    procedure sgFacturasPortalFilterDone(Sender: TObject);
  
  private
    procedure BuscarFacturas;
    function NormalizarFecha(const Valor: string): string;
...
function TFormImportarFcCompraAFIP.NormalizarFecha(const Valor: string): string;
var
  Fecha: TDateTime;
  FS: TFormatSettings;
begin
  ...
  if TryStrToDate(Valor, Fecha, FS) then
    Result := FormatDateTime('dd/mm/yyyy', Fecha)
  else if TryISO8601ToDate(Valor, Fecha) then
    Result := FormatDateTime('dd/mm/yyyy', Fecha)
  else
    Result := ''; // no se pudo interpretar
...
begin
  ...
  if Not(cheBox_Portal.Checked) then
   begin
     sgFacturasPortal.RemoveAllFilters;
     for col := 7 to 31 do
       begin
         if ((Col<>8) and (Col<>9)) then
           sgFacturasPortal.Cells[col, sgFacturasPortal.RowCount - 1] :=
           FormatFloat(',0.00', sgFacturasPortal.ColumnSum(col,1,sgFacturasPortal.RowCount-2));
       end;
      sgFacturasPortal.FloatingFooter.Invalidate;
   end;
end;
...
begin
  ...
  QBorrarLote.ExecSQL;
  QBorrarLote.Close;
  ShowMessage('Lote Borrado... verificar....');
end;
...
begin
  ...
  if ComBuscadorStock.rOk then
    begin
      ...
      CDSRelacionProveeCONCPETO.AsString        := VarToStr(ComBuscadorStock.Id);
      CDSRelacionProveeMUESTRACONCPETO.AsString := ComBuscadorStock.Data.FieldByName('DETALLE_STK').AsString;
...
procedure TFormImportarFcCompraAFIP.btBuscarFCClick(Sender: TObject);
begin
  inherited;
  BuscarFacturas;
end;
    ...
    for row := 1 to sgFacturasPortal.RowCount -1 do
      for col := 0 to sgFacturasPortal.ColCount-1 do
        begin
          if col=0 Then
            begin
              Cadena :=NormalizarFecha(sgFacturasPortal.Cells[0, row]);
              sgFacturasPortal.Cells[0, row]:=Cadena;
            end;
  ...
  for col := 7 to 31 do
   begin
     if ((Col<>8) and (Col<>9)) then
       sgFacturasPortal.Cells[col, sgFacturasPortal.RowCount - 1] :=
       FormatFloat('0.00', sgFacturasPortal.ColumnSum(col,1,sgFacturasPortal.RowCount-2));
     //sgFacturasPortal.Floats(Col,sgFacturas
   end;
  sgFacturasPortal.FloatingFooter.Invalidate;
  BuscarFacturas;
end;

procedure TFormImportarFcCompraAFIP.BuscarFacturas;
var i:Integer;
 desde,hasta:TDateTime;
begin
...
procedure TFormImportarFcCompraAFIP.DBGrid1ColEnter(Sender: TObject);
begin
  inherited;
...
begin
  ...
  if (cbSucursalPortal.KeyValue>-1) and (sgFacturasPortal.RowCount>1) then
    begin
      ...
      for r := 1 to sgFacturasPortal.RowCount-2 do
        begin
          pgBar.Position:=r;
          Application.ProcessMessages;
          DMMain_FD.fdcGestion.StartTransaction;
          try
            ...
            if (CDSRelacionCpbte.Locate('SUCURSAL;CODIGO_AFIP',VarArrayOf([IntToStr(cbSucursalPortal.KeyValue),(sgFacturasPortal.Cells[cbTipoComprobante.ItemIndex,r])]),[])) Then
            ...
            spGenerarFacturaPortal.ParamByName('COMP_TOTAL').Value         := Abs(StrToFloat(StringReplace(sgFacturasPortal.Cells[cbImporteTotal.ItemIndex,r],',','.',[rfReplaceAll])));
            ...
            spGenerarFacturaPortal.ParamByName('COMP_IMP_INTERNOS').Value  := Abs(StrToFloatDef(sgFacturasPortal.Cells[cbImpInternos.ItemIndex,r],0));
    ...
    ShowMessage('Falta la Sucursal o No hay Movimientos.....');
  spGenerarFacturaPortal.Close;
  pgBar.Position:=0;
end;
...
begin
  ...
  if (sgFacturasPortal.Cells[1,ARow]='8') Then Nro:='B';
  ...
  if (sgFacturasPortal.Cells[1,ARow]='81') Then Tp:='FO'
  else
  if (sgFacturasPortal.Cells[1,ARow]='12') Then Tp:='ND'
  else
  if (sgFacturasPortal.Cells[1,ARow]='8') Then Tp:='NC';
...
procedure TFormImportarFcCompraAFIP.sgFacturasPortalFilterDone(Sender: TObject);
var Col:Integer;
begin
  inherited;
 for Col := 7 to 31 do
  ...
  if (sgFacturas.RowCount>1 ) then
    begin
      ...
      while (r<sgFacturas.RowCount) do
        begin
          if Not(CDSRelacionCpbte.Locate('SUCURSAL;CODIGO_AFIP',VarArrayOf([IntToStr(dbcSucursal.KeyValue),(sgFacturas.Cells[1,r])]),[])) Then
            begin
              CDSRelacionCpbte.Append;
              case StrToInt(Trim(sgFacturasPortal.Cells[1,r])) of
                1 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura A';
                6 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura B';
                11: CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura C';
                2 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Debito A';
                7 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Debito B';
                12: CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Debito C';
                3 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Credito A';
                8 : CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Credito B';
                13: CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Credito C';
              end;
              CDSRelacionCpbteCOMPROBANTE_AFIP.AsString := Trim(sgFacturas.Cells[1,r]);
              CDSRelacionCpbteID_COMPROBANTE.AsInteger  := -1;
              CDSRelacionCpbteSUCURSAL.Value         := dbcSucursal.KeyValue;
              CDSRelacionCpbteCODIGO_AFIP.Value      := StrToInt(Trim(sgFacturas.Cells[1,r]));
...
begin
  inherited;
  if (sgFacturasPortal.RowCount>1 ) and (cbSucursalPortal.KeyValue>0) then
    begin
      r:=1;
      while (r<sgFacturasPortal.RowCount-1) do
        begin
          if Not(CDSRelacionCpbte.Locate('SUCURSAL;CODIGO_AFIP',VarArrayOf([IntToStr(cbSucursalPortal.KeyValue),(sgFacturasPortal.Cells[1,r])]),[])) Then
            begin
              CDSRelacionCpbte.Append;
              case StrToInt(Trim(sgFacturasPortal.Cells[1,r])) of
                1 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura A';
                6 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura B';
                11: CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+' - '+'Factura C';
                2 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+' - '+'N.Debito A';
                7 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'N.Debito B';
                12: CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'N.Debito C';
                3 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'N.Credito A';
                8 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'N.Credito B';
                13: CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'N.Credito C';
                81: CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'Tk.A';
              end;
              ...
              CDSRelacionCpbteCODIGO_AFIP.Value      := StrToInt(Trim(sgFacturasPortal.Cells[1,r]));
              CDSRelacionCpbte.Post;
            end
          else
            if (CDSRelacionCpbte.Locate('SUCURSAL;CODIGO_AFIP',VarArrayOf([IntToStr(cbSucursalPortal.KeyValue),(sgFacturasPortal.Cells[1,r])]),[])) Then
              begin
                CDSRelacionCpbte.Edit;
                case StrToInt(Trim(sgFacturasPortal.Cells[1,r])) of
                  1 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'Factura A';
                  6 : CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'Factura B';
                  11: CDSRelacionCpbteCOMPROBANTE_AFIP.Value := Trim(sgFacturasPortal.Cells[1,r])+'-'+'Factura C';
