...
uses
  ...
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs,Vui.Qz, Vcl.ComCtrls, Vcl.StdCtrls,
  Vcl.ExtCtrls,Httpapp, VirtualUI_SDK;

type
  TFormPrintVirtual = class(TForm)
    pnBase: TPanel;
    Label1: TLabel;
    ...
    procedure btPrintPDFClick(Sender: TObject);
  ...
  public
    { Public declarations }
  published
    property ArchivoImprimir:String read FArchivoImprimir write FArchivoImprimir;
  end;

var
  FormPrintVirtual: TFormPrintVirtual;
...
procedure TFormPrintVirtual.btPrintPDFClick(Sender: TObject);
begin
   FQz.PrintPDF(ExpandFilename(FArchivoImprimir));
//   FQz.PrintPDF(ExpandFilename('vui-qz\assets\pdf_sample.pdf'));

end;

procedure TFormPrintVirtual.Button1Click(Sender: TObject);
var
  SL : TStringList;
  S : string;
begin
  ...
  try
    SL.LoadFromFile(FArchivoImprimir);
    FQz.BeginDoc;
    for S in SL do
      FQz.PrintText(S);
    FQz.EndDoc;
...
procedure TFormPrintVirtual.cmbPrintersChange(Sender: TObject);
begin
  FQz.DefaultPrinter := cmbPrinters.Text;
end;

procedure TFormPrintVirtual.FormClose(Sender: TObject;
...
procedure TFormPrintVirtual.FormCreate(Sender: TObject);
begin
  FQz                 := TVuiQz.Create;
...
procedure TFormPrintVirtual.FormDestroy(Sender: TObject);
begin
  FormPrintVirtual:=nil;
end;

procedure TFormPrintVirtual.FormShow(Sender: TObject);
begin
  FQz.Init(QZ_RUNTIME_CERTIFICATE, QZ_RUNTIME_PRIVATE_KEY);
  lbArchivo.Caption:=FArchivoImprimir;
end;

procedure TFormPrintVirtual.PrinterChanged(Sender: TObject);
begin
  cmbPrinters.Items.Assign(FQz.PrinterList);
  cmbPrinters.ItemIndex := cmbPrinters.Items.IndexOf(FQz.DefaultPrinter);
  // EnablePanel(pnBase,true);
end;

procedure TFormPrintVirtual.StateChanged(Sender: TObject);
begin
