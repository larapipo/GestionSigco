inherited FormLecturaXML: TFormLecturaXML
  Left = 530
  Top = 98
  Caption = 'Lectura Archivos Sis.Adici'#243'n'
  ClientHeight = 675
  ClientWidth = 969
  ExplicitWidth = 987
  ExplicitHeight = 716
  PixelsPerInch = 96
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 969
    Height = 553
    ExplicitWidth = 969
    ExplicitHeight = 553
    object SpeedButton1: TSpeedButton
      Left = 423
      Top = 444
      Width = 23
      Height = 22
      Action = BuscarPathXML
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
    object SpeedButton2: TSpeedButton
      Left = 423
      Top = 478
      Width = 23
      Height = 22
      Action = BuscarPathXTR
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
    object Label3: TLabel
      Left = 7
      Top = 7
      Width = 41
      Height = 16
      Caption = 'Cajas'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object RxLabel1: TJvLabel
      Left = 739
      Top = 1
      Width = 106
      Height = 13
      Caption = 'Fecha de Facturacion'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel2: TJvLabel
      Left = 739
      Top = 41
      Width = 77
      Height = 13
      Caption = 'Sucursal de Vta'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel6: TJvLabel
      Left = 739
      Top = 80
      Width = 178
      Height = 13
      Caption = 'Tipo de Comprobante de Facturaci'#243'n'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel3: TJvLabel
      Left = 739
      Top = 160
      Width = 100
      Height = 13
      Caption = 'Deposito Mercaderia'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object JvLabel1: TJvLabel
      Left = 741
      Top = 203
      Width = 133
      Height = 13
      Caption = 'Caja de ingresos de Fondos'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object Label1: TLabel
      Left = 914
      Top = 445
      Width = 34
      Height = 13
      Caption = 'Rubros'
    end
    object Label2: TLabel
      Left = 904
      Top = 491
      Width = 51
      Height = 13
      Caption = 'Sub Rubro'
      Visible = False
    end
    object Label5: TLabel
      Left = 16
      Top = 429
      Width = 107
      Height = 13
      Caption = 'Archivo de Datos XML'
    end
    object Label6: TLabel
      Left = 16
      Top = 465
      Width = 112
      Height = 13
      Caption = 'Archivo de Mapeo XRT'
    end
    object JvLabel2: TJvLabel
      Left = 741
      Top = 252
      Width = 153
      Height = 13
      Caption = 'Codigo de Articulos Eceptuados'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object JvLabel3: TJvLabel
      Left = 732
      Top = 404
      Width = 83
      Height = 39
      Caption = 'Importe Eceptuado en el Total'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      WordWrap = True
    end
    object Label7: TLabel
      Left = 658
      Top = 447
      Width = 168
      Height = 13
      Caption = 'Rubro para Dar de Alta los Articulos'
    end
    object JvLabel4: TJvLabel
      Left = 741
      Top = 122
      Width = 166
      Height = 13
      Caption = 'Tipo de Comprobante de N.Credito'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object Label11: TLabel
      Left = 16
      Top = 505
      Width = 230
      Height = 13
      Caption = 'Articulo por Defecto en caso de no Existir alguno'
    end
    object lbArchivos: TListBox
      Left = 5
      Top = 24
      Width = 215
      Height = 399
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = lbArchivosClick
      OnKeyDown = lbArchivosKeyDown
    end
    object edArchivosXML: TEdit
      Left = 16
      Top = 444
      Width = 401
      Height = 21
      TabOrder = 1
      Text = 'edArchivosXML'
    end
    object edArchivosXRT: TEdit
      Left = 16
      Top = 479
      Width = 401
      Height = 21
      TabOrder = 2
      Text = 'edArchivosXRT'
    end
    object BitBtn1: TBitBtn
      Left = 452
      Top = 477
      Width = 89
      Height = 25
      Action = Recuperar
      Caption = 'Recuperar'
      Glyph.Data = {
        
        523B160000000000000000001717171717170000000000000000}
      TabOrder = 3
    end
    object pcMovimientos: TJvPageControl
      Left = 226
      Top = 3
      Width = 500
      Height = 438
      ActivePage = pag2
      MultiLine = True
      OwnerDraw = True
      TabOrder = 4
      TabWidth = 200
      TabPainter = JvTabDefaultPainter1
      ParentColor = False
      Color = clInactiveCaption
      object Pag1: TTabSheet
        Caption = 'Articulos'
        ExplicitLeft = 0
        ExplicitTop = 0
        ExplicitWidth = 0
        ExplicitHeight = 0
        object JvDBStatusLabel1: TJvDBStatusLabel
          Left = 2
          Top = 353
          Width = 96
          Height = 13
          DataSource = DSArticulos
          Style = lsRecordNo
          Transparent = True
        end
        object Label4: TLabel
          Left = 163
          Top = 353
          Width = 65
          Height = 13
          Caption = 'Rubro Detalle'
        end
        object cxDBTextEdit1: TDBText
          Left = 234
          Top = 353
          Width = 71
          Height = 13
          AutoSize = True
          Color = clInactiveCaption
          DataField = 'RUBRO'
          DataSource = DSArticulos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentColor = False
          ParentFont = False
        end
        object DBText1: TDBText
          Left = 234
          Top = 368
          Width = 42
          Height = 13
          AutoSize = True
          Color = clInactiveCaption
          DataField = 'COD_RUBRO'
          DataSource = DSArticulos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentColor = False
          ParentFont = False
        end
        object dbgArticulos: TJvDBGrid
          Left = 0
          Top = 0
          Width = 492
          Height = 345
          Align = alTop
          DataSource = DSArticulos
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          TitleButtons = True
          OnTitleBtnClick = dbgArticulosTitleBtnClick
          AlternateRowColor = clInfoBk
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Expanded = False
              FieldName = 'COD_ARTICU'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 62
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DESCRIPCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'IMPORTE1'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'IMPORTE2'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
        object BitBtn3: TBitBtn
          Left = 389
          Top = 356
          Width = 87
          Height = 25
          Action = VerificaArticulos
          Caption = 'Verifica Articulos'
          TabOrder = 1
        end
        object pbArticulos: TProgressBar
          Left = 0
          Top = 367
          Width = 155
          Height = 16
          Smooth = True
          TabOrder = 2
        end
      end
      object pag2: TTabSheet
        Caption = 'Detalle de Venta'
        ImageIndex = 1
        object JvDBStatusLabel2: TJvDBStatusLabel
          Left = 5
          Top = 363
          Width = 96
          Height = 13
          DataSource = DSVtas
          Style = lsRecordNo
          Transparent = True
        end
        object Label8: TLabel
          Left = 304
          Top = 389
          Width = 57
          Height = 13
          Caption = 'Total Gral'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label9: TLabel
          Left = 270
          Top = 343
          Width = 92
          Height = 13
          Caption = 'Total a Facturar'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object Label10: TLabel
          Left = 252
          Top = 366
          Width = 110
          Height = 13
          Caption = 'Total Negativo(NC)'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlue
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
        object dbgVentas: TJvDBGrid
          Left = 0
          Top = 0
          Width = 492
          Height = 337
          Align = alTop
          DataSource = DSVtas
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          TitleButtons = True
          OnTitleBtnClick = dbgVentasTitleBtnClick
          AlternateRowColor = clMoneyGreen
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Expanded = False
              FieldName = 'COD_ARTICU'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 48
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DESCRIPCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 222
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CANTIDAD'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'UNITARIO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 68
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'IMPORTE_C_DESCUENTO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
        object edTotalVta: TJvCalcEdit
          Left = 368
          Top = 340
          Width = 121
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          ShowButton = False
          TabOrder = 1
          DisabledTextColor = clNavy
          DecimalPlacesAlwaysShown = False
        end
        object btFactura: TBitBtn
          Left = 178
          Top = 364
          Width = 57
          Height = 22
          Action = Facturar
          Caption = 'Facturar'
          TabOrder = 2
        end
        object pbVentas: TProgressBar
          Left = 3
          Top = 380
          Width = 154
          Height = 16
          Smooth = True
          TabOrder = 3
        end
        object btNCredito: TBitBtn
          Left = 178
          Top = 385
          Width = 57
          Height = 22
          Action = HacerNC
          Caption = 'HacerNC'
          TabOrder = 4
        end
        object edTotalGral: TJvCalcEdit
          Left = 368
          Top = 386
          Width = 121
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          ShowButton = False
          TabOrder = 5
          DisabledTextColor = clNavy
          DisabledColor = clMoneyGreen
          DecimalPlacesAlwaysShown = False
        end
        object edNegativos: TJvCalcEdit
          Left = 368
          Top = 363
          Width = 121
          Height = 21
          DisplayFormat = ',0.00'
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clNavy
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
          ShowButton = False
          TabOrder = 6
          DisabledTextColor = clNavy
          DecimalPlacesAlwaysShown = False
        end
      end
    end
    object dbcSucursal: TJvDBLookupCombo
      Left = 741
      Top = 53
      Width = 217
      Height = 21
      EmptyValue = '-1'
      LookupField = 'CODIGO'
      LookupDisplay = 'DETALLE'
      LookupSource = DSSucursal
      TabOrder = 5
      OnChange = dbcSucursalChange
    end
    object dbcComprobante: TJvDBLookupCombo
      Left = 741
      Top = 95
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSComprobante
      TabOrder = 6
    end
    object dbcDeposito: TJvDBLookupCombo
      Left = 741
      Top = 175
      Width = 217
      Height = 21
      EmptyValue = '-1'
      LookupField = 'ID'
      LookupDisplay = 'NOMBRE'
      LookupSource = DSDeposito
      TabOrder = 7
    end
    object dbcCtas: TJvDBLookupCombo
      Left = 741
      Top = 217
      Width = 217
      Height = 21
      EmptyValue = '-1'
      LookupField = 'ID_CUENTA_CAJA'
      LookupDisplay = 'NOMBRE'
      LookupSource = DSCajas
      TabOrder = 8
    end
    object cbRubro: TJvDBLookupCombo
      Left = 658
      Top = 464
      Width = 297
      Height = 21
      DisplayEmpty = 'Todos Los Rubros'
      EmptyValue = '***'
      LookupField = 'CODIGO_RUBRO'
      LookupDisplay = 'DETALLE_RUBRO'
      LookupSource = DSRubros
      TabOrder = 9
      OnChange = cbRubroChange
    end
    object cbSubRubro: TJvDBLookupCombo
      Left = 659
      Top = 510
      Width = 297
      Height = 21
      DisplayEmpty = 'Todos los Subrubros'
      EmptyValue = '*****'
      LookupField = 'CODIGO_SUBRUBRO'
      LookupDisplay = 'DETALLE_SUBRUBRO'
      LookupSource = DSSubRubro
      TabOrder = 10
      Visible = False
      OnEnter = cbSubRubroEnter
    end
    object dbeFecha: TJvDBDateEdit
      Left = 741
      Top = 20
      Width = 116
      Height = 21
      DataField = 'Fecha'
      DataSource = DSCabecera
      ShowNullDate = False
      TabOrder = 11
    end
    object dbgExcepciones: TDBGrid
      Left = 741
      Top = 271
      Width = 164
      Height = 120
      DataSource = DSEceptuados
      TabOrder = 12
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end>
    end
    object edEceptuado: TJvCalcEdit
      Left = 829
      Top = 406
      Width = 121
      Height = 21
      DisplayFormat = ',0.00'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      ShowButton = False
      TabOrder = 13
      DisabledTextColor = clNavy
      DecimalPlacesAlwaysShown = False
    end
    object dbcNotaCredito: TJvDBLookupCombo
      Left = 741
      Top = 137
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSCompNC
      TabOrder = 14
    end
    object chNegativos: TCheckBox
      Left = 536
      Top = 444
      Width = 97
      Height = 17
      Caption = 'Valores Negativos'
      TabOrder = 15
      OnClick = chNegativosClick
    end
    object ceCodigo: TJvComboEdit
      Left = 18
      Top = 520
      Width = 73
      Height = 21
      ButtonWidth = 16
      ClickKey = 16397
      Color = clWhite
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 16
      Text = ''
      OnButtonClick = BuscarArticuloExecute
    end
    object edDetalle: TEdit
      Left = 97
      Top = 520
      Width = 312
      Height = 21
      Color = clInfoBk
      Enabled = False
      TabOrder = 17
    end
  end
  inherited ToolBar1: TToolBar
    Top = 553
    Width = 969
    ExplicitTop = 553
    ExplicitWidth = 969
    inherited btConfirma: TBitBtn
      Visible = False
    end
    inherited btNuevo: TBitBtn
      Visible = False
    end
    inherited btCancelar: TBitBtn
      Visible = False
    end
    inherited btModificar: TBitBtn
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited btBuscar: TBitBtn
      Visible = False
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
    inherited btBorrar: TBitBtn
      Visible = False
    end
  end
  inherited Panel1: TPanel
    Top = 583
    Width = 969
    ExplicitTop = 583
    ExplicitWidth = 969
    inherited sbEstado: TStatusBar
      Width = 807
      ExplicitWidth = 807
    end
  end
  inherited ActionList1: TActionList
    object BuscarPathXML: TAction
      OnExecute = BuscarPathXMLExecute
    end
    object BuscarPathXTR: TAction
      OnExecute = BuscarPathXTRExecute
    end
    object LeerArchisoXML: TAction
      Caption = 'LeerArchisoXML'
      OnExecute = LeerArchisoXMLExecute
    end
    object Facturar: TAction
      Caption = 'Facturar'
      OnExecute = FacturarExecute
    end
    object VerificaArticulos: TAction
      Caption = 'Verifica Articulos'
      OnExecute = VerificaArticulosExecute
    end
    object HacerNC: TAction
      Caption = 'HacerNC'
      OnExecute = HacerNCExecute
    end
    object BuscarArticulo: TAction
      Caption = 'BuscarArticulo'
      OnExecute = BuscarArticuloExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 472
    Top = 104
  end
  inherited ImageList1: TImageList
    Left = 304
    Top = 94
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    Left = 792
    Top = 320
  end
  inherited QBrowse: TSQLQuery
    Left = 8
    Top = 600
  end
  inherited QUltimoCodigo: TSQLQuery
    SQL.Strings = (
      'select max(codigo_stk) from stock')
    Left = 56
    Top = 632
  end
  object CDSArticulos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'XMLDSPArticulos'
    Left = 376
    Top = 240
    object CDSArticulosCOD_ARTICU: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'COD_ARTICU'
      Size = 6
    end
    object CDSArticulosDESCRIPCION: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DESCRIPCION'
      Size = 45
    end
    object CDSArticulosIMPORTE1: TFloatField
      DisplayLabel = 'Precio 1'
      FieldName = 'IMPORTE1'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosIMPORTE2: TFloatField
      DisplayLabel = 'Precio 2'
      FieldName = 'IMPORTE2'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosCOD_RUBRO: TStringField
      DisplayLabel = 'Rubro'
      FieldName = 'COD_RUBRO'
      Size = 3
    end
    object CDSArticulosRUBRO: TStringField
      DisplayLabel = 'Rubro Detalle'
      FieldName = 'RUBRO'
      Size = 25
    end
  end
  object DSArticulos: TDataSource
    DataSet = CDSArticulos
    Left = 432
    Top = 224
  end
  object OpenDialog1: TOpenDialog
    Left = 640
    Top = 320
  end
  object CDSVtas: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'XMLDSPVtas'
    AfterOpen = CDSVtasAfterOpen
    AfterRefresh = CDSVtasAfterRefresh
    Left = 368
    Top = 288
    object CDSVtasCOD_ARTICU: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'COD_ARTICU'
      Size = 6
    end
    object CDSVtasDESCRIPCION: TStringField
      DisplayLabel = 'Descripcion'
      FieldName = 'DESCRIPCION'
      Size = 45
    end
    object CDSVtasUNITARIO: TFloatField
      DisplayLabel = 'Uni.'
      FieldName = 'UNITARIO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasCANTIDAD: TFloatField
      DisplayLabel = 'Cant.'
      FieldName = 'CANTIDAD'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSVtasIMPORTE_C_DESCUENTO: TFloatField
      DisplayLabel = 'Imp.+Desc.'
      FieldName = 'IMPORTE_C_DESCUENTO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
  end
  object DSVtas: TDataSource
    DataSet = CDSVtas
    Left = 440
    Top = 280
  end
  object sdDirectorio: TJvSelectDirectory
    Left = 576
    Top = 304
  end
  object JvTabDefaultPainter1: TJvTabDefaultPainter
    ActiveFont.Charset = DEFAULT_CHARSET
    ActiveFont.Color = clBlack
    ActiveFont.Height = -11
    ActiveFont.Name = 'MS Sans Serif'
    ActiveFont.Style = [fsBold]
    ActiveColorTo = clAqua
    InactiveFont.Charset = DEFAULT_CHARSET
    InactiveFont.Color = clBlack
    InactiveFont.Height = -11
    InactiveFont.Name = 'MS Sans Serif'
    InactiveFont.Style = []
    DisabledFont.Charset = DEFAULT_CHARSET
    DisabledFont.Color = clBlack
    DisabledFont.Height = -11
    DisabledFont.Name = 'MS Sans Serif'
    DisabledFont.Style = []
    ShowFocus = True
    Left = 192
    Top = 456
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Left = 40
    Top = 136
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 48
    Top = 88
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 35
    end
  end
  object DSSucursal: TDataSource
    DataSet = CDSSucursal
    Left = 48
    Top = 40
  end
  object DSComprobante: TDataSource
    DataSet = CDSComprobantes
    Left = 128
    Top = 40
  end
  object CDSComprobantes: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 120
    Top = 88
    object CDSComprobantesID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Origin = 'DBMAIN.COMPROBANTES.ID_COMPROBANTE'
    end
    object CDSComprobantesTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Origin = 'DBMAIN.COMPROBANTES.TIPO_COMPROB'
      Size = 2
    end
    object CDSComprobantesCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Origin = 'DBMAIN.COMPROBANTES.CLASE_COMPROB'
      Size = 2
    end
    object CDSComprobantesDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Origin = 'DBMAIN.COMPROBANTES.DENOMINACION'
      Size = 15
    end
    object CDSComprobantesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'DBMAIN.COMPROBANTES.SUCURSAL'
    end
  end
  object DSPComprobante: TDataSetProvider
    DataSet = QComproba
    Left = 120
    Top = 144
  end
  object DSPDepositos: TDataSetProvider
    DataSet = QDepositos
    Left = 224
    Top = 144
  end
  object CDSDepositos: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPDepositos'
    Left = 224
    Top = 88
    object CDSDepositosID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object CDSDepositosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 25
    end
    object CDSDepositosSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Required = True
    end
    object CDSDepositosCENTRALIZA_STOCK: TStringField
      FieldName = 'CENTRALIZA_STOCK'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object DSDeposito: TDataSource
    DataSet = CDSDepositos
    Left = 224
    Top = 32
  end
  object QComproba: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select   ID_COMPROBANTE,TIPO_COMPROB,'
      '    CLASE_COMPROB, DENOMINACION,SUCURSAL from comprobantes'
      'where tipo_comprob in ('#39'FO'#39','#39'TK'#39') and'
      '      sucursal=:sucursal and'
      '      compra_venta= '#39'V'#39)
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 120
    Top = 208
    object QComprobaID_COMPROBANTE: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_COMPROBANTE'
      Origin = 'DBMAIN.COMPROBANTES.ID_COMPROBANTE'
    end
    object QComprobaTIPO_COMPROB: TStringField
      DisplayWidth = 2
      FieldName = 'TIPO_COMPROB'
      Origin = 'DBMAIN.COMPROBANTES.TIPO_COMPROB'
      Size = 2
    end
    object QComprobaCLASE_COMPROB: TStringField
      DisplayWidth = 2
      FieldName = 'CLASE_COMPROB'
      Origin = 'DBMAIN.COMPROBANTES.CLASE_COMPROB'
      Size = 2
    end
    object QComprobaDENOMINACION: TStringField
      DisplayWidth = 15
      FieldName = 'DENOMINACION'
      Origin = 'DBMAIN.COMPROBANTES.DENOMINACION'
      Size = 15
    end
    object QComprobaSUCURSAL: TIntegerField
      DisplayWidth = 10
      FieldName = 'SUCURSAL'
      Origin = 'DBMAIN.COMPROBANTES.SUCURSAL'
    end
  end
  object QCajas: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      
        'select c.id_caja,c.numerocaja, c.fecha_incio,c.fecha_cierre,c.id' +
        '_cuenta_caja,cc.nombre from caja_cab c'
      'left join cuenta_caja cc on cc.id_cuenta=c.id_cuenta_caja'
      'where c.estado=0 '
      'order By c.id_caja desc')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 584
    Top = 496
  end
  object DSPCajas: TDataSetProvider
    DataSet = QCajas
    Left = 632
    Top = 504
  end
  object CDSCajas: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPCajas'
    Left = 680
    Top = 504
    object CDSCajasID_CAJA: TIntegerField
      FieldName = 'ID_CAJA'
      Required = True
    end
    object CDSCajasNUMEROCAJA: TIntegerField
      FieldName = 'NUMEROCAJA'
    end
    object CDSCajasFECHA_INCIO: TSQLTimeStampField
      FieldName = 'FECHA_INCIO'
      Required = True
    end
    object CDSCajasFECHA_CIERRE: TSQLTimeStampField
      FieldName = 'FECHA_CIERRE'
    end
    object CDSCajasID_CUENTA_CAJA: TIntegerField
      FieldName = 'ID_CUENTA_CAJA'
      Required = True
    end
    object CDSCajasNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 25
    end
  end
  object DSCajas: TDataSource
    DataSet = CDSCajas
    Left = 768
    Top = 504
  end
  object QDepositos: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select d.* from depositos d where d.sucursal=:sucursal')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 232
    Top = 200
  end
  object DSRubros: TDataSource
    DataSet = CDSRubro
    Left = 584
    Top = 552
  end
  object CDSRubro: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPRubro'
    Left = 632
    Top = 552
    object CDSRubroCODIGO_RUBRO: TStringField
      FieldName = 'CODIGO_RUBRO'
      Required = True
      Size = 3
    end
    object CDSRubroDETALLE_RUBRO: TStringField
      FieldName = 'DETALLE_RUBRO'
      Required = True
      Size = 35
    end
  end
  object DSPRubro: TDataSetProvider
    DataSet = DMBuscadores.QBuscaRubro
    Left = 672
    Top = 552
  end
  object DSPSubRubro: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSubRubro
    Left = 688
    Top = 608
  end
  object CDSSubRubro: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'rubro'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'rubro'
        ParamType = ptInput
      end>
    ProviderName = 'DSPSubRubro'
    Left = 632
    Top = 608
    object CDSSubRubroCODIGO_SUBRUBRO: TStringField
      FieldName = 'CODIGO_SUBRUBRO'
      Required = True
      Size = 5
    end
    object CDSSubRubroDETALLE_SUBRUBRO: TStringField
      FieldName = 'DETALLE_SUBRUBRO'
      Required = True
      Size = 45
    end
    object CDSSubRubroCODIGO_RUBRO: TStringField
      FieldName = 'CODIGO_RUBRO'
      Required = True
      Size = 3
    end
  end
  object DSSubRubro: TDataSource
    DataSet = CDSSubRubro
    Left = 584
    Top = 608
  end
  object spABMStock: TSQLStoredProc
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Precision = 8
        Name = 'CODIGO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 45
        Name = 'DETALLE'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 4
        Name = 'MARCA'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 3
        Name = 'RUBRO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 5
        Name = 'SUBRUBRO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 1
        Name = 'MODO_GRAVAMEN'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'TASA_IVA'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'SOBRETASA_IVA'
        ParamType = ptInput
      end
      item
        DataType = ftFMTBcd
        Precision = 8
        NumericScale = 3
        Name = 'COSTO_GRAVADO'
        ParamType = ptInput
      end
      item
        DataType = ftFMTBcd
        Precision = 8
        NumericScale = 3
        Name = 'COSTO_EXENTO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 200
        Name = 'DETALLE_ADICIONAL'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 20
        Name = 'CODIGO_ALTERNATIVO'
        ParamType = ptInput
      end
      item
        DataType = ftFMTBcd
        Precision = 8
        NumericScale = 3
        Name = 'PRESENTACION_CANTIDAD'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 3
        Name = 'PRESENTACION_UNIDAD'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Precision = 4
        Name = 'MONEDA'
        ParamType = ptInput
      end>
    SQLConnection = DMMain_2.SQLConectionMain
    StoredProcName = 'ABM_STOCK'
    Left = 888
    Top = 592
  end
  object QExisteCdigo: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select codigo_stk from stock'
      'where codigo_stk=:codigo')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 808
    Top = 592
    object QExisteCdigoCODIGO_STK: TStringField
      DisplayWidth = 8
      FieldName = 'CODIGO_STK'
      Size = 8
    end
  end
  object QTraeCodigo: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select codigo_stk,Detalle_stk,rubro_stk,subrubro_stk from stock'
      'where reemplazo_stk = :codigo')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 280
    Top = 624
    object QTraeCodigoCODIGO_STK: TStringField
      FieldName = 'CODIGO_STK'
      Required = True
      Size = 8
    end
    object QTraeCodigoDETALLE_STK: TStringField
      FieldName = 'DETALLE_STK'
      Size = 45
    end
    object QTraeCodigoRUBRO_STK: TStringField
      FieldName = 'RUBRO_STK'
      Required = True
      Size = 3
    end
    object QTraeCodigoSUBRUBRO_STK: TStringField
      FieldName = 'SUBRUBRO_STK'
      Required = True
      Size = 5
    end
  end
  object spABMSubRubro: TSQLStoredProc
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Precision = 5
        Name = 'CODIGO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 3
        Name = 'CODIGO_RUBRO'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Precision = 35
        Name = 'DETALLE'
        ParamType = ptInput
      end>
    SQLConnection = DMMain_2.SQLConectionMain
    StoredProcName = 'ABM_SUBRUBRO'
    Left = 888
    Top = 344
  end
  object CDSCabecera: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'XMLDSPCabecera'
    Left = 384
    Top = 344
    object CDSCabeceraFecha: TDateField
      FieldName = 'Fecha'
    end
    object CDSCabeceraHora: TTimeField
      FieldName = 'Hora'
    end
    object CDSCabeceraSucursal: TStringField
      FieldName = 'Sucursal'
      Size = 15
    end
  end
  object DSCabecera: TDataSource
    DataSet = CDSCabecera
    Left = 448
    Top = 336
  end
  object QBuscadorSubRubro: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'descripcion'
        ParamType = ptInput
      end>
    SQL.Strings = (
      
        'select r.* from subrubros r where r.detalle_subrubro= :descripci' +
        'on')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 472
    Top = 592
    object QBuscadorSubRubroCODIGO_SUBRUBRO: TStringField
      FieldName = 'CODIGO_SUBRUBRO'
      Required = True
      Size = 5
    end
    object QBuscadorSubRubroDETALLE_SUBRUBRO: TStringField
      FieldName = 'DETALLE_SUBRUBRO'
      Required = True
      Size = 45
    end
    object QBuscadorSubRubroCODIGO_RUBRO: TStringField
      FieldName = 'CODIGO_RUBRO'
      Required = True
      Size = 3
    end
    object QBuscadorSubRubroCONTROL_SUBRUBRO: TStringField
      FieldName = 'CONTROL_SUBRUBRO'
      FixedChar = True
      Size = 1
    end
  end
  object QCodigoFree: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'rubro'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select * from buscar_codigosubrubro_free (:rubro)')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 408
    Top = 624
    object QCodigoFreeNEWCODIGO: TIntegerField
      FieldName = 'NEWCODIGO'
    end
  end
  object QUltimoCodigoSub: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'rubro'
        ParamType = ptInput
      end>
    SQL.Strings = (
      
        'select max(codigo_subrubro) from subrubros where codigo_rubro=:r' +
        'ubro')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 344
    Top = 576
    object QUltimoCodigoSubMAX: TStringField
      FieldName = 'MAX'
      Size = 5
    end
  end
  object QEceptuados: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select * from TABLA_CODIGO_ECEPTUADOS')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 624
    Top = 224
    object QEceptuadosCODIGO: TStringField
      FieldName = 'CODIGO'
      Required = True
    end
  end
  object DSEceptuados: TDataSource
    DataSet = CDSEceptuados
    Left = 632
    Top = 64
  end
  object CDSEceptuados: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPEceptuados'
    AfterPost = CDSEceptuadosAfterPost
    AfterDelete = CDSEceptuadosAfterDelete
    Left = 624
    Top = 112
    object CDSEceptuadosCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Required = True
    end
  end
  object DSPEceptuados: TDataSetProvider
    DataSet = QEceptuados
    Left = 624
    Top = 168
  end
  object DSCompNC: TDataSource
    DataSet = CDSCompNC
    Left = 48
    Top = 200
  end
  object CDSCompNC: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPCompNC'
    Left = 48
    Top = 248
    object CDSCompNCID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSCompNCTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 4
    end
    object CDSCompNCCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 4
    end
    object CDSCompNCDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSCompNCSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
  end
  object DSPCompNC: TDataSetProvider
    DataSet = QCompNC
    Left = 48
    Top = 304
  end
  object QCompNC: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    SQL.Strings = (
      'select   ID_COMPROBANTE,TIPO_COMPROB,'
      '    CLASE_COMPROB, DENOMINACION,SUCURSAL from comprobantes'
      'where tipo_comprob in ('#39'NC'#39') and'
      '      sucursal=:sucursal and'
      '      compra_venta= '#39'V'#39)
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 40
    Top = 368
    object QCompNCID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object QCompNCTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 4
    end
    object QCompNCCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 4
    end
    object QCompNCDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object QCompNCSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
  end
  object QStock: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    SQL.Strings = (
      
        'select s.codigo_stk,s.detalle_stk from stock s where s.codigo_st' +
        'k=:codigo')
    SQLConnection = DMMain_2.SQLConectionMain
    Left = 136
    Top = 360
    object QStockCODIGO_STK: TStringField
      DisplayWidth = 8
      FieldName = 'CODIGO_STK'
      Size = 8
    end
    object QStockDETALLE_STK: TStringField
      DisplayWidth = 45
      FieldName = 'DETALLE_STK'
      Size = 45
    end
  end
  object XMLDSPVtas: TXMLTransformProvider
    Left = 294
    Top = 243
  end
  object XMLDSPArticulos: TXMLTransformProvider
    Left = 294
    Top = 291
  end
  object XMLDSPCabecera: TXMLTransformProvider
    Left = 286
    Top = 339
  end
end
