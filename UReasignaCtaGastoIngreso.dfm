inherited FormReasignaCtasGastosIngresos: TFormReasignaCtasGastosIngresos
  Left = 403
  Top = 85
  Caption = 'Reasignaci'#243'n de Ctas de Gastos e Ingresos'
  ClientHeight = 647
  ClientWidth = 610
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 628
  ExplicitHeight = 688
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 610
    Height = 505
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 610
    ExplicitHeight = 505
    object Label2: TLabel
      Left = 288
      Top = 8
      Width = 31
      Height = 13
      Caption = 'Desde'
    end
    object Label3: TLabel
      Left = 440
      Top = 8
      Width = 28
      Height = 13
      Caption = 'Hasta'
    end
    object UpDown1: TUpDown
      Left = 578
      Top = 7
      Width = 16
      Height = 20
      Hint = 'Cambia el Per'#237'odo'
      Max = 10000
      ParentShowHint = False
      Position = 100
      ShowHint = True
      TabOrder = 2
      OnClick = UpDown1Click
    end
    object pgcMov: TPageControl
      Left = 0
      Top = 32
      Width = 601
      Height = 489
      ActivePage = tsEgresos
      TabOrder = 3
      object tsEgresos: TTabSheet
        Caption = 'Egresos'
        object Label1: TLabel
          Left = 16
          Top = 3
          Width = 32
          Height = 13
          Caption = 'Rubro:'
        end
        object dbtRubEgreso: TDBText
          Left = 50
          Top = 3
          Width = 68
          Height = 13
          AutoSize = True
          Color = 16776176
          DataField = 'MUESTRARUBRO'
          DataSource = DSDetalleEgresos
          ParentColor = False
        end
        object Label4: TLabel
          Left = 280
          Top = 3
          Width = 37
          Height = 13
          Caption = 'Cuenta:'
        end
        object dbtCtaEgreso: TDBText
          Left = 320
          Top = 3
          Width = 64
          Height = 13
          AutoSize = True
          Color = 16776176
          DataField = 'MUESTRACUENTA'
          DataSource = DSDetalleEgresos
          ParentColor = False
        end
        object Label8: TLabel
          Left = 8
          Top = 405
          Width = 33
          Height = 13
          Caption = 'Detalle'
          FocusControl = DBEdit2
        end
        object DBEdit2: TDBEdit
          Left = 8
          Top = 420
          Width = 580
          Height = 21
          Color = clWhite
          DataField = 'DETALLE'
          DataSource = DSDetalleEgresos
          TabOrder = 1
        end
        object dbgEgresos: TJvDBGrid
          Left = 3
          Top = 22
          Width = 585
          Height = 377
          DataSource = DSDetalleEgresos
          Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          PopupMenu = PopupMenu1
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnColEnter = dbgEgresosColEnter
          OnDblClick = dbgEgresosDblClick
          OnEnter = dbgEgresosEnter
          OnKeyPress = dbgEgresosKeyPress
          TitleButtons = True
          OnTitleBtnClick = dbgEgresosTitleBtnClick
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <
            item
              ControlName = 'FechaEgreso'
              FieldName = 'FECHAGASTO'
              FitCell = fcCellSize
              LeaveOnEnterKey = True
              LeaveOnUpDownKey = True
            end>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'FECHACPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 88
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHAGASTO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 100
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'CLASECPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 25
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'NROCPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 112
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'RUBROGASTO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 72
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CODIGOGASTO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 78
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'IMPORTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 68
              Visible = True
            end>
        end
        object FechaEgreso: TJvDBDateEdit
          Left = 454
          Top = 136
          Width = 121
          Height = 21
          DataField = 'FECHAGASTO'
          DataSource = DSDetalleEgresos
          ShowNullDate = False
          TabOrder = 2
          Visible = False
        end
      end
      object tsIngresos: TTabSheet
        Caption = 'Ingresos'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ImageIndex = 1
        ParentFont = False
        object Label5: TLabel
          Left = 16
          Top = 3
          Width = 32
          Height = 13
          Caption = 'Rubro:'
        end
        object dbtRubIngreso: TDBText
          Left = 50
          Top = 3
          Width = 70
          Height = 13
          AutoSize = True
          Color = 16776176
          DataField = 'MUESTRARUBRO'
          DataSource = DSDetalleIngresos
          ParentColor = False
        end
        object Label6: TLabel
          Left = 280
          Top = 3
          Width = 37
          Height = 13
          Caption = 'Cuenta:'
        end
        object dbtCtaIngreso: TDBText
          Left = 326
          Top = 3
          Width = 66
          Height = 13
          AutoSize = True
          Color = 16776176
          DataField = 'MUESTRACUENTA'
          DataSource = DSDetalleIngresos
          ParentColor = False
        end
        object Label7: TLabel
          Left = 8
          Top = 405
          Width = 33
          Height = 13
          Caption = 'Detalle'
          FocusControl = DBEdit1
        end
        object DBEdit1: TDBEdit
          Left = 8
          Top = 420
          Width = 585
          Height = 21
          Color = clWhite
          DataField = 'DETALLE'
          DataSource = DSDetalleIngresos
          TabOrder = 1
        end
        object dbgIngresos: TJvDBGrid
          Left = 6
          Top = 24
          Width = 584
          Height = 377
          DataSource = DSDetalleIngresos
          Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnColEnter = dbgIngresosColEnter
          OnDblClick = dbgIngresosDblClick
          OnEnter = dbgIngresosEnter
          OnKeyPress = dbgIngresosKeyPress
          TitleButtons = True
          OnTitleBtnClick = dbgIngresosTitleBtnClick
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <
            item
              ControlName = 'FechaIngreso'
              FieldName = 'FECHAINGRESO'
              FitCell = fcCellSize
              LeaveOnEnterKey = True
              LeaveOnUpDownKey = True
            end>
          RowsHeight = 17
          TitleRowHeight = 17
          Columns = <
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'FECHACPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 88
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FECHAINGRESO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 100
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'CLASECPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 25
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'NROCPBTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 112
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'RUBROINGRESO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 72
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CODIGOINGRESO'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 78
              Visible = True
            end
            item
              Color = clInfoBk
              Expanded = False
              FieldName = 'IMPORTE'
              ReadOnly = True
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 68
              Visible = True
            end>
        end
        object FechaIngreso: TJvDBDateEdit
          Left = 454
          Top = 136
          Width = 121
          Height = 21
          DataField = 'FECHAINGRESO'
          DataSource = DSDetalleIngresos
          ShowNullDate = False
          TabOrder = 2
          Visible = False
        end
      end
    end
    object hasta: TJvDateEdit
      Left = 474
      Top = 7
      Width = 105
      Height = 21
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 0
    end
    object desde: TJvDateEdit
      Left = 325
      Top = 8
      Width = 105
      Height = 21
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 1
    end
  end
  inherited ToolBar1: TToolBar
    Top = 505
    Width = 610
    ExplicitTop = 505
    ExplicitWidth = 610
    inherited btNuevo: TBitBtn
      Width = 28
      Visible = False
      ExplicitWidth = 28
    end
    inherited btCancelar: TBitBtn
      Left = 105
      ExplicitLeft = 105
    end
    inherited btModificar: TBitBtn
      Left = 180
      Width = 22
      Visible = False
      ExplicitLeft = 180
      ExplicitWidth = 22
    end
    inherited Pr: TSpeedButton
      Left = 202
      Visible = False
      ExplicitLeft = 202
    end
    inherited btBuscar: TBitBtn
      Left = 217
      Width = 20
      ExplicitLeft = 217
      ExplicitWidth = 20
    end
    inherited Ne: TSpeedButton
      Left = 237
      Visible = False
      ExplicitLeft = 237
    end
    inherited btBorrar: TBitBtn
      Left = 252
      Visible = False
      ExplicitLeft = 252
    end
    inherited btSalir: TBitBtn
      Left = 327
      ExplicitLeft = 327
    end
    object BitBtn1: TBitBtn
      Left = 402
      Top = 0
      Width = 92
      Height = 29
      Action = Recuperar
      Caption = 'Recuperar'
      Glyph.Data = {
        
        0303}
      NumGlyphs = 2
      TabOrder = 7
    end
  end
  inherited Panel1: TPanel
    Top = 535
    Width = 610
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 535
    ExplicitWidth = 610
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
    end
    inherited sbEstado: TStatusBar
      Width = 521
      ExplicitLeft = 88
      ExplicitWidth = 521
    end
  end
  inherited ActionList1: TActionList
    Top = 120
    inherited Buscar: TAction
      Enabled = False
      Visible = False
    end
    object BuscarRubro: TAction
      Caption = 'BuscarRubro'
      OnExecute = BuscarRubroExecute
    end
    object BuscarCta: TAction
      Caption = 'BuscarCta'
      OnExecute = BuscarCtaExecute
    end
    object ReasignaRub: TAction
      Caption = 'Reasigna Rub'
      OnExecute = ReasignaRubExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 496
    Top = 80
  end
  inherited ImageList1: TImageList
    Left = 160
    Top = 120
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    Left = 432
    Top = 352
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 536
    Top = 368
  end
  object DSDetalleEgresos: TDataSource
    DataSet = CDSDetalleEgresos
    Left = 56
    Top = 128
  end
  object CDSDetalleEgresos: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPDetalleEgresos'
    BeforeInsert = CDSDetalleEgresosBeforeInsert
    Left = 352
    Top = 152
    object CDSDetalleEgresosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSDetalleEgresosCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSDetalleEgresosNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSDetalleEgresosFECHACPBTE: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHACPBTE'
      Origin = 'FECHACPBTE'
    end
    object CDSDetalleEgresosFECHAGASTO: TSQLTimeStampField
      DisplayLabel = 'F.Gasto'
      FieldName = 'FECHAGASTO'
      Origin = 'FECHAGASTO'
    end
    object CDSDetalleEgresosRUBROGASTO: TStringField
      DisplayLabel = 'Rubro'
      FieldName = 'RUBROGASTO'
      Origin = 'RUBROGASTO'
      Size = 3
    end
    object CDSDetalleEgresosCODIGOGASTO: TStringField
      DisplayLabel = 'Cuenta'
      FieldName = 'CODIGOGASTO'
      Origin = 'CODIGOGASTO'
      Size = 6
    end
    object CDSDetalleEgresosDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 100
    end
    object CDSDetalleEgresosMUESTRARUBRO: TStringField
      FieldName = 'MUESTRARUBRO'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
    object CDSDetalleEgresosMUESTRACUENTA: TStringField
      FieldName = 'MUESTRACUENTA'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
    object CDSDetalleEgresosID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
      ProviderFlags = []
    end
    object CDSDetalleEgresosIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
  end
  object DSPDetalleEgresos: TDataSetProvider
    DataSet = QDetalleEgresos
    Options = []
    Left = 248
    Top = 152
  end
  object DSDetalleIngresos: TDataSource
    DataSet = CDSDetalleIngresos
    Left = 48
    Top = 192
  end
  object DSPDetalleIngresos: TDataSetProvider
    DataSet = QDetalleIngresos
    Options = []
    Left = 248
    Top = 192
  end
  object CDSDetalleIngresos: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPDetalleIngresos'
    BeforeInsert = CDSDetalleEgresosBeforeInsert
    Left = 368
    Top = 192
    object CDSDetalleIngresosID: TIntegerField
      FieldName = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSDetalleIngresosCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object CDSDetalleIngresosNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Size = 13
    end
    object CDSDetalleIngresosFECHACPBTE: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHACPBTE'
    end
    object CDSDetalleIngresosFECHAINGRESO: TSQLTimeStampField
      DisplayLabel = 'F.Ingrs.'
      FieldName = 'FECHAINGRESO'
    end
    object CDSDetalleIngresosRUBROINGRESO: TStringField
      DisplayLabel = 'Rubro'
      FieldName = 'RUBROINGRESO'
      Size = 3
    end
    object CDSDetalleIngresosCODIGOINGRESO: TStringField
      DisplayLabel = 'Cuenta'
      FieldName = 'CODIGOINGRESO'
      Size = 6
    end
    object CDSDetalleIngresosDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 100
    end
    object CDSDetalleIngresosMUESTRARUBRO: TStringField
      FieldName = 'MUESTRARUBRO'
      ProviderFlags = []
      Size = 50
    end
    object CDSDetalleIngresosMUESTRACUENTA: TStringField
      FieldName = 'MUESTRACUENTA'
      ProviderFlags = []
      Size = 50
    end
    object CDSDetalleIngresosID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      ProviderFlags = []
    end
    object CDSDetalleIngresosIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
  end
  object CDSBuscaRubro: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPBuscaRubro'
    Left = 104
    Top = 384
    object CDSBuscaRubroCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 3
    end
    object CDSBuscaRubroDETALLE: TStringField
      DisplayLabel = 'Detalle'
      DisplayWidth = 35
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 50
    end
  end
  object DSPBuscaRubro: TDataSetProvider
    DataSet = QBuscaRubro
    Left = 168
    Top = 384
  end
  object CDSBuscaCuenta: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'rubro'
        ParamType = ptInput
      end>
    ProviderName = 'DSPBuscaCuenta'
    Left = 384
    Top = 296
    object CDSBuscaCuentaCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 6
    end
    object CDSBuscaCuentaCODIGO_RUBRO: TStringField
      FieldName = 'CODIGO_RUBRO'
      Origin = 'CODIGO_RUBRO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 3
    end
    object CDSBuscaCuentaDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 50
    end
  end
  object DSPBuscaCuenta: TDataSetProvider
    DataSet = QBuscaCuentas
    Left = 448
    Top = 296
  end
  object PopupMenu1: TPopupMenu
    Left = 308
    Top = 416
    object ReasignaRub1: TMenuItem
      Action = ReasignaRub
    end
  end
  object ComBuscadorCuentas: TComBuscador
    Data = CDSBuscaCuenta
    Campo = 'CODIGO'
    Titulo = 'Cuentas'
    rOk = False
    Left = 212
    Top = 312
  end
  object ComBuscadorRubros: TComBuscador
    Data = CDSBuscaRubro
    Campo = 'CODIGO'
    Titulo = 'Rubros'
    rOk = False
    Left = 80
    Top = 584
  end
  object ComBuscadorRubro: TComBuscador
    Data = CDSBuscaRubro
    Campo = 'CODIGO'
    Titulo = 'Rubros'
    rOk = False
    Left = 44
    Top = 440
  end
  object QDetalleEgresos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select ec.id,'
      '       ec.clasecpbte,'
      '       ec.nrocpbte,ec.fechacpbte,'
      '       ec.fechagasto,ec.rubrogasto,'
      '       ec.codigogasto,ec.detalle,ec.importe,'
      '       gr.detalle as MuestraRubro,'
      
        '       gc.detalle as MuestraCuenta,ec.id_cpbte from egr_caja_det' +
        'alle ec'
      'left join gastos_rubros gr on gr.codigo=ec.rubrogasto'
      'left join gastos_cuentas gc on gc.codigo=ec.codigogasto'
      'where ec.fechacpbte between :desde and :hasta')
    Left = 156
    Top = 152
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QDetalleEgresosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QDetalleEgresosCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object QDetalleEgresosNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object QDetalleEgresosFECHACPBTE: TSQLTimeStampField
      FieldName = 'FECHACPBTE'
      Origin = 'FECHACPBTE'
    end
    object QDetalleEgresosFECHAGASTO: TSQLTimeStampField
      FieldName = 'FECHAGASTO'
      Origin = 'FECHAGASTO'
    end
    object QDetalleEgresosRUBROGASTO: TStringField
      FieldName = 'RUBROGASTO'
      Origin = 'RUBROGASTO'
      Size = 3
    end
    object QDetalleEgresosCODIGOGASTO: TStringField
      FieldName = 'CODIGOGASTO'
      Origin = 'CODIGOGASTO'
      Size = 6
    end
    object QDetalleEgresosDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 100
    end
    object QDetalleEgresosIMPORTE: TFloatField
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
    end
    object QDetalleEgresosMUESTRARUBRO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRARUBRO'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
    object QDetalleEgresosMUESTRACUENTA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRACUENTA'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
    object QDetalleEgresosID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
    end
  end
  object QDetalleIngresos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select ic.id,ic.clasecpbte,ic.nrocpbte,ic.fechacpbte,ic.id_cpbte' +
        ','
      
        '  ic.fechaingreso,ic.rubroingreso,ic.codigoingreso,ic.detalle,ic' +
        '.importe,gr.detalle as MuestraRubro,'
      'gc.detalle as MuestraCuenta from ing_caja_detalle ic'
      'left join gastos_rubros gr on gr.codigo=ic.rubroingreso'
      'left join gastos_cuentas gc on gc.codigo=ic.codigoingreso'
      'where ic.fechacpbte between :desde and :hasta')
    Left = 156
    Top = 200
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QDetalleIngresosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QDetalleIngresosCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object QDetalleIngresosNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object QDetalleIngresosFECHACPBTE: TSQLTimeStampField
      FieldName = 'FECHACPBTE'
      Origin = 'FECHACPBTE'
    end
    object QDetalleIngresosID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
    end
    object QDetalleIngresosFECHAINGRESO: TSQLTimeStampField
      FieldName = 'FECHAINGRESO'
      Origin = 'FECHAINGRESO'
    end
    object QDetalleIngresosRUBROINGRESO: TStringField
      FieldName = 'RUBROINGRESO'
      Origin = 'RUBROINGRESO'
      Size = 3
    end
    object QDetalleIngresosCODIGOINGRESO: TStringField
      FieldName = 'CODIGOINGRESO'
      Origin = 'CODIGOINGRESO'
      Size = 6
    end
    object QDetalleIngresosDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 100
    end
    object QDetalleIngresosIMPORTE: TFloatField
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
    end
    object QDetalleIngresosMUESTRARUBRO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRARUBRO'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
    object QDetalleIngresosMUESTRACUENTA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRACUENTA'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 50
    end
  end
  object QBuscaCuentas: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from gastos_cuentas'
      'where codigo_rubro=:rubro'
      'order by detalle')
    Left = 540
    Top = 296
    ParamData = <
      item
        Position = 1
        Name = 'rubro'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QBuscaRubro: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from Gastos_Rubros order by detalle')
    Left = 244
    Top = 376
  end
  object QReasignaRubrosMovimientos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'update egr_caja_detalle e set e.rubrogasto = (select g.codigo_ru' +
        'bro from gastos_cuentas g where g.codigo=e.codigogasto)')
    Left = 344
    Top = 576
  end
  object QRubro: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from gastos_rubros where codigo=:codigo')
    Left = 388
    Top = 400
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 3
        Value = Null
      end>
    object QRubroCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 3
    end
    object QRubroDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 50
    end
  end
  object QCta: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from Gastos_Cuentas'
      'where codigo=:codigo and codigo_rubro=:rubro')
    Left = 508
    Top = 464
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 6
        Value = Null
      end
      item
        Name = 'RUBRO'
        DataType = ftString
        ParamType = ptInput
        Size = 3
      end>
    object QCtaCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 6
    end
    object QCtaCODIGO_RUBRO: TStringField
      FieldName = 'CODIGO_RUBRO'
      Origin = 'CODIGO_RUBRO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 3
    end
    object QCtaDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 50
    end
    object QCtaEXCLUYE: TStringField
      FieldName = 'EXCLUYE'
      Origin = 'EXCLUYE'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
end
