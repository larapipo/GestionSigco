...
begin
  ...
  try
    ...
    BIO_free(keyfile); // <-- Esto libera el archivo
  end;
end;
...
begin
  ...
  try
    ...
    fCertificate := PEM_read_bio_X509_AUX(certfile, c, nil, nil);
    if fCertificate = nil then
      raise EOpenSSL.Create('No se pudo leer el certificado');
  finally
    BIO_free(certfile); // <-- Esto cierra el archivo
  end;
end;
