inherited FormListadoCostoArtVendidos: TFormListadoCostoArtVendidos
  Left = 222
  Top = 87
  Caption = 'Listado de Costo de Mercaderia Vendidas'
  ClientHeight = 684
  ClientWidth = 1047
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 1065
  ExplicitHeight = 725
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1047
    Height = 631
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1047
    ExplicitHeight = 631
    object PageControl1: TPageControl
      Left = 0
      Top = 57
      Width = 1047
      Height = 574
      ActivePage = CtroCosto
      Align = alClient
      TabOrder = 0
      object TabSheet1: TTabSheet
        Caption = 'General'
        DesignSize = (
          1039
          546)
        object Label1: TLabel
          Left = 377
          Top = 438
          Width = 41
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Sucursal'
          ExplicitTop = 401
        end
        object Label2: TLabel
          Left = 377
          Top = 484
          Width = 57
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Vendedores'
          ExplicitTop = 447
        end
        object Label3: TLabel
          Left = 605
          Top = 484
          Width = 30
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Zonas'
          ExplicitTop = 447
        end
        object RxLabel10: TJvLabel
          Left = 5
          Top = 411
          Width = 31
          Height = 13
          Caption = 'Grupo'
          Anchors = [akLeft, akBottom]
          ShadowSize = 1
          ShadowPos = spLeftTop
          Transparent = True
          ExplicitTop = 374
        end
        object dbgDetalle: TDBGrid
          Left = 0
          Top = 0
          Width = 1039
          Height = 405
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          DataSource = DSDetalle
          FixedColor = clBlue
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDrawColumnCell = dbgDetalleDrawColumnCell
          Columns = <
            item
              Expanded = False
              FieldName = 'CODIGOARTICULO'
              Title.Alignment = taCenter
              Title.Caption = 'Codigo'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 71
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DETALLE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 317
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CANTIDAD'
              Title.Alignment = taCenter
              Title.Caption = 'Cant.'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'COSTO_TOTAL'
              Title.Alignment = taCenter
              Title.Caption = '$ Costo'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'PRECIOVTA_UNITARIO'
              Title.Alignment = taCenter
              Title.Caption = '$ Vta'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TOTAL_COSTO_VENTA'
              Title.Alignment = taCenter
              Title.Caption = 'Total Costo'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 78
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TOTAL_VENTA'
              Title.Alignment = taCenter
              Title.Caption = 'Total Vta.'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 74
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'COMISION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clNavy
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 80
              Visible = True
            end>
        end
        object dbcGrupo: TJvDBLookupCombo
          Left = 5
          Top = 430
          Width = 171
          Height = 21
          DisplayEmpty = 'Sin Selecci'#243'n'
          EmptyValue = '-1'
          Anchors = [akLeft, akBottom]
          LookupField = 'ID'
          LookupDisplay = 'DETALLE'
          LookupSource = DSFiltroCab
          TabOrder = 1
          OnChange = dbcGrupoChange
        end
        object chFiltro: TCheckBox
          Left = 182
          Top = 430
          Width = 97
          Height = 17
          Anchors = [akLeft, akBottom]
          Caption = 'Usa Filtro'
          TabOrder = 2
          OnClick = chFiltroClick
        end
        object dbgDetallefiltro: TDBGrid
          Left = 5
          Top = 457
          Width = 331
          Height = 84
          Anchors = [akLeft, akBottom]
          DataSource = DSFiltroDet
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 3
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'MUESTRACOMPROBANTE'
              Title.Alignment = taCenter
              Title.Caption = 'Cpbte.'
              Title.Color = clAqua
              Width = 184
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MUESTRASUC'
              Title.Alignment = taCenter
              Title.Caption = 'Sucursal'
              Title.Color = clAqua
              Width = 122
              Visible = True
            end>
        end
        object dbcSucursal: TJvDBLookupCombo
          Left = 375
          Top = 457
          Width = 210
          Height = 21
          DisplayEmpty = 'Todas las Sucursales'
          EmptyValue = '-1'
          Anchors = [akLeft, akBottom]
          LookupField = 'CODIGO'
          LookupDisplay = 'DETALLE'
          LookupSource = DSSucursales
          TabOrder = 4
        end
        object dbcVendedor: TJvDBLookupCombo
          Left = 375
          Top = 503
          Width = 210
          Height = 21
          DisplayEmpty = 'Todos Los Vendedores'
          EmptyValue = '******'
          Anchors = [akLeft, akBottom]
          LookupField = 'CODIGO'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSPersonal
          TabOrder = 5
        end
        object dbcZona: TJvDBLookupCombo
          Left = 605
          Top = 503
          Width = 210
          Height = 21
          DisplayEmpty = 'Todas las Zonas'
          EmptyValue = '-1'
          Anchors = [akLeft, akBottom]
          LookupField = 'ID_ZONA'
          LookupDisplay = 'DETALLE'
          LookupSource = DSZona
          TabOrder = 6
        end
        object chStock: TCheckBox
          Left = 605
          Top = 461
          Width = 183
          Height = 17
          Anchors = [akLeft, akBottom]
          Caption = 'Los Art.con Seguimiento de Stock'
          TabOrder = 7
        end
        object chRtoObra: TCheckBox
          Left = 605
          Top = 430
          Width = 183
          Height = 17
          Anchors = [akLeft, akBottom]
          Caption = 'Incluir Rtos.de Obra'
          TabOrder = 8
        end
      end
      object TabSheet2: TTabSheet
        Caption = 'Cruzado'
        ImageIndex = 1
        object cxGrid1: TcxGrid
          Left = 0
          Top = 0
          Width = 1039
          Height = 546
          Align = alClient
          TabOrder = 0
          LevelTabs.Style = 8
          object cxGrid1DBTableView1: TcxGridDBTableView
            Navigator.Buttons.CustomButtons = <>
            Navigator.Buttons.First.Visible = True
            Navigator.Buttons.PriorPage.Visible = True
            Navigator.Buttons.Prior.Visible = True
            Navigator.Buttons.Next.Visible = True
            Navigator.Buttons.NextPage.Visible = True
            Navigator.Buttons.Last.Visible = True
            Navigator.Buttons.Insert.Visible = True
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Visible = True
            Navigator.Buttons.Edit.Visible = True
            Navigator.Buttons.Post.Visible = True
            Navigator.Buttons.Cancel.Visible = True
            Navigator.Buttons.Refresh.Visible = True
            Navigator.Buttons.SaveBookmark.Visible = True
            Navigator.Buttons.GotoBookmark.Visible = True
            Navigator.Buttons.Filter.Visible = True
            ScrollbarAnnotations.CustomAnnotations = <>
            DataController.DataSource = DSDetalle
            DataController.Summary.DefaultGroupSummaryItems = <>
            DataController.Summary.FooterSummaryItems = <>
            DataController.Summary.SummaryGroups = <>
            object cxGrid1DBTableView1DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRAARTICULO'
              Visible = False
              GroupIndex = 1
              Options.ShowEditButtons = isebNever
              Options.CellMerging = True
            end
            object cxGrid1DBTableView1CLASE_CPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'CLASE_CPBTE'
            end
            object cxGrid1DBTableView1TIPO_CPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'TIPO_CPBTE'
            end
            object cxGrid1DBTableView1NROCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'NROCPBTE'
            end
            object cxGrid1DBTableView1FECHACPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'FECHACPBTE'
              Width = 124
            end
            object cxGrid1DBTableView1CANTIDAD: TcxGridDBColumn
              DataBinding.FieldName = 'CANTIDAD'
            end
            object cxGrid1DBTableView1COSTO_TOTAL: TcxGridDBColumn
              Caption = 'Costo Unit.'
              DataBinding.FieldName = 'COSTO_TOTAL'
            end
            object cxGrid1DBTableView1PRECIOVTA_UNITARIO: TcxGridDBColumn
              DataBinding.FieldName = 'PRECIOVTA_UNITARIO'
            end
            object cxGrid1DBTableView1TOTAL_COSTO_VENTA: TcxGridDBColumn
              Caption = 'Costo Vta'
              DataBinding.FieldName = 'TOTAL_COSTO_VENTA'
            end
            object cxGrid1DBTableView1TOTAL_VENTA: TcxGridDBColumn
              Caption = 'Tot.de Vta'
              DataBinding.FieldName = 'TOTAL_VENTA'
            end
            object cxGrid1DBTableView1T_COSTO_CIVA: TcxGridDBColumn
              DataBinding.FieldName = 'T_COSTO_CIVA'
              Width = 87
            end
            object cxGrid1DBTableView1T_VTA_CIVA: TcxGridDBColumn
              DataBinding.FieldName = 'T_VTA_CIVA'
              Width = 81
            end
            object cxGrid1DBTableView1RENT_SOBRECOSTO: TcxGridDBColumn
              DataBinding.FieldName = 'RENT_SOBRECOSTO'
            end
            object cxGrid1DBTableView1RENT_SOBREVTA: TcxGridDBColumn
              DataBinding.FieldName = 'RENT_SOBREVTA'
            end
            object cxGrid1DBTableView1DECLARA_IVA: TcxGridDBColumn
              Caption = 'Iva'
              DataBinding.FieldName = 'DECLARA_IVA'
            end
            object cxGrid1DBTableView1CENTRO_COSTO: TcxGridDBColumn
              DataBinding.FieldName = 'CENTRO_COSTO'
              Visible = False
              GroupIndex = 0
            end
          end
          object cxGrid1Level1: TcxGridLevel
            GridView = cxGrid1DBTableView1
          end
        end
      end
      object CtroCosto: TTabSheet
        Caption = 'Centro de Costo'
        ImageIndex = 2
        object DBGrid1: TDBGrid
          Left = 0
          Top = 0
          Width = 1039
          Height = 489
          Align = alTop
          DataSource = DSGastos
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
        end
      end
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 1047
      Height = 57
      Align = alTop
      BevelOuter = bvNone
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 1
      object Label4: TLabel
        Left = 340
        Top = 1
        Width = 268
        Height = 26
        Caption = 
          'Los Item Resaltado, su costo fue tomado desde la ficha, '#13#10'por no' +
          ' tener costo cargado en el momento de la vta.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object RxLabel1: TJvLabel
        Left = 829
        Top = 4
        Width = 33
        Height = 13
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel2: TJvLabel
        Left = 933
        Top = 3
        Width = 30
        Height = 13
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object Label5: TLabel
        Left = 16
        Top = 13
        Width = 271
        Height = 25
        Caption = 'Listado Mercaderia Vendida'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -21
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Button1: TButton
        Left = 640
        Top = 23
        Width = 125
        Height = 19
        Caption = 'Filtrar comprobantes'
        TabOrder = 0
        OnClick = Button1Click
      end
      object Hasta: TJvDateEdit
        Left = 924
        Top = 19
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 1
        OnAcceptDate = HastaAcceptDate
      end
      object Desde: TJvDateEdit
        Left = 829
        Top = 19
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 3
        OnAcceptDate = DesdeAcceptDate
      end
      object chBuscar: TAdvOfficeCheckBox
        Left = 579
        Top = 0
        Width = 200
        Height = 17
        BiDiMode = bdRightToLeft
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        TabStop = True
        Alignment = taLeftJustify
        Caption = 'Ejecutar Busqueda al cambiar'
        ReturnIsTab = False
        Version = '1.8.4.1'
      end
      object UpDown1: TUpDown
        Left = 1019
        Top = 19
        Width = 16
        Height = 20
        Hint = 'Cambia el Per'#237'odo'
        Min = -100000
        Max = 10000
        ParentShowHint = False
        Position = 100
        ShowHint = True
        TabOrder = 4
        OnClick = UpDown1Click
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 631
    Width = 1047
    Align = alBottom
    ButtonHeight = 28
    ExplicitTop = 631
    ExplicitWidth = 1047
    inherited btConfirma: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btNuevo: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btCancelar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btModificar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited Pr: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBuscar: TBitBtn
      Height = 28
      DoubleBuffered = True
      ExplicitHeight = 28
    end
    inherited Ne: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBorrar: TBitBtn
      Height = 28
      DoubleBuffered = True
      Visible = False
      ExplicitHeight = 28
    end
    inherited btSalir: TBitBtn
      Height = 28
      DoubleBuffered = True
      ExplicitHeight = 28
    end
    object ToolButton1: TToolButton
      Left = 557
      Top = 0
      Width = 8
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object BitBtn1: TBitBtn
      Left = 565
      Top = 0
      Width = 75
      Height = 28
      Action = Imprimir
      Caption = '&Imprimir'
      ImageIndex = 9
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      TabOrder = 7
    end
    object BitBtn4: TBitBtn
      Left = 640
      Top = 0
      Width = 96
      Height = 28
      Action = ExportarXLS
      Caption = 'Exportar XLS'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 8
    end
  end
  inherited Panel1: TPanel
    Top = 661
    Width = 1047
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 661
    ExplicitWidth = 1047
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
      Visible = False
    end
    inherited sbEstado: TStatusBar
      Width = 958
      SimplePanel = True
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 958
    end
  end
  inherited ActionList1: TActionList
    Left = 416
    Top = 160
    object Imprimir: TAction
      Caption = '&Imprimir'
      ImageIndex = 9
      OnExecute = ImprimirExecute
    end
    object ExportarXLS: TAction
      Caption = 'Exportar XLS'
      OnExecute = ExportarXLSExecute
    end
    object ListadoPorCtrCosto: TAction
      Caption = 'ListadoPorCtrCosto'
      OnExecute = ListadoPorCtrCostoExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 432
  end
  inherited ImageList1: TImageList
    Left = 256
    Top = 24
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 48
    Top = 320
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 112
  end
  object DSDetalle: TDataSource
    DataSet = CDSDetalle
    Left = 248
    Top = 104
  end
  object frListado: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39638.819200300920000000
    ReportOptions.LastChange = 39638.819200300920000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 536
    Top = 136
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frDBMov: TfrxDBDataset
    UserName = 'frDBMov'
    CloseDataSource = False
    DataSet = CDSDetalle
    BCDToCurrency = False
    DataSetOptions = []
    Left = 464
    Top = 128
    FieldDefs = <
      item
        FieldName = 'ID_COMPROBANTE'
        FieldAlias = 'ID_COMPROBANTE'
      end
      item
        FieldName = 'TIPO_CPBTE'
        FieldAlias = 'TIPO_CPBTE'
      end
      item
        FieldName = 'CLASE_CPBTE'
        FieldAlias = 'CLASE_CPBTE'
      end
      item
        FieldName = 'NROCPBTE'
        FieldAlias = 'NROCPBTE'
      end
      item
        FieldName = 'FECHACPBTE'
        FieldAlias = 'FECHACPBTE'
      end
      item
        FieldName = 'CODIGOARTICULO'
        FieldAlias = 'CODIGOARTICULO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'CANTIDAD'
        FieldAlias = 'CANTIDAD'
      end
      item
        FieldName = 'COSTO_TOTAL'
        FieldAlias = 'COSTO_TOTAL'
      end
      item
        FieldName = 'PRECIOVTA_UNITARIO'
        FieldAlias = 'PRECIOVTA_UNITARIO'
      end
      item
        FieldName = 'TOTAL_COSTO_VENTA'
        FieldAlias = 'TOTAL_COSTO_VENTA'
      end
      item
        FieldName = 'TOTAL_VENTA'
        FieldAlias = 'TOTAL_VENTA'
      end
      item
        FieldName = 'PORCENTAJE'
        FieldAlias = 'PORCENTAJE'
      end
      item
        FieldName = 'ACTUALIZADO'
        FieldAlias = 'ACTUALIZADO'
      end
      item
        FieldName = 'RENTABILIDAD'
        FieldAlias = 'RENTABILIDAD'
      end
      item
        FieldName = 'T_COSTO_CIVA'
        FieldAlias = 'T_COSTO_CIVA'
      end
      item
        FieldName = 'T_VTA_CIVA'
        FieldAlias = 'T_VTA_CIVA'
      end
      item
        FieldName = 'RENT_SOBRECOSTO'
        FieldAlias = 'RENT_SOBRECOSTO'
      end
      item
        FieldName = 'RENT_SOBREVTA'
        FieldAlias = 'RENT_SOBREVTA'
      end
      item
        FieldName = 'IVA_TASA'
        FieldAlias = 'IVA_TASA'
      end
      item
        FieldName = 'DECLARA_IVA'
        FieldAlias = 'DECLARA_IVA'
      end
      item
        FieldName = 'MUESTRAARTICULO'
        FieldAlias = 'MUESTRAARTICULO'
      end
      item
        FieldName = 'COMISION'
        FieldAlias = 'COMISION'
      end>
  end
  object DataToXLS: TDataToXLS
    DataSet = CDSDetalle
    Columns = <
      item
        DataField = 'TIPO_CPBTE'
        Title = 'Tp'
      end
      item
        DataField = 'CLASE_CPBTE'
        Title = 'Cl'
      end
      item
        DataField = 'NROCPBTE'
        Title = 'Nro.Cpbte'
      end
      item
        DataField = 'FECHACPBTE'
        Title = 'Fecha'
      end
      item
        DataField = 'CODIGOARTICULO'
        Title = 'Codigo Art.'
      end
      item
        DataField = 'DETALLE'
        Title = 'Detalle'
      end
      item
        DataField = 'CANTIDAD'
        Title = 'Cantidad'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'COSTO_TOTAL'
        Title = 'Costo Total'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'PRECIOVTA_UNITARIO'
        Title = 'Precio Vta Unit.'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'TOTAL_COSTO_VENTA'
        Title = 'T.Costo Vta'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'TOTAL_VENTA'
        Title = 'T.de Vta'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'PORCENTAJE'
        Title = 'Porcent.'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'ACTUALIZADO'
        Title = 'Actual.'
      end
      item
        DataField = 'RENTABILIDAD'
        Title = 'Rentabil.'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'COMISION'
        Title = 'Comision'
        DataType = ctDouble
      end>
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -13
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Header.Font.Charset = DEFAULT_CHARSET
    Header.Font.Color = clWindowText
    Header.Font.Height = -13
    Header.Font.Name = 'Arial'
    Header.Font.Style = [fsBold]
    Left = 592
    Top = 200
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'txt'
    FileName = 'citi'
    InitialDir = 'c:\'
    Title = 'Archivo de Retenciones'
    Left = 336
    Top = 224
  end
  object CDSDetalle: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'vendedor'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'zona'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'control'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'obra'
        ParamType = ptInput
      end>
    ProviderName = 'DSPDetalle'
    OnCalcFields = CDSDetalleCalcFields
    Left = 168
    Top = 104
    object CDSDetalleID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Origin = 'ID_COMPROBANTE'
    end
    object CDSDetalleTIPO_CPBTE: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPO_CPBTE'
      Origin = 'TIPO_CPBTE'
      Size = 2
    end
    object CDSDetalleCLASE_CPBTE: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASE_CPBTE'
      Origin = 'CLASE_CPBTE'
      Size = 2
    end
    object CDSDetalleNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSDetalleFECHACPBTE: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHACPBTE'
      Origin = 'FECHACPBTE'
    end
    object CDSDetalleCODIGOARTICULO: TStringField
      DisplayLabel = 'Codigo Art.'
      FieldName = 'CODIGOARTICULO'
      Origin = 'CODIGOARTICULO'
      Size = 8
    end
    object CDSDetalleDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 50
    end
    object CDSDetalleCANTIDAD: TFMTBCDField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetalleCOSTO_TOTAL: TFMTBCDField
      DisplayLabel = 'Costo Total'
      FieldName = 'COSTO_TOTAL'
      Origin = 'COSTO_TOTAL'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetallePRECIOVTA_UNITARIO: TFMTBCDField
      DisplayLabel = 'Precio Vta Unit.'
      FieldName = 'PRECIOVTA_UNITARIO'
      Origin = 'PRECIOVTA_UNITARIO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetalleTOTAL_COSTO_VENTA: TFMTBCDField
      DisplayLabel = 'T.Costo Vta'
      FieldName = 'TOTAL_COSTO_VENTA'
      Origin = 'TOTAL_COSTO_VENTA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetalleTOTAL_VENTA: TFMTBCDField
      DisplayLabel = 'T.de Vta'
      FieldName = 'TOTAL_VENTA'
      Origin = 'TOTAL_VENTA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetallePORCENTAJE: TFMTBCDField
      DisplayLabel = 'Porcent.'
      FieldName = 'PORCENTAJE'
      Origin = 'PORCENTAJE'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetalleACTUALIZADO: TStringField
      DisplayLabel = 'Actual.'
      FieldName = 'ACTUALIZADO'
      Origin = 'ACTUALIZADO'
      Size = 1
    end
    object CDSDetalleRENTABILIDAD: TFMTBCDField
      DisplayLabel = 'Rentabil.'
      FieldName = 'RENTABILIDAD'
      Origin = 'RENTABILIDAD'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
      Precision = 15
      Size = 3
    end
    object CDSDetalleT_COSTO_CIVA: TFMTBCDField
      DisplayLabel = 'Costo C/IVA'
      FieldKind = fkCalculated
      FieldName = 'T_COSTO_CIVA'
      DisplayFormat = ',0.00'
      Calculated = True
    end
    object CDSDetalleT_VTA_CIVA: TFMTBCDField
      DisplayLabel = 'Vta.C/IVA'
      FieldKind = fkCalculated
      FieldName = 'T_VTA_CIVA'
      DisplayFormat = ',0.00'
      Calculated = True
    end
    object CDSDetalleRENT_SOBRECOSTO: TFMTBCDField
      DisplayLabel = 'Rent.S/Costo'
      FieldKind = fkCalculated
      FieldName = 'RENT_SOBRECOSTO'
      DisplayFormat = ',0.00'
      Calculated = True
    end
    object CDSDetalleRENT_SOBREVTA: TFMTBCDField
      DisplayLabel = 'Rent.S/Vta'
      FieldKind = fkCalculated
      FieldName = 'RENT_SOBREVTA'
      DisplayFormat = ',0.00'
      Calculated = True
    end
    object CDSDetalleIVA_TASA: TFMTBCDField
      FieldName = 'IVA_TASA'
      Origin = 'IVA_TASA'
      DisplayFormat = ',0.00'
      Precision = 15
      Size = 2
    end
    object CDSDetalleDECLARA_IVA: TStringField
      FieldName = 'DECLARA_IVA'
      Origin = 'DECLARA_IVA'
      Size = 1
    end
    object CDSDetalleMUESTRAARTICULO: TStringField
      DisplayLabel = 'Articulo'
      FieldKind = fkCalculated
      FieldName = 'MUESTRAARTICULO'
      Size = 60
      Calculated = True
    end
    object CDSDetalleCOMISION: TFMTBCDField
      DisplayLabel = '% Comisi'
      FieldName = 'COMISION'
      Origin = 'COMISION'
      DisplayFormat = '0.00'
      Precision = 15
      Size = 2
    end
    object CDSDetalleCENTRO_COSTO: TStringField
      DisplayLabel = 'Centro de Costo'
      FieldName = 'CENTRO_COSTO'
      Size = 45
    end
  end
  object DSPDetalle: TDataSetProvider
    DataSet = QDetalle
    Options = []
    Left = 136
    Top = 160
  end
  object DSFiltroCab: TDataSource
    DataSet = QFiltroCab
    Left = 696
    Top = 464
  end
  object DSFiltroDet: TDataSource
    DataSet = QFiltroDet
    Left = 728
    Top = 528
  end
  object DSZona: TDataSource
    DataSet = CDSZona
    Left = 256
    Top = 496
  end
  object CDSZona: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPZona'
    Left = 312
    Top = 496
    object CDSZonaID_ZONA: TIntegerField
      FieldName = 'ID_ZONA'
      Required = True
    end
    object CDSZonaDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 25
    end
  end
  object DSPZona: TDataSetProvider
    DataSet = DMBuscadores.QBuscaZona
    Options = []
    Left = 376
    Top = 496
  end
  object DSSucursales: TDataSource
    DataSet = CDSSucursal
    Left = 72
    Top = 488
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 128
    Top = 488
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 35
    end
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Options = []
    Left = 192
    Top = 488
  end
  object DSPPersonal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaPersonal
    Options = []
    Left = 192
    Top = 536
  end
  object CDSPersonal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPersonal'
    Left = 128
    Top = 536
    object CDSPersonalNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 50
    end
    object CDSPersonalCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 6
    end
  end
  object DSPersonal: TDataSource
    DataSet = CDSPersonal
    Left = 72
    Top = 536
  end
  object QDetalle: TFDQuery
    Connection = DMMain_FD.fdcGestion
    FormatOptions.AssignedValues = [fvMapRules]
    FormatOptions.OwnMapRules = True
    FormatOptions.MapRules = <
      item
        SourceDataType = dtDateTime
        TargetDataType = dtDate
      end
      item
        SourceDataType = dtDouble
        TargetDataType = dtFmtBCD
      end>
    SQL.Strings = (
      
        'select * from LISTA_COSTO_ART_VDO_OBRA (:desde,:hasta,:vendedor,' +
        ':sucursal,:zona,:control,:obra)'
      'order by codigoarticulo,tipo_cpbte')
    Left = 52
    Top = 152
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'vendedor'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'zona'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 6
        Name = 'control'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 7
        Name = 'obra'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QFiltroCab: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from TABLA_CPBTE_CCVTA_CAB')
    Left = 612
    Top = 456
    object QFiltroCabID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QFiltroCabDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 25
    end
    object QFiltroCabGRUPO: TSmallintField
      FieldName = 'GRUPO'
      Origin = 'GRUPO'
    end
  end
  object QFiltroDet: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select tc.*,'
      'c.denominacion as MuestraComprobante,'
      'c.tipo_comprob as MuestraTipo,'
      'c.clase_comprob as MuestraClase,'
      's.detalle  as MuestraSuc from tabla_cpbte_ccvta_det tc'
      'left join comprobantes c on c.id_comprobante=tc.id_cpbte'
      'left join sucursal s on s.codigo=c.sucursal'
      'where tc.id_cab=:id'
      'order by s.detalle,c.tipo_comprob,c.clase_comprob')
    Left = 584
    Top = 529
    ParamData = <
      item
        Position = 1
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QFiltroDetID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QFiltroDetID_CAB: TIntegerField
      FieldName = 'ID_CAB'
      Origin = 'ID_CAB'
      Required = True
    end
    object QFiltroDetID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
    end
    object QFiltroDetMUESTRACOMPROBANTE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRACOMPROBANTE'
      Origin = 'DENOMINACION'
      ProviderFlags = []
      ReadOnly = True
      Size = 15
    end
    object QFiltroDetMUESTRATIPO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRATIPO'
      Origin = 'TIPO_COMPROB'
      ProviderFlags = []
      ReadOnly = True
      Size = 2
    end
    object QFiltroDetMUESTRACLASE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRACLASE'
      Origin = 'CLASE_COMPROB'
      ProviderFlags = []
      ReadOnly = True
      Size = 2
    end
    object QFiltroDetMUESTRASUC: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRASUC'
      Origin = 'DETALLE'
      ProviderFlags = []
      ReadOnly = True
      Size = 35
    end
  end
  object QGastos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    FormatOptions.AssignedValues = [fvMapRules]
    FormatOptions.OwnMapRules = True
    FormatOptions.MapRules = <
      item
        SourceDataType = dtDateTime
        TargetDataType = dtDate
      end
      item
        SourceDataType = dtDateTimeStamp
        TargetDataType = dtDate
      end>
    SQL.Strings = (
      
        'select tipo,ctrocosto,nombre,sum(debe) as debe ,sum(haber) as ha' +
        'ber from LST_POR_CTRO_COSTO_new(:desde,:hasta,:ingresos,:vtas,:p' +
        'ersonal) where ctrocosto= :ct or :ct = -1'
      ''
      'group by tipo,ctrocosto,nombre'
      '')
    Left = 664
    Top = 378
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'ingresos'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'vtas'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'personal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 6
        Name = 'ct'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 7
        Name = 'ct'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object DSPGastos: TDataSetProvider
    DataSet = QGastos
    Options = []
    Left = 748
    Top = 372
  end
  object CDSGastos: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'ingresos'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'vtas'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'personal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'ct'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'ct'
        ParamType = ptInput
      end>
    ProviderName = 'DSPGastos'
    Left = 828
    Top = 372
    object CDSGastosTIPO: TStringField
      FieldName = 'TIPO'
      Size = 1
    end
    object CDSGastosCTROCOSTO: TIntegerField
      FieldName = 'CTROCOSTO'
    end
    object CDSGastosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 100
    end
    object CDSGastosDEBE: TFloatField
      FieldName = 'DEBE'
      ReadOnly = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
    object CDSGastosHABER: TFloatField
      FieldName = 'HABER'
      ReadOnly = True
      DisplayFormat = ',0.00;-,0.00;-'
    end
  end
  object DSGastos: TDataSource
    DataSet = CDSGastos
    Left = 891
    Top = 380
  end
end
