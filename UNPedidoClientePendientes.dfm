inherited FormNPedidoClientePendientes: TFormNPedidoClientePendientes
  Left = 95
  Top = 191
  Caption = 'N.de Pedido Pendientes'
  ClientHeight = 608
  ClientWidth = 911
  StyleElements = [seFont, seClient, seBorder]
  OnActivate = FormActivate
  ExplicitWidth = 929
  ExplicitHeight = 649
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 911
    Height = 555
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 911
    ExplicitHeight = 555
    object Splitter1: TSplitter
      Left = 329
      Top = 0
      Height = 251
      ExplicitLeft = 353
      ExplicitTop = -1
      ExplicitHeight = 222
    end
    object Splitter2: TSplitter
      Left = 0
      Top = 251
      Width = 911
      Height = 4
      Cursor = crVSplit
      Align = alBottom
      ExplicitLeft = 1
      ExplicitTop = 264
      ExplicitWidth = 895
    end
    object Panel2: TPanel
      Left = 0
      Top = 0
      Width = 329
      Height = 251
      Align = alLeft
      Caption = 'Panel2'
      TabOrder = 0
      object dbgResultados: TDBGrid
        Left = 1
        Top = 26
        Width = 327
        Height = 224
        Align = alClient
        DataSource = DSPendientesClientes
        DrawingStyle = gdsGradient
        GradientEndColor = clBlue
        GradientStartColor = clGreen
        Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        PopupMenu = PopupMenu1
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = VerNPExecute
        OnTitleClick = dbgResultadosTitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'FECHAPEDIDO'
            Title.Alignment = taCenter
            Title.Caption = 'Fecha'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 94
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NROCPBTE'
            Title.Alignment = taCenter
            Title.Caption = 'Nro.Cpbte'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 89
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODIGO'
            Title.Alignment = taCenter
            Title.Caption = 'Codigo'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 62
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Alignment = taCenter
            Title.Caption = 'Nombre'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end>
      end
      object Panel7: TPanel
        Left = 1
        Top = 1
        Width = 327
        Height = 25
        Align = alTop
        BevelOuter = bvNone
        Color = clNavy
        ParentBackground = False
        TabOrder = 1
        object Label1: TLabel
          Left = 7
          Top = 5
          Width = 105
          Height = 13
          Caption = 'Pedido por Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
    object Panel3: TPanel
      Left = 0
      Top = 255
      Width = 911
      Height = 300
      Align = alBottom
      Caption = 'Panel3'
      TabOrder = 1
      object Splitter3: TSplitter
        Left = 1
        Top = 156
        Width = 909
        Height = 4
        Cursor = crVSplit
        Align = alBottom
        ExplicitTop = 155
        ExplicitWidth = 837
      end
      object dbgArticulosPorCliente: TDBGrid
        Left = 1
        Top = 25
        Width = 909
        Height = 131
        Align = alClient
        DataSource = DSArticulosPorCliente
        DrawingStyle = gdsGradient
        GradientEndColor = clFuchsia
        GradientStartColor = clBlue
        Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        PopupMenu = PopupMenu3
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = VerNPExecute
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGOARTICULO'
            Title.Alignment = taCenter
            Title.Caption = 'Codigo'
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 119
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DETALLE'
            Title.Alignment = taCenter
            Title.Caption = 'Detalle'
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 320
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PEDIDA'
            Title.Alignment = taCenter
            Title.Caption = 'Pedida'
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 106
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ENTREGADA'
            Title.Alignment = taCenter
            Title.Caption = 'Entregada'
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 91
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FALTANTE'
            Title.Alignment = taCenter
            Title.Caption = 'Faltante'
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 94
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FECHA'
            Title.Alignment = taCenter
            Title.Color = clGreen
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end>
      end
      object Panel5: TPanel
        Left = 1
        Top = 160
        Width = 909
        Height = 139
        Align = alBottom
        Caption = 'Panel5'
        TabOrder = 1
        object Splitter4: TSplitter
          Left = 449
          Top = 25
          Height = 113
        end
        object dbgTotales: TDBGrid
          Left = 1
          Top = 25
          Width = 448
          Height = 113
          Align = alLeft
          DataSource = DSTotales
          DrawingStyle = gdsGradient
          GradientEndColor = clFuchsia
          GradientStartColor = clBlue
          Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          PopupMenu = PopupMenu2
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDblClick = VerNPExecute
          OnTitleClick = dbgTotalesTitleClick
          Columns = <
            item
              Expanded = False
              FieldName = 'CODIGOARTICULO'
              Title.Alignment = taCenter
              Title.Caption = 'Codigo'
              Title.Color = clMaroon
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clYellow
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 62
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DETALLE'
              Title.Alignment = taCenter
              Title.Caption = 'Detalle'
              Title.Color = clMaroon
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clYellow
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FALTANTE'
              Title.Alignment = taCenter
              Title.Caption = 'Cant.Faltante'
              Title.Color = clMaroon
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clYellow
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 89
              Visible = True
            end>
        end
        object DBGrid3: TDBGrid
          Left = 452
          Top = 25
          Width = 456
          Height = 113
          Align = alClient
          DataSource = DSArticulosClientes
          DrawingStyle = gdsGradient
          GradientEndColor = clBlue
          GradientStartColor = clFuchsia
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          PopupMenu = PopupMenu2
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clSilver
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = [fsBold]
          Columns = <
            item
              Expanded = False
              FieldName = 'CODIGO'
              Title.Alignment = taCenter
              Title.Color = clMoneyGreen
              Width = 72
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOMBRE'
              Title.Alignment = taCenter
              Title.Color = clMoneyGreen
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CANTIDAD_FALTANTE'
              Title.Alignment = taCenter
              Title.Color = clMoneyGreen
              Visible = True
            end>
        end
        object Panel9: TPanel
          Left = 1
          Top = 1
          Width = 907
          Height = 24
          Align = alTop
          Color = clNavy
          ParentBackground = False
          TabOrder = 2
          object Label4: TLabel
            Left = 7
            Top = 6
            Width = 176
            Height = 13
            Caption = 'Total de Mercaderia Pendiente'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
          object Label5: TLabel
            Left = 639
            Top = 6
            Width = 189
            Height = 13
            Caption = 'Clientes Por Articulos Pendientes'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWhite
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = [fsBold]
            ParentFont = False
          end
        end
      end
      object Panel8: TPanel
        Left = 1
        Top = 1
        Width = 909
        Height = 24
        Align = alTop
        Caption = ' '
        Color = clNavy
        ParentBackground = False
        TabOrder = 2
        object Label3: TLabel
          Left = 12
          Top = 6
          Width = 232
          Height = 13
          Caption = 'Total de Articulos Pendientes del Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
    object Panel4: TPanel
      Left = 332
      Top = 0
      Width = 579
      Height = 251
      Align = alClient
      Caption = 'Panel4'
      TabOrder = 2
      object dbgPendientesDetalles: TDBGrid
        Left = 1
        Top = 26
        Width = 577
        Height = 224
        Align = alClient
        DataSource = DSPendientesDetalles
        DrawingStyle = gdsGradient
        GradientEndColor = clGreen
        GradientStartColor = clBlue
        Options = [dgTitles, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = VerNPExecute
        OnTitleClick = dbgPendientesDetallesTitleClick
        Columns = <
          item
            Expanded = False
            FieldName = 'CODIGOARTICULO'
            Title.Alignment = taCenter
            Title.Caption = 'Codigo'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 72
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DETALLE'
            Title.Alignment = taCenter
            Title.Caption = 'Detalle'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CANTIDAD_FALTANTE'
            Title.Alignment = taCenter
            Title.Caption = 'Faltante'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 84
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CANTIDAD_PEDIDIA'
            Title.Alignment = taCenter
            Title.Caption = 'Pedido'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 96
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CANTIDAD_ENTREGADA'
            Title.Alignment = taCenter
            Title.Caption = 'Entregado'
            Title.Color = clBlue
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 92
            Visible = True
          end>
      end
      object Panel6: TPanel
        Left = 1
        Top = 1
        Width = 577
        Height = 25
        Align = alTop
        BevelOuter = bvNone
        Color = clNavy
        ParentBackground = False
        TabOrder = 1
        object Label2: TLabel
          Left = 6
          Top = 6
          Width = 227
          Height = 13
          Caption = 'Articulos Pendientes por Pedido/Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWhite
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 555
    Width = 911
    Align = alBottom
    ExplicitTop = 555
    ExplicitWidth = 911
    inherited btConfirma: TBitBtn
      DoubleBuffered = True
      Visible = False
    end
    inherited btNuevo: TBitBtn
      DoubleBuffered = True
      Visible = False
    end
    inherited btCancelar: TBitBtn
      DoubleBuffered = True
      Visible = False
    end
    inherited btModificar: TBitBtn
      DoubleBuffered = True
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited btBuscar: TBitBtn
      DoubleBuffered = True
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
    inherited btBorrar: TBitBtn
      DoubleBuffered = True
      Visible = False
    end
    inherited btSalir: TBitBtn
      DoubleBuffered = True
    end
    object chbCodAlter: TCheckBox
      Left = 557
      Top = 0
      Width = 97
      Height = 29
      Caption = 'Ver Cod.Alternativo'
      TabOrder = 7
      OnClick = chbCodAlterClick
    end
  end
  inherited Panel1: TPanel
    Top = 585
    Width = 911
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 585
    ExplicitWidth = 911
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
      Visible = False
    end
    inherited sbEstado: TStatusBar
      Width = 822
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 822
    end
  end
  inherited ActionList1: TActionList
    Left = 640
    Top = 56
    object VerNP: TAction
      Caption = 'VerNP'
      OnExecute = VerNPExecute
    end
    object AnularPedido: TAction
      Caption = 'Anular Pedido'
      OnExecute = AnularPedidoExecute
    end
    object ImprimirPendientesClientes: TAction
      Caption = 'Imprimir Pendientes Clientes'
      OnExecute = ImprimirPendientesClientesExecute
    end
    object ImrimirPendienteDelCliente: TAction
      Caption = 'Imrimir Pendiente Del Cliente'
      OnExecute = ImrimirPendienteDelClienteExecute
    end
    object ControlNP: TAction
      Caption = 'ControlNP'
      OnExecute = ControlNPExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 680
    Top = 40
  end
  inherited ImageList1: TImageList
    Left = 192
    Top = 32
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Left = 464
    Top = 184
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 104
    Top = 584
  end
  inherited QBrowse2: TFDQuery
    Left = 152
    Top = 608
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 136
    Top = 616
  end
  object DSPendientesDetalles: TDataSource
    DataSet = CDSPendientesDetalles
    Left = 560
    Top = 160
  end
  object DSPPendientesDetalles: TDataSetProvider
    DataSet = QPendientesDetalles
    Options = []
    Left = 456
    Top = 107
  end
  object CDSPendientesDetalles: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPPendientesDetalles'
    Left = 536
    Top = 96
    object CDSPendientesDetallesID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSPendientesDetallesID_CABNP: TIntegerField
      FieldName = 'ID_CABNP'
      Origin = 'ID_CABNP'
    end
    object CDSPendientesDetallesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Size = 2
    end
    object CDSPendientesDetallesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSPendientesDetallesNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSPendientesDetallesFECHAPEDIOD: TSQLTimeStampField
      FieldName = 'FECHAPEDIOD'
      Origin = 'FECHAPEDIOD'
    end
    object CDSPendientesDetallesCODIGOARTICULO: TStringField
      FieldName = 'CODIGOARTICULO'
      Origin = 'CODIGOARTICULO'
      OnGetText = CDSPendientesDetallesCODIGOARTICULOGetText
      Size = 8
    end
    object CDSPendientesDetallesDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSPendientesDetallesUNIDAD: TStringField
      FieldName = 'UNIDAD'
      Origin = 'UNIDAD'
      Size = 3
    end
    object CDSPendientesDetallesDESGLOZAIVA: TStringField
      FieldName = 'DESGLOZAIVA'
      Origin = 'DESGLOZAIVA'
      Size = 1
    end
    object CDSPendientesDetallesMODO_GRAVAMEN: TStringField
      FieldName = 'MODO_GRAVAMEN'
      Origin = 'MODO_GRAVAMEN'
      Size = 1
    end
    object CDSPendientesDetallesTIPOIVA_TASA: TIntegerField
      FieldName = 'TIPOIVA_TASA'
      Origin = 'TIPOIVA_TASA'
    end
    object CDSPendientesDetallesTIPOIVA_SOBRETASA: TIntegerField
      FieldName = 'TIPOIVA_SOBRETASA'
      Origin = 'TIPOIVA_SOBRETASA'
    end
    object CDSPendientesDetallesDEPOSITO: TIntegerField
      FieldName = 'DEPOSITO'
      Origin = 'DEPOSITO'
    end
    object CDSPendientesDetallesAFECTA_STOCK: TStringField
      FieldName = 'AFECTA_STOCK'
      Origin = 'AFECTA_STOCK'
      Size = 1
    end
    object CDSPendientesDetallesCALCULA_SOBRETASA: TStringField
      FieldName = 'CALCULA_SOBRETASA'
      Origin = 'CALCULA_SOBRETASA'
      Size = 1
    end
    object CDSPendientesDetallesGRAVADO_IB: TStringField
      FieldName = 'GRAVADO_IB'
      Origin = 'GRAVADO_IB'
      FixedChar = True
      Size = 1
    end
    object CDSPendientesDetallesID_MONEDA: TIntegerField
      FieldName = 'ID_MONEDA'
      Origin = 'ID_MONEDA'
    end
    object CDSPendientesDetallesMUESTRACODALTER: TStringField
      FieldName = 'MUESTRACODALTER'
      Origin = 'REEMPLAZO_STK'
      ProviderFlags = []
    end
    object CDSPendientesDetallesCANTIDAD_PEDIDIA: TFloatField
      FieldName = 'CANTIDAD_PEDIDIA'
      Origin = 'CANTIDAD_PEDIDIA'
      DisplayFormat = ',0.00'
    end
    object CDSPendientesDetallesCANTIDAD_ENTREGADA: TFloatField
      FieldName = 'CANTIDAD_ENTREGADA'
      Origin = 'CANTIDAD_ENTREGADA'
      DisplayFormat = ',0.00'
    end
    object CDSPendientesDetallesCANTIDAD_FALTANTE: TFloatField
      FieldName = 'CANTIDAD_FALTANTE'
      Origin = 'CANTIDAD_FALTANTE'
      DisplayFormat = ',0.00'
    end
    object CDSPendientesDetallesCANTIDAD_UNIDADES: TFloatField
      FieldName = 'CANTIDAD_UNIDADES'
      Origin = 'CANTIDAD_UNIDADES'
      Required = True
      DisplayFormat = ',0.00'
    end
  end
  object CDSPendientesClientes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPendientesClientes'
    AfterScroll = CDSPendientesClientesAfterScroll
    Left = 224
    Top = 88
    object CDSPendientesClientesID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSPendientesClientesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSPendientesClientesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSPendientesClientesNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSPendientesClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object CDSPendientesClientesLETRANP: TStringField
      FieldName = 'LETRANP'
      Origin = 'LETRANP'
      Size = 1
    end
    object CDSPendientesClientesSUCNP: TStringField
      FieldName = 'SUCNP'
      Origin = 'SUCNP'
      Size = 4
    end
    object CDSPendientesClientesNUMERONP: TStringField
      FieldName = 'NUMERONP'
      Origin = 'NUMERONP'
      Size = 8
    end
    object CDSPendientesClientesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 35
    end
    object CDSPendientesClientesRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      Origin = 'RAZONSOCIAL'
      Size = 35
    end
    object CDSPendientesClientesDIRECCION: TStringField
      FieldName = 'DIRECCION'
      Origin = 'DIRECCION'
      Size = 35
    end
    object CDSPendientesClientesCPOSTAL: TStringField
      FieldName = 'CPOSTAL'
      Origin = 'CPOSTAL'
      Size = 8
    end
    object CDSPendientesClientesLOCALIDAD: TStringField
      FieldName = 'LOCALIDAD'
      Origin = 'LOCALIDAD'
      Size = 15
    end
    object CDSPendientesClientesTIPOIVA: TIntegerField
      FieldName = 'TIPOIVA'
      Origin = 'TIPOIVA'
    end
    object CDSPendientesClientesCUIT: TStringField
      FieldName = 'CUIT'
      Origin = 'CUIT'
      Size = 13
    end
    object CDSPendientesClientesLISTAPRECIO: TIntegerField
      FieldName = 'LISTAPRECIO'
      Origin = 'LISTAPRECIO'
    end
    object CDSPendientesClientesFECHAPEDIDO: TSQLTimeStampField
      FieldName = 'FECHAPEDIDO'
      Origin = 'FECHAPEDIDO'
    end
    object CDSPendientesClientesFECHAVTO: TSQLTimeStampField
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object CDSPendientesClientesCONDICIONVTA: TIntegerField
      FieldName = 'CONDICIONVTA'
      Origin = 'CONDICIONVTA'
    end
    object CDSPendientesClientesIMPRESO: TStringField
      FieldName = 'IMPRESO'
      Origin = 'IMPRESO'
      Size = 1
    end
    object CDSPendientesClientesOBSERVACION1: TStringField
      FieldName = 'OBSERVACION1'
      Origin = 'OBSERVACION1'
      Size = 60
    end
    object CDSPendientesClientesOBSERVACION2: TStringField
      FieldName = 'OBSERVACION2'
      Origin = 'OBSERVACION2'
      Size = 60
    end
    object CDSPendientesClientesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSPendientesClientesZONA: TIntegerField
      FieldName = 'ZONA'
      Origin = 'ZONA'
    end
    object CDSPendientesClientesLDR: TIntegerField
      FieldName = 'LDR'
      Origin = 'LDR'
    end
    object CDSPendientesClientesDEPOSITO: TIntegerField
      FieldName = 'DEPOSITO'
      Origin = 'DEPOSITO'
    end
    object CDSPendientesClientesCALCULA_SOBRETASA: TStringField
      FieldName = 'CALCULA_SOBRETASA'
      Origin = 'CALCULA_SOBRETASA'
      Size = 1
    end
    object CDSPendientesClientesDESGLOZAIVA: TStringField
      FieldName = 'DESGLOZAIVA'
      Origin = 'DESGLOZAIVA'
      Size = 1
    end
    object CDSPendientesClientesVENDEDOR: TStringField
      FieldName = 'VENDEDOR'
      Origin = 'VENDEDOR'
      Size = 6
    end
    object CDSPendientesClientesUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'USUARIO'
      Size = 50
    end
    object CDSPendientesClientesRENGLONES: TSmallintField
      FieldName = 'RENGLONES'
      Origin = 'RENGLONES'
    end
    object CDSPendientesClientesORIGEN_ID_NOTA_PEDIDO: TIntegerField
      FieldName = 'ORIGEN_ID_NOTA_PEDIDO'
      Origin = 'ORIGEN_ID_NOTA_PEDIDO'
    end
    object CDSPendientesClientesORIGEN_TIPOCPBTE: TStringField
      FieldName = 'ORIGEN_TIPOCPBTE'
      Origin = 'ORIGEN_TIPOCPBTE'
      Size = 2
    end
    object CDSPendientesClientesORIGEN_NROCPBTE: TStringField
      FieldName = 'ORIGEN_NROCPBTE'
      Origin = 'ORIGEN_NROCPBTE'
      Size = 13
    end
    object CDSPendientesClientesCOPIA: TSmallintField
      FieldName = 'COPIA'
      Origin = 'COPIA'
      Required = True
    end
    object CDSPendientesClientesENTREGA_ID_CPBTE: TIntegerField
      FieldName = 'ENTREGA_ID_CPBTE'
      Origin = 'ENTREGA_ID_CPBTE'
    end
    object CDSPendientesClientesENTREGA_TIPOCPBTE: TStringField
      FieldName = 'ENTREGA_TIPOCPBTE'
      Origin = 'ENTREGA_TIPOCPBTE'
      Size = 2
    end
    object CDSPendientesClientesENTREGA_NROCPBTE: TStringField
      FieldName = 'ENTREGA_NROCPBTE'
      Origin = 'ENTREGA_NROCPBTE'
      Size = 13
    end
    object CDSPendientesClientesESTADO: TStringField
      FieldName = 'ESTADO'
      Origin = 'ESTADO'
      FixedChar = True
      Size = 1
    end
    object CDSPendientesClientesPRESUPUESTO_ID: TIntegerField
      FieldName = 'PRESUPUESTO_ID'
      Origin = 'PRESUPUESTO_ID'
    end
    object CDSPendientesClientesPRESUPUESTO_NROCPBTE: TStringField
      FieldName = 'PRESUPUESTO_NROCPBTE'
      Origin = 'PRESUPUESTO_NROCPBTE'
      Size = 13
    end
  end
  object DSPPendientesClientes: TDataSetProvider
    DataSet = QPendientesClientes
    Options = []
    Left = 128
    Top = 83
  end
  object DSPendientesClientes: TDataSource
    DataSet = CDSPendientesClientes
    Left = 112
    Top = 144
  end
  object DSArticulosPorCliente: TDataSource
    DataSet = CDSArticulosPorCliente
    Left = 368
    Top = 288
  end
  object DSTotales: TDataSource
    DataSet = CDSTotales
    Left = 352
    Top = 448
  end
  object DSArticulosClientes: TDataSource
    DataSet = CDSArticulosClientes
    Left = 792
    Top = 448
  end
  object CDSTotales: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'FECHAPEDIDO'
        DataType = ftDateTime
      end
      item
        Name = 'CODIGOARTICULO'
        DataType = ftString
        Size = 8
      end
      item
        Name = 'DETALLE'
        DataType = ftString
        Size = 45
      end
      item
        Name = 'FALTANTE'
        DataType = ftFMTBcd
        Precision = 15
        Size = 3
      end
      item
        Name = 'MUESTRACODALTER'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'CODIGO'
        DataType = ftString
        Size = 6
      end
      item
        Name = 'NOMBRE'
        DataType = ftString
        Size = 35
      end>
    IndexDefs = <
      item
        Name = 'CODIGOARTICULO'
        Fields = 'CODIGOARTICULO'
      end
      item
        Name = 'DETALLE'
        Fields = 'DETALLE'
      end
      item
        Name = 'FALTANTE'
        Fields = 'FALTANTE'
        Options = [ixDescending]
      end>
    Params = <>
    ProviderName = 'DSPTotales'
    StoreDefs = True
    AfterScroll = CDSTotalesAfterScroll
    Left = 256
    Top = 448
    object CDSTotalesCODIGOARTICULO: TStringField
      FieldName = 'CODIGOARTICULO'
      OnGetText = CDSTotalesCODIGOARTICULOGetText
      Size = 8
    end
    object CDSTotalesDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 45
    end
    object CDSTotalesFALTANTE: TFloatField
      FieldName = 'FALTANTE'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSTotalesMUESTRACODALTER: TStringField
      FieldName = 'MUESTRACODALTER'
    end
    object CDSTotalesCODIGO: TStringField
      FieldName = 'CODIGO'
      Size = 6
    end
    object CDSTotalesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 35
    end
    object CDSTotalesFECHAPEDIDO: TSQLTimeStampField
      FieldName = 'FECHAPEDIDO'
    end
  end
  object DSPTotales: TDataSetProvider
    DataSet = QTotales
    Options = []
    Left = 176
    Top = 443
  end
  object PopupMenu1: TPopupMenu
    Left = 644
    Top = 144
    object AnularPedido1: TMenuItem
      Action = AnularPedido
    end
  end
  object frxReporte: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 40045.758306921300000000
    ReportOptions.LastChange = 40045.968650115740000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      ''
      'begin'
      ''
      'end.')
    Left = 496
    Top = 624
    Datasets = <>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 0
      LeftMargin = 10.000000000000000000
      RightMargin = 10.000000000000000000
      TopMargin = 10.000000000000000000
      BottomMargin = 10.000000000000000000
      Frame.Typ = []
      MirrorMode = []
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 18.897650000000000000
        Width = 718.110700000000000000
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 83.149660000000000000
        Width = 718.110700000000000000
        RowCount = 0
      end
      object PageFooter1: TfrxPageFooter
        FillType = ftBrush
        FillGap.Top = 0
        FillGap.Left = 0
        FillGap.Bottom = 0
        FillGap.Right = 0
        Frame.Typ = []
        Height = 22.677180000000000000
        Top = 147.401670000000000000
        Width = 718.110700000000000000
        object Memo1: TfrxMemoView
          AllowVectorExport = True
          Left = -151.181200000000000000
          Top = -0.000000000000004247
          Width = 75.590600000000000000
          Height = 18.897650000000000000
          Frame.Typ = []
          HAlign = haRight
          Memo.UTF8W = (
            '[Page#]')
        end
      end
    end
  end
  object frxDBTotales: TfrxDBDataset
    UserName = 'frxDBTotales'
    CloseDataSource = False
    DataSet = CDSTotales
    BCDToCurrency = False
    DataSetOptions = []
    Left = 776
    Top = 568
    FieldDefs = <
      item
        FieldName = 'CODIGOARTICULO'
        FieldAlias = 'CODIGOARTICULO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'FALTANTE'
        FieldAlias = 'FALTANTE'
      end
      item
        FieldName = 'MUESTRACODALTER'
        FieldAlias = 'MUESTRACODALTER'
      end>
  end
  object frxDBArticulosClientes: TfrxDBDataset
    UserName = 'frxDBArticulosClientes'
    CloseDataSource = False
    DataSet = CDSArticulosClientes
    BCDToCurrency = False
    DataSetOptions = []
    Left = 688
    Top = 568
    FieldDefs = <
      item
        FieldName = 'CODIGO'
        FieldAlias = 'CODIGO'
      end
      item
        FieldName = 'NOMBRE'
        FieldAlias = 'NOMBRE'
      end
      item
        FieldName = 'CANTIDAD_FALTANTE'
        FieldAlias = 'CANTIDAD_FALTANTE'
      end
      item
        FieldName = 'MUESTRACODALTER'
        FieldAlias = 'MUESTRACODALTER'
      end>
  end
  object PopupMenu2: TPopupMenu
    Left = 764
    Top = 352
    object ImprimirPendientesClientes1: TMenuItem
      Action = ImprimirPendientesClientes
    end
  end
  object frxDBPendientesDelCliente: TfrxDBDataset
    UserName = 'frxDBPendientesDelCliente'
    CloseDataSource = False
    DataSet = CDSArticulosPorCliente
    BCDToCurrency = False
    DataSetOptions = []
    Left = 616
    Top = 336
    FieldDefs = <
      item
        FieldName = 'CODIGOARTICULO'
        FieldAlias = 'CODIGOARTICULO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'FALTANTE'
        FieldAlias = 'FALTANTE'
      end
      item
        FieldName = 'PEDIDA'
        FieldAlias = 'PEDIDA'
      end
      item
        FieldName = 'ENTREGADA'
        FieldAlias = 'ENTREGADA'
      end
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'MUESTRACODALTER'
        FieldAlias = 'MUESTRACODALTER'
      end>
  end
  object PopupMenu3: TPopupMenu
    Left = 700
    Top = 304
    object MenuItem1: TMenuItem
      Action = ImrimirPendienteDelCliente
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object ExportarExcel1: TMenuItem
      Caption = 'Exportar Excel'
      OnClick = ExportarExcel1Click
    end
  end
  object CDSArticulosPorCliente: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPArticulosPorCliente'
    Left = 304
    Top = 288
    object CDSArticulosPorClienteCODIGOARTICULO: TStringField
      FieldName = 'CODIGOARTICULO'
      OnGetText = CDSArticulosPorClienteCODIGOARTICULOGetText
      Size = 8
    end
    object CDSArticulosPorClienteDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 45
    end
    object CDSArticulosPorClienteFALTANTE: TFloatField
      FieldName = 'FALTANTE'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosPorClientePEDIDA: TFloatField
      FieldName = 'PEDIDA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosPorClienteENTREGADA: TFloatField
      FieldName = 'ENTREGADA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosPorClienteFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      DisplayWidth = 16
      FieldName = 'FECHA'
    end
    object CDSArticulosPorClienteMUESTRACODALTER: TStringField
      FieldName = 'MUESTRACODALTER'
    end
  end
  object DSPArticulosPorCliente: TDataSetProvider
    DataSet = QArticulosPorCliente
    Options = []
    Left = 232
    Top = 296
  end
  object QArticulosPorCliente: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select d.codigoarticulo,d.detalle,sum(d.cantidad_faltante) as Fa' +
        'ltante,'
      
        '  sum(d.cantidad_pedidia) as Pedida, sum(d.cantidad_entregada) a' +
        's Entregada,max(c.fechapedido) as Fecha,s.reemplazo_stk as Muest' +
        'raCodAlter  from npedidodet_cliente d'
      'left join npedidocab_cliente c on c.id=d.id_cabnp'
      'left join stock s on s.codigo_stk=d.codigoarticulo'
      
        'where c.codigo=:codigo and d.cantidad_faltante>0 and c.estado in' +
        ' ('#39'N'#39','#39'P'#39')'
      'group by d.codigoarticulo,d.detalle,s.reemplazo_stk')
    Left = 144
    Top = 288
    ParamData = <
      item
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QTotales: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select c.fechapedido,d.codigoarticulo,d.detalle,sum(d.cantidad_f' +
        'altante) as Faltante,'
      
        's.reemplazo_stk as MuestraCodAlter,c.codigo,c.nombre from npedid' +
        'odet_cliente d'
      'left join npedidocab_cliente c on c.id=d.id_cabnp'
      'left join stock s on s.codigo_stk=d.codigoarticulo'
      'where c.estado in ('#39'N'#39','#39'P'#39')  and d.cantidad_faltante>0'
      
        'group by c.fechapedido,d.codigoarticulo,d.detalle,s.reemplazo_st' +
        'k,c.codigo,c.nombre')
    Left = 112
    Top = 448
  end
  object spControlNP: TFDStoredProc
    Connection = DMMain_FD.fdcGestion
    StoredProcName = 'CONTROL_DIAS_NPCLIENTES'
    Left = 440
    Top = 464
  end
  object CDSArticulosClientes: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPArticulosClientes'
    Left = 696
    Top = 440
    object CDSArticulosClientesCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      OnGetText = CDSArticulosClientesCODIGOGetText
      Size = 6
    end
    object CDSArticulosClientesNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      Size = 35
    end
    object CDSArticulosClientesCANTIDAD_FALTANTE: TFloatField
      DisplayLabel = 'Faltante'
      FieldName = 'CANTIDAD_FALTANTE'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSArticulosClientesMUESTRACODALTER: TStringField
      FieldName = 'MUESTRACODALTER'
    end
    object CDSArticulosClientesDETALLE_STK: TStringField
      FieldName = 'DETALLE_STK'
      Size = 45
    end
    object CDSArticulosClientesCODIGO_STK: TStringField
      FieldName = 'CODIGO_STK'
      Size = 8
    end
  end
  object DSPArticulosClientes: TDataSetProvider
    DataSet = QArticulosClientes
    Options = []
    Left = 608
    Top = 440
  end
  object QArticulosClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select c.codigo,c.nombre,d.cantidad_faltante,s.reemplazo_stk as ' +
        'MuestraCodAlter,s.detalle_stk,s.codigo_stk from npedidodet_clien' +
        'te d'
      'left join npedidocab_cliente c on c.id=d.id_cabnp'
      'left join stock s on s.codigo_stk=d.codigoarticulo'
      
        'where d.codigoarticulo=:codigo and c.estado in ('#39'N'#39','#39'P'#39')  and d.' +
        'cantidad_faltante>0')
    Left = 520
    Top = 440
    ParamData = <
      item
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object DataToXLS: TDataToXLS
    DataSet = CDSTotales
    Columns = <
      item
        DataField = 'CODIGOARTICULO'
        Title = 'CODIGOARTICULO'
      end
      item
        DataField = 'MUESTRACODALTER'
        Title = 'MUESTRACODALTER'
      end
      item
        DataField = 'DETALLE'
        Title = 'DETALLE'
      end
      item
        DataField = 'FALTANTE'
        Title = 'FALTANTE'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'CODIGO'
        Title = 'CODIGO'
      end
      item
        DataField = 'NOMBRE'
        Title = 'NOMBRE'
      end
      item
        DataField = 'FECHAPEDIDO'
        Title = 'FECHAPEDIDO'
      end>
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -13
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Header.Font.Charset = DEFAULT_CHARSET
    Header.Font.Color = clWindowText
    Header.Font.Height = -13
    Header.Font.Name = 'Arial'
    Header.Font.Style = [fsBold]
    Left = 648
    Top = 290
  end
  object SaveDialog: TSaveDialog
    DefaultExt = 'xls'
    Left = 740
    Top = 184
  end
  object QPendientesClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select n.* from npedidocab_cliente n'
      'where n.estado in ( '#39'N'#39','#39'P'#39')'
      'order by n.razonsocial')
    Left = 48
    Top = 80
  end
  object QPendientesDetalles: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select d.*,s.reemplazo_stk as MuestraCodAlter from npedidodet_cl' +
        'iente d'
      'left join stock s on s.codigo_stk=d.codigoarticulo'
      'where d.id_cabnp=:id')
    Left = 380
    Top = 104
    ParamData = <
      item
        Position = 1
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object spAnular: TFDStoredProc
    Connection = DMMain_FD.fdcGestion
    StoredProcName = 'ANULAR_NOTAPEDIDO_CLIENTE'
    Left = 496
    Top = 312
    ParamData = <
      item
        Position = 1
        Name = 'ID'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
end
