...
uses
  ...
  dxSkinWXI, frCoreClasses, AdvOfficeButtons, AdvProgr, VCL.TMSFNCTypes,
...
type
  TFormControlAsignacionRto_Fc = class(TFormABMBase)
    ...
    AsignacionManual: TTabSheet;
    AsiganacionAutomatica: TTabSheet;
    Panel2: TPanel;
    DBGFacturas: TDBGrid;
    QFCRto2: TFDQuery;
    DSPFCRto2: TDataSetProvider;
    CDSFCRto2: TClientDataSet;
    DSFCRto2: TDataSource;
    CDSFCRto2ID_FC: TIntegerField;
    CDSFCRto2FECHAVTA: TSQLTimeStampField;
    CDSFCRto2TIPOCPBTE: TStringField;
    CDSFCRto2CLASECPBTE: TStringField;
    CDSFCRto2LETRAFAC: TStringField;
    CDSFCRto2SUCFAC: TStringField;
    CDSFCRto2NUMEROFAC: TStringField;
    CDSFCRto2NROCPBTE: TStringField;
    CDSFCRto2NRORTO: TStringField;
    CDSFCRto2TOTAL: TFloatField;
    CDSFCRto2CODIGO: TStringField;
    cxGrid1: TcxGrid;
    ...
    CDSFCRto2IDREMITO: TIntegerField;
    Panel3: TPanel;
    btConsultar: TButton;
    Asignar: TAction;
...
//  ActiveControl:=tvListado;
  ...
  if ceCliente.text <> '' then
    CDSFcRto2.Params.ParamByName('codigo').AsString:= ceCliente.Text
  else
  CDSFcRto2.Params.ParamByName('codigo').AsString:= '******';
  CDSFCRto2.Params.ParamByName('Desde').AsDate   := Desde.Date;
  CDSFCRto2.Params.ParamByName('Hasta').AsDate   := Hasta.Date;
  CDSFCRto2.open;
...
procedure TFormControlAsignacionRto_Fc.AsignarExecute(Sender: TObject);
var CantArreglados, CantTotal, CantSinFactura :Integer;
begin
  CantArreglados      := 0;
  CantTotal           := 0;
  CantSinFactura      := 0;
  Cancela             := False;
  inherited;
  pgProgreso.Minimum  := 1;
  pgProgreso.Maximum  := CDSFCRto2.RecordCount-1;
  pgProgreso.Value    := 1;

  CDSFCRto2.first;
  CDSFCRto2.DisableControls;
  Screen.Cursor:=crHourGlass;
  while (not CDSFCRto2.Eof) and (Cancela=False) do
    begin
      if CDSRtos.Locate('ID_RTO',CDSFCRto2IDREMITO.AsInteger,[])  then
        begin
          CDSRtos.Edit;
          CDSRtosIDFACTURA.AsInteger:= CDSFCRto2ID_FC.AsInteger;
          CDSRtosNROFACT.asstring   := CDSFCRto2NROCPBTE.asstring;
          CDSRtos.Post;
          if chAplicaCorreccion.Checked then
            CDSRtos.ApplyUpdates(0);
          CantArreglados            := CantArreglados+1;
        end;
      CantTotal:=CantTotal+1;
      CDSFCRto2.Next;
      pgProgreso.Value:= pgProgreso.Value+1;
      Application.ProcessMessages;
    end;
  CDSFCRto2.EnableControls;
  pgProgreso.Value        := 0;
  Application.ProcessMessages;
  ...
  while not CDSRtos.Eof do
    begin
      if ( CDSRtosIDFACTURA.Asstring ='')  then
        CantSinFactura:=CantSinFactura+1;
       CDSRtos.next;
    end;
  CDSRtos.EnableControls;
  Screen.Cursor:=crDefault;

  ShowMessage('Asignación Terminada'+#13+
              '------------------------------------------'+#13+
              'Cantidad Total Remitos            : '+ IntToStr(CantTotal) +#13+
              'Cantidad Remitos Sin Factura : '+ IntToStr(CantSinFactura) +#13+
              'Cantidad Remitos Arreglados  : '+ IntToStr(CantArreglados));
end;
...
procedure TFormControlAsignacionRto_Fc.DBGFacturasDblClick(Sender: TObject);
begin
  inherited;
  if not(CDSFCRto2.IsEmpty) and (CDSFCRto2IDREMITO.AsString<>'') then
    begin
      if Not(Assigned(FormRemitos)) then
        FormRemitos:=TFormRemitos.Create(Self);
      FormRemitos.DatoNew   :=CDSFCRto2IDREMITO.AsString;
      FormRemitos.TipoCpbte :='RE';
      FormRemitos.Recuperar.Execute;
      FormRemitos.Show;
    end;
...
procedure TFormControlAsignacionRto_Fc.DBGFacturasTitleClick(Column: TColumn);
var Campo :string;
begin
  inherited;
  try
