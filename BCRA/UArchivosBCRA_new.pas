...
uses
  ...
  JvComponentBase, JvFormPlacement, Vcl.ImgList, vcl.wwDialog, vcl.wwIDlg,
...
procedure TFormArchivosBCRA.BorrarCheques;
var i:Integer;
Dato:String;
begin
  for i:= 0 to mCheques.Lines.Count-1 do
    begin
      Dato:=Trim(mCheques.Lines[i]);
    end;

end;

procedure TFormArchivosBCRA.BorrarCheuqes;
var i:integer;
begin
  AdvProgress.Min      := 0;
  ...
  for i:= 0 to mCheques.Lines.Count-1 do
    begin
      if Cancelar then
        Exit;
      if (I div 2000)=0 Then JvThBorrado.Synchronize(IncProgreso);
      QBorrarChe.Close;
