...
uses
  ...
  JvFormPlacement, ImgList, wwDialog, wwidlg, ActnList, JvLabel, JvDBControls,
...
uses UBuscadorTipoCpbte, UTransferencia,DMBuscadoresForm,DMMainForm_2,
...
begin
  ...
  if Trim(DMBuscadores.QbuscaStockCODIGO_STK.AsString)='' then
    begin
      ...
      DMMain_2.CDSCodigoBarra.Params.ParamByName('CodigoBarra').AsString:=Codigo;
      DMMain_2.CDSCodigoBarra.Open;
      If DMMain_2.CDSCodigoBarra.Fields[0].AsString='' Then
        begin
          DMMain_2.CDSCodigoBarra.Close;
          DMMain_2.CDSCodigoBarra.Params.ParamByName('CodigoBarra').AsString:=Copy(Codigo,1,length(Trim(Codigo))-1);
          DMMain_2.CDSCodigoBarra.Open;
        end;
      If DMMain_2.CDSCodigoBarra.Fields[0].AsString='' Then
        begin
  ...
  //***************************************************
        ...
        begin
          if DMMain_2.CDSCodigoBarra.RecordCount>1 Then
            ...
            CodTxt:=DMMain_2.CDSCodigoBarra.Fields[0].AsString;
          ...
          if ExisteArchivo(FileNameColectora) then
            begin
              ...
              while Not Eof(ArchiTxt) do
                 begin
                   ReadLn(ArchiTxt,s);
                   Codigo  := Copy(s,1,pos(',',s));
                   s:=copy(s,pos(',',s)+1,100);
                   s:=Trim(s);
                   Cantidad:=Copy(s,1,pos(',',s));
                   s:=copy(s,pos(',',s)+1,100);
                   s:=Trim(s);
                   deposito:=s;
                   ArmarLista;
