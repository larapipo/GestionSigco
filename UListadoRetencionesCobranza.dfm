inherited FormListadoRetencionesCobranza: TFormListadoRetencionesCobranza
  Left = 257
  Top = 97
  Caption = 
    'Listado de Retenciones Percibidas en Cobranzas/Bancos/Tarjetas d' +
    'e Credito'
  ClientHeight = 678
  ClientWidth = 1016
  Font.Color = clDefault
  StyleElements = [seFont, seClient, seBorder]
  OnResize = FormResize
  ExplicitWidth = 1034
  ExplicitHeight = 719
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1016
    Height = 625
    Align = alClient
    Color = clMenuHighlight
    Font.Color = clBlack
    ParentFont = False
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1007
    ExplicitHeight = 625
    object Label1: TLabel
      Left = 11
      Top = 7
      Width = 269
      Height = 16
      Caption = 'Codigo Regimen Para Exportacion Ganancia'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object RxLabel1: TJvLabel
      Left = 691
      Top = 5
      Width = 43
      Height = 16
      Caption = 'Desde'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ShadowPos = spLeftTop
      Transparent = True
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -13
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
    end
    object RxLabel3: TJvLabel
      Left = 795
      Top = 5
      Width = 38
      Height = 16
      Caption = 'Hasta'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ShadowPos = spLeftTop
      Transparent = True
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -13
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
    end
    object RxLabel4: TJvLabel
      Left = 8
      Top = 49
      Width = 43
      Height = 16
      Caption = 'Cliente'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ShadowPos = spLeftTop
      Transparent = True
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -13
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
    end
    object JvLabel: TJvLabel
      Left = 821
      Top = 53
      Width = 120
      Height = 16
      Caption = 'Separador Decimal'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ShadowPos = spLeftTop
      Transparent = True
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -13
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
    end
    object Label2: TLabel
      Left = 318
      Top = 7
      Width = 109
      Height = 16
      Caption = 'Sucursal de Venta'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 594
      Top = 7
      Width = 75
      Height = 16
      Caption = 'Filtro Cpbtes'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object UpDown1: TUpDown
      Left = 895
      Top = 25
      Width = 18
      Height = 22
      Hint = 'Cambia el Per'#237'odo'
      Max = 10000
      ParentShowHint = False
      Position = 100
      ShowHint = True
      TabOrder = 2
      OnClick = UpDown1Click
    end
    object edCodigoReg: TEdit
      Left = 7
      Top = 26
      Width = 103
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 3
    end
    object edNombreCliente: TEdit
      Left = 89
      Top = 68
      Width = 312
      Height = 21
      Color = clInfoBk
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 5
    end
    object Desde: TJvDateEdit
      Left = 691
      Top = 26
      Width = 97
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      ParentFont = False
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 0
    end
    object Hasta: TJvDateEdit
      Left = 795
      Top = 26
      Width = 97
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      ParentFont = False
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 1
    end
    object ceCliente: TJvComboEdit
      Left = 8
      Top = 68
      Width = 73
      Height = 21
      ButtonWidth = 16
      ClickKey = 16397
      Color = clBtnFace
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      ParentFont = False
      TabOrder = 4
      Text = ''
      OnButtonClick = BuscadorClienteExecute
      OnKeyPress = ceClienteKeyPress
    end
    object cbSeparador: TComboBox
      Left = 874
      Top = 68
      Width = 39
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ItemIndex = 1
      ParentFont = False
      TabOrder = 7
      Text = '.'
      Items.Strings = (
        ','
        '.')
    end
    object dbcSucursal: TJvDBLookupCombo
      Left = 210
      Top = 26
      Width = 217
      Height = 21
      DisplayEmpty = 'Todas las Sucursales'
      EmptyValue = '-1'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      LookupField = 'CODIGO'
      LookupDisplay = 'DETALLE'
      LookupSource = DSSucursales
      ParentFont = False
      TabOrder = 9
      OnClick = BuscarExecute
    end
    object pcRetenciones: TPageControl
      Left = 0
      Top = 104
      Width = 1016
      Height = 521
      ActivePage = tsGeneral
      Align = alBottom
      Anchors = [akLeft, akTop, akRight, akBottom]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      OnResize = pcRetencionesResize
      ExplicitWidth = 1007
      object tsGeneral: TTabSheet
        Caption = 'General'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        object dbgMov: TJvDBGrid
          Left = 0
          Top = 0
          Width = 1008
          Height = 493
          Align = alClient
          DataSource = DSRetenciones
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          PopupMenu = PopupMenu1
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWhite
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnDblClick = dbgMovDblClick
          SelectColumnsDialogStrings.Caption = 'Select columns'
          SelectColumnsDialogStrings.OK = '&OK'
          SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
          EditControls = <>
          RowsHeight = 17
          TitleRowHeight = 17
          OnCheckIfBooleanField = dbgMovCheckIfBooleanField
          Columns = <
            item
              Expanded = False
              FieldName = 'FECHA_RETENCION'
              Title.Alignment = taCenter
              Title.Caption = 'Fecha Ret.'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 86
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOMBRE_CLIENTE'
              Title.Alignment = taCenter
              Title.Caption = 'Cliente'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 279
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NRO_RETENCION'
              Title.Alignment = taCenter
              Title.Caption = 'Nro.Retenci'#243'n'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TIPOCPBTE'
              Title.Alignment = taCenter
              Title.Caption = 'Tp'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 31
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NROCPBTE'
              Title.Alignment = taCenter
              Title.Caption = 'Nro.Cpbte'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 97
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DETALLE'
              Title.Alignment = taCenter
              Title.Caption = 'Detalle'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 131
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'IMPORTE'
              Title.Alignment = taCenter
              Title.Caption = 'Importe'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 99
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'CONTA'
              Title.Alignment = taCenter
              Title.Caption = 'Cont.'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 41
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'JURIDICCION'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end>
        end
      end
      object tsTCredito: TTabSheet
        Caption = 'Tarjetas de Credito'
        ImageIndex = 1
        DesignSize = (
          1008
          493)
        object cxDBPivotGrid1: TcxDBPivotGrid
          Left = 3
          Top = 15
          Width = 417
          Height = 428
          Anchors = [akLeft, akTop, akBottom]
          DataSource = DSRetTC
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Groups = <>
          OptionsView.ColumnFields = False
          OptionsView.ColumnGrandTotalText = 'Total'
          OptionsView.DataFields = False
          OptionsView.FilterFields = False
          OptionsView.FilterSeparator = False
          OptionsView.RowGrandTotalText = 'Total'
          OptionsView.RowGrandTotalWidth = 193
          OptionsView.TotalsForSingleValues = True
          ParentFont = False
          TabOrder = 0
          object cxDBPivotGrid1Field1: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'FECHA'
            Visible = True
            UniqueName = 'Fecha'
          end
          object cxDBPivotGrid1Field2: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'FECHA_RETENCION'
            Visible = True
            UniqueName = 'Fecha Ret'
          end
          object cxDBPivotGrid1Field3: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'IMPORTE'
            Visible = True
            UniqueName = 'Importe'
          end
          object cxDBPivotGrid1Field4: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 1
            DataBinding.FieldName = 'TC_NOMBRE'
            Visible = True
            UniqueName = 'Nombre T.C'
          end
          object cxDBPivotGrid1Field5: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'DETALLE'
            Visible = True
            UniqueName = 'Detalle'
          end
        end
        object cxDBPivotGrid2: TcxDBPivotGrid
          Left = 488
          Top = 15
          Width = 417
          Height = 428
          Anchors = [akLeft, akTop, akBottom]
          DataSource = DSRetBco
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Groups = <>
          OptionsView.ColumnFields = False
          OptionsView.ColumnGrandTotalText = 'Total'
          OptionsView.DataFields = False
          OptionsView.FilterFields = False
          OptionsView.RowGrandTotalText = 'Total'
          OptionsView.RowGrandTotalWidth = 162
          OptionsView.TotalsForSingleValues = True
          ParentFont = False
          TabOrder = 1
          object cxDBPivotGridField1: TcxDBPivotGridField
            AreaIndex = 0
            DataBinding.FieldName = 'FECHA'
            Visible = True
            UniqueName = 'Fecha'
          end
          object cxDBPivotGridField2: TcxDBPivotGridField
            AreaIndex = 1
            DataBinding.FieldName = 'FECHA_RETENCION'
            Visible = True
            UniqueName = 'Fecha Ret.'
          end
          object cxDBPivotGridField3: TcxDBPivotGridField
            Area = faData
            AreaIndex = 0
            DataBinding.FieldName = 'IMPORTE'
            Visible = True
            UniqueName = 'Importe'
          end
          object cxDBPivotGridField4: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 1
            IsCaptionAssigned = True
            Caption = 'Bco.Nombre'
            DataBinding.FieldName = 'BCO_NOMBRE'
            Visible = True
            UniqueName = 'Bco.Nombre'
          end
          object cxDBPivotGridField5: TcxDBPivotGridField
            Area = faRow
            AreaIndex = 0
            DataBinding.FieldName = 'DETALLE'
            Visible = True
            UniqueName = 'Detalle'
          end
        end
        object BitBtn1: TBitBtn
          Left = 833
          Top = 450
          Width = 72
          Height = 23
          Anchors = [akLeft, akBottom]
          Caption = '&Imprimir'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Glyph.Data = {
            
            0303030303030303030303030303030303030303030303030303}
          NumGlyphs = 2
          ParentFont = False
          TabOrder = 2
          OnClick = BitBtn1Click
        end
        object BitBtn2: TBitBtn
          Left = 348
          Top = 450
          Width = 72
          Height = 23
          Anchors = [akLeft, akBottom]
          Caption = '&Imprimir'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Glyph.Data = {
            
            0303030303030303030303030303030303030303030303030303}
          NumGlyphs = 2
          ParentFont = False
          TabOrder = 3
          OnClick = BitBtn2Click
        end
        object btExcelGastos: TBitBtn
          Left = 272
          Top = 450
          Width = 70
          Height = 23
          Anchors = [akLeft, akBottom]
          Caption = 'Excel'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Glyph.Data = {
            
            00FFFF00FFFF00FFFF00FF000000000000000000FF00FF000000}
          ParentFont = False
          TabOrder = 4
          OnClick = btExcelGastosClick
        end
        object BitBtn3: TBitBtn
          Left = 757
          Top = 450
          Width = 70
          Height = 23
          Anchors = [akLeft, akBottom]
          Caption = 'Excel'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Glyph.Data = {
            
            00FFFF00FFFF00FFFF00FF000000000000000000FF00FF000000}
          ParentFont = False
          TabOrder = 5
          OnClick = BitBtn3Click
        end
      end
    end
    object cbFiltroCpbte: TComboBox
      Left = 605
      Top = 29
      Width = 39
      Height = 21
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 12
      Text = '**'
      OnClick = cbFiltroCpbteClick
      Items.Strings = (
        '**'
        'LQ'
        'GX'
        'RC')
    end
    object chcpbteContabilizan: TAdvOfficeCheckBox
      Left = 650
      Top = 53
      Width = 165
      Height = 36
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 10
      TabStop = True
      OnClick = chcpbteContabilizanClick
      Alignment = taRightJustify
      Caption = 'Solo Los Comprobantes que Contabilizan'
      ReturnIsTab = False
      Version = '1.8.4.2'
    end
    object chbTodosLosClientes: TAdvOfficeCheckBox
      Left = 212
      Top = 47
      Width = 165
      Height = 17
      Checked = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      TabStop = True
      OnClick = chbTodosLosClientesClick
      Alignment = taRightJustify
      Caption = 'Todos los Clientes'
      ReturnIsTab = False
      State = cbChecked
      Version = '1.8.4.2'
    end
    object rgInforma: TAdvOfficeRadioGroup
      Left = 443
      Top = 10
      Width = 118
      Height = 79
      CaptionFont.Charset = DEFAULT_CHARSET
      CaptionFont.Color = clWindowText
      CaptionFont.Height = -11
      CaptionFont.Name = 'Tahoma'
      CaptionFont.Style = []
      Version = '1.8.4.2'
      Caption = ' Informa Ret.IIBB'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentBackground = False
      ParentCtl3D = True
      ParentFont = False
      TabOrder = 8
      ItemIndex = 2
      Items.Strings = (
        '1'#186' Quincena'
        '2'#186' Quincena'
        'Mensual')
      Ellipsis = False
    end
  end
  inherited ToolBar1: TToolBar
    Top = 625
    Width = 1016
    Align = alBottom
    ExplicitTop = 625
    ExplicitWidth = 1007
    inherited btConfirma: TBitBtn
      Width = 39
      DoubleBuffered = True
      Visible = False
      ExplicitWidth = 39
    end
    inherited btNuevo: TBitBtn
      Left = 41
      Width = 32
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 41
      ExplicitWidth = 32
    end
    inherited btCancelar: TBitBtn
      Left = 73
      Width = 40
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 73
      ExplicitWidth = 40
    end
    inherited btModificar: TBitBtn
      Left = 113
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 113
    end
    inherited Pr: TSpeedButton
      Left = 188
      Visible = False
      ExplicitLeft = 188
    end
    inherited btBuscar: TBitBtn
      Left = 203
      DoubleBuffered = True
      ExplicitLeft = 203
    end
    inherited Ne: TSpeedButton
      Left = 278
      Visible = False
      ExplicitLeft = 278
    end
    inherited btBorrar: TBitBtn
      Left = 293
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 293
    end
    inherited btSalir: TBitBtn
      Left = 368
      DoubleBuffered = True
      ExplicitLeft = 368
    end
    object ToolButton1: TToolButton
      Left = 443
      Top = 0
      Width = 22
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object btImprimir: TBitBtn
      Left = 465
      Top = 0
      Width = 75
      Height = 29
      Caption = 'Imprimir'
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
      TabOrder = 7
      OnClick = btImprimirClick
    end
  end
  inherited Panel1: TPanel
    Top = 655
    Width = 1016
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 655
    ExplicitWidth = 1007
    inherited DBStatusLabel1: TJvDBStatusLabel
      Visible = False
    end
    inherited sbEstado: TStatusBar
      Width = 854
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 845
    end
  end
  inherited ActionList1: TActionList
    Left = 400
    Top = 128
    object BuscadorCliente: TAction [9]
      Caption = 'BuscadorCliente'
      OnExecute = BuscadorClienteExecute
    end
    object ExportarIIBBTxtWeb: TAction
      Caption = 'Exportar IIBB a Txt (web)'
      OnExecute = ExportarIIBBTxtWebExecute
    end
    object ExportaIVATxt: TAction
      Caption = 'Exporta IVA a Txt'
      OnExecute = ExportaIVATxtExecute
    end
    object ExportarGanancia: TAction
      Caption = 'Exportar Ret.Gan.'
      OnExecute = ExportarGananciaExecute
    end
    object ExportarXLS: TAction
      Caption = 'Exportar a XLS'
      OnExecute = ExportarXLSExecute
    end
    object ExportarIIBBSifereTxt: TAction
      Caption = 'Exportar IIBB Txt (SIFERE)'
      OnExecute = ExportarIIBBSifereTxtExecute
    end
    object ExportarIIBB_Mensual: TAction
      Caption = 'Exportar IIBB Mensual'
      OnExecute = ExportarIIBB_MensualExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 304
    Top = 144
  end
  inherited ImageList1: TImageList
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Left = 608
    Top = 480
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 24
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 608
    Top = 536
  end
  inherited QBrowse2: TFDQuery
    Top = 304
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 32
    Top = 440
  end
  object PopupMenu1: TPopupMenu
    Left = 376
    Top = 176
    object ExportarTxt1: TMenuItem
      Action = ExportaIVATxt
    end
    object ExportarIVATxt1: TMenuItem
      Action = ExportarIIBBTxtWeb
    end
    object ExportarRetGan1: TMenuItem
      Action = ExportarGanancia
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object ExportaraXLS1: TMenuItem
      Action = ExportarXLS
    end
    object N2: TMenuItem
      Caption = '-'
    end
    object ExportarIIBBTxt1: TMenuItem
      Action = ExportarIIBBSifereTxt
    end
  end
  object SaveDialog1: TSaveDialog
    DefaultExt = 'txt'
    FileName = 'Retenciones'
    Title = 'Archivo de Retenciones'
    Left = 296
    Top = 216
  end
  object frRetenciones: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39638.833204930550000000
    ReportOptions.LastChange = 39638.833204930550000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 440
    Top = 176
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frDBDSRetenciones: TfrxDBDataset
    UserName = 'frDBDSRetenciones'
    CloseDataSource = False
    DataSet = CDSRetenciones
    BCDToCurrency = False
    DataSetOptions = []
    Left = 544
    Top = 184
    FieldDefs = <
      item
        FieldName = 'ID_MOV_RETENCIONES'
        FieldAlias = 'ID_MOV_RETENCIONES'
      end
      item
        FieldName = 'ID_RETENCION'
        FieldAlias = 'ID_RETENCION'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'TIPOCPBTE'
        FieldAlias = 'TIPOCPBTE'
      end
      item
        FieldName = 'CLASECPBTE'
        FieldAlias = 'CLASECPBTE'
      end
      item
        FieldName = 'NROCPBTE'
        FieldAlias = 'NROCPBTE'
      end
      item
        FieldName = 'ID_CPBTE'
        FieldAlias = 'ID_CPBTE'
      end
      item
        FieldName = 'NRO_RETENCION'
        FieldAlias = 'NRO_RETENCION'
      end
      item
        FieldName = 'CODIGO_CLIENTE'
        FieldAlias = 'CODIGO_CLIENTE'
      end
      item
        FieldName = 'CUIT_CLIENTE'
        FieldAlias = 'CUIT_CLIENTE'
      end
      item
        FieldName = 'JURIDICCION'
        FieldAlias = 'JURIDICCION'
      end
      item
        FieldName = 'NOMBRE_CLIENTE'
        FieldAlias = 'NOMBRE_CLIENTE'
      end
      item
        FieldName = 'SUCURSAL'
        FieldAlias = 'SUCURSAL'
      end
      item
        FieldName = 'MUESTRASUC'
        FieldAlias = 'MUESTRASUC'
      end
      item
        FieldName = 'CONTABILIZA'
        FieldAlias = 'CONTABILIZA'
      end
      item
        FieldName = 'CONTA'
        FieldAlias = 'CONTA'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'FECHA_RETENCION'
        FieldAlias = 'FECHA_RETENCION'
      end>
  end
  object DSRetenciones: TDataSource
    DataSet = CDSRetenciones
    Left = 432
    Top = 352
  end
  object DataToXLS1: TDataToXLS
    DataSet = CDSRetenciones
    Columns = <
      item
        DataField = 'ID_MOV_RETENCIONES'
        Title = 'ID_MOV_RETENCIONES'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'ID_RETENCION'
        Title = 'ID_RETENCION'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'DETALLE'
        Title = 'DETALLE'
      end
      item
        DataField = 'TIPOCPBTE'
        Title = 'TIPOCPBTE'
      end
      item
        DataField = 'CLASECPBTE'
        Title = 'CLASECPBTE'
      end
      item
        DataField = 'NROCPBTE'
        Title = 'NROCPBTE'
      end
      item
        DataField = 'ID_CPBTE'
        Title = 'ID_CPBTE'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'FECHA'
        Title = 'FECHA'
      end
      item
        DataField = 'FECHA_RETENCION'
        Title = 'FECHA_RETENCION'
      end
      item
        DataField = 'NRO_RETENCION'
        Title = 'NRO_RETENCION'
      end
      item
        DataField = 'IMPORTE'
        Title = 'IMPORTE'
        Alignment = taRightJustify
        DataType = ctDouble
      end
      item
        DataField = 'CODIGO_CLIENTE'
        Title = 'CODIGO_CLIENTE'
      end
      item
        DataField = 'CUIT_CLIENTE'
        Title = 'CUIT_CLIENTE'
      end
      item
        DataField = 'JURIDICCION'
        Title = 'JURIDICCION'
        Alignment = taRightJustify
        DataType = ctInteger
      end
      item
        DataField = 'NOMBRE_CLIENTE'
        Title = 'NOMBRE_CLIENTE'
      end
      item
        DataField = 'SUCURSAL'
        Title = 'SUCURSAL'
        Alignment = taRightJustify
        DataType = ctInteger
      end
      item
        DataField = 'MUESTRASUC'
        Title = 'MUESTRASUC'
      end
      item
        DataField = 'CONTABILIZA'
        Title = 'CONTABILIZA'
      end>
    Title.Font.Charset = DEFAULT_CHARSET
    Title.Font.Color = clWindowText
    Title.Font.Height = -13
    Title.Font.Name = 'Arial'
    Title.Font.Style = [fsBold]
    Header.Font.Charset = DEFAULT_CHARSET
    Header.Font.Color = clWindowText
    Header.Font.Height = -13
    Header.Font.Name = 'Arial'
    Header.Font.Style = [fsBold]
    Left = 760
    Top = 520
  end
  object CDSRetenciones: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaInicial'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaFinal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPRetenciones'
    OnCalcFields = CDSRetencionesCalcFields
    Left = 376
    Top = 352
    object CDSRetencionesID_MOV_RETENCIONES: TIntegerField
      FieldName = 'ID_MOV_RETENCIONES'
      Origin = 'ID_MOV_RETENCIONES'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSRetencionesID_RETENCION: TIntegerField
      FieldName = 'ID_RETENCION'
      Origin = 'ID_RETENCION'
      Required = True
    end
    object CDSRetencionesDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object CDSRetencionesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSRetencionesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSRetencionesNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Required = True
      Size = 13
    end
    object CDSRetencionesID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
      Required = True
    end
    object CDSRetencionesNRO_RETENCION: TStringField
      FieldName = 'NRO_RETENCION'
      Origin = 'NRO_RETENCION'
    end
    object CDSRetencionesCODIGO_CLIENTE: TStringField
      FieldName = 'CODIGO_CLIENTE'
      Origin = 'CODIGO_CLIENTE'
      Required = True
      Size = 6
    end
    object CDSRetencionesCUIT_CLIENTE: TStringField
      FieldName = 'CUIT_CLIENTE'
      Origin = 'CUIT_CLIENTE'
      Size = 13
    end
    object CDSRetencionesJURIDICCION: TIntegerField
      DisplayLabel = 'Juridic.'
      FieldName = 'JURIDICCION'
      Origin = 'JURIDICCION'
    end
    object CDSRetencionesNOMBRE_CLIENTE: TStringField
      FieldName = 'NOMBRE_CLIENTE'
      Origin = 'NOMBRE_CLIENTE'
      Size = 35
    end
    object CDSRetencionesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSRetencionesMUESTRASUC: TStringField
      FieldName = 'MUESTRASUC'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 35
    end
    object CDSRetencionesCONTABILIZA: TStringField
      FieldName = 'CONTABILIZA'
      Origin = 'CONTABILIZA'
      Size = 1
    end
    object CDSRetencionesCONTA: TBooleanField
      DisplayLabel = 'Contabil.'
      FieldKind = fkCalculated
      FieldName = 'CONTA'
      Calculated = True
    end
    object CDSRetencionesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
    object CDSRetencionesFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSRetencionesFECHA_RETENCION: TSQLTimeStampField
      DisplayLabel = 'F.Ret'
      FieldName = 'FECHA_RETENCION'
      Origin = 'FECHA_RETENCION'
    end
  end
  object DSPRetenciones: TDataSetProvider
    DataSet = QRet
    Options = []
    Left = 320
    Top = 352
  end
  object DSPEmpresa: TDataSetProvider
    DataSet = DMMain_2.QEmpresa
    Options = []
    Left = 72
    Top = 456
  end
  object CDSEmpresa: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPEmpresa'
    Left = 128
    Top = 440
    object CDSEmpresaNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Required = True
      Size = 30
    end
    object CDSEmpresaACTIVIDAD: TStringField
      FieldName = 'ACTIVIDAD'
      Size = 30
    end
    object CDSEmpresaDIRECCION: TStringField
      FieldName = 'DIRECCION'
      Size = 30
    end
    object CDSEmpresaLOCALIDAD: TStringField
      FieldName = 'LOCALIDAD'
      Size = 30
    end
    object CDSEmpresaCORREO: TStringField
      FieldName = 'CORREO'
      Size = 30
    end
    object CDSEmpresaTELEFONO1: TStringField
      FieldName = 'TELEFONO1'
      Size = 10
    end
    object CDSEmpresaTELEFONO2: TStringField
      FieldName = 'TELEFONO2'
      Size = 10
    end
    object CDSEmpresaCUIT: TStringField
      FieldName = 'CUIT'
      Size = 13
    end
    object CDSEmpresaCONDICIONIVA: TStringField
      FieldName = 'CONDICIONIVA'
      Size = 15
    end
    object CDSEmpresaNRO_AGTE_RET_INGBRUTOS: TStringField
      FieldName = 'NRO_AGTE_RET_INGBRUTOS'
      Size = 15
    end
    object CDSEmpresaLOGO: TBlobField
      FieldName = 'LOGO'
      Size = 1
    end
    object CDSEmpresaCPOSTAL: TStringField
      FieldName = 'CPOSTAL'
      Size = 8
    end
    object CDSEmpresaNRO_AGTE_RET_GANANCIA: TStringField
      FieldName = 'NRO_AGTE_RET_GANANCIA'
      Size = 15
    end
    object CDSEmpresaAG_RET_GANANCIA: TStringField
      FieldName = 'AG_RET_GANANCIA'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSEmpresaAG_RET_ING_BRUTOS: TStringField
      FieldName = 'AG_RET_ING_BRUTOS'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSEmpresaAG_PERCEPCION_ING_BRUTOS: TStringField
      FieldName = 'AG_PERCEPCION_ING_BRUTOS'
      Required = True
      FixedChar = True
      Size = 1
    end
    object CDSEmpresaTASA_PERCEPCION_IB: TFMTBCDField
      FieldName = 'TASA_PERCEPCION_IB'
      Precision = 15
      Size = 3
    end
    object CDSEmpresaMINO_IMP_PERCEP_IB: TFMTBCDField
      FieldName = 'MINO_IMP_PERCEP_IB'
      Precision = 15
      Size = 3
    end
    object CDSEmpresaPROVINCIA: TIntegerField
      FieldName = 'PROVINCIA'
    end
  end
  object DSSucursales: TDataSource
    DataSet = CDSSucursal
    Left = 672
    Top = 184
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 680
    Top = 232
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 35
    end
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Options = []
    Left = 672
    Top = 280
  end
  object DSPRetTC: TDataSetProvider
    DataSet = QRetTC
    Options = []
    Left = 320
    Top = 432
  end
  object CDSRetTC: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaInicial'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaFinal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPRetTC'
    Left = 376
    Top = 432
    object IntegerField1: TIntegerField
      FieldName = 'ID_MOV_RETENCIONES'
      Origin = 'ID_MOV_RETENCIONES'
      Required = True
    end
    object IntegerField2: TIntegerField
      FieldName = 'ID_RETENCION'
      Origin = 'ID_RETENCION'
      Required = True
    end
    object StringField1: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object StringField2: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object StringField3: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object StringField4: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Required = True
      Size = 13
    end
    object IntegerField3: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
      Required = True
    end
    object StringField5: TStringField
      DisplayLabel = 'Nro.Retencion'
      FieldName = 'NRO_RETENCION'
      Origin = 'NRO_RETENCION'
    end
    object StringField6: TStringField
      FieldName = 'CODIGO_CLIENTE'
      Origin = 'CODIGO_CLIENTE'
      Required = True
      Size = 6
    end
    object StringField7: TStringField
      DisplayLabel = 'Cuit'
      FieldName = 'CUIT_CLIENTE'
      Origin = 'CUIT_CLIENTE'
      Size = 13
    end
    object IntegerField4: TIntegerField
      FieldName = 'JURIDICCION'
      Origin = 'JURIDICCION'
    end
    object StringField8: TStringField
      FieldName = 'NOMBRE_CLIENTE'
      Origin = 'NOMBRE_CLIENTE'
      Size = 35
    end
    object IntegerField5: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object StringField9: TStringField
      DisplayLabel = 'Sucursal'
      FieldName = 'MUESTRASUC'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 35
    end
    object StringField10: TStringField
      FieldName = 'CONTABILIZA'
      Origin = 'CONTABILIZA'
      Size = 1
    end
    object CDSRetTCTC_CODIGO: TIntegerField
      DisplayLabel = 'Codigo TC'
      FieldName = 'TC_CODIGO'
      Origin = 'TC_CODIGO'
    end
    object CDSRetTCTC_NOMBRE: TStringField
      DisplayLabel = 'Nombre T.C'
      FieldName = 'TC_NOMBRE'
      Origin = 'TC_NOMBRE'
      Size = 15
    end
    object CDSRetTCIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
    object CDSRetTCFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSRetTCFECHA_RETENCION: TSQLTimeStampField
      DisplayLabel = 'Fecha Ret.'
      FieldName = 'FECHA_RETENCION'
      Origin = 'FECHA_RETENCION'
    end
  end
  object DSRetTC: TDataSource
    DataSet = CDSRetTC
    Left = 440
    Top = 432
  end
  object DSPRetBco: TDataSetProvider
    DataSet = QRetBco
    Options = []
    Left = 584
    Top = 352
  end
  object CDSRetBco: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaInicial'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'FechaFinal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPRetBco'
    Left = 656
    Top = 352
    object CDSRetBcoID_MOV_RETENCIONES: TIntegerField
      FieldName = 'ID_MOV_RETENCIONES'
      Origin = 'ID_MOV_RETENCIONES'
    end
    object CDSRetBcoID_RETENCION: TIntegerField
      FieldName = 'ID_RETENCION'
      Origin = 'ID_RETENCION'
    end
    object CDSRetBcoDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object CDSRetBcoTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Size = 2
    end
    object CDSRetBcoCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSRetBcoNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSRetBcoID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Origin = 'ID_CPBTE'
    end
    object CDSRetBcoNRO_RETENCION: TStringField
      FieldName = 'NRO_RETENCION'
      Origin = 'NRO_RETENCION'
    end
    object CDSRetBcoCODIGO_CLIENTE: TStringField
      FieldName = 'CODIGO_CLIENTE'
      Origin = 'CODIGO_CLIENTE'
      Size = 6
    end
    object CDSRetBcoCUIT_CLIENTE: TStringField
      FieldName = 'CUIT_CLIENTE'
      Origin = 'CUIT_CLIENTE'
      Size = 13
    end
    object CDSRetBcoJURIDICCION: TIntegerField
      FieldName = 'JURIDICCION'
      Origin = 'JURIDICCION'
    end
    object CDSRetBcoNOMBRE_CLIENTE: TStringField
      FieldName = 'NOMBRE_CLIENTE'
      Origin = 'NOMBRE_CLIENTE'
      Size = 35
    end
    object CDSRetBcoSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSRetBcoCONTABILIZA: TStringField
      FieldName = 'CONTABILIZA'
      Origin = 'CONTABILIZA'
      Size = 1
    end
    object CDSRetBcoTC_CODIGO: TIntegerField
      FieldName = 'TC_CODIGO'
      Origin = 'TC_CODIGO'
    end
    object CDSRetBcoTC_NOMBRE: TStringField
      FieldName = 'TC_NOMBRE'
      Origin = 'TC_NOMBRE'
      Size = 15
    end
    object CDSRetBcoBCO_CODIGO: TIntegerField
      DisplayLabel = 'Cod.Bco'
      FieldName = 'BCO_CODIGO'
      Origin = 'BCO_CODIGO'
    end
    object CDSRetBcoBCO_NOMBRE: TStringField
      DisplayLabel = 'Bco.Nombre'
      FieldName = 'BCO_NOMBRE'
      Origin = 'BCO_NOMBRE'
      Size = 25
    end
    object CDSRetBcoMUESTRASUC: TStringField
      DisplayLabel = 'Sucursal'
      FieldName = 'MUESTRASUC'
      Origin = 'DETALLE'
      ProviderFlags = []
      Size = 35
    end
    object CDSRetBcoIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
    object CDSRetBcoFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSRetBcoFECHA_RETENCION: TSQLTimeStampField
      DisplayLabel = 'Fecha Ret'
      FieldName = 'FECHA_RETENCION'
      Origin = 'FECHA_RETENCION'
    end
  end
  object DSRetBco: TDataSource
    DataSet = CDSRetBco
    Left = 720
    Top = 344
  end
  object dxComponentPrinter2: TdxComponentPrinter
    CurrentLink = dxComponentPrinter2Link2
    Version = 0
    Left = 860
    Top = 383
    PixelsPerInch = 96
    object dxComponentPrinter2Link1: TcxPivotGridReportLink
      Active = True
      Component = cxDBPivotGrid2
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.GrayShading = True
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 12700
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportDocument.CreationDate = 45153.790490520830000000
      Color = clInactiveCaptionText
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      OptionsExpanding.AutoExpandColumns = True
      OptionsExpanding.AutoExpandRows = True
      OptionsFormatting.LookAndFeelKind = lfFlat
      PixelsPerInch = 96
      BuiltInReportLink = True
    end
    object dxComponentPrinter2Link2: TcxPivotGridReportLink
      Active = True
      Component = cxDBPivotGrid1
      PrinterPage.DMPaper = 9
      PrinterPage.Footer = 6350
      PrinterPage.GrayShading = True
      PrinterPage.Header = 6350
      PrinterPage.Margins.Bottom = 12700
      PrinterPage.Margins.Left = 12700
      PrinterPage.Margins.Right = 12700
      PrinterPage.Margins.Top = 12700
      PrinterPage.Orientation = poLandscape
      PrinterPage.PageSize.X = 210000
      PrinterPage.PageSize.Y = 297000
      PrinterPage._dxMeasurementUnits_ = 0
      PrinterPage._dxLastMU_ = 2
      ReportDocument.CreationDate = 45153.790490520830000000
      OptionsExpanding.AutoExpandColumns = True
      OptionsExpanding.AutoExpandRows = True
      OptionsFormatting.LookAndFeelKind = lfStandard
      PixelsPerInch = 96
      BuiltInReportLink = True
    end
  end
  object QClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'SELECT C.Nombre, c.Razon_social,c.codigo, c.nro_de_cuit , c.Dire' +
        'ccion_Comercial FROM Clientes C '
      'where C.codigo= :codigo')
    Left = 128
    Top = 176
    ParamData = <
      item
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 6
        Value = Null
      end>
    object QClientesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Required = True
      Size = 35
    end
    object QClientesRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      Origin = 'RAZON_SOCIAL'
      Size = 35
    end
    object QClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 6
    end
    object QClientesNRO_DE_CUIT: TStringField
      FieldName = 'NRO_DE_CUIT'
      Origin = 'NRO_DE_CUIT'
      Size = 13
    end
    object QClientesDIRECCION_COMERCIAL: TStringField
      FieldName = 'DIRECCION_COMERCIAL'
      Origin = 'DIRECCION_COMERCIAL'
      Size = 35
    end
  end
  object QRetBco: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select l.*,s.detalle as MuestraSuc from listado_retenciones ( :c' +
        'odigo, :FechaInicial, :FechaFinal ) l'
      'left join sucursal s on s.codigo= l.sucursal'
      
        'where ( l.sucursal = :sucursal or :sucursal = -1 ) and (l.tipocp' +
        'bte='#39'GX'#39')'
      
        'order By l.sucursal,l.id_retencion,l.tc_nombre, l.Fecha_retencio' +
        'n')
    Left = 520
    Top = 352
    ParamData = <
      item
        Position = 1
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'FechaInicial'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'FechaFinal'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QRetTC: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select l.*,s.detalle as MuestraSuc from listado_retenciones ( :c' +
        'odigo, :FechaInicial, :FechaFinal ) l'
      'left join sucursal s on s.codigo= l.sucursal'
      
        'where ( l.sucursal = :sucursal or :sucursal = -1 ) and (l.tipocp' +
        'bte='#39'LQ'#39')'
      
        'order By l.sucursal,l.id_retencion,l.tc_nombre, l.Fecha_retencio' +
        'n')
    Left = 248
    Top = 432
    ParamData = <
      item
        Position = 1
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'FechaInicial'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'FechaFinal'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QRet: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select l.*,s.detalle as MuestraSuc from listado_retenciones ( :c' +
        'odigo, :FechaInicial, :FechaFinal ) l'
      'left join sucursal s on s.codigo= l.sucursal'
      'where ( l.sucursal = :sucursal or :sucursal = -1 )'
      'order By l.sucursal, l.id_retencion, l.Fecha_retencion')
    Left = 256
    Top = 352
    ParamData = <
      item
        Position = 1
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'FechaInicial'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'FechaFinal'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 5
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object frxPDFExport1: TfrxPDFExport
    UseFileCache = True
    ShowProgress = True
    OverwritePrompt = False
    DataOnly = False
    EmbedFontsIfProtected = False
    InteractiveFormsFontSubset = 'A-Z,a-z,0-9,#43-#47 '
    OpenAfterExport = False
    PrintOptimized = False
    Outline = False
    Background = False
    HTMLTags = True
    Quality = 95
    Transparency = False
    Author = 'FastReport'
    Subject = 'FastReport PDF export'
    Creator = 'FastReport'
    ProtectionFlags = [ePrint, eModify, eCopy, eAnnot]
    HideToolbar = False
    HideMenubar = False
    HideWindowUI = False
    FitWindow = False
    CenterWindow = False
    PrintScaling = False
    PdfA = False
    PDFStandard = psNone
    PDFVersion = pv17
    Left = 912
    Top = 512
  end
end
