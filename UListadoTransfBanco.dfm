inherited FormListadoTransfBanco: TFormListadoTransfBanco
  Left = 81
  Top = 99
  Caption = 'Transferencias de Bancos'
  ClientHeight = 683
  ClientWidth = 1146
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 1164
  ExplicitHeight = 724
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1146
    Height = 635
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1146
    ExplicitHeight = 635
    object Splitter1: TSplitter
      Left = 0
      Top = 296
      Width = 1146
      Height = 5
      Cursor = crVSplit
      Align = alTop
      Color = clGreen
      ParentColor = False
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 1146
      Height = 57
      Align = alTop
      BevelOuter = bvNone
      Color = clMenuHighlight
      ParentBackground = False
      TabOrder = 0
      object RxLabel1: TJvLabel
        Left = 576
        Top = 8
        Width = 33
        Height = 13
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel3: TJvLabel
        Left = 680
        Top = 8
        Width = 30
        Height = 13
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel2: TJvLabel
        Left = 8
        Top = 8
        Width = 70
        Height = 13
        Caption = 'Cuenta Banco'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel4: TJvLabel
        Left = 280
        Top = 8
        Width = 98
        Height = 13
        Caption = 'Nro.de Cta Bancaria'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel5: TJvLabel
        Left = 408
        Top = 8
        Width = 33
        Height = 13
        Caption = 'Banco'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -11
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object UpDown1: TUpDown
        Left = 782
        Top = 24
        Width = 16
        Height = 20
        Hint = 'Cambia el Per'#237'odo'
        Max = 10000
        ParentShowHint = False
        Position = 100
        ShowHint = True
        TabOrder = 0
        OnClick = UpDown1Click
      end
      object edNombreCta: TEdit
        Left = 72
        Top = 24
        Width = 193
        Height = 21
        Color = clInfoBk
        Enabled = False
        TabOrder = 1
      end
      object edNroCta: TEdit
        Left = 280
        Top = 24
        Width = 121
        Height = 21
        Color = clInfoBk
        Enabled = False
        TabOrder = 2
      end
      object edBanco: TEdit
        Left = 408
        Top = 24
        Width = 129
        Height = 21
        Color = clInfoBk
        Enabled = False
        TabOrder = 3
      end
      object ceCtaBco: TJvComboEdit
        Left = 8
        Top = 24
        Width = 57
        Height = 21
        ButtonWidth = 16
        ClickKey = 16397
        Glyph.Data = {
          4E000000424D4E000000000000003E000000280000000A000000040000000100
          010000000000100000000000000000000000020000000200000000000000FFFF
          FF00FFC000003300000033000000FFC00000}
        TabOrder = 4
        Text = ''
        OnButtonClick = BuscadorCtaExecute
        OnKeyDown = ceCtaBcoKeyDown
        OnKeyPress = ceCtaBcoKeyPress
      end
      object Desde: TJvDateEdit
        Left = 576
        Top = 24
        Width = 97
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 5
        OnAcceptDate = DesdeAcceptDate
      end
      object Hasta: TJvDateEdit
        Left = 680
        Top = 24
        Width = 97
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 6
      end
    end
    object Panel3: TPanel
      Left = 152
      Top = 176
      Width = 185
      Height = 41
      Caption = 'Panel3'
      TabOrder = 1
    end
    object pnRealizadas: TPanel
      Left = 0
      Top = 57
      Width = 1146
      Height = 239
      Align = alTop
      BevelOuter = bvNone
      Caption = 'pnRealizadas'
      TabOrder = 2
      DesignSize = (
        1146
        239)
      object RxLabel6: TJvLabel
        Left = 8
        Top = 13
        Width = 130
        Height = 16
        Caption = 'Transf.Realizadas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ShadowSize = 1
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -13
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object dbgRealizadas: TDBGrid
        Left = 5
        Top = 32
        Width = 1132
        Height = 172
        Anchors = [akLeft, akTop, akRight, akBottom]
        DataSource = DSTxEmitidas
        DrawingStyle = gdsClassic
        FixedColor = clBlue
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        PopupMenu = PopupMenu1
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = dbgRealizadasDblClick
        OnMouseMove = dbgRealizadasMouseMove
        OnTitleClick = dbgRealizadasTitleClick
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA'
            Title.Alignment = taCenter
            Title.Caption = 'Fecha'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 74
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TIPO_COMPROB'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 28
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CLASE_COMPROB'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 26
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NRO_COMPROBANTE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 90
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'NUMERO'
            Title.Alignment = taCenter
            Title.Caption = 'Numero'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 83
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRECAJA'
            Title.Alignment = taCenter
            Title.Caption = 'Caja'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 176
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUMEROCAJA'
            Title.Alignment = taCenter
            Title.Caption = 'Numero'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 74
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'UNIDADES'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'IMPORTE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 107
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MUESTRABCO'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWhite
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 150
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWhite
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 275
            Visible = True
          end>
      end
      object edRealizadas: TJvCalcEdit
        Left = 648
        Top = 210
        Width = 121
        Height = 21
        DisplayFormat = ',0.00'
        Anchors = [akLeft, akBottom]
        ShowButton = False
        TabOrder = 1
        DecimalPlacesAlwaysShown = False
      end
    end
    object pnRecibidas: TPanel
      Left = 0
      Top = 301
      Width = 1146
      Height = 334
      Align = alClient
      BevelOuter = bvNone
      Caption = 'pnRecibidas'
      TabOrder = 3
      DesignSize = (
        1146
        334)
      object RxLabel7: TJvLabel
        Left = 8
        Top = 10
        Width = 122
        Height = 16
        Caption = 'Transf.Recibidas'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ShadowSize = 1
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -13
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object dbgRecibidas: TDBGrid
        Left = 1
        Top = 32
        Width = 1137
        Height = 254
        Anchors = [akLeft, akTop, akRight, akBottom]
        DataSource = DSTxRecibidas
        DrawingStyle = gdsClassic
        FixedColor = clGreen
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        PopupMenu = PopupMenu1
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = dbgRecibidasDblClick
        OnMouseMove = dbgRecibidasMouseMove
        OnTitleClick = dbgRecibidasTitleClick
        Columns = <
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'FECHA'
            Title.Alignment = taCenter
            Title.Caption = 'Fecha'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 79
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TIPO_COMPROB'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 29
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CLASE_COMPROB'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 23
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NRO_COMPROBANTE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 94
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'NUMERO'
            Title.Alignment = taCenter
            Title.Caption = 'Numero'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 79
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRECAJA'
            Title.Alignment = taCenter
            Title.Caption = 'Caja'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 174
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUMEROCAJA'
            Title.Alignment = taCenter
            Title.Caption = 'Numero'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 72
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'UNIDADES'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = False
          end
          item
            Expanded = False
            FieldName = 'IMPORTE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 97
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MUESTRABCO'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWhite
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOMBRE'
            Title.Alignment = taCenter
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clYellow
            Title.Font.Height = -11
            Title.Font.Name = 'MS Sans Serif'
            Title.Font.Style = [fsBold]
            Width = 265
            Visible = True
          end>
      end
      object edRecibidas: TJvCalcEdit
        Left = 648
        Top = 292
        Width = 121
        Height = 21
        DisplayFormat = ',0.00'
        Anchors = [akLeft, akBottom]
        ShowButton = False
        TabOrder = 1
        DecimalPlacesAlwaysShown = False
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 635
    Width = 1146
    Height = 25
    Align = alBottom
    AutoSize = True
    ButtonHeight = 25
    ExplicitTop = 635
    ExplicitWidth = 1146
    ExplicitHeight = 25
    inherited btConfirma: TBitBtn
      Width = 27
      Height = 25
      DoubleBuffered = True
      Visible = False
      ExplicitWidth = 27
      ExplicitHeight = 25
    end
    inherited btNuevo: TBitBtn
      Left = 29
      Width = 36
      Height = 25
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 29
      ExplicitWidth = 36
      ExplicitHeight = 25
    end
    inherited btCancelar: TBitBtn
      Left = 65
      Height = 25
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 65
      ExplicitHeight = 25
    end
    inherited btModificar: TBitBtn
      Left = 140
      Height = 25
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 140
      ExplicitHeight = 25
    end
    inherited Pr: TSpeedButton
      Left = 215
      Height = 25
      Visible = False
      ExplicitLeft = 215
      ExplicitHeight = 25
    end
    inherited btBuscar: TBitBtn
      Left = 230
      Height = 25
      DoubleBuffered = True
      ExplicitLeft = 230
      ExplicitHeight = 25
    end
    inherited Ne: TSpeedButton
      Left = 305
      Height = 25
      Visible = False
      ExplicitLeft = 305
      ExplicitHeight = 25
    end
    inherited btBorrar: TBitBtn
      Left = 320
      Height = 25
      DoubleBuffered = True
      Visible = False
      ExplicitLeft = 320
      ExplicitHeight = 25
    end
    inherited btSalir: TBitBtn
      Left = 395
      Height = 25
      DoubleBuffered = True
      ExplicitLeft = 395
      ExplicitHeight = 25
    end
    object btImprimir: TBitBtn
      Left = 470
      Top = 0
      Width = 75
      Height = 25
      Action = Imprimir
      Caption = '&Imprimir'
      Glyph.Data = {
        
        33333337FFFF7733333333300000033333333337777773333333}
      NumGlyphs = 2
      TabOrder = 7
    end
  end
  inherited Panel1: TPanel
    Top = 660
    Width = 1146
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 660
    ExplicitWidth = 1146
    inherited sbEstado: TStatusBar
      Width = 1057
      OnDblClick = sbEstadoDblClick
      ExplicitWidth = 1057
    end
  end
  inherited ActionList1: TActionList
    Left = 504
    Top = 160
    object BuscadorCta: TAction
      Caption = 'Busca'
      OnExecute = BuscadorCtaExecute
    end
    object Imprimir: TAction
      Caption = '&Imprimir'
      OnExecute = ImprimirExecute
    end
    object ExportarCSV: TAction
      Caption = 'Exportar a CSV'
      OnExecute = ExportarCSVExecute
    end
  end
  inherited ImageList1: TImageList
    Left = 219
    Top = 78
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 128
    Top = 584
  end
  inherited QBrowse2: TFDQuery
    Left = 8
    Top = 584
  end
  inherited QUltimoCodigo2: TFDQuery
    Left = 216
    Top = 560
  end
  object DSTxEmitidas: TDataSource
    DataSet = CDSTxEmitidas
    Left = 264
    Top = 464
  end
  object DSTxRecibidas: TDataSource
    DataSet = CDSTxRecibidas
    Left = 256
    Top = 416
  end
  object frDSTxRecibidas: TfrxDBDataset
    UserName = 'frDSTxRecibidas'
    CloseDataSource = False
    DataSet = CDSTxRecibidas
    BCDToCurrency = False
    DataSetOptions = []
    Left = 689
    Top = 390
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'NUMERO'
        FieldAlias = 'NUMERO'
      end
      item
        FieldName = 'ID_CUENTA_BANCO'
        FieldAlias = 'ID_CUENTA_BANCO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'CLASE_COMPROB'
        FieldAlias = 'CLASE_COMPROB'
      end
      item
        FieldName = 'NRO_COMPROBANTE'
        FieldAlias = 'NRO_COMPROBANTE'
      end
      item
        FieldName = 'CTACAJA'
        FieldAlias = 'CTACAJA'
      end
      item
        FieldName = 'NOMBRECAJA'
        FieldAlias = 'NOMBRECAJA'
      end
      item
        FieldName = 'TIPO_COMPROB'
        FieldAlias = 'TIPO_COMPROB'
      end
      item
        FieldName = 'NRO_COMPROBANTE_1'
        FieldAlias = 'NRO_COMPROBANTE_1'
      end
      item
        FieldName = 'FECHA_OPERACION'
        FieldAlias = 'FECHA_OPERACION'
      end
      item
        FieldName = 'NUMEROCAJA'
        FieldAlias = 'NUMEROCAJA'
      end
      item
        FieldName = 'ID_COMPROBANTE'
        FieldAlias = 'ID_COMPROBANTE'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end
      item
        FieldName = 'UNIDADES'
        FieldAlias = 'UNIDADES'
      end
      item
        FieldName = 'MUESTRABCO'
        FieldAlias = 'MUESTRABCO'
      end
      item
        FieldName = 'ID'
        FieldAlias = 'ID'
      end
      item
        FieldName = 'NOMBRE'
        FieldAlias = 'NOMBRE'
      end>
  end
  object frResumen: TfrxReport
    Version = '2024.1.8'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Por defecto'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39638.831343668980000000
    ReportOptions.LastChange = 39638.831343668980000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      'begin'
      ''
      'end.')
    Left = 528
    Top = 408
    Datasets = <>
    Variables = <>
    Style = <>
  end
  object frDSTxEmitidas: TfrxDBDataset
    UserName = 'frDSTxEmitidas'
    CloseDataSource = False
    DataSet = CDSTxEmitidas
    BCDToCurrency = False
    DataSetOptions = []
    Left = 784
    Top = 392
    FieldDefs = <
      item
        FieldName = 'FECHA'
        FieldAlias = 'FECHA'
      end
      item
        FieldName = 'NUMERO'
        FieldAlias = 'NUMERO'
      end
      item
        FieldName = 'ID_CUENTA_BANCO'
        FieldAlias = 'ID_CUENTA_BANCO'
      end
      item
        FieldName = 'DETALLE'
        FieldAlias = 'DETALLE'
      end
      item
        FieldName = 'CLASE_COMPROB'
        FieldAlias = 'CLASE_COMPROB'
      end
      item
        FieldName = 'NRO_COMPROBANTE'
        FieldAlias = 'NRO_COMPROBANTE'
      end
      item
        FieldName = 'CTACAJA'
        FieldAlias = 'CTACAJA'
      end
      item
        FieldName = 'NOMBRECAJA'
        FieldAlias = 'NOMBRECAJA'
      end
      item
        FieldName = 'TIPO_COMPROB'
        FieldAlias = 'TIPO_COMPROB'
      end
      item
        FieldName = 'NRO_COMPROBANTE_1'
        FieldAlias = 'NRO_COMPROBANTE_1'
      end
      item
        FieldName = 'FECHA_OPERACION'
        FieldAlias = 'FECHA_OPERACION'
      end
      item
        FieldName = 'NUMEROCAJA'
        FieldAlias = 'NUMEROCAJA'
      end
      item
        FieldName = 'ID_COMPROBANTE'
        FieldAlias = 'ID_COMPROBANTE'
      end
      item
        FieldName = 'IMPORTE'
        FieldAlias = 'IMPORTE'
      end
      item
        FieldName = 'UNIDADES'
        FieldAlias = 'UNIDADES'
      end
      item
        FieldName = 'MUESTRABCO'
        FieldAlias = 'MUESTRABCO'
      end
      item
        FieldName = 'ID'
        FieldAlias = 'ID'
      end
      item
        FieldName = 'NOMBRE'
        FieldAlias = 'NOMBRE'
      end>
  end
  object CDSTxEmitidas: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPTxEmitidas'
    Left = 184
    Top = 368
    object CDSTxEmitidasFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
      Required = True
    end
    object CDSTxEmitidasNUMERO: TIntegerField
      FieldName = 'NUMERO'
    end
    object CDSTxEmitidasID_CUENTA_BANCO: TIntegerField
      FieldName = 'ID_CUENTA_BANCO'
      Required = True
    end
    object CDSTxEmitidasDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object CDSTxEmitidasCLASE_COMPROB: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASE_COMPROB'
      Size = 2
    end
    object CDSTxEmitidasNRO_COMPROBANTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NRO_COMPROBANTE'
      Size = 13
    end
    object CDSTxEmitidasCTACAJA: TIntegerField
      FieldName = 'CTACAJA'
    end
    object CDSTxEmitidasNOMBRECAJA: TStringField
      FieldName = 'NOMBRECAJA'
      Size = 25
    end
    object CDSTxEmitidasTIPO_COMPROB: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPO_COMPROB'
      Size = 2
    end
    object CDSTxEmitidasNRO_COMPROBANTE_1: TStringField
      FieldName = 'NRO_COMPROBANTE_1'
      Size = 13
    end
    object CDSTxEmitidasFECHA_OPERACION: TSQLTimeStampField
      FieldName = 'FECHA_OPERACION'
    end
    object CDSTxEmitidasNUMEROCAJA: TIntegerField
      FieldName = 'NUMEROCAJA'
    end
    object CDSTxEmitidasID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
    end
    object CDSTxEmitidasIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
    object CDSTxEmitidasUNIDADES: TFloatField
      DisplayLabel = 'Unidades'
      FieldName = 'UNIDADES'
      DisplayFormat = ',0.00'
    end
    object CDSTxEmitidasMUESTRABCO: TStringField
      DisplayLabel = 'Cta. Bancaria'
      FieldName = 'MUESTRABCO'
      ReadOnly = True
      Size = 25
    end
    object CDSTxEmitidasID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object CDSTxEmitidasNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      ReadOnly = True
      Size = 100
    end
  end
  object DSPTxEmitidas: TDataSetProvider
    DataSet = QTxEm
    Options = []
    Left = 120
    Top = 360
  end
  object CDSTxRecibidas: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPTxRecibidas'
    Left = 176
    Top = 416
    object CDSTxRecibidasFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
      Origin = 'FECHA'
      Required = True
    end
    object CDSTxRecibidasNUMERO: TIntegerField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
    end
    object CDSTxRecibidasID_CUENTA_BANCO: TIntegerField
      FieldName = 'ID_CUENTA_BANCO'
      Origin = 'ID_CUENTA_BANCO'
      Required = True
    end
    object CDSTxRecibidasDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      ProviderFlags = []
    end
    object CDSTxRecibidasCLASE_COMPROB: TStringField
      DisplayLabel = 'Cl'
      FieldName = 'CLASE_COMPROB'
      Origin = 'CLASE_COMPROB'
      ProviderFlags = []
      Size = 2
    end
    object CDSTxRecibidasNRO_COMPROBANTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NRO_COMPROBANTE'
      Origin = 'NRO_COMPROBANTE'
      ProviderFlags = []
      Size = 13
    end
    object CDSTxRecibidasCTACAJA: TIntegerField
      FieldName = 'CTACAJA'
      Origin = 'ID_CUENTA'
      ProviderFlags = []
    end
    object CDSTxRecibidasNOMBRECAJA: TStringField
      FieldName = 'NOMBRECAJA'
      Origin = 'NOMBRE'
      ProviderFlags = []
      Size = 25
    end
    object CDSTxRecibidasTIPO_COMPROB: TStringField
      DisplayLabel = 'Tp'
      FieldName = 'TIPO_COMPROB'
      Origin = 'TIPO_COMPROB'
      ProviderFlags = []
      Size = 2
    end
    object CDSTxRecibidasNRO_COMPROBANTE_1: TStringField
      FieldName = 'NRO_COMPROBANTE_1'
      Origin = 'NRO_COMPROBANTE'
      ProviderFlags = []
      Size = 13
    end
    object CDSTxRecibidasFECHA_OPERACION: TSQLTimeStampField
      FieldName = 'FECHA_OPERACION'
      Origin = 'FECHA_OPERACION'
      ProviderFlags = []
    end
    object CDSTxRecibidasNUMEROCAJA: TIntegerField
      FieldName = 'NUMEROCAJA'
      Origin = 'NUMEROCAJA'
      ProviderFlags = []
    end
    object CDSTxRecibidasID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Origin = 'ID_COMPROBANTE'
      ProviderFlags = []
    end
    object CDSTxRecibidasIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      Origin = 'IMPORTE'
      DisplayFormat = ',0.00'
    end
    object CDSTxRecibidasUNIDADES: TFloatField
      DisplayLabel = 'Unidades'
      FieldName = 'UNIDADES'
      Origin = 'UNIDADES'
      DisplayFormat = ',0.00'
    end
    object CDSTxRecibidasMUESTRABCO: TStringField
      DisplayLabel = 'Cta. Bancaria'
      FieldName = 'MUESTRABCO'
      ReadOnly = True
      Size = 25
    end
    object CDSTxRecibidasID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object CDSTxRecibidasNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      DisplayWidth = 30
      FieldName = 'NOMBRE'
      ReadOnly = True
    end
  end
  object DSPTxRecibidas: TDataSetProvider
    DataSet = QTxRecibidas
    Options = []
    Left = 112
    Top = 416
  end
  object CDSBuscadorCta: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPBuscadorCta'
    Left = 360
    Top = 336
    object CDSBuscadorCtaID_CUENTA: TIntegerField
      DisplayLabel = 'Id'
      DisplayWidth = 10
      FieldName = 'ID_CUENTA'
      Required = True
    end
    object CDSBuscadorCtaNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      DisplayWidth = 25
      FieldName = 'NOMBRE'
      Size = 25
    end
    object CDSBuscadorCtaNRO_CUENTA: TStringField
      DisplayLabel = 'Nro.Cta.Bancaria'
      DisplayWidth = 15
      FieldName = 'NRO_CUENTA'
      Size = 15
    end
    object CDSBuscadorCtaID_TIPO_CTA: TIntegerField
      FieldName = 'ID_TIPO_CTA'
      Required = True
      Visible = False
    end
    object CDSBuscadorCtaID_BANCO: TIntegerField
      FieldName = 'ID_BANCO'
      Visible = False
    end
    object CDSBuscadorCtaCUIT: TStringField
      FieldName = 'CUIT'
      Visible = False
      Size = 13
    end
    object CDSBuscadorCtaRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      Visible = False
      Size = 35
    end
  end
  object DSPBuscadorCta: TDataSetProvider
    DataSet = QBuscCta
    Options = []
    Left = 304
    Top = 336
  end
  object ComBuscadorCta: TComBuscador
    Data = CDSBuscadorCta
    Campo = 'ID_CUENTA'
    Titulo = 'Cuentas'
    rOk = False
    Left = 632
    Top = 208
  end
  object QBuscCta: TFDQuery
    ObjectView = False
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from cuenta_caja where id_tipo_cta=2')
    Left = 232
    Top = 336
  end
  object QTxRecibidas: TFDQuery
    ObjectView = False
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select t.id, t.fecha, t.numero,t.id_cuenta_banco, cbco.nombre as' +
        ' MuestraBco, t.importe,t.unidades,'
      
        'tc.detalle,cm.clase_comprob,cm.nro_comprobante,cc.id_cuenta as C' +
        'taCaja, cc.nombre as NombreCaja,cm.tipo_comprob,'
      
        'cm.nro_comprobante,cm.fecha_operacion,cab.numerocaja as NumeroCa' +
        'ja,cm.id_comprobante,'
      
        '(case when cm.tipo_comprob='#39'RC'#39' then (select r.nombre from recib' +
        'os r where r.id_rc=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'IC'#39' then (select i.detalle from ing_' +
        'caja i where i.id_ingreso=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'FO'#39' then (select f.nombre from fcvta' +
        'cab f where f.id_fc=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'AV'#39' then (select aas.nombre from anu' +
        'lacion_adelantos_sueldo aas where aas.id=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'XP'#39' then (select xp.nombre from anul' +
        'acion_opago xp where xp.id_anulacion=cm.id_comprobante)'
      'end) as Nombre'
      ''
      ''
      ' from transf_banco_recibidas t'
      'left join caja_mov cm on cm.id_mov_caja=T.id_mov_caja'
      'left join tipo_comprobante tc on tc.sigla=cm.tipo_comprob'
      'left join cuenta_caja cc on cc.id_cuenta=cm.id_cuenta_caja'
      'left join caja_cab cab on cab.id_caja=cm.id_caja'
      
        'left join cuenta_caja cbco on cbco.Id_cuenta  = t.id_cuenta_banc' +
        'o'
      ''
      
        'where t.fecha between :desde and :hasta and  (t.id_cuenta_banco=' +
        ':id or (:id=-1))'
      '  order by tc.detalle,cm.nro_comprobante')
    Left = 48
    Top = 416
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QTxEm: TFDQuery
    ObjectView = False
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select t.id, t.fecha, t.numero,t.id_cuenta_banco,cbco.nombre as ' +
        'MuestraBco, t.importe,t.unidades,'
      
        'tc.detalle,cm.clase_comprob,cm.nro_comprobante,cc.id_cuenta as C' +
        'taCaja, cc.nombre as NombreCaja,cm.tipo_comprob,'
      
        'cm.nro_comprobante,cm.fecha_operacion,cab.numerocaja as NumeroCa' +
        'ja,cm.id_comprobante,'
      
        '(case when cm.tipo_comprob='#39'OP'#39' then (select o.nombre from orden' +
        'pago o where o.id_op=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'EC'#39' then (select e.detalle from egr_' +
        'caja e where e.id_egreso=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'FO'#39' then (select f.nombre from fccom' +
        'pcab f where f.id_fc=cm.id_comprobante)'
      
        '      when cm.tipo_comprob='#39'VA'#39' then (select v.nombre from adela' +
        'ntos_sueldo v where v.id=cm.id_comprobante)'
      'end) as Nombre'
      'from transf_banco_emitidas t'
      'left join caja_mov cm on cm.id_mov_caja=T.id_mov_caja'
      'left join tipo_comprobante tc on tc.sigla=cm.tipo_comprob'
      'left join cuenta_caja cc on cc.id_cuenta=cm.id_cuenta_caja'
      'left join caja_cab cab on cab.id_caja=cm.id_caja'
      
        'left join cuenta_caja cbco on cbco.Id_cuenta  = t.id_cuenta_banc' +
        'o'
      
        'where t.fecha between :desde and :hasta and  (t.id_cuenta_banco=' +
        ':id or (:id = -1 ))'
      'order by tc.detalle,cm.nro_comprobante')
    Left = 56
    Top = 360
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QCtaBco: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select c.*,b.Nombre As MuestraBanco from cuenta_caja c '
      'join bancos b on b.id_banco=c.id_banco'
      'where c.id_cuenta=:id ')
    Left = 456
    Top = 448
    ParamData = <
      item
        Name = 'ID'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object QCtaBcoID_CUENTA: TIntegerField
      FieldName = 'ID_CUENTA'
      Origin = 'ID_CUENTA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QCtaBcoID_TIPO_CTA: TIntegerField
      FieldName = 'ID_TIPO_CTA'
      Origin = 'ID_TIPO_CTA'
      Required = True
    end
    object QCtaBcoNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 25
    end
    object QCtaBcoNRO_CUENTA: TStringField
      FieldName = 'NRO_CUENTA'
      Origin = 'NRO_CUENTA'
      Size = 15
    end
    object QCtaBcoID_BANCO: TIntegerField
      FieldName = 'ID_BANCO'
      Origin = 'ID_BANCO'
    end
    object QCtaBcoCUIT: TStringField
      FieldName = 'CUIT'
      Origin = 'CUIT'
      Size = 13
    end
    object QCtaBcoRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      Origin = 'RAZONSOCIAL'
      Size = 35
    end
    object QCtaBcoMUESTRABANCO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRABANCO'
      Origin = 'NOMBRE'
      ProviderFlags = []
      ReadOnly = True
    end
  end
  object PopupMenu1: TPopupMenu
    Left = 360
    Top = 176
    object CambiarRealizadas: TMenuItem
      Caption = 'Cambiar Cta en Realizaadas'
      OnClick = CambiarRealizadasClick
    end
    object CambiarRecibidas: TMenuItem
      Caption = 'Cambiar Cta en Recibidas'
      OnClick = CambiarRecibidasClick
    end
    object N1: TMenuItem
      Caption = '-'
    end
    object ExportaraCSV1: TMenuItem
      Action = ExportarCSV
    end
  end
  object OpenDialog1: TOpenDialog
    Left = 1008
    Top = 421
  end
end
