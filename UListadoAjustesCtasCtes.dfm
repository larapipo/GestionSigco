inherited FormListadoAjustesCC: TFormListadoAjustesCC
  Caption = 'Listado de Ajustes en Ctas.Ctes.'
  ClientHeight = 747
  ClientWidth = 1106
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 1124
  ExplicitHeight = 788
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1106
    Height = 569
    Caption = 'con'
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1106
    ExplicitHeight = 569
    object pcAjustes: TPageControl
      Left = 0
      Top = 57
      Width = 1106
      Height = 506
      ActivePage = pgVentas
      Align = alTop
      TabOrder = 0
      object pgVentas: TTabSheet
        Caption = 'Ajustes sobre Clientes'
        object cxGrid1: TcxGrid
          Left = 0
          Top = 0
          Width = 1098
          Height = 472
          Align = alTop
          PopupMenu = PopupMenu1
          TabOrder = 0
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          object cxGrid1DBTableView1: TcxGridDBTableView
            OnDblClick = cxGrid1DBTableView1DblClick
            DataController.DataSource = DSClientes
            DataController.Options = [dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding, dcoImmediatePost]
            OptionsData.Deleting = False
            OptionsData.Inserting = False
            OptionsView.GroupByBox = False
            object cxGrid1DBTableView1ID: TcxGridDBColumn
              DataBinding.FieldName = 'ID'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1TIPOCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'TIPOCPBTE'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1CLASECPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'CLASECPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 36
            end
            object cxGrid1DBTableView1FECHA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 75
            end
            object cxGrid1DBTableView1FECHAVTO: TcxGridDBColumn
              DataBinding.FieldName = 'FECHAVTO'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 75
            end
            object cxGrid1DBTableView1LETRA: TcxGridDBColumn
              DataBinding.FieldName = 'LETRA'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1SUC: TcxGridDBColumn
              DataBinding.FieldName = 'SUC'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1NUMERO: TcxGridDBColumn
              DataBinding.FieldName = 'NUMERO'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1NROCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'NROCPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 85
            end
            object cxGrid1DBTableView1SUCURSAL: TcxGridDBColumn
              DataBinding.FieldName = 'SUCURSAL'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 36
            end
            object cxGrid1DBTableView1CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 60
            end
            object cxGrid1DBTableView1MUESTRANOMBRE: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRANOMBRE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 211
            end
            object cxGrid1DBTableView1DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1DEBE: TcxGridDBColumn
              DataBinding.FieldName = 'DEBE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 80
            end
            object cxGrid1DBTableView1HABER: TcxGridDBColumn
              DataBinding.FieldName = 'HABER'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 80
            end
            object cxGrid1DBTableView1SALDO: TcxGridDBColumn
              DataBinding.FieldName = 'SALDO'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1USUARIO: TcxGridDBColumn
              DataBinding.FieldName = 'USUARIO'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1FECHA_HORA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA_HORA'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1ID_FACTURA_CONSOLIDADA: TcxGridDBColumn
              DataBinding.FieldName = 'ID_FACTURA_CONSOLIDADA'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1ID_FC_CUOTAS: TcxGridDBColumn
              DataBinding.FieldName = 'ID_FC_CUOTAS'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1LOTE: TcxGridDBColumn
              DataBinding.FieldName = 'LOTE'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1MUESTRAMOTIVO: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRAMOTIVO'
              HeaderAlignmentHorz = taCenter
            end
          end
          object cxGrid1Level1: TcxGridLevel
            GridView = cxGrid1DBTableView1
          end
        end
      end
      object pgCompras: TTabSheet
        Caption = 'Ajustes sobre Proveedores'
        ImageIndex = 1
        object cxGrid2: TcxGrid
          Left = 0
          Top = 0
          Width = 1098
          Height = 472
          Align = alTop
          PopupMenu = PopupMenu2
          TabOrder = 0
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          object cxGridDBTableView1: TcxGridDBTableView
            OnDblClick = cxGridDBTableView1DblClick
            DataController.DataSource = DSProvee
            DataController.Options = [dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding, dcoImmediatePost]
            OptionsData.Deleting = False
            OptionsData.Inserting = False
            OptionsView.GroupByBox = False
            object cxGridDBTableView1ID: TcxGridDBColumn
              DataBinding.FieldName = 'ID'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1TIPOCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'TIPOCPBTE'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1CLASECPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'CLASECPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 36
            end
            object cxGridDBTableView1FECHA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 75
            end
            object cxGridDBTableView1FECHAVTO: TcxGridDBColumn
              DataBinding.FieldName = 'FECHAVTO'
              Visible = False
              HeaderAlignmentHorz = taCenter
              Width = 75
            end
            object cxGridDBTableView1LETRA: TcxGridDBColumn
              DataBinding.FieldName = 'LETRA'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1SUC: TcxGridDBColumn
              DataBinding.FieldName = 'SUC'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1NUMERO: TcxGridDBColumn
              DataBinding.FieldName = 'NUMERO'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1NROCPBTE: TcxGridDBColumn
              DataBinding.FieldName = 'NROCPBTE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 85
            end
            object cxGridDBTableView1SUCURSAL: TcxGridDBColumn
              DataBinding.FieldName = 'SUCURSAL'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 36
            end
            object cxGridDBTableView1CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 60
            end
            object cxGridDBTableView1MUESTRANOMBRE: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRANOMBRE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 211
            end
            object cxGridDBTableView1DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGridDBTableView1DEBE: TcxGridDBColumn
              DataBinding.FieldName = 'DEBE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 80
            end
            object cxGridDBTableView1HABER: TcxGridDBColumn
              DataBinding.FieldName = 'HABER'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 80
            end
            object cxGridDBTableView1SALDO: TcxGridDBColumn
              DataBinding.FieldName = 'SALDO'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1USUARIO: TcxGridDBColumn
              DataBinding.FieldName = 'USUARIO'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1FECHA_HORA: TcxGridDBColumn
              DataBinding.FieldName = 'FECHA_HORA'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1LOTE: TcxGridDBColumn
              DataBinding.FieldName = 'LOTE'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1MOTIVO_AJUSTE: TcxGridDBColumn
              DataBinding.FieldName = 'MOTIVO_AJUSTE'
              Visible = False
              HeaderAlignmentHorz = taCenter
            end
            object cxGridDBTableView1MUESTRAMOTIVO: TcxGridDBColumn
              DataBinding.FieldName = 'MUESTRAMOTIVO'
              HeaderAlignmentHorz = taCenter
            end
          end
          object cxGridLevel1: TcxGridLevel
            GridView = cxGridDBTableView1
          end
        end
      end
    end
    object pnCabecera: TPanel
      Left = 0
      Top = 0
      Width = 1106
      Height = 57
      Align = alTop
      BevelOuter = bvNone
      Color = clHighlight
      ParentBackground = False
      TabOrder = 1
      object RxLabel1: TJvLabel
        Left = 686
        Top = 5
        Width = 43
        Height = 16
        Caption = 'Desde'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -13
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object RxLabel3: TJvLabel
        Left = 796
        Top = 5
        Width = 38
        Height = 16
        Caption = 'Hasta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = DEFAULT_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -13
        HotTrackFont.Name = 'MS Sans Serif'
        HotTrackFont.Style = []
      end
      object JvLabel1: TJvLabel
        Left = 19
        Top = 9
        Width = 283
        Height = 30
        Caption = 'Ajustes de Cta.Corriente'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -21
        Font.Name = 'Arial Black'
        Font.Style = [fsBold]
        ParentFont = False
        ShadowPos = spLeftTop
        Transparent = True
        HotTrackFont.Charset = ANSI_CHARSET
        HotTrackFont.Color = clWindowText
        HotTrackFont.Height = -21
        HotTrackFont.Name = 'Arial Black'
        HotTrackFont.Style = []
      end
      object Hasta: TJvDateEdit
        Left = 796
        Top = 24
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 0
      end
      object UpDown1: TUpDown
        Left = 887
        Top = 24
        Width = 16
        Height = 20
        Hint = 'Cambia el Per'#237'odo'
        Min = -1000000000
        Max = 100000000
        ParentShowHint = False
        Position = 100
        ShowHint = True
        TabOrder = 1
        OnClick = UpDown1Click
      end
      object Desde: TJvDateEdit
        Left = 686
        Top = 24
        Width = 89
        Height = 21
        Glyph.Data = {
          
          80008080800080808000808080008080800080808000FF00FF00}
        ImageKind = ikCustom
        NumGlyphs = 2
        PopupColor = clBtnFace
        ShowNullDate = False
        TabOrder = 2
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 569
    Width = 1106
    ExplicitTop = 569
    ExplicitWidth = 1106
    inherited btConfirma: TBitBtn
      Visible = False
    end
    inherited btNuevo: TBitBtn
      Visible = False
    end
    inherited btCancelar: TBitBtn
      Visible = False
    end
    inherited btModificar: TBitBtn
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
    inherited btBorrar: TBitBtn
      Visible = False
    end
  end
  inherited Panel1: TPanel
    Top = 599
    Width = 1106
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 599
    ExplicitWidth = 1106
    inherited DBStatusLabel1: TJvDBStatusLabel
      Height = 21
    end
    inherited sbEstado: TStatusBar
      Width = 1017
      ExplicitWidth = 1017
    end
  end
  inherited ActionList1: TActionList
    Left = 584
    Top = 208
    object ExportarCliente: TAction
      Caption = 'Exportar Ajustes de Clientes'
      OnExecute = ExportarClienteExecute
    end
    object ExportarProveedores: TAction
      Caption = 'Exportar Ajustes de Proveedores'
      OnExecute = ExportarProveedoresExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 568
    Top = 72
  end
  inherited ImageList1: TImageList
    Left = 280
    Top = 96
    Bitmap = {
      
      000000000000}
  end
  object CDSProvee: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPProvee'
    AfterPost = CDSProveeAfterPost
    Left = 416
    Top = 560
    object CDSProveeID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Visible = False
    end
    object CDSProveeTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Visible = False
      Size = 2
    end
    object CDSProveeCLASECPBTE: TStringField
      DisplayLabel = 'Cl.'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSProveeLETRA: TStringField
      FieldName = 'LETRA'
      Origin = 'LETRA'
      Visible = False
      FixedChar = True
      Size = 1
    end
    object CDSProveeSUC: TStringField
      FieldName = 'SUC'
      Origin = 'SUC'
      Visible = False
      Size = 4
    end
    object CDSProveeNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Visible = False
      Size = 8
    end
    object CDSProveeNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSProveeSUCURSAL: TIntegerField
      DisplayLabel = 'Suc.'
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSProveeCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object CDSProveeDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object CDSProveeOBSERVACIONES: TStringField
      FieldName = 'OBSERVACIONES'
      Origin = 'OBSERVACIONES'
      Size = 100
    end
    object CDSProveeUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'USUARIO'
      Visible = False
      Size = 50
    end
    object CDSProveeLOTE: TIntegerField
      FieldName = 'LOTE'
      Origin = 'LOTE'
      Visible = False
    end
    object CDSProveeMOTIVO_AJUSTE: TIntegerField
      FieldName = 'MOTIVO_AJUSTE'
      Origin = 'MOTIVO_AJUSTE'
      Visible = False
    end
    object CDSProveeMUESTRANOMBRE: TStringField
      DisplayLabel = 'Proveedor'
      FieldName = 'MUESTRANOMBRE'
      Origin = 'NOMBRE'
      ProviderFlags = []
      Size = 35
    end
    object CDSProveeMUESTRAMOTIVO: TStringField
      DisplayLabel = 'Motivo'
      FieldKind = fkLookup
      FieldName = 'MUESTRAMOTIVO'
      LookupDataSet = CDSListaMotProvee
      LookupKeyFields = 'ID'
      LookupResultField = 'DETALLE'
      KeyFields = 'MOTIVO_AJUSTE'
      ProviderFlags = []
      Size = 50
      Lookup = True
    end
    object CDSProveeDEBE: TFloatField
      DisplayLabel = 'Debe'
      FieldName = 'DEBE'
      Origin = 'DEBE'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSProveeHABER: TFloatField
      DisplayLabel = 'Haber'
      FieldName = 'HABER'
      Origin = 'HABER'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSProveeSALDO: TFloatField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      Origin = 'SALDO'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSProveeFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
    end
    object CDSProveeFECHAVTO: TSQLTimeStampField
      DisplayLabel = 'Fecha Vto'
      FieldName = 'FECHAVTO'
    end
    object CDSProveeFECHA_HORA: TSQLTimeStampField
      FieldName = 'FECHA_HORA'
    end
  end
  object CDSClientes: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end>
    ProviderName = 'DSPClientes'
    AfterPost = CDSClientesAfterPost
    Left = 408
    Top = 496
    object CDSClientesID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Visible = False
    end
    object CDSClientesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Visible = False
      Size = 2
    end
    object CDSClientesCLASECPBTE: TStringField
      DisplayLabel = 'Cl.'
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSClientesLETRA: TStringField
      FieldName = 'LETRA'
      Origin = 'LETRA'
      Visible = False
      FixedChar = True
      Size = 1
    end
    object CDSClientesSUC: TStringField
      FieldName = 'SUC'
      Origin = 'SUC'
      Visible = False
      Size = 4
    end
    object CDSClientesNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Visible = False
      Size = 8
    end
    object CDSClientesNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSClientesSUCURSAL: TIntegerField
      DisplayLabel = 'Suc.'
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSClientesCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object CDSClientesDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object CDSClientesOBSERVACIONES: TStringField
      DisplayLabel = 'Observ.'
      FieldName = 'OBSERVACIONES'
      Origin = 'OBSERVACIONES'
      Size = 100
    end
    object CDSClientesUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'USUARIO'
      Visible = False
      Size = 50
    end
    object CDSClientesID_FACTURA_CONSOLIDADA: TIntegerField
      FieldName = 'ID_FACTURA_CONSOLIDADA'
      Origin = 'ID_FACTURA_CONSOLIDADA'
      Visible = False
    end
    object CDSClientesID_FC_CUOTAS: TIntegerField
      FieldName = 'ID_FC_CUOTAS'
      Origin = 'ID_FC_CUOTAS'
      Visible = False
    end
    object CDSClientesLOTE: TIntegerField
      FieldName = 'LOTE'
      Origin = 'LOTE'
      Visible = False
    end
    object CDSClientesMOTIVO_AJUSTE: TIntegerField
      FieldName = 'MOTIVO_AJUSTE'
      Origin = 'MOTIVO_AJUSTE'
    end
    object CDSClientesMUESTRANOMBRE: TStringField
      DisplayLabel = 'Cliente Nombre'
      FieldName = 'MUESTRANOMBRE'
      Origin = 'NOMBRE'
      ProviderFlags = []
      Size = 35
    end
    object CDSClientesMUESTRAMOTIVO: TStringField
      DisplayLabel = 'Motivo Ajuste'
      FieldKind = fkLookup
      FieldName = 'MUESTRAMOTIVO'
      LookupDataSet = CDSListaMotClientes
      LookupKeyFields = 'ID'
      LookupResultField = 'DETALLE'
      KeyFields = 'MOTIVO_AJUSTE'
      ProviderFlags = []
      Size = 50
      Lookup = True
    end
    object CDSClientesDEBE: TFloatField
      DisplayLabel = 'Debe'
      FieldName = 'DEBE'
      Origin = 'DEBE'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSClientesHABER: TFloatField
      DisplayLabel = 'Haber'
      FieldName = 'HABER'
      Origin = 'HABER'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSClientesSALDO: TFloatField
      DisplayLabel = 'Saldo'
      FieldName = 'SALDO'
      Origin = 'SALDO'
      DisplayFormat = ',0.00;,0.00;-'
    end
    object CDSClientesFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object CDSClientesFECHAVTO: TSQLTimeStampField
      DisplayLabel = 'Fecha Vto'
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object CDSClientesFECHA_HORA: TSQLTimeStampField
      FieldName = 'FECHA_HORA'
      Origin = 'FECHA_HORA'
    end
  end
  object DSPClientes: TDataSetProvider
    DataSet = QAjustesClientes
    Left = 336
    Top = 496
  end
  object DSPProvee: TDataSetProvider
    DataSet = QAjustesProvee
    UpdateMode = upWhereKeyOnly
    Left = 328
    Top = 560
  end
  object DSClientes: TDataSource
    DataSet = CDSClientes
    Left = 512
    Top = 504
  end
  object DSProvee: TDataSource
    DataSet = CDSProvee
    Left = 512
    Top = 560
  end
  object DSPListaMotClientes: TDataSetProvider
    DataSet = DMBuscadores.QLstMotAjClientes
    Left = 640
    Top = 504
  end
  object DSPListaMotProveedores: TDataSetProvider
    DataSet = DMBuscadores.QLstMotAjProvee
    Left = 640
    Top = 560
  end
  object CDSListaMotClientes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPListaMotClientes'
    Left = 752
    Top = 504
    object CDSListaMotClientesID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object CDSListaMotClientesDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 50
    end
  end
  object CDSListaMotProvee: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPListaMotProveedores'
    Left = 752
    Top = 560
  end
  object QAjustesClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select aj.*,c.nombre as MuestraNombre from ajuste_cc_clientes aj'
      'left join clientes c on c.codigo=aj.codigo'
      'where aj.fecha between :desde and :hasta')
    Left = 252
    Top = 489
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QAjustesClientesID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QAjustesClientesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Size = 2
    end
    object QAjustesClientesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object QAjustesClientesLETRA: TStringField
      FieldName = 'LETRA'
      Origin = 'LETRA'
      FixedChar = True
      Size = 1
    end
    object QAjustesClientesSUC: TStringField
      FieldName = 'SUC'
      Origin = 'SUC'
      Size = 4
    end
    object QAjustesClientesNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Size = 8
    end
    object QAjustesClientesNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object QAjustesClientesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object QAjustesClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object QAjustesClientesDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object QAjustesClientesDEBE: TFloatField
      FieldName = 'DEBE'
      Origin = 'DEBE'
    end
    object QAjustesClientesHABER: TFloatField
      FieldName = 'HABER'
      Origin = 'HABER'
    end
    object QAjustesClientesSALDO: TFloatField
      FieldName = 'SALDO'
      Origin = 'SALDO'
    end
    object QAjustesClientesOBSERVACIONES: TStringField
      FieldName = 'OBSERVACIONES'
      Origin = 'OBSERVACIONES'
      Size = 100
    end
    object QAjustesClientesUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'USUARIO'
      Size = 50
    end
    object QAjustesClientesID_FACTURA_CONSOLIDADA: TIntegerField
      FieldName = 'ID_FACTURA_CONSOLIDADA'
      Origin = 'ID_FACTURA_CONSOLIDADA'
    end
    object QAjustesClientesID_FC_CUOTAS: TIntegerField
      FieldName = 'ID_FC_CUOTAS'
      Origin = 'ID_FC_CUOTAS'
    end
    object QAjustesClientesLOTE: TIntegerField
      FieldName = 'LOTE'
      Origin = 'LOTE'
    end
    object QAjustesClientesMOTIVO_AJUSTE: TIntegerField
      FieldName = 'MOTIVO_AJUSTE'
      Origin = 'MOTIVO_AJUSTE'
    end
    object QAjustesClientesMUESTRANOMBRE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRANOMBRE'
      Origin = 'NOMBRE'
      ProviderFlags = []
      ReadOnly = True
      Size = 35
    end
    object QAjustesClientesFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object QAjustesClientesFECHAVTO: TSQLTimeStampField
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object QAjustesClientesFECHA_HORA: TSQLTimeStampField
      FieldName = 'FECHA_HORA'
      Origin = 'FECHA_HORA'
    end
  end
  object QAjustesProvee: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select aj.*,p.nombre as MuestraNombre from ajuste_cc_provee aj'
      'left join poveedor p on p.codigo=aj.codigo'
      'where aj.fecha between :desde and :hasta')
    Left = 248
    Top = 560
    ParamData = <
      item
        Position = 1
        Name = 'desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end>
    object QAjustesProveeID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QAjustesProveeTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Size = 2
    end
    object QAjustesProveeCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object QAjustesProveeLETRA: TStringField
      FieldName = 'LETRA'
      Origin = 'LETRA'
      FixedChar = True
      Size = 1
    end
    object QAjustesProveeSUC: TStringField
      FieldName = 'SUC'
      Origin = 'SUC'
      Size = 4
    end
    object QAjustesProveeNUMERO: TStringField
      FieldName = 'NUMERO'
      Origin = 'NUMERO'
      Size = 8
    end
    object QAjustesProveeNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object QAjustesProveeSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object QAjustesProveeCODIGO: TStringField
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      Size = 6
    end
    object QAjustesProveeDETALLE: TStringField
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
    end
    object QAjustesProveeDEBE: TFloatField
      FieldName = 'DEBE'
      Origin = 'DEBE'
    end
    object QAjustesProveeHABER: TFloatField
      FieldName = 'HABER'
      Origin = 'HABER'
    end
    object QAjustesProveeSALDO: TFloatField
      FieldName = 'SALDO'
      Origin = 'SALDO'
    end
    object QAjustesProveeOBSERVACIONES: TStringField
      FieldName = 'OBSERVACIONES'
      Origin = 'OBSERVACIONES'
      Size = 100
    end
    object QAjustesProveeUSUARIO: TStringField
      FieldName = 'USUARIO'
      Origin = 'USUARIO'
      Size = 50
    end
    object QAjustesProveeLOTE: TIntegerField
      FieldName = 'LOTE'
      Origin = 'LOTE'
    end
    object QAjustesProveeMOTIVO_AJUSTE: TIntegerField
      FieldName = 'MOTIVO_AJUSTE'
      Origin = 'MOTIVO_AJUSTE'
    end
    object QAjustesProveeMUESTRANOMBRE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'MUESTRANOMBRE'
      Origin = 'NOMBRE'
      ProviderFlags = []
      ReadOnly = True
      Size = 35
    end
    object QAjustesProveeFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
      Origin = 'FECHA'
    end
    object QAjustesProveeFECHAVTO: TSQLTimeStampField
      FieldName = 'FECHAVTO'
      Origin = 'FECHAVTO'
    end
    object QAjustesProveeFECHA_HORA: TSQLTimeStampField
      FieldName = 'FECHA_HORA'
      Origin = 'FECHA_HORA'
    end
  end
  object SaveDialog: TSaveDialog
    Left = 644
    Top = 265
  end
  object PopupMenu1: TPopupMenu
    Left = 316
    Top = 201
    object ExportarAjustesdeClientes1: TMenuItem
      Action = ExportarCliente
    end
  end
  object PopupMenu2: TPopupMenu
    Left = 428
    Top = 201
    object ExportarAjustesdeProveedores1: TMenuItem
      Action = ExportarProveedores
    end
  end
end
