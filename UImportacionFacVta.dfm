inherited FormImportacionFacturasVtaGral: TFormImportacionFacturasVtaGral
  Left = 165
  Top = 94
  Caption = 'Importacion Facturas Vta.Gral.'
  ClientHeight = 684
  ClientWidth = 1029
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 1047
  ExplicitHeight = 725
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 1029
    Height = 631
    Align = alClient
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 1029
    ExplicitHeight = 631
    object RxLabel2: TJvLabel
      Left = 444
      Top = 405
      Width = 89
      Height = 13
      Caption = 'Sucursal de Venta'
      Anchors = [akLeft, akBottom]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitTop = 343
    end
    object RxLabel6: TJvLabel
      Left = 786
      Top = 7
      Width = 48
      Height = 13
      Caption = 'Factura A'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object RxLabel3: TJvLabel
      Left = 444
      Top = 449
      Width = 100
      Height = 13
      Caption = 'Deposito Mercaderia'
      Anchors = [akLeft, akBottom]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitTop = 387
    end
    object Label1: TLabel
      Left = 14
      Top = 450
      Width = 64
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Articulo (21%)'
      ExplicitTop = 388
    end
    object Label3: TLabel
      Left = 14
      Top = 493
      Width = 73
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Articulo (10.5%)'
      ExplicitTop = 431
    end
    object Label2: TLabel
      Left = 8
      Top = 2
      Width = 90
      Height = 13
      Caption = 'Archivo a Procesar'
    end
    object BuscarArchivo: TSpeedButton
      Left = 398
      Top = 20
      Width = 23
      Height = 22
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
      OnClick = BuscarArchivoClick
    end
    object JvLabel2: TJvLabel
      Left = 786
      Top = 46
      Width = 48
      Height = 13
      Caption = 'Factura B'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel4: TJvLabel
      Left = 786
      Top = 86
      Width = 67
      Height = 13
      Caption = 'Fc.Contado A'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel5: TJvLabel
      Left = 786
      Top = 126
      Width = 67
      Height = 13
      Caption = 'Fc.Contado B'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel6: TJvLabel
      Left = 786
      Top = 166
      Width = 56
      Height = 13
      Caption = 'N.Credito A'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel3: TJvLabel
      Left = 786
      Top = 450
      Width = 67
      Height = 13
      Caption = 'Fc.Contado C'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      Visible = False
      ExplicitLeft = 761
    end
    object JvLabel10: TJvLabel
      Left = 786
      Top = 410
      Width = 48
      Height = 13
      Caption = 'Factura C'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      Visible = False
      ExplicitLeft = 761
    end
    object Label4: TLabel
      Left = 14
      Top = 405
      Width = 71
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Articulo (Exent)'
      ExplicitTop = 343
    end
    object spAyudaFormato: TSpeedButton
      Left = 427
      Top = 20
      Width = 23
      Height = 22
      Flat = True
      Glyph.Data = {
        
        0303030303030303030303030303030303030303030303030303}
      NumGlyphs = 2
      OnClick = spAyudaFormatoClick
    end
    object JvLabel7: TJvLabel
      Left = 786
      Top = 205
      Width = 56
      Height = 13
      Caption = 'N.Credito B'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel8: TJvLabel
      Left = 786
      Top = 245
      Width = 59
      Height = 13
      Caption = 'N.Debitos A'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel9: TJvLabel
      Left = 786
      Top = 285
      Width = 59
      Height = 13
      Caption = 'N.Debitos B'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object Label5: TLabel
      Left = 14
      Top = 535
      Width = 64
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Articulo (27%)'
      ExplicitTop = 473
    end
    object JvLabel1: TJvLabel
      Left = 786
      Top = 327
      Width = 36
      Height = 13
      Caption = 'Tiket A'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object JvLabel11: TJvLabel
      Left = 786
      Top = 365
      Width = 36
      Height = 13
      Caption = 'Tiket B'
      Anchors = [akTop, akRight]
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
      ExplicitLeft = 761
    end
    object Label8: TLabel
      Left = 14
      Top = 579
      Width = 51
      Height = 13
      Anchors = [akLeft, akBottom]
      Caption = 'Cliente C.F'
      ParentShowHint = False
      ShowHint = True
      ExplicitTop = 517
    end
    object dbcSucursal: TJvDBLookupCombo
      Left = 444
      Top = 422
      Width = 217
      Height = 21
      EmptyValue = '-1'
      Anchors = [akLeft, akBottom]
      LookupField = 'CODIGO'
      LookupDisplay = 'DETALLE'
      LookupSource = DSSucursal
      TabOrder = 0
      OnClick = dbcSucursalClick
    end
    object dbcFacturaA: TJvDBLookupCombo
      Left = 786
      Top = 20
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSFc_A
      TabOrder = 1
    end
    object dbcDeposito: TJvDBLookupCombo
      Left = 444
      Top = 466
      Width = 217
      Height = 21
      EmptyValue = '-1'
      Anchors = [akLeft, akBottom]
      LookupField = 'ID'
      LookupDisplay = 'NOMBRE'
      LookupSource = DSDeposito
      TabOrder = 2
    end
    object ceCodigo: TJvComboEdit
      Left = 14
      Top = 470
      Width = 73
      Height = 21
      Anchors = [akLeft, akBottom]
      ButtonWidth = 16
      ClickKey = 16397
      Color = clBtnFace
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 3
      Text = ''
      OnButtonClick = BuscarArticuloExecute
    end
    object ceCodigo105: TJvComboEdit
      Left = 14
      Top = 510
      Width = 73
      Height = 21
      Anchors = [akLeft, akBottom]
      ButtonWidth = 16
      ClickKey = 16397
      Color = clBtnFace
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 4
      Text = ''
      OnButtonClick = BuscarArticuloExecute
    end
    object edDetalle2: TEdit
      Left = 95
      Top = 510
      Width = 312
      Height = 21
      Anchors = [akLeft, akBottom]
      Color = clInfoBk
      Enabled = False
      TabOrder = 5
    end
    object edDetalle: TEdit
      Left = 95
      Top = 470
      Width = 312
      Height = 21
      Anchors = [akLeft, akBottom]
      Color = clInfoBk
      Enabled = False
      TabOrder = 6
    end
    object edPath: TEdit
      Left = 7
      Top = 21
      Width = 385
      Height = 21
      TabOrder = 7
    end
    object dbcFacturaB: TJvDBLookupCombo
      Left = 786
      Top = 60
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSFc_B
      TabOrder = 8
    end
    object dbcF_ContadoA: TJvDBLookupCombo
      Left = 786
      Top = 100
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSFo_A
      TabOrder = 9
    end
    object dbcF_ContadoB: TJvDBLookupCombo
      Left = 786
      Top = 140
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSFo_B
      TabOrder = 10
    end
    object dbcNc_A: TJvDBLookupCombo
      Left = 786
      Top = 178
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSNc_A
      TabOrder = 11
    end
    object dbcNd_A: TJvDBLookupCombo
      Left = 786
      Top = 260
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSNd_A
      TabOrder = 12
    end
    object dbcNd_B: TJvDBLookupCombo
      Left = 786
      Top = 300
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSNd_B
      TabOrder = 13
    end
    object dbcNc_B: TJvDBLookupCombo
      Left = 786
      Top = 218
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSNc_B
      TabOrder = 14
    end
    object dbcF_ContadoC: TJvDBLookupCombo
      Left = 786
      Top = 465
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      TabOrder = 15
      Visible = False
    end
    object dbcFacturaC: TJvDBLookupCombo
      Left = 786
      Top = 423
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      TabOrder = 16
      Visible = False
    end
    object ceCodigoCero: TJvComboEdit
      Left = 14
      Top = 427
      Width = 73
      Height = 21
      Anchors = [akLeft, akBottom]
      ButtonWidth = 16
      ClickKey = 16397
      Color = clBtnFace
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 17
      Text = ''
      OnButtonClick = BuscarArticuloExecute
    end
    object edDetalleExento: TEdit
      Left = 95
      Top = 427
      Width = 312
      Height = 21
      Anchors = [akLeft, akBottom]
      Color = clInfoBk
      Enabled = False
      TabOrder = 18
    end
    object BitBtn1: TBitBtn
      Left = 717
      Top = 582
      Width = 115
      Height = 34
      Action = Importar
      Anchors = [akRight, akBottom]
      Caption = 'Importar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        33333333333333333333}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 19
    end
    object ceCodigo27: TJvComboEdit
      Left = 14
      Top = 552
      Width = 73
      Height = 21
      Anchors = [akLeft, akBottom]
      ButtonWidth = 16
      ClickKey = 16397
      Color = clBtnFace
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 20
      Text = ''
      OnButtonClick = BuscarArticuloExecute
    end
    object edDetalle27: TEdit
      Left = 95
      Top = 552
      Width = 312
      Height = 21
      Anchors = [akLeft, akBottom]
      Color = clInfoBk
      Enabled = False
      TabOrder = 21
    end
    object chExcel2013Plus: TCheckBox
      Left = 427
      Top = 7
      Width = 83
      Height = 13
      Caption = 'Excel 2003+'
      Checked = True
      State = cbChecked
      TabOrder = 22
    end
    object dbc_TkA: TJvDBLookupCombo
      Left = 786
      Top = 342
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSTkA
      TabOrder = 23
    end
    object dbc_TkB: TJvDBLookupCombo
      Left = 786
      Top = 380
      Width = 217
      Height = 21
      DisplayAllFields = True
      EmptyValue = '-1'
      Anchors = [akTop, akRight]
      LookupField = 'ID_COMPROBANTE'
      LookupDisplay = 'tipo_comprob;clase_comprob;DENOMINACION'
      LookupSource = DSTkB
      TabOrder = 24
    end
    object ceCliente: TJvComboEdit
      Left = 14
      Top = 597
      Width = 73
      Height = 21
      Anchors = [akLeft, akBottom]
      ButtonWidth = 16
      ClickKey = 16397
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      TabOrder = 25
      Text = ''
      OnButtonClick = ceClienteButtonClick
      OnKeyDown = ceClienteKeyDown
      OnKeyPress = ceClienteKeyPress
    end
    object edNombreCliente: TEdit
      Left = 95
      Top = 597
      Width = 257
      Height = 21
      Anchors = [akLeft, akBottom]
      Color = clInfoBk
      Enabled = False
      TabOrder = 26
    end
    object pcImportacion: TPageControl
      Left = 0
      Top = 48
      Width = 770
      Height = 351
      ActivePage = pgImportadas
      Anchors = [akLeft, akTop, akRight, akBottom]
      TabOrder = 27
      object pgImpotar: TTabSheet
        Caption = 'Datos A Importar'
        object JvDBStatusLabel1: TJvDBStatusLabel
          Left = 3
          Top = 245
          Width = 96
          Height = 13
          DataSource = DSMovXLS
          Style = lsRecordNo
          Transparent = True
        end
        object dbgMovimientos: TDBAdvGrid
          Left = 0
          Top = 0
          Width = 762
          Height = 323
          Align = alClient
          ColCount = 2
          DrawingStyle = gdsClassic
          FixedColor = clWhite
          RowCount = 2
          FixedRows = 1
          TabOrder = 0
          ActiveCellFont.Charset = DEFAULT_CHARSET
          ActiveCellFont.Color = 4474440
          ActiveCellFont.Height = -11
          ActiveCellFont.Name = 'Tahoma'
          ActiveCellFont.Style = [fsBold]
          ActiveCellColor = 11565130
          ActiveCellColorTo = 11565130
          BorderColor = 11250603
          ControlLook.FixedGradientFrom = clWhite
          ControlLook.FixedGradientTo = clWhite
          ControlLook.FixedGradientHoverFrom = clGray
          ControlLook.FixedGradientHoverTo = clWhite
          ControlLook.FixedGradientHoverMirrorFrom = clWhite
          ControlLook.FixedGradientHoverMirrorTo = clWhite
          ControlLook.FixedGradientHoverBorder = 11645361
          ControlLook.FixedGradientDownFrom = clWhite
          ControlLook.FixedGradientDownTo = clWhite
          ControlLook.FixedGradientDownMirrorFrom = clWhite
          ControlLook.FixedGradientDownMirrorTo = clWhite
          ControlLook.FixedGradientDownBorder = 11250603
          ControlLook.DropDownHeader.Font.Charset = DEFAULT_CHARSET
          ControlLook.DropDownHeader.Font.Color = clWindowText
          ControlLook.DropDownHeader.Font.Height = -11
          ControlLook.DropDownHeader.Font.Name = 'Tahoma'
          ControlLook.DropDownHeader.Font.Style = []
          ControlLook.DropDownHeader.Visible = True
          ControlLook.DropDownHeader.Buttons = <>
          ControlLook.DropDownFooter.Font.Charset = DEFAULT_CHARSET
          ControlLook.DropDownFooter.Font.Color = clWindowText
          ControlLook.DropDownFooter.Font.Height = -11
          ControlLook.DropDownFooter.Font.Name = 'Tahoma'
          ControlLook.DropDownFooter.Font.Style = []
          ControlLook.DropDownFooter.Visible = True
          ControlLook.DropDownFooter.Buttons = <>
          ControlLook.ToggleSwitch.BackgroundBorderWidth = 1.000000000000000000
          ControlLook.ToggleSwitch.ButtonBorderWidth = 1.000000000000000000
          ControlLook.ToggleSwitch.CaptionFont.Charset = DEFAULT_CHARSET
          ControlLook.ToggleSwitch.CaptionFont.Color = clWindowText
          ControlLook.ToggleSwitch.CaptionFont.Height = -12
          ControlLook.ToggleSwitch.CaptionFont.Name = 'Segoe UI'
          ControlLook.ToggleSwitch.CaptionFont.Style = []
          ControlLook.ToggleSwitch.Shadow = False
          Filter = <>
          FilterDropDown.Font.Charset = DEFAULT_CHARSET
          FilterDropDown.Font.Color = clWindowText
          FilterDropDown.Font.Height = -11
          FilterDropDown.Font.Name = 'Tahoma'
          FilterDropDown.Font.Style = []
          FilterDropDown.TextChecked = 'Checked'
          FilterDropDown.TextUnChecked = 'Unchecked'
          FilterDropDownClear = '(All)'
          FilterEdit.TypeNames.Strings = (
            'Starts with'
            'Ends with'
            'Contains'
            'Not contains'
            'Equal'
            'Not equal'
            'Larger than'
            'Smaller than'
            'Clear')
          FixedColWidth = 20
          FixedRowHeight = 22
          FixedFont.Charset = DEFAULT_CHARSET
          FixedFont.Color = clWindowText
          FixedFont.Height = -11
          FixedFont.Name = 'Tahoma'
          FixedFont.Style = [fsBold]
          FloatFormat = '%.2f'
          HoverButtons.Buttons = <>
          HTMLSettings.ImageFolder = 'images'
          HTMLSettings.ImageBaseName = 'img'
          Look = glCustom
          PrintSettings.DateFormat = 'dd/mm/yyyy'
          PrintSettings.Font.Charset = DEFAULT_CHARSET
          PrintSettings.Font.Color = clWindowText
          PrintSettings.Font.Height = -11
          PrintSettings.Font.Name = 'Tahoma'
          PrintSettings.Font.Style = []
          PrintSettings.FixedFont.Charset = DEFAULT_CHARSET
          PrintSettings.FixedFont.Color = clWindowText
          PrintSettings.FixedFont.Height = -11
          PrintSettings.FixedFont.Name = 'Tahoma'
          PrintSettings.FixedFont.Style = []
          PrintSettings.HeaderFont.Charset = DEFAULT_CHARSET
          PrintSettings.HeaderFont.Color = clWindowText
          PrintSettings.HeaderFont.Height = -11
          PrintSettings.HeaderFont.Name = 'Tahoma'
          PrintSettings.HeaderFont.Style = []
          PrintSettings.FooterFont.Charset = DEFAULT_CHARSET
          PrintSettings.FooterFont.Color = clWindowText
          PrintSettings.FooterFont.Height = -11
          PrintSettings.FooterFont.Name = 'Tahoma'
          PrintSettings.FooterFont.Style = []
          PrintSettings.PageNumSep = '/'
          SearchFooter.ColorTo = clNone
          SearchFooter.FindNextCaption = 'Find &next'
          SearchFooter.FindPrevCaption = 'Find &previous'
          SearchFooter.Font.Charset = DEFAULT_CHARSET
          SearchFooter.Font.Color = clWindowText
          SearchFooter.Font.Height = -11
          SearchFooter.Font.Name = 'Tahoma'
          SearchFooter.Font.Style = []
          SearchFooter.HighLightCaption = 'Highlight'
          SearchFooter.HintClose = 'Close'
          SearchFooter.HintFindNext = 'Find next occurrence'
          SearchFooter.HintFindPrev = 'Find previous occurrence'
          SearchFooter.HintHighlight = 'Highlight occurrences'
          SearchFooter.MatchCaseCaption = 'Match case'
          SearchFooter.ResultFormat = '(%d of %d)'
          SelectionColor = 13744549
          SelectionTextColor = clWindowText
          SortSettings.HeaderColor = clWhite
          SortSettings.HeaderColorTo = clWhite
          SortSettings.HeaderMirrorColor = clWhite
          SortSettings.HeaderMirrorColorTo = clWhite
          Version = '2.5.1.23'
          AutoCreateColumns = True
          AutoRemoveColumns = True
          Columns = <
            item
              Borders = []
              BorderPen.Color = clSilver
              ButtonHeight = 18
              CheckFalse = 'N'
              CheckTrue = 'Y'
              Color = clWindow
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = []
              HeaderFont.Charset = DEFAULT_CHARSET
              HeaderFont.Color = 3881787
              HeaderFont.Height = -11
              HeaderFont.Name = 'Tahoma'
              HeaderFont.Style = []
              PrintBorders = [cbTop, cbLeft, cbRight, cbBottom]
              PrintFont.Charset = DEFAULT_CHARSET
              PrintFont.Color = clWindowText
              PrintFont.Height = -11
              PrintFont.Name = 'Tahoma'
              PrintFont.Style = []
              Width = 20
            end
            item
              Borders = []
              BorderPen.Color = clSilver
              ButtonHeight = 18
              CheckFalse = 'N'
              CheckTrue = 'Y'
              Color = clWindow
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clWindowText
              Font.Height = -11
              Font.Name = 'Tahoma'
              Font.Style = []
              HeaderFont.Charset = DEFAULT_CHARSET
              HeaderFont.Color = 3881787
              HeaderFont.Height = -11
              HeaderFont.Name = 'Tahoma'
              HeaderFont.Style = []
              PrintBorders = [cbTop, cbLeft, cbRight, cbBottom]
              PrintFont.Charset = DEFAULT_CHARSET
              PrintFont.Color = clWindowText
              PrintFont.Height = -11
              PrintFont.Name = 'Tahoma'
              PrintFont.Style = []
              Width = 64
            end>
          DataSource = DSMovXLS
          InvalidPicture.Data = {
            
            FFC003FF}
          ShowUnicode = False
          ExplicitWidth = 747
          ExplicitHeight = 261
          ColWidths = (
            20
            64)
          RowHeights = (
            22
            22)
        end
      end
      object pgImportadas: TTabSheet
        Caption = 'Facturas Importadas'
        ImageIndex = 1
        DesignSize = (
          762
          323)
        object JvDBStatusLabel2: TJvDBStatusLabel
          Left = 546
          Top = 3
          Width = 96
          Height = 13
          DataSource = DSMemory
          Style = lsRecordNo
          Transparent = True
        end
        object dbgImportados: TDBGrid
          Left = 3
          Top = 37
          Width = 537
          Height = 283
          Anchors = [akBottom]
          Color = clMoneyGreen
          DataSource = DSMemory
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'ID'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clWindowText
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TIPOCPBTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlue
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 30
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NROCPBTE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clGreen
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 90
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOMBRE'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clGreen
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 215
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TOTAL'
              Title.Alignment = taCenter
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clGreen
              Title.Font.Height = -11
              Title.Font.Name = 'MS Sans Serif'
              Title.Font.Style = [fsBold]
              Width = 78
              Visible = True
            end>
        end
        object BitBtn2: TBitBtn
          Left = 553
          Top = 220
          Width = 129
          Height = 29
          Caption = 'Borrar Importados'
          Glyph.Data = {
            
            FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
          TabOrder = 1
          OnClick = BitBtn2Click
        end
      end
    end
    object btCancelarImpo: TBitBtn
      Left = 838
      Top = 582
      Width = 117
      Height = 34
      Hint = 'Cancelar los Cambios'
      Anchors = [akRight, akBottom]
      Cancel = True
      Caption = 'Ca&ncelar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Glyph.Data = {
        
        B40001059F00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentFont = False
      TabOrder = 28
      OnClick = btCancelarImpoClick
    end
  end
  inherited ToolBar1: TToolBar
    Top = 631
    Width = 1029
    Align = alBottom
    ButtonHeight = 28
    ExplicitTop = 631
    ExplicitWidth = 1029
    inherited btConfirma: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btNuevo: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btCancelar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btModificar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited Pr: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBuscar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited Ne: TSpeedButton
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btBorrar: TBitBtn
      Height = 28
      Visible = False
      ExplicitHeight = 28
    end
    inherited btSalir: TBitBtn
      Height = 28
      ExplicitHeight = 28
    end
    object ToolButton1: TToolButton
      Left = 557
      Top = 0
      Width = 173
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
    object btNovedades: TButton
      Left = 730
      Top = 0
      Width = 116
      Height = 28
      Caption = 'Ver Novedades'
      TabOrder = 7
      OnClick = btNovedadesClick
    end
  end
  inherited Panel1: TPanel
    Top = 661
    Width = 1029
    Align = alBottom
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 661
    ExplicitWidth = 1029
    inherited sbEstado: TStatusBar
      Width = 867
      ExplicitWidth = 867
    end
  end
  object pnAvisoBase: TJvPanel [3]
    Left = 82
    Top = 80
    Width = 873
    Height = 214
    Color = clHighlight
    Padding.Left = 5
    Padding.Top = 5
    Padding.Right = 5
    Padding.Bottom = 5
    ParentBackground = False
    TabOrder = 3
    object pnAviso: TPanel
      Left = 6
      Top = 6
      Width = 861
      Height = 202
      Align = alClient
      Color = clGradientInactiveCaption
      ParentBackground = False
      TabOrder = 0
      object Label6: TLabel
        Left = 6
        Top = 87
        Width = 387
        Height = 91
        Caption = 
          'Establecer este formato para la planilla de Facturas, respetando' +
          ' los nombre de las '#13#10'columnas.'#13#10'En nombre de la hoja dejar que d' +
          'iga Hoja1, el que deja por defecto el Excel'#13#10'TP: FC=  Fac'#13#10'     ' +
          '  FO= Fc.Ctdo'#13#10'       NC= N.Cred.'#13#10'       ND= N.Deb          TK ' +
          '= Tikets'
      end
      object Label7: TLabel
        Left = 419
        Top = 87
        Width = 147
        Height = 52
        Caption = 
          'CondicionIVA : 1 = Inscripto'#13#10'                        4 = No Ins' +
          'cripto'#13#10'                        3 = Consu.Final'#13#10'               ' +
          '         5 = Monotributo'
      end
      object Label9: TLabel
        Left = 486
        Top = 142
        Width = 323
        Height = 39
        Caption = 
          'Si Nombred de Cliente esta Vacio se tomara como codigo el defind' +
          'o en  los parametros actuales, Los Importes de IVA que no haya, ' +
          'puede ser omitidos Fecha en DD/MM/YYYY'
        WordWrap = True
      end
      object DBGrid1: TDBGrid
        Left = 1
        Top = 1
        Width = 859
        Height = 73
        Align = alTop
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = vBGrid1DblClick
        Columns = <
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'TP'
            Width = 36
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Letra'
            Width = 54
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Prefijo'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Numero'
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Fecha'
            Width = 61
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Nombre'
            Width = 81
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'CUIT'
            Width = 56
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'CondicionIVA'
            Width = 85
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Iva21'
            Width = 35
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Iva105'
            Width = 51
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'exento'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Iva27'
            Width = 57
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'IIBB'
            Width = 57
            Visible = True
          end
          item
            Expanded = False
            Title.Alignment = taCenter
            Title.Caption = 'Total'
            Width = 50
            Visible = True
          end>
      end
    end
  end
  inherited ActionList1: TActionList
    Left = 456
    Top = 32
    object Importar: TAction
      Caption = 'Importar'
      OnExecute = ImportarExecute
    end
    object BuscarArticulo: TAction
      Caption = 'BuscarArticulo'
      OnExecute = BuscarArticuloExecute
    end
  end
  inherited DSBase: TDataSource
    Left = 520
    Top = 24
  end
  inherited ImageList1: TImageList
    Top = 48
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    Top = 312
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 24
    Top = 288
  end
  inherited JvAppIniFileStorage1: TJvAppIniFileStorage
    Left = 384
    Top = 344
  end
  object DSFc_B: TDataSource
    DataSet = CDSFc_B
    Left = 696
    Top = 208
  end
  object DSNd_B: TDataSource
    DataSet = CDSNd_B
    Left = 688
    Top = 536
  end
  object CDSNd_B: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'ND'#39' and letra = '#39'B'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 640
    Top = 544
    object CDSNd_BID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSNd_BTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNd_BCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNd_BDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSNd_BSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSNd_BLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSFo_C: TDataSource
    DataSet = CDSFo_C
    Left = 928
    Top = 384
  end
  object CDSFo_C: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FO'#39' and letra = '#39'C'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 880
    Top = 384
    object CDSFo_CID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSFo_CTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_CCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_CDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSFo_CSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSFo_CLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSFc_C: TDataSource
    DataSet = CDSFc_C
    Left = 824
    Top = 384
  end
  object CDSFc_C: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FC'#39' and letra = '#39'C'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 744
    Top = 232
    object CDSFc_CID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSFc_CTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_CCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_CDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSFc_CSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSFc_CLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSNd_A: TDataSource
    DataSet = CDSNd_A
    Left = 696
    Top = 488
  end
  object CDSNd_A: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'ND'#39' and letra = '#39'A'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 488
    object CDSNd_AID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSNd_ATIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNd_ACLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNd_ADENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSNd_ASUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSNd_ALETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSNc_B: TDataSource
    DataSet = CDSNc_B
    Left = 696
    Top = 432
  end
  object CDSNc_B: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'NC'#39' and letra = '#39'B'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 432
    object CDSNc_BID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSNc_BTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNc_BCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNc_BDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSNc_BSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSNc_BLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSNc_A: TDataSource
    DataSet = CDSNc_A
    Left = 696
    Top = 376
  end
  object CDSNc_A: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'NC'#39' and letra = '#39'A'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 376
    object CDSNc_AID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSNc_ATIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNc_ACLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSNc_ADENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSNc_ASUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSNc_ALETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSFo_B: TDataSource
    DataSet = CDSFo_B
    Left = 696
    Top = 320
  end
  object CDSFo_B: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FO'#39' and letra = '#39'B'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 320
    object CDSFo_BID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSFo_BTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_BCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_BDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSFo_BSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSFo_BLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSFo_A: TDataSource
    DataSet = CDSFo_A
    Left = 696
    Top = 264
  end
  object CDSFo_A: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FO'#39' and letra = '#39'A'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 264
    object CDSFo_AID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSFo_ATIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_ACLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFo_ADENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSFo_ASUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSFo_ALETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object CDSFc_B: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FC'#39' and letra = '#39'B'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 208
    object CDSFc_BID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object CDSFc_BTIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_BCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_BDENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object CDSFc_BSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object CDSFc_BLETRA: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSFc_A: TDataSource
    DataSet = CDSFc_A
    Left = 704
    Top = 168
  end
  object CDSFc_A: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'FC'#39' and letra = '#39'A'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 648
    Top = 168
    object CDSFc_AID_COMPROBANTE: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Origin = 'ID_COMPROBANTE'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object CDSFc_ATIPO_COMPROB: TStringField
      FieldName = 'TIPO_COMPROB'
      Origin = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_ACLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Origin = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object CDSFc_ADENOMINACION: TStringField
      FieldName = 'DENOMINACION'
      Origin = 'DENOMINACION'
      Size = 15
    end
    object CDSFc_ASUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
    end
    object CDSFc_ALETRA: TStringField
      FieldName = 'LETRA'
      Origin = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSPComprobante: TDataSetProvider
    DataSet = QComproba
    Options = []
    Left = 576
    Top = 176
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 616
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 35
    end
  end
  object DSSucursal: TDataSource
    DataSet = CDSSucursal
    Left = 704
  end
  object DSDeposito: TDataSource
    DataSet = QDepositos
    Left = 504
    Top = 88
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Left = 576
  end
  object DSPStock: TDataSetProvider
    DataSet = DMBuscadores.QStockCodigo_Detalle
    Options = []
    Left = 188
    Top = 160
  end
  object CDSStock: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPStock'
    Left = 244
    Top = 160
    object CDSStockCODIGO_STK: TStringField
      FieldName = 'CODIGO_STK'
      Size = 8
    end
    object CDSStockDETALLE_STK: TStringField
      FieldName = 'DETALLE_STK'
      Size = 45
    end
  end
  object ADOConnection: TADOConnection
    ConnectionString = 
      'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=C:\Documents and Se' +
      'ttings\usuario\Mis documentos\Padron Embargos.xls;Extended Prope' +
      'rties=Excel 8.0;Persist Security Info=False'
    LoginPrompt = False
    Mode = cmShareDenyNone
    Provider = 'Microsoft.Jet.OLEDB.4.0'
    Left = 400
    Top = 520
  end
  object ADODSMovXLS: TADODataSet
    Connection = ADOConnection
    CursorType = ctStatic
    CommandText = 'Hoja1$'
    CommandType = cmdTableDirect
    Parameters = <>
    Left = 352
    Top = 520
  end
  object DSMovXLS: TDataSource
    DataSet = ADODSMovXLS
    Left = 440
    Top = 520
  end
  object OpenDialog: TOpenDialog
    Left = 512
    Top = 304
  end
  object CDSTkA: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'TK'#39' and letra = '#39'A'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 512
    Top = 456
    object IntegerField1: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object StringField1: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object StringField2: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object StringField3: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object IntegerField2: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object StringField4: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSTkA: TDataSource
    DataSet = CDSTkA
    Left = 560
    Top = 456
  end
  object CDSTkB: TClientDataSet
    Aggregates = <>
    Filter = 'Tipo_comprob = '#39'TK'#39' and letra = '#39'B'#39
    Filtered = True
    Params = <
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPComprobante'
    Left = 512
    Top = 512
    object IntegerField3: TIntegerField
      FieldName = 'ID_COMPROBANTE'
      Required = True
    end
    object StringField5: TStringField
      FieldName = 'TIPO_COMPROB'
      Required = True
      Size = 2
    end
    object StringField6: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
    object StringField7: TStringField
      FieldName = 'DENOMINACION'
      Size = 15
    end
    object IntegerField4: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object StringField8: TStringField
      FieldName = 'LETRA'
      FixedChar = True
      Size = 1
    end
  end
  object DSTkB: TDataSource
    DataSet = CDSTkB
    Left = 560
    Top = 512
  end
  object DSPClientes: TDataSetProvider
    DataSet = DMMain_2.QClientes
    Left = 336
    Top = 168
  end
  object CDSClientes: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'codigo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPClientes'
    Left = 408
    Top = 168
    object CDSClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Required = True
      Size = 6
    end
    object CDSClientesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Required = True
      Size = 35
    end
    object CDSClientesRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      Size = 35
    end
  end
  object QVerificaCpbte: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select f.id_fc from fcvtacab f'
      'where f.letrafac=:letra and'
      'f.sucfac=:suc and'
      'f.numerofac=:numero')
    Left = 512
    Top = 624
    ParamData = <
      item
        Name = 'LETRA'
        DataType = ftOraInterval
        ParamType = ptInput
        Size = 1
        Value = Null
      end
      item
        Name = 'SUC'
        DataType = ftString
        ParamType = ptInput
        Size = 4
      end
      item
        Name = 'NUMERO'
        DataType = ftString
        ParamType = ptInput
        Size = 8
      end>
    object QVerificaCpbteID_FC: TIntegerField
      FieldName = 'ID_FC'
      Origin = 'ID_FC'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
  end
  object QBuscaClienteCuit: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select Max(c.codigo) as Codigo from clientes c where c.nro_de_cu' +
        'it = :cuit')
    Left = 328
    Top = 448
    ParamData = <
      item
        Name = 'CUIT'
        DataType = ftString
        ParamType = ptInput
        Size = 13
        Value = Null
      end>
    object QBuscaClienteCuitCODIGO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInKey]
      ReadOnly = True
      Size = 6
    end
  end
  object QComproba: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select ID_COMPROBANTE,TIPO_COMPROB,'
      '    CLASE_COMPROB, DENOMINACION,SUCURSAL,letra from comprobantes'
      'where tipo_comprob in ('#39'FC'#39','#39'NC'#39','#39'ND'#39','#39'FO'#39','#39'TK'#39') and'
      '      sucursal=:sucursal and'
      '      compra_venta= '#39'V'#39)
    Left = 504
    Top = 176
    ParamData = <
      item
        Position = 1
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QDepositos: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select d.* from depositos d where d.sucursal=:sucursal')
    Left = 464
    Top = 96
    ParamData = <
      item
        Name = 'SUCURSAL'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object QDepositosID: TIntegerField
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QDepositosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Origin = 'NOMBRE'
      Size = 25
    end
    object QDepositosSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      Origin = 'SUCURSAL'
      Required = True
    end
    object QDepositosCENTRALIZA_STOCK: TStringField
      FieldName = 'CENTRALIZA_STOCK'
      Origin = 'CENTRALIZA_STOCK'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object spIngresarCliente: TFDStoredProc
    Connection = DMMain_FD.fdcGestion
    StoredProcName = 'INGRESAR_CLIENTE'
    Left = 856
    Top = 512
    ParamData = <
      item
        Position = 1
        Name = 'CODIGO'
        DataType = ftString
        ParamType = ptInput
        Size = 6
      end
      item
        Position = 2
        Name = 'NOMBRE'
        DataType = ftString
        ParamType = ptInput
        Size = 35
      end
      item
        Position = 3
        Name = 'RAZON_SOCIAL'
        DataType = ftString
        ParamType = ptInput
        Size = 35
      end
      item
        Position = 4
        Name = 'DIRECCION'
        DataType = ftString
        ParamType = ptInput
        Size = 35
      end
      item
        Position = 5
        Name = 'TIPO_IVA'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 6
        Name = 'CUIT'
        DataType = ftString
        ParamType = ptInput
        Size = 13
      end
      item
        Position = 7
        Name = 'C_POSTAL'
        DataType = ftString
        ParamType = ptInput
        Size = 8
      end
      item
        Position = 8
        Name = 'LOCALIDAD'
        DataType = ftString
        ParamType = ptInput
        Size = 15
      end
      item
        Position = 9
        Name = 'TELEFONO'
        DataType = ftString
        ParamType = ptInput
        Size = 10
      end
      item
        Position = 10
        Name = 'VENDEDOR'
        DataType = ftString
        ParamType = ptInput
        Size = 6
      end
      item
        Position = 11
        Name = 'ZONA'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 12
        Name = 'OBSERVACIONES'
        DataType = ftMemo
        ParamType = ptInput
      end
      item
        Position = 13
        Name = 'MAIL'
        DataType = ftString
        ParamType = ptInput
        Size = 100
      end
      item
        Position = 14
        Name = 'CUENTA_OP'
        DataType = ftString
        ParamType = ptInput
        Size = 10
      end
      item
        Position = 15
        Name = 'TIPODOC'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 16
        Name = 'NRODOCUMENTO'
        DataType = ftString
        ParamType = ptInput
        Size = 15
      end
      item
        Position = 17
        Name = 'SUCURSAL'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 18
        Name = 'CELULAR'
        DataType = ftString
        ParamType = ptInput
        Size = 11
      end
      item
        Position = 19
        Name = 'ID_POSTAL'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 20
        Name = 'CODIGO_ALTA'
        DataType = ftString
        ParamType = ptOutput
        Size = 6
      end>
  end
  object JvDBGridExcelExport1: TJvDBGridExcelExport
    Caption = 'Exporting to MS Excel...'
    Grid = DBGrid1
    Close = scNever
    AutoFit = False
    Left = 128
    Top = 105
  end
  object SaveDialog1: TSaveDialog
    Left = 216
    Top = 105
  end
  object MemoryData: TJvMemoryData
    FieldDefs = <>
    Left = 309
    Top = 641
    object MemoryDataTIPOCPBTE: TStringField
      DisplayLabel = 'Tp.'
      FieldName = 'TIPOCPBTE'
      Size = 2
    end
    object MemoryDataNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      FieldName = 'NROCPBTE'
    end
    object MemoryDataNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      FieldName = 'NOMBRE'
      Size = 45
    end
    object MemoryDataTOTAL: TFloatField
      DisplayLabel = 'Total'
      FieldName = 'TOTAL'
      DisplayFormat = ',0.00'
    end
    object MemoryDataID: TIntegerField
      FieldName = 'ID'
    end
  end
  object DSMemory: TDataSource
    DataSet = MemoryData
    Left = 368
    Top = 633
  end
  object QBorrarFc: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'delete from fcvtacab f where f.id_fc = :id')
    Left = 632
    Top = 624
    ParamData = <
      item
        Name = 'ID'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
  end
  object QBuscaClienteNombre: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select Max(c.codigo) as Codigo from clientes c where c.razon_soc' +
        'ial = :nombre')
    Left = 432
    Top = 448
    ParamData = <
      item
        Name = 'NOMBRE'
        DataType = ftString
        ParamType = ptInput
        Size = 35
        Value = Null
      end>
    object QBuscaClienteNombreCODIGO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      ProviderFlags = [pfInKey]
      ReadOnly = True
      Size = 6
    end
  end
  object mtMovXLS: TFDMemTable
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    Left = 256
    Top = 392
  end
end
