inherited FormPlanillaPedido: TFormPlanillaPedido
  Left = 171
  Top = 88
  Caption = 'Planilla de Pedido de Mercaderia'
  ClientHeight = 657
  ClientWidth = 897
  ExplicitWidth = 915
  ExplicitHeight = 698
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 897
    Height = 604
    Align = alClient
    ExplicitWidth = 897
    ExplicitHeight = 604
    object pcPlanilla: TPageControl
      Left = 0
      Top = 0
      Width = 897
      Height = 604
      ActivePage = pagPedido
      Align = alClient
      TabOrder = 0
      object pagDetalle: TTabSheet
        Caption = 'Planilla de Pedido'
        DesignSize = (
          889
          576)
        object Label4: TLabel
          Left = 591
          Top = 487
          Width = 80
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Total Acumulado'
          ExplicitTop = 499
        end
        object Label6: TLabel
          Left = 298
          Top = 456
          Width = 242
          Height = 37
          Caption = 'EN Construccion'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -32
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object dbgPlanilla: TcxGrid
          Left = 0
          Top = 0
          Width = 889
          Height = 450
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 0
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          object dbgPlanillaDBTableView1: TcxGridDBTableView
            Navigator.Buttons.First.Visible = True
            Navigator.Buttons.PriorPage.Visible = True
            Navigator.Buttons.Prior.Visible = True
            Navigator.Buttons.Next.Visible = True
            Navigator.Buttons.NextPage.Visible = True
            Navigator.Buttons.Last.Visible = True
            Navigator.Buttons.Insert.Visible = True
            Navigator.Buttons.Append.Visible = False
            Navigator.Buttons.Delete.Visible = True
            Navigator.Buttons.Edit.Visible = True
            Navigator.Buttons.Post.Visible = True
            Navigator.Buttons.Cancel.Visible = True
            Navigator.Buttons.Refresh.Visible = True
            Navigator.Buttons.SaveBookmark.Visible = True
            Navigator.Buttons.GotoBookmark.Visible = True
            Navigator.Buttons.Filter.Visible = True
            OnCellDblClick = dbgPlanillaDBTableView1CellDblClick
            DataController.DataSource = DSTablaPedido
            OptionsData.Inserting = False
            OptionsView.NoDataToDisplayInfoText = 'Sin Datos'
            OptionsView.GroupByBox = False
            OptionsView.RowSeparatorColor = clGradientInactiveCaption
            Styles.GroupByBox = cxStyle1
            Styles.Header = cxStyle1
            object dbgPlanillaDBTableView1CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 72
            end
            object dbgPlanillaDBTableView1DETALLE: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object dbgPlanillaDBTableView1DET_RUBRO: TcxGridDBColumn
              DataBinding.FieldName = 'DET_RUBRO'
              Visible = False
              FooterAlignmentHorz = taRightJustify
              GroupIndex = 0
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              IsCaptionAssigned = True
            end
            object dbgPlanillaDBTableView1DET_SUBRUB: TcxGridDBColumn
              DataBinding.FieldName = 'DET_SUBRUB'
              Visible = False
              GroupIndex = 1
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              IsCaptionAssigned = True
            end
            object dbgPlanillaDBTableView1EXISTENCIA: TcxGridDBColumn
              DataBinding.FieldName = 'EXISTENCIA'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object dbgPlanillaDBTableView1PEDIDOS: TcxGridDBColumn
              DataBinding.FieldName = 'PEDIDOS'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
            end
            object dbgPlanillaDBTableView1A_INGRESAR: TcxGridDBColumn
              DataBinding.FieldName = 'A_INGRESAR'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object dbgPlanillaDBTableView1A_PEDIR: TcxGridDBColumn
              DataBinding.FieldName = 'A_PEDIR'
              HeaderAlignmentHorz = taCenter
              Width = 77
            end
            object dbgPlanillaDBTableView1COSTO: TcxGridDBColumn
              Caption = 'Costo'
              DataBinding.FieldName = 'COSTO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
          end
          object dbgPlanillaLevel1: TcxGridLevel
            GridView = dbgPlanillaDBTableView1
          end
        end
        object dbcListaPrecios: TJvDBLookupCombo
          Left = 338
          Top = 540
          Width = 331
          Height = 21
          DropDownWidth = 300
          DisplayAllFields = True
          Color = clWhite
          DisplayEmpty = 'Sin Lista Selecionada'
          EmptyValue = '0'
          Anchors = [akLeft, akBottom]
          LookupField = 'ID_LISTA'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSlistaprecios
          TabOrder = 1
        end
        object chCodAlt: TCheckBox
          Left = 11
          Top = 511
          Width = 137
          Height = 17
          Anchors = [akLeft, akBottom]
          Caption = 'Ver Por Cod.Alternativo'
          TabOrder = 2
          OnClick = chCodAltClick
        end
        object dbcListaProveeor: TJvDBLookupCombo
          Left = 338
          Top = 517
          Width = 331
          Height = 21
          DropDownWidth = 300
          DisplayAllFields = True
          Color = clWhite
          DisplayEmpty = 'Sin Lista Selecionada'
          EmptyValue = '0'
          Enabled = False
          Anchors = [akLeft, akBottom]
          LookupField = 'ID'
          LookupDisplay = 'NOMBRE'
          LookupSource = DSListaProveedor
          TabOrder = 3
        end
        object rgTipoLista: TRadioGroup
          Left = 195
          Top = 511
          Width = 140
          Height = 51
          Anchors = [akLeft, akBottom]
          Caption = 'Modo de Valorizaci'#243'n'
          ItemIndex = 1
          Items.Strings = (
            'Lista del Proveedor'
            'Lista de Precios')
          TabOrder = 4
          OnClick = rgTipoListaClick
        end
        object btFiltroRubro: TButton
          Left = 808
          Top = 511
          Width = 77
          Height = 23
          Anchors = [akLeft, akBottom]
          Caption = 'Filtrar Rubros'
          TabOrder = 5
          OnClick = btFiltroRubroClick
        end
        object btGenerarResumen: TBitBtn
          Left = 766
          Top = 536
          Width = 119
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = '&Generar Resumen'
          Glyph.Data = {
            
            FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
          TabOrder = 6
          OnClick = btGenerarResumenClick
        end
        object edTotal2: TJvCalcEdit
          Left = 681
          Top = 484
          Width = 121
          Height = 21
          DisplayFormat = ',0.00'
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 7
          DecimalPlacesAlwaysShown = False
        end
      end
      object pagPedido: TTabSheet
        Caption = 'Resumen de Pedido'
        ImageIndex = 1
        DesignSize = (
          889
          576)
        object Label2: TLabel
          Left = 234
          Top = 531
          Width = 49
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Proveedor'
          ParentShowHint = False
          ShowHint = True
          ExplicitTop = 540
        end
        object Label3: TLabel
          Left = 3
          Top = 477
          Width = 71
          Height = 13
          Anchors = [akLeft, akBottom]
          Caption = 'Observaciones'
          ExplicitTop = 486
        end
        object Label5: TLabel
          Left = 287
          Top = 320
          Width = 242
          Height = 37
          Caption = 'EN Construccion'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -32
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object btHacerOC: TBitBtn
          Left = 714
          Top = 544
          Width = 88
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = 'Hacer O.C'
          Glyph.Data = {
            
            1F7C1F7C1F7C}
          TabOrder = 2
          OnClick = btHacerOCClick
        end
        object ceProvee: TJvComboEdit
          Left = 234
          Top = 550
          Width = 73
          Height = 21
          Anchors = [akLeft, akBottom]
          ButtonWidth = 16
          ClickKey = 16397
          Glyph.Data = {
            4E000000424D4E000000000000003E000000280000000A000000040000000100
            010000000000100000000000000000000000020000000200000000000000FFFF
            FF00FFC000003300000033000000FFC00000}
          TabOrder = 3
          Text = ''
          OnButtonClick = ceProveeButtonClick
          OnKeyDown = ceProveeKeyDown
          OnKeyPress = ceProveeKeyPress
        end
        object edNombre: TEdit
          Left = 313
          Top = 550
          Width = 369
          Height = 21
          Anchors = [akLeft, akBottom]
          Color = clInfoBk
          Enabled = False
          TabOrder = 4
        end
        object edTotal: TJvCalcEdit
          Left = 576
          Top = 493
          Width = 121
          Height = 21
          DisplayFormat = ',0.00'
          Anchors = [akLeft, akBottom]
          ShowButton = False
          TabOrder = 1
          DecimalPlacesAlwaysShown = False
        end
        object edNotas: TEdit
          Left = 4
          Top = 493
          Width = 429
          Height = 21
          Anchors = [akLeft, akBottom]
          TabOrder = 0
        end
        object chConfirmacion: TCheckBox
          Left = 714
          Top = 521
          Width = 135
          Height = 17
          Anchors = [akLeft, akBottom]
          Caption = 'Confirmacion Automatica'
          TabOrder = 5
        end
        object cxGrid1: TcxGrid
          Left = 0
          Top = 49
          Width = 889
          Height = 185
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          TabOrder = 6
          LookAndFeel.Kind = lfOffice11
          LookAndFeel.NativeStyle = False
          object cxGrid1DBTableView1: TcxGridDBTableView
            DataController.DataSource = DSPla_Det
            OptionsView.GroupByBox = False
            object cxGrid1DBTableView1CODIGO: TcxGridDBColumn
              DataBinding.FieldName = 'CODIGO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 69
            end
            object cxGrid1DBTableView1DETALLE_STK: TcxGridDBColumn
              DataBinding.FieldName = 'DETALLE_STK'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1CANTIDAD: TcxGridDBColumn
              DataBinding.FieldName = 'CANTIDAD'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1IMPORTE_UNITARIO: TcxGridDBColumn
              DataBinding.FieldName = 'IMPORTE_UNITARIO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1DESCUENTO: TcxGridDBColumn
              DataBinding.FieldName = 'DESCUENTO'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
            object cxGrid1DBTableView1IMPORTE_TOTAL: TcxGridDBColumn
              DataBinding.FieldName = 'IMPORTE_TOTAL'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
              Width = 92
            end
            object cxGrid1DBTableView1REEMPLAZO_STK: TcxGridDBColumn
              DataBinding.FieldName = 'REEMPLAZO_STK'
              HeaderAlignmentHorz = taCenter
              Options.Editing = False
              Options.Focusing = False
            end
          end
          object cxGrid1Level1: TcxGridLevel
            GridView = cxGrid1DBTableView1
          end
        end
        object pnCabecera2: TPanel
          Left = 0
          Top = 0
          Width = 889
          Height = 49
          Align = alTop
          TabOrder = 7
          object Label1: TLabel
            Left = 3
            Top = 16
            Width = 110
            Height = 13
            Caption = 'Pedidos Solicitados por'
          end
          object dbcSolicitante: TJvDBLookupCombo
            Left = 126
            Top = 13
            Width = 244
            Height = 21
            DisplayEmpty = 'Sin Comporador'
            EmptyValue = '*'
            LookupField = 'CODIGO'
            LookupDisplay = 'NOMBRE'
            LookupSource = DSSolicitante
            TabOrder = 0
          end
          object dbeFecha: TJvDateEdit
            Left = 691
            Top = 13
            Width = 101
            Height = 21
            DefaultToday = True
            ShowNullDate = False
            TabOrder = 1
          end
        end
      end
    end
  end
  inherited ToolBar1: TToolBar
    Top = 604
    Width = 897
    Align = alBottom
    ExplicitTop = 604
    ExplicitWidth = 897
    inherited btConfirma: TBitBtn
      Visible = False
    end
    inherited btNuevo: TBitBtn
      Visible = False
    end
    inherited btCancelar: TBitBtn
      Visible = False
    end
    inherited btModificar: TBitBtn
      Visible = False
    end
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited btBuscar: TBitBtn
      Width = 74
      ExplicitWidth = 74
    end
    inherited Ne: TSpeedButton
      Left = 391
      Visible = False
      ExplicitLeft = 391
    end
    inherited btBorrar: TBitBtn
      Left = 406
      Visible = False
      ExplicitLeft = 406
    end
    inherited btSalir: TBitBtn
      Left = 481
      ExplicitLeft = 481
    end
    object ToolButton1: TToolButton
      Left = 556
      Top = 0
      Width = 44
      Caption = 'ToolButton1'
      Style = tbsSeparator
    end
  end
  inherited Panel1: TPanel
    Top = 634
    Width = 897
    Align = alBottom
    ExplicitTop = 634
    ExplicitWidth = 897
    inherited sbEstado: TStatusBar
      Width = 735
      ExplicitWidth = 735
    end
  end
  inherited ActionList1: TActionList
    Left = 416
    Top = 96
  end
  inherited DSBase: TDataSource
    Left = 792
    Top = 96
  end
  inherited ImageList1: TImageList
    Left = 168
    Top = 152
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    PreventResize = False
    Left = 368
    Top = 384
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 128
    Top = 264
  end
  inherited ComBuscadorBase: TComBuscador
    Left = 392
    Top = 216
  end
  inherited QBrowse2: TFDQuery
    Left = 176
  end
  object QTablaPedido: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from TABLA_PEDIDO_STOCK ( :lista,:modo )')
    Left = 504
    Top = 120
    ParamData = <
      item
        Position = 1
        Name = 'lista'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'modo'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object CDSTablaPedido: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'lista'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'modo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPTablaPedido'
    AfterPost = CDSTablaPedidoAfterPost
    AfterDelete = CDSTablaPedidoAfterPost
    Left = 656
    Top = 120
    object CDSTablaPedidoCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      Origin = 'CODIGO'
      OnGetText = CDSTablaPedidoCODIGOGetText
      Size = 8
    end
    object CDSTablaPedidoDETALLE: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSTablaPedidoCOD_RUBRO: TStringField
      FieldName = 'COD_RUBRO'
      Origin = 'COD_RUBRO'
      Visible = False
      Size = 3
    end
    object CDSTablaPedidoDET_RUBRO: TStringField
      FieldName = 'DET_RUBRO'
      Origin = 'DET_RUBRO'
      Size = 35
    end
    object CDSTablaPedidoCOD_SUBRUB: TStringField
      FieldName = 'COD_SUBRUB'
      Origin = 'COD_SUBRUB'
      Visible = False
      Size = 5
    end
    object CDSTablaPedidoDET_SUBRUB: TStringField
      FieldName = 'DET_SUBRUB'
      Origin = 'DET_SUBRUB'
      Size = 45
    end
    object CDSTablaPedidoEXISTENCIA: TFloatField
      DisplayLabel = 'Stk Actual'
      FieldName = 'EXISTENCIA'
      Origin = 'EXISTENCIA'
      DisplayFormat = '0.00;0.00;-'
      EditFormat = '0.00;0.00;-'
    end
    object CDSTablaPedidoPEDIDOS: TFloatField
      DisplayLabel = 'Pedido x Clientes'
      FieldName = 'PEDIDOS'
      Origin = 'PEDIDOS'
      DisplayFormat = '0.00;0.00;-'
      EditFormat = '0.00;0.00;-'
    end
    object CDSTablaPedidoA_INGRESAR: TFloatField
      DisplayLabel = 'A Recibir'
      FieldName = 'A_INGRESAR'
      Origin = 'A_INGRESAR'
      DisplayFormat = '0.00;0.00;-'
      EditFormat = '0.00;0.00;-'
    end
    object CDSTablaPedidoA_PEDIR: TFloatField
      DisplayLabel = 'A Pedir'
      FieldKind = fkInternalCalc
      FieldName = 'A_PEDIR'
      DisplayFormat = '0.00;0.00;-'
      EditFormat = '0.00;0.00;-'
    end
    object CDSTablaPedidoCOSTO: TFloatField
      FieldName = 'COSTO'
      Origin = 'COSTO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSTablaPedidoCOSTO_FINAL: TFloatField
      FieldName = 'COSTO_FINAL'
      Origin = 'COSTO_FINAL'
    end
    object CDSTablaPedidoPRECIO_VTA: TFloatField
      FieldName = 'PRECIO_VTA'
      Origin = 'PRECIO_VTA'
    end
    object CDSTablaPedidoPRECIO_VTA_FINAL: TFloatField
      FieldName = 'PRECIO_VTA_FINAL'
      Origin = 'PRECIO_VTA_FINAL'
    end
    object CDSTablaPedidoCODIGO_ALTERNATIVO: TStringField
      FieldName = 'CODIGO_ALTERNATIVO'
      Origin = 'CODIGO_ALTERNATIVO'
    end
  end
  object DSPTablaPedido: TDataSetProvider
    DataSet = QTablaPedido
    Options = []
    Left = 584
    Top = 120
  end
  object DSTablaPedido: TDataSource
    DataSet = CDSTablaPedido
    Left = 744
    Top = 128
  end
  object CDSPla_Det: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPPla_det'
    AfterPost = CDSPla_DetAfterPost
    OnNewRecord = CDSPla_DetNewRecord
    Left = 644
    Top = 280
    object CDSPla_DetCODIGO: TStringField
      DisplayLabel = 'Codigo'
      FieldName = 'CODIGO'
      OnGetText = CDSPla_DetCODIGOGetText
      Size = 8
    end
    object CDSPla_DetCANTIDAD: TFloatField
      DisplayLabel = 'Cantidad'
      FieldName = 'CANTIDAD'
      OnSetText = CDSPla_DetCANTIDADSetText
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSPla_DetIMPORTE_UNITARIO: TFloatField
      DisplayLabel = 'Imp.Unitario'
      FieldName = 'IMPORTE_UNITARIO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSPla_DetIMPORTE_TOTAL: TFloatField
      DisplayLabel = 'Importe Total'
      FieldName = 'IMPORTE_TOTAL'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSPla_DetDETALLE_STK: TStringField
      DisplayLabel = 'Detalle'
      FieldName = 'DETALLE_STK'
      ProviderFlags = []
      Size = 45
    end
    object CDSPla_DetDESCUENTO: TFloatField
      DisplayLabel = 'Descuento'
      FieldName = 'DESCUENTO'
      OnSetText = CDSPla_DetDESCUENTOSetText
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object CDSPla_DetREEMPLAZO_STK: TStringField
      DisplayLabel = 'Cod.Alternativo'
      FieldName = 'REEMPLAZO_STK'
    end
  end
  object DSPla_Det: TDataSource
    DataSet = CDSPla_Det
    Left = 712
    Top = 280
  end
  object QListaPrecios: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select l.id as id_lista,l.nombre from listaprecioespecialcab l')
    Left = 184
    Top = 136
    object intgrfldQListaPrecID_LISTA: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_LISTA'
    end
    object QListaPreciosNOMBRE: TStringField
      DisplayWidth = 50
      FieldName = 'NOMBRE'
      Size = 50
    end
  end
  object DSPListaPrecios: TDataSetProvider
    DataSet = QListaPrecios
    Options = []
    Left = 136
    Top = 112
  end
  object CDSListaPrecios: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPListaPrecios'
    Left = 64
    Top = 96
    object CDSListaPreciosID_LISTA: TIntegerField
      FieldName = 'ID_LISTA'
    end
    object CDSListaPreciosNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 50
    end
  end
  object DSlistaprecios: TDataSource
    DataSet = CDSListaPrecios
    Left = 264
    Top = 136
  end
  object QListasProveedor: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select l.nombre,l.codigo_provee,l.fecha,l.id from LISTA_PRECIOS_' +
        'PROV_CABECERA l'
      'order by l.nombre')
    Left = 64
    Top = 376
  end
  object DSPListaProveedor: TDataSetProvider
    DataSet = QListasProveedor
    Options = []
    Left = 136
    Top = 376
  end
  object CDSListaProveedor: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPBuscadorLista'
    Left = 216
    Top = 352
    object CDSListaProveedorCODIGO_PROVEE: TStringField
      DisplayLabel = 'Codigo'
      DisplayWidth = 6
      FieldName = 'CODIGO_PROVEE'
      Size = 6
    end
    object CDSListaProveedorNOMBRE: TStringField
      DisplayLabel = 'Nombre'
      DisplayWidth = 45
      FieldName = 'NOMBRE'
      Size = 45
    end
    object CDSListaProveedorFECHA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      DisplayWidth = 34
      FieldName = 'FECHA'
    end
    object CDSListaProveedorID: TIntegerField
      DisplayLabel = 'Id'
      DisplayWidth = 10
      FieldName = 'ID'
      Required = True
    end
  end
  object DSListaProveedor: TDataSource
    DataSet = CDSListaProveedor
    Left = 312
    Top = 360
  end
  object DSPSolicitante: TDataSetProvider
    DataSet = DMBuscadores.QBuscaVendedores
    Options = []
    Left = 584
    Top = 344
  end
  object CDSSolicitante: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSolicitante'
    Left = 720
    Top = 344
    object CDSSolicitanteCODIGO: TStringField
      FieldName = 'CODIGO'
      Required = True
      Size = 6
    end
    object CDSSolicitanteNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Size = 50
    end
  end
  object DSSolicitante: TDataSource
    DataSet = CDSSolicitante
    Left = 529
    Top = 342
  end
  object DSPProvee: TDataSetProvider
    DataSet = DMMain_FD.QProveedores
    UpdateMode = upWhereKeyOnly
    Left = 552
    Top = 560
  end
  object CDSProveedor: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'CODIGO'
        Attributes = [faRequired]
        DataType = ftString
        Size = 6
      end
      item
        Name = 'NOMBRE'
        Attributes = [faRequired]
        DataType = ftString
        Size = 35
      end
      item
        Name = 'RAZON_SOCIAL'
        DataType = ftString
        Size = 35
      end
      item
        Name = 'N_DE_CUENTA'
        DataType = ftString
        Size = 14
      end
      item
        Name = 'ID_COD_POSTAL'
        DataType = ftInteger
      end
      item
        Name = 'COD_POSTAL'
        DataType = ftString
        Size = 8
      end
      item
        Name = 'LOCALIDAD'
        DataType = ftString
        Size = 15
      end
      item
        Name = 'DIRECCION'
        DataType = ftString
        Size = 35
      end
      item
        Name = 'TELEFONO_1'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'TELEFONO_2'
        DataType = ftString
        Size = 10
      end
      item
        Name = 'CONDICION_IVA'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'N_DE_CUIT'
        DataType = ftString
        Size = 13
      end
      item
        Name = 'ACTIVO'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'RETIENE_IVA'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'RETIENE_IB'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'RETIENE_GANANCIAS'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'IVA_SERVICIOS'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'PRECIOS_CON_IVA'
        DataType = ftString
        Size = 1
      end
      item
        Name = 'OBSERVACIONES'
        DataType = ftMemo
        Size = 1
      end
      item
        Name = 'NOMBRE_REPRESENTANTE'
        DataType = ftString
        Size = 35
      end
      item
        Name = 'DIRECCION_REPRESENTANTE'
        DataType = ftString
        Size = 35
      end
      item
        Name = 'TELEFONO_REPRESENTANTE'
        DataType = ftString
        Size = 25
      end
      item
        Name = 'TIPO_PROVEEDOR'
        Attributes = [faRequired]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'FECHAULTIMACOMPRA'
        DataType = ftDateTime
      end
      item
        Name = 'FECHAALTA'
        DataType = ftDateTime
      end
      item
        Name = 'SALDO_INICIAL'
        DataType = ftFMTBcd
        Precision = 15
        Size = 3
      end
      item
        Name = 'ID_PERC_IVA'
        DataType = ftInteger
      end
      item
        Name = 'ID_PERC_IBB'
        DataType = ftInteger
      end
      item
        Name = 'ID_PERC_IBB_2'
        DataType = ftInteger
      end
      item
        Name = 'RUBRO'
        DataType = ftInteger
      end
      item
        Name = 'ID_FACTURAPORDEFECTO'
        DataType = ftInteger
      end
      item
        Name = 'EXENTO_GANACIA'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'CODIGO_REGIMEN_RET_GANACIA'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'EXENTO_RETENCION_IB'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'ID_TASA_RETENCION_IB'
        Attributes = [faRequired]
        DataType = ftInteger
      end
      item
        Name = 'NRO_IIBB'
        DataType = ftString
        Size = 15
      end
      item
        Name = 'CORREO'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'CONV_MULTI'
        Attributes = [faRequired, faFixed]
        DataType = ftString
        Size = 1
      end
      item
        Name = 'MONEDA_PRECIOS'
        DataType = ftInteger
      end
      item
        Name = 'TELEDISCADO'
        DataType = ftString
        Size = 6
      end
      item
        Name = 'DESPERIVA'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DESPERIB'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'DESPERIB2'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'MUESTRADETALLERETIIBB'
        DataType = ftString
        Size = 25
      end
      item
        Name = 'MUESTRATASARETIIBB'
        DataType = ftFMTBcd
        Precision = 15
        Size = 3
      end
      item
        Name = 'MUESTRAMINIMPONIRETIIBB'
        DataType = ftFMTBcd
        Precision = 15
        Size = 3
      end>
    IndexDefs = <>
    Params = <
      item
        DataType = ftString
        Name = 'Codigo'
        ParamType = ptUnknown
      end>
    ProviderName = 'DSPProvee'
    StoreDefs = True
    Left = 648
    Top = 560
    object CDSProveedorCODIGO: TStringField
      FieldName = 'CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 6
    end
    object CDSProveedorNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Required = True
      Size = 35
    end
    object CDSProveedorRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      Size = 35
    end
    object CDSProveedorN_DE_CUENTA: TStringField
      FieldName = 'N_DE_CUENTA'
      Size = 14
    end
    object CDSProveedorID_COD_POSTAL: TIntegerField
      FieldName = 'ID_COD_POSTAL'
    end
  end
  object QPla_Det: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select d.*,s.detalle_stk,s.reemplazo_stk from pla_pedido_det d'
      'left join stock s on s.codigo_stk=d.codigo')
    Left = 508
    Top = 280
  end
  object DSPPla_det: TDataSetProvider
    DataSet = QPla_Det
    Options = []
    Left = 588
    Top = 280
  end
  object cxStyleRepository1: TcxStyleRepository
    Left = 396
    Top = 168
    PixelsPerInch = 96
    object cxStyle1: TcxStyle
    end
    object cxStyle2: TcxStyle
    end
    object cxStyle3: TcxStyle
    end
    object cxStyle4: TcxStyle
    end
    object cxGridBandedTableViewStyleSheet1: TcxGridBandedTableViewStyleSheet
      Styles.Background = cxStyle2
      Styles.Footer = cxStyle1
      Styles.BandBackground = cxStyle3
      Styles.BandHeader = cxStyle4
      BuiltIn = True
    end
  end
end
