inherited FormControlImpuestos: TFormControlImpuestos
  Left = 251
  Top = 129
  Caption = 'Controla C'#225'lculo de Facturas de Venta'
  ClientHeight = 727
  ClientWidth = 942
  StyleElements = [seFont, seClient, seBorder]
  ExplicitWidth = 960
  ExplicitHeight = 768
  TextHeight = 13
  inherited pnPrincipal: TPanel
    Width = 942
    Height = 491
    StyleElements = [seFont, seClient, seBorder]
    ExplicitWidth = 942
    ExplicitHeight = 491
    object Label1: TLabel
      Left = 12
      Top = 364
      Width = 86
      Height = 13
      Caption = 'Totales en Detalle'
    end
    object RxLabel1: TJvLabel
      Left = 7
      Top = 3
      Width = 33
      Height = 13
      Caption = 'Desde'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel2: TJvLabel
      Left = 119
      Top = 3
      Width = 30
      Height = 13
      Caption = 'Hasta'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel3: TJvLabel
      Left = 8
      Top = 48
      Width = 113
      Height = 13
      Caption = 'Datos del Comprobante'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel4: TJvLabel
      Left = 8
      Top = 384
      Width = 133
      Height = 13
      Caption = 'Impuestos del Comprobante'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel5: TJvLabel
      Left = 256
      Top = 3
      Width = 43
      Height = 13
      Caption = 'Sucrusal'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object RxLabel7: TJvLabel
      Left = 600
      Top = 48
      Width = 98
      Height = 13
      Caption = 'Dif. Encab-Neto Imp'
      ShadowSize = 1
      ShadowPos = spLeftTop
      Transparent = True
    end
    object Label2: TLabel
      Left = 480
      Top = 7
      Width = 32
      Height = 13
      Caption = 'Cliente'
    end
    object JvDBStatusLabel1: TJvDBStatusLabel
      Left = 471
      Top = 48
      Width = 96
      Height = 13
      DataSource = DSBase
      Style = lsRecordNo
      Transparent = True
    end
    object JvDBStatusLabel2: TJvDBStatusLabel
      Left = 649
      Top = 360
      Width = 96
      Height = 13
      DataSource = DSFcDetalle
      Style = lsRecordNo
      Transparent = True
    end
    object Label3: TLabel
      Left = 823
      Top = 2
      Width = 46
      Height = 13
      Caption = 'Tipo Cpbt'
    end
    object Label4: TLabel
      Left = 875
      Top = 2
      Width = 51
      Height = 13
      Caption = 'Clase Cpbt'
    end
    object SpeedButton1: TSpeedButton
      Left = 560
      Top = 463
      Width = 20
      Height = 22
      Caption = '<<'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      OnClick = SpeedButton1Click
    end
    object UpDown1: TUpDown
      Left = 226
      Top = 19
      Width = 24
      Height = 20
      Hint = 'Cambia el Per'#237'odo'
      Min = -32768
      Max = 32767
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnClick = UpDown1Click
    end
    object cbSuc: TJvDBLookupCombo
      Left = 256
      Top = 21
      Width = 209
      Height = 21
      DisplayEmpty = 'Todas'
      EmptyValue = '-1'
      LookupField = 'CODIGO'
      LookupDisplay = 'DETALLE'
      LookupSource = DSSucrusal
      TabOrder = 6
      OnClick = cbSucClick
    end
    object Desde: TJvDateEdit
      Left = 7
      Top = 19
      Width = 105
      Height = 21
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 0
    end
    object Hasta: TJvDateEdit
      Left = 119
      Top = 19
      Width = 105
      Height = 21
      Glyph.Data = {
        
        80008080800080808000808080008080800080808000FF00FF00}
      ImageKind = ikCustom
      NumGlyphs = 2
      PopupColor = clBtnFace
      ShowNullDate = False
      TabOrder = 1
    end
    object edTotaDetalle: TJvCalcEdit
      Left = 560
      Top = 360
      Width = 81
      Height = 21
      Margins.Left = 1
      Margins.Top = 1
      AutoSize = False
      DisplayFormat = '0.00;-0.00'
      ShowButton = False
      TabOrder = 3
      DecimalPlacesAlwaysShown = False
    end
    object edTotalGravado: TJvCalcEdit
      Left = 440
      Top = 360
      Width = 81
      Height = 21
      Margins.Left = 1
      Margins.Top = 1
      AutoSize = False
      DisplayFormat = '0.00;-0.00'
      ShowButton = False
      TabOrder = 9
      DecimalPlacesAlwaysShown = False
    end
    object edTotalExento: TJvCalcEdit
      Left = 352
      Top = 360
      Width = 81
      Height = 21
      Margins.Left = 1
      Margins.Top = 1
      AutoSize = False
      DisplayFormat = '0.00;-0.00'
      ShowButton = False
      TabOrder = 10
      DecimalPlacesAlwaysShown = False
    end
    object dbgCabeceras: TJvDBGrid
      Left = 8
      Top = 64
      Width = 737
      Height = 135
      DataSource = DSBase
      DrawingStyle = gdsGradient
      PopupMenu = PopupMenu1
      TabOrder = 11
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDblClick = BuscarCpbteExecute
      OnTitleBtnClick = dbgCabecerasTitleBtnClick
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'FECHAVTA'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'SUCURSAL'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 33
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ID_FC'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 47
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TIPOCPBTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 38
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CLASECPBTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 29
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NROCPBTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NETOEXEN2'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 69
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NETOGRAV2'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 70
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DSTOIMPORTE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 83
          Visible = True
        end
        item
          Color = clMoneyGreen
          Expanded = False
          FieldName = 'TOTAL'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          PopupMenu = PopupMenu1
          Title.Alignment = taCenter
          Title.Color = clNavy
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clMaroon
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 72
          Visible = True
        end
        item
          Color = clSilver
          Expanded = False
          FieldName = 'TotalCalculado'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 85
          Visible = True
        end>
    end
    object dbgFcDetalle: TJvDBGrid
      Left = 8
      Top = 205
      Width = 737
      Height = 150
      DataSource = DSFcDetalle
      DrawingStyle = gdsGradient
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDrawColumnCell = dbgFcDetalleDrawColumnCell
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGOARTICULO'
          Title.Alignment = taCenter
          Title.Caption = 'Codigo'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DETALLE'
          Title.Alignment = taCenter
          Title.Caption = 'Detalle'
          Width = 140
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'CANTIDAD'
          Title.Alignment = taCenter
          Title.Caption = 'Can'
          Width = 61
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'MODO_GRAVAMEN'
          Title.Alignment = taCenter
          Title.Caption = 'M.G'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESGLOZAIVA'
          Title.Alignment = taCenter
          Title.Caption = 'Desg.'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IVA_TASA'
          Title.Alignment = taCenter
          Title.Caption = 'Tasa'
          Width = 43
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IVA_SOBRETASA'
          Title.Alignment = taCenter
          Title.Caption = 'Tasa'
          Width = 40
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNITARIO_EXENTO'
          Title.Alignment = taCenter
          Title.Caption = 'Exent'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNITARIO_GRAVADO'
          Title.Alignment = taCenter
          Title.Caption = 'Grav'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UNITARIO_TOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Sub Tot.'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TOTAL'
          Title.Alignment = taCenter
          Title.Caption = 'Total'
          Width = 91
          Visible = True
        end>
    end
    object dbgImpuesto: TJvDBGrid
      Left = 9
      Top = 405
      Width = 545
      Height = 80
      DataSource = DSFcImpuesto
      DrawingStyle = gdsGradient
      TabOrder = 8
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDrawColumnCell = dbgImpuestoDrawColumnCell
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO_GRAVAMEN'
          ReadOnly = True
          Title.Alignment = taCenter
          Width = 59
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DETALLE'
          ReadOnly = True
          Title.Alignment = taCenter
          Width = 131
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'TASA'
          ReadOnly = True
          Title.Alignment = taCenter
          Width = 51
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'NETO'
          Title.Alignment = taCenter
          Width = 75
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IMPORTE'
          Title.Alignment = taCenter
          Width = 69
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ImpuestoCalculado'
          ReadOnly = True
          Title.Alignment = taCenter
          Width = 118
          Visible = True
        end>
    end
    object ceCliente: TJvComboEdit
      Left = 481
      Top = 21
      Width = 73
      Height = 21
      AlwaysEnableButton = True
      ButtonWidth = 16
      ClickKey = 16397
      Glyph.Data = {
        4E000000424D4E000000000000003E000000280000000A000000040000000100
        010000000000100000000000000000000000020000000200000000000000FFFF
        FF00FFC000003300000033000000FFC00000}
      MaxLength = 6
      TabOrder = 12
      Text = ''
      OnButtonClick = BuscarClienteExecute
      OnClick = BuscarClienteExecute
    end
    object edNombreCliente: TEdit
      Left = 560
      Top = 21
      Width = 257
      Height = 21
      Color = clInfoBk
      Enabled = False
      TabOrder = 13
    end
    object chbClientes: TCheckBox
      Left = 532
      Top = 1
      Width = 166
      Height = 17
      Caption = 'Filtrar por Este Cliente'
      TabOrder = 14
      OnClick = chbClientesClick
    end
    object chbDif: TCheckBox
      Left = 777
      Top = 45
      Width = 81
      Height = 17
      Caption = 'Filtrar C/Dif'
      TabOrder = 15
      OnClick = chbDifClick
    end
    object btVerifica: TButton
      Left = 768
      Top = 361
      Width = 75
      Height = 25
      Caption = 'Verificar'
      TabOrder = 16
      OnClick = btVerificaClick
    end
    object dbgDiferenciaNeto: TJvDBGrid
      Left = 768
      Top = 64
      Width = 105
      Height = 133
      Color = 16776176
      DataSource = DSBase
      DrawingStyle = gdsGradient
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 4
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      TitleButtons = True
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'DiferenciaNeto'
          Visible = True
        end>
    end
    object dbgDifDetalle: TJvDBGrid
      Left = 768
      Top = 203
      Width = 105
      Height = 152
      Color = 16776176
      DataSource = DSBase
      DrawingStyle = gdsGradient
      Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 7
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      TitleButtons = True
      SelectColumnsDialogStrings.Caption = 'Select columns'
      SelectColumnsDialogStrings.OK = '&OK'
      SelectColumnsDialogStrings.NoSelectionWarning = 'At least one column must be visible!'
      EditControls = <>
      RowsHeight = 17
      TitleRowHeight = 17
      Columns = <
        item
          Expanded = False
          FieldName = 'Dif_enc_det'
          Width = 76
          Visible = True
        end>
    end
    object btRevision: TButton
      Left = 606
      Top = 458
      Width = 116
      Height = 25
      Caption = 'Revision Impuestos'
      TabOrder = 17
      OnClick = btRevisionClick
    end
    object cbCpbte: TComboBox
      Left = 823
      Top = 21
      Width = 39
      Height = 21
      TabOrder = 18
      Text = '**'
      OnChange = cbCpbteChange
      Items.Strings = (
        '**'
        'FC'
        'FO'
        'NC'
        'ND'
        'TK')
    end
    object ch2decimales: TCheckBox
      Left = 752
      Top = 440
      Width = 106
      Height = 17
      Caption = 'Calculo en 2 Deci.'
      TabOrder = 19
    end
    object edImpuestos: TJvCalcEdit
      Left = 560
      Top = 405
      Width = 81
      Height = 21
      Margins.Left = 1
      Margins.Top = 1
      AutoSize = False
      DisplayFormat = '0.00;-0.00'
      ShowButton = False
      TabOrder = 20
      DecimalPlacesAlwaysShown = False
    end
    object chCambio: TCheckBox
      Left = 424
      Top = 387
      Width = 106
      Height = 17
      Caption = 'Aplicar cambio'
      TabOrder = 21
    end
  end
  inherited ToolBar1: TToolBar
    Top = 491
    Width = 942
    ExplicitTop = 491
    ExplicitWidth = 942
    inherited Pr: TSpeedButton
      Visible = False
    end
    inherited Ne: TSpeedButton
      Visible = False
    end
  end
  inherited Panel1: TPanel
    Top = 521
    Width = 942
    StyleElements = [seFont, seClient, seBorder]
    ExplicitTop = 521
    ExplicitWidth = 942
    inherited sbEstado: TStatusBar
      Width = 853
      SimplePanel = True
      ExplicitWidth = 853
    end
  end
  object JvDBLookupCombo1: TJvDBLookupCombo [3]
    Left = 868
    Top = 21
    Width = 53
    Height = 21
    LookupField = 'CLASE_COMPROB'
    LookupDisplay = 'CLASE_COMPROB'
    LookupSource = DSTipoComp
    TabOrder = 3
    OnClick = cbCpbteChange
  end
  inherited ActionList1: TActionList
    Left = 464
    Top = 56
    inherited Agregar: TAction
      Visible = False
    end
    inherited Borrar: TAction
      Visible = False
    end
    inherited Modificar: TAction
      Visible = False
    end
    inherited Confirma: TAction
      Visible = False
    end
    inherited Cancelar: TAction
      Visible = False
    end
    object BuscarCpbte: TAction
      Caption = 'BuscarCpbte'
      OnExecute = BuscarCpbteExecute
    end
    object BuscarCliente: TAction
      Caption = 'BuscarCliente'
      OnExecute = BuscarClienteExecute
    end
    object ActualizaTotal: TAction
      Caption = 'ActualizaTotal'
      OnExecute = ActualizaTotalExecute
    end
  end
  inherited DSBase: TDataSource
    AutoEdit = True
    DataSet = CDSFcCab
    Left = 312
    Top = 112
  end
  inherited ImageList1: TImageList
    Left = 256
    Top = 80
    Bitmap = {
      
      000000000000}
  end
  inherited JvFormStorage: TJvFormStorage
    Left = 688
    Top = 408
  end
  inherited QUltimoCodigo: TSQLQuery
    Left = 248
    Top = 504
  end
  object DSPFcCab: TDataSetProvider
    DataSet = QFcCab
    Options = []
    Left = 120
    Top = 112
  end
  object DSPFcDetalle: TDataSetProvider
    DataSet = QFcDetalle
    Options = []
    Left = 304
    Top = 232
  end
  object DSFcDetalle: TDataSource
    DataSet = CDSFcDetalle
    Left = 488
    Top = 232
  end
  object DSPFcImpuesto: TDataSetProvider
    DataSet = QFcImpuesto
    Left = 168
    Top = 376
  end
  object DSFcImpuesto: TDataSource
    DataSet = CDSFcImpuesto
    Left = 344
    Top = 384
  end
  object DSSucrusal: TDataSource
    DataSet = CDSSucursal
    Left = 602
    Top = 515
  end
  object QFcCab: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select fc.id_fc,fc.tipocpbte,fc.codigo,fc.razonsocial,fc.sucursa' +
        'l,'
      'fc.clasecpbte,fc.nrocpbte, fc.netoexen2,fc.netograv2,'
      
        'fc.dstoimporte,fc.total, fc.fechavta,(select sum(imp.importe) fr' +
        'om FcVtaImpuesto imp'
      'where imp.id_FcVtaCab=fc.id_fc ) as impuestos from FcVtaCab fc'
      ''
      
        'where fc.fechavta>=:Desde and fc.fechavta<=:hasta and fc.anulado' +
        '<>'#39'S'#39' and'
      '(fc.sucursal= :suc or :suc = -1)'
      'order by fc.fechavta')
    Left = 64
    Top = 112
    ParamData = <
      item
        Position = 1
        Name = 'Desde'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 2
        Name = 'hasta'
        DataType = ftDate
        ParamType = ptInput
      end
      item
        Position = 3
        Name = 'suc'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Position = 4
        Name = 'suc'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QFcCabID_FC: TIntegerField
      FieldName = 'ID_FC'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QFcCabTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 2
    end
    object QFcCabCODIGO: TStringField
      FieldName = 'CODIGO'
      ProviderFlags = []
      Required = True
      Size = 6
    end
    object QFcCabRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      ProviderFlags = []
      Size = 35
    end
    object QFcCabSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
      ProviderFlags = []
      Required = True
    end
    object QFcCabCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      ProviderFlags = []
      Required = True
      Size = 2
    end
    object QFcCabNROCPBTE: TStringField
      FieldName = 'NROCPBTE'
      ProviderFlags = []
      Size = 13
    end
    object QFcCabNETOEXEN2: TFloatField
      FieldName = 'NETOEXEN2'
      ProviderFlags = []
      Required = True
    end
    object QFcCabNETOGRAV2: TFloatField
      FieldName = 'NETOGRAV2'
      ProviderFlags = []
      Required = True
    end
    object QFcCabDSTOIMPORTE: TFloatField
      FieldName = 'DSTOIMPORTE'
      ProviderFlags = []
      Required = True
    end
    object QFcCabTOTAL: TFloatField
      FieldName = 'TOTAL'
      Required = True
    end
    object QFcCabFECHAVTA: TSQLTimeStampField
      FieldName = 'FECHAVTA'
      ProviderFlags = []
      Required = True
    end
    object QFcCabIMPUESTOS: TFloatField
      FieldName = 'IMPUESTOS'
      ProviderFlags = []
    end
  end
  object CDSFcCab: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftDate
        Name = 'Desde'
        ParamType = ptInput
      end
      item
        DataType = ftDate
        Name = 'hasta'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'suc'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'suc'
        ParamType = ptInput
      end>
    ProviderName = 'DSPFcCab'
    AfterScroll = CDSFcCabAfterScroll
    OnCalcFields = CDSFcCabCalcFields
    Left = 192
    Top = 112
    object CDSFcCabID_FC: TIntegerField
      DisplayLabel = 'Id'
      DisplayWidth = 10
      FieldName = 'ID_FC'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object CDSFcCabTIPOCPBTE: TStringField
      DisplayLabel = 'Tp'
      DisplayWidth = 2
      FieldName = 'TIPOCPBTE'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Size = 2
    end
    object CDSFcCabCLASECPBTE: TStringField
      DisplayLabel = 'Cl'
      DisplayWidth = 2
      FieldName = 'CLASECPBTE'
      ProviderFlags = []
      Size = 2
    end
    object CDSFcCabNROCPBTE: TStringField
      DisplayLabel = 'Nro.Cpbte'
      DisplayWidth = 13
      FieldName = 'NROCPBTE'
      ProviderFlags = []
      Size = 13
    end
    object CDSFcCabTotalCalculado: TFloatField
      DisplayLabel = 'Re-Calculado'
      DisplayWidth = 10
      FieldKind = fkInternalCalc
      FieldName = 'TotalCalculado'
      ProviderFlags = []
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object CDSFcCabDiferenciaNeto: TCurrencyField
      DisplayWidth = 10
      FieldKind = fkInternalCalc
      FieldName = 'DiferenciaNeto'
      ProviderFlags = []
      DisplayFormat = '0.00'
      EditFormat = '0.00'
      currency = False
    end
    object CDSFcCabDif_enc_det: TFloatField
      DisplayWidth = 10
      FieldKind = fkInternalCalc
      FieldName = 'Dif_enc_det'
      ProviderFlags = []
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
    object CDSFcCabSUCURSAL: TIntegerField
      DisplayLabel = 'Suc'
      DisplayWidth = 10
      FieldName = 'SUCURSAL'
      ProviderFlags = []
    end
    object CDSFcCabNETOEXEN2: TFloatField
      DisplayLabel = 'N.Exento'
      FieldName = 'NETOEXEN2'
      ProviderFlags = []
      Required = True
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcCabNETOGRAV2: TFloatField
      DisplayLabel = 'N.Grav'
      FieldName = 'NETOGRAV2'
      ProviderFlags = []
      Required = True
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcCabDSTOIMPORTE: TFloatField
      DisplayLabel = 'Dsto'
      FieldName = 'DSTOIMPORTE'
      ProviderFlags = []
      Required = True
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcCabTOTAL: TFloatField
      DisplayLabel = 'Total'
      FieldName = 'TOTAL'
      Required = True
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcCabFECHAVTA: TSQLTimeStampField
      DisplayLabel = 'Fecha'
      FieldName = 'FECHAVTA'
      ProviderFlags = []
      Required = True
    end
    object CDSFcCabCODIGO: TStringField
      FieldName = 'CODIGO'
      ProviderFlags = []
      Required = True
      Size = 6
    end
    object CDSFcCabRAZONSOCIAL: TStringField
      FieldName = 'RAZONSOCIAL'
      ProviderFlags = []
      Size = 35
    end
    object CDSFcCabIMPUESTOS: TFloatField
      FieldName = 'IMPUESTOS'
      ProviderFlags = []
    end
  end
  object QFcDetalle: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select id,tipocpbte,clasecpbte,nrocpbte,codigoarticulo,detalle,c' +
        'antidad,unitario_total,'
      
        'unitario_gravado,unitario_exento,desglozaiva,modo_gravamen,iva_t' +
        'asa,iva_sobretasa,'
      'descuento,total from FcVtaDet'
      'where id_cabFac=:id')
    Left = 240
    Top = 232
    ParamData = <
      item
        Position = 1
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object CDSFcDetalle: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPFcDetalle'
    Left = 384
    Top = 232
    object CDSFcDetalleID: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'ID'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object CDSFcDetalleTIPOCPBTE: TStringField
      DisplayWidth = 2
      FieldName = 'TIPOCPBTE'
      Origin = 'TIPOCPBTE'
      Size = 2
    end
    object CDSFcDetalleCLASECPBTE: TStringField
      DisplayWidth = 2
      FieldName = 'CLASECPBTE'
      Origin = 'CLASECPBTE'
      Size = 2
    end
    object CDSFcDetalleNROCPBTE: TStringField
      DisplayWidth = 13
      FieldName = 'NROCPBTE'
      Origin = 'NROCPBTE'
      Size = 13
    end
    object CDSFcDetalleCODIGOARTICULO: TStringField
      DisplayWidth = 8
      FieldName = 'CODIGOARTICULO'
      Origin = 'CODIGOARTICULO'
      Size = 8
    end
    object CDSFcDetalleDETALLE: TStringField
      DisplayWidth = 45
      FieldName = 'DETALLE'
      Origin = 'DETALLE'
      Size = 45
    end
    object CDSFcDetalleDESGLOZAIVA: TStringField
      DisplayWidth = 1
      FieldName = 'DESGLOZAIVA'
      Origin = 'DESGLOZAIVA'
      Size = 1
    end
    object CDSFcDetalleMODO_GRAVAMEN: TStringField
      DisplayWidth = 1
      FieldName = 'MODO_GRAVAMEN'
      Origin = 'MODO_GRAVAMEN'
      Size = 1
    end
    object CDSFcDetalleUNITARIO_TOTAL: TFloatField
      FieldName = 'UNITARIO_TOTAL'
      Origin = 'UNITARIO_TOTAL'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleUNITARIO_GRAVADO: TFloatField
      FieldName = 'UNITARIO_GRAVADO'
      Origin = 'UNITARIO_GRAVADO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleUNITARIO_EXENTO: TFloatField
      FieldName = 'UNITARIO_EXENTO'
      Origin = 'UNITARIO_EXENTO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleCANTIDAD: TFloatField
      FieldName = 'CANTIDAD'
      Origin = 'CANTIDAD'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleIVA_TASA: TFloatField
      FieldName = 'IVA_TASA'
      Origin = 'IVA_TASA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleIVA_SOBRETASA: TFloatField
      FieldName = 'IVA_SOBRETASA'
      Origin = 'IVA_SOBRETASA'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleDESCUENTO: TFloatField
      FieldName = 'DESCUENTO'
      Origin = 'DESCUENTO'
      Required = True
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcDetalleTOTAL: TFloatField
      FieldName = 'TOTAL'
      Origin = 'TOTAL'
      DisplayFormat = '0.00'
      EditFormat = '0.00'
    end
  end
  object QFcImpuesto: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from FcVtaImpuesto'
      'where id_FcVtaCab=:id')
    Left = 96
    Top = 376
    ParamData = <
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QFcImpuestoID_IMPUESTO: TIntegerField
      FieldName = 'ID_IMPUESTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QFcImpuestoID_FCVTACAB: TIntegerField
      FieldName = 'ID_FCVTACAB'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QFcImpuestoCODIGO_GRAVAMEN: TIntegerField
      FieldName = 'CODIGO_GRAVAMEN'
      ProviderFlags = []
    end
    object QFcImpuestoDETALLE: TStringField
      FieldName = 'DETALLE'
      ProviderFlags = []
    end
    object QFcImpuestoNETO: TFloatField
      FieldName = 'NETO'
    end
    object QFcImpuestoTASA: TFloatField
      FieldName = 'TASA'
      ProviderFlags = []
    end
    object QFcImpuestoIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
  end
  object CDSFcImpuesto: TClientDataSet
    Aggregates = <>
    AggregatesActive = True
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPFcImpuesto'
    AfterOpen = CDSFcImpuestoAfterOpen
    AfterScroll = CDSFcImpuestoAfterScroll
    OnCalcFields = CDSFcImpuestoCalcFields
    Left = 256
    Top = 376
    object CDSFcImpuestoID_IMPUESTO: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_IMPUESTO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object CDSFcImpuestoID_FCVTACAB: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_FCVTACAB'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object CDSFcImpuestoCODIGO_GRAVAMEN: TIntegerField
      DisplayLabel = 'Codigo'
      DisplayWidth = 10
      FieldName = 'CODIGO_GRAVAMEN'
      ProviderFlags = []
    end
    object CDSFcImpuestoDETALLE: TStringField
      DisplayLabel = 'Detalle'
      DisplayWidth = 20
      FieldName = 'DETALLE'
      ProviderFlags = []
    end
    object CDSFcImpuestoImpuestoCalculado: TFloatField
      DisplayLabel = 'Impuesto. Re calculado'
      DisplayWidth = 10
      FieldKind = fkCalculated
      FieldName = 'ImpuestoCalculado'
      ProviderFlags = []
      DisplayFormat = '0.00'
      EditFormat = '0.00'
      Calculated = True
    end
    object CDSFcImpuestoNETO: TFloatField
      DisplayLabel = 'Neto'
      FieldName = 'NETO'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcImpuestoTASA: TFloatField
      DisplayLabel = '%'
      FieldName = 'TASA'
      ProviderFlags = []
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
    object CDSFcImpuestoIMPORTE: TFloatField
      DisplayLabel = 'Importe'
      FieldName = 'IMPORTE'
      DisplayFormat = ',0.00'
      EditFormat = ',0.00'
    end
  end
  object CDSSucursal: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DSPSucursal'
    Left = 528
    Top = 512
    object CDSSucursalCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object CDSSucursalDETALLE: TStringField
      FieldName = 'DETALLE'
      Size = 35
    end
  end
  object DSPSucursal: TDataSetProvider
    DataSet = DMBuscadores.QBuscaSucursal
    Left = 448
    Top = 512
  end
  object QClientes: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'SELECT C.*  FROM Clientes C'
      'where (c.codigo=:codigo)')
    Left = 56
    Top = 176
    ParamData = <
      item
        Name = 'codigo'
        DataType = ftString
        ParamType = ptInput
      end>
    object QClientesCODIGO: TStringField
      FieldName = 'CODIGO'
      Required = True
      Size = 6
    end
    object QClientesNOMBRE: TStringField
      FieldName = 'NOMBRE'
      Required = True
      Size = 35
    end
    object QClientesRAZON_SOCIAL: TStringField
      FieldName = 'RAZON_SOCIAL'
      Size = 35
    end
    object QClientesDIRECCION_PARTICULAR: TStringField
      FieldName = 'DIRECCION_PARTICULAR'
      Size = 35
    end
    object QClientesDIRECCION_COMERCIAL: TStringField
      FieldName = 'DIRECCION_COMERCIAL'
      Size = 35
    end
    object QClientesTELEFONO_PARTICULAR: TStringField
      FieldName = 'TELEFONO_PARTICULAR'
      Size = 10
    end
    object QClientesTELEFONO_COMERCIAL_1: TStringField
      FieldName = 'TELEFONO_COMERCIAL_1'
      Size = 10
    end
    object QClientesTELEFONO_COMERCIAL_2: TStringField
      FieldName = 'TELEFONO_COMERCIAL_2'
      Size = 10
    end
    object QClientesTELEFONO_CELULAR: TStringField
      FieldName = 'TELEFONO_CELULAR'
      Size = 11
    end
    object QClientesID_COD_POSTAL: TIntegerField
      FieldName = 'ID_COD_POSTAL'
    end
    object QClientesCPOSTAL: TStringField
      FieldName = 'CPOSTAL'
      Size = 8
    end
    object QClientesLOCALIDAD: TStringField
      FieldName = 'LOCALIDAD'
      Size = 15
    end
    object QClientesCONDICION_IVA: TIntegerField
      FieldName = 'CONDICION_IVA'
      Required = True
    end
    object QClientesSUCURSAL: TIntegerField
      FieldName = 'SUCURSAL'
    end
    object QClientesACTIVO: TStringField
      FieldName = 'ACTIVO'
      FixedChar = True
      Size = 1
    end
  end
  object QCaja: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select estado from caja_cab where id_caja=:id ')
    Left = 168
    Top = 584
    ParamData = <
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QCajaESTADO: TIntegerField
      FieldName = 'ESTADO'
      Required = True
    end
  end
  object QCerrarCaja: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update caja_cab c set c.estado=1 where c.id_caja=:id_caja')
    Left = 408
    Top = 584
    ParamData = <
      item
        Name = 'id_caja'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QReAbrirCaja: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update caja_cab c set c.estado=0 where c.id_caja= :id_caja')
    Left = 536
    Top = 584
    ParamData = <
      item
        Name = 'id_caja'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object QCajaActualAbierta: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select c.id_caja from caja_cab c where c.estado = 0 and c.id_cue' +
        'nta_caja= :id_cta_caja')
    Left = 288
    Top = 584
    ParamData = <
      item
        Name = 'id_cta_caja'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QCajaActualAbiertaID_CAJA: TIntegerField
      FieldName = 'ID_CAJA'
      Required = True
    end
  end
  object QBuscaAplicaiones: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select m.* from movaplicaccvta m '
      'where m.aplica_id_cpbte=:id and '
      '           m.aplica_tipocpbte=:tipo ')
    Left = 760
    Top = 472
    ParamData = <
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'tipo'
        DataType = ftString
        ParamType = ptInput
      end>
    object QBuscaAplicaionesID_MOV: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object QBuscaAplicaionesID_MOVCCVTA: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object QBuscaAplicaionesFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object QBuscaAplicaionesCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object QBuscaAplicaionesID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object QBuscaAplicaionesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object QBuscaAplicaionesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object QBuscaAplicaionesNUMEROCPBTE: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object QBuscaAplicaionesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
    object QBuscaAplicaionesAPLICA_ID_CPBTE: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object QBuscaAplicaionesAPLICA_TIPOCPBTE: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object QBuscaAplicaionesAPLICA_CLASECPBTE: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object QBuscaAplicaionesAPLICA_NROCPBTE: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object QBuscaAplicaionesDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object QBuscaAplicaionesCONCILIADO: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object DSPBuscaAplicaciones: TDataSetProvider
    DataSet = QBuscaAplicaiones
    Left = 800
    Top = 456
  end
  object CDSBuscaAplicaciones: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'tipo'
        ParamType = ptInput
      end>
    ProviderName = 'DSPBuscaAplicaciones'
    Left = 864
    Top = 456
    object CDSBuscaAplicacionesID_MOV: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object CDSBuscaAplicacionesID_MOVCCVTA: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object CDSBuscaAplicacionesFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object CDSBuscaAplicacionesCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object CDSBuscaAplicacionesID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object CDSBuscaAplicacionesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSBuscaAplicacionesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object CDSBuscaAplicacionesNUMEROCPBTE: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object CDSBuscaAplicacionesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
    object CDSBuscaAplicacionesAPLICA_ID_CPBTE: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object CDSBuscaAplicacionesAPLICA_TIPOCPBTE: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object CDSBuscaAplicacionesAPLICA_CLASECPBTE: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object CDSBuscaAplicacionesAPLICA_NROCPBTE: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object CDSBuscaAplicacionesDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object CDSBuscaAplicacionesCONCILIADO: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object QActualizarMovCC: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      ' update movccvta m set m.saldo = m.saldo - :total'
      '   where m.id_movccvta = :id')
    Left = 712
    Top = 576
    ParamData = <
      item
        Name = 'total'
        DataType = ftFloat
        ParamType = ptInput
      end
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object IntegerField1: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object IntegerField2: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object SQLTimeStampField1: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object StringField1: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object IntegerField3: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object StringField2: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object StringField3: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object StringField4: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object FMTBCDField1: TFloatField
      FieldName = 'IMPORTE'
    end
    object IntegerField4: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object StringField5: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object StringField6: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object StringField7: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object StringField8: TStringField
      FieldName = 'DETALLE'
    end
    object StringField9: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object QAplicaciones: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select * from MovAplicaCCVta where Id_MovCCVta = :Id')
    Left = 728
    Top = 528
    ParamData = <
      item
        Name = 'Id'
        DataType = ftInteger
        ParamType = ptInput
      end>
    object QAplicacionesID_MOV: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object QAplicacionesID_MOVCCVTA: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object QAplicacionesFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object QAplicacionesCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object QAplicacionesID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object QAplicacionesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object QAplicacionesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object QAplicacionesNUMEROCPBTE: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object QAplicacionesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
    object QAplicacionesAPLICA_ID_CPBTE: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object QAplicacionesAPLICA_TIPOCPBTE: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object QAplicacionesAPLICA_CLASECPBTE: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object QAplicacionesAPLICA_NROCPBTE: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object QAplicacionesDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object QAplicacionesCONCILIADO: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object DSPAplicaciones: TDataSetProvider
    DataSet = QAplicaciones
    Options = [poRetainServerOrder]
    UpdateMode = upWhereKeyOnly
    Left = 808
    Top = 536
  end
  object CDSAplicaciones: TClientDataSet
    Aggregates = <>
    IndexFieldNames = 'ID_MOVCCVTA'
    MasterFields = 'ID_MOVCCVTA'
    Params = <
      item
        DataType = ftInteger
        Name = 'Id'
        ParamType = ptInput
      end>
    ProviderName = 'DSPAplicaciones'
    Left = 864
    Top = 536
    object CDSAplicacionesID_MOV: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object CDSAplicacionesID_MOVCCVTA: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object CDSAplicacionesFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object CDSAplicacionesCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object CDSAplicacionesID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object CDSAplicacionesTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSAplicacionesCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object CDSAplicacionesNUMEROCPBTE: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object CDSAplicacionesIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
    object CDSAplicacionesAPLICA_ID_CPBTE: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object CDSAplicacionesAPLICA_TIPOCPBTE: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object CDSAplicacionesAPLICA_CLASECPBTE: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object CDSAplicacionesAPLICA_NROCPBTE: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object CDSAplicacionesDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object CDSAplicacionesCONCILIADO: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object QActualizarMovACta: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'update movactaccvta m set m.importe = m.importe - :importe'
      '  where m.id_cpbte   = :id_cpbte and'
      '        m.tipocpbte  = :tipocpbte and'
      '        m.clasecpbte = :clasecpbte')
    Left = 840
    Top = 584
    ParamData = <
      item
        Name = 'importe'
        DataType = ftFloat
        ParamType = ptInput
      end
      item
        Name = 'id_cpbte'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'tipocpbte'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'clasecpbte'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object CDSAplicacionDeNC: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'id'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'tipo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'clase'
        ParamType = ptInput
      end>
    ProviderName = 'DSPAplicacionDeNC'
    Left = 656
    Top = 632
    object CDSAplicacionDeNCID_MOV: TIntegerField
      FieldName = 'ID_MOV'
      Required = True
    end
    object CDSAplicacionDeNCID_MOVCCVTA: TIntegerField
      FieldName = 'ID_MOVCCVTA'
      Required = True
    end
    object CDSAplicacionDeNCFECHA: TSQLTimeStampField
      FieldName = 'FECHA'
    end
    object CDSAplicacionDeNCCLIENTE: TStringField
      FieldName = 'CLIENTE'
      Required = True
      Size = 6
    end
    object CDSAplicacionDeNCID_CPBTE: TIntegerField
      FieldName = 'ID_CPBTE'
      Required = True
    end
    object CDSAplicacionDeNCTIPOCPBTE: TStringField
      FieldName = 'TIPOCPBTE'
      Required = True
      Size = 2
    end
    object CDSAplicacionDeNCCLASECPBTE: TStringField
      FieldName = 'CLASECPBTE'
      Size = 2
    end
    object CDSAplicacionDeNCNUMEROCPBTE: TStringField
      FieldName = 'NUMEROCPBTE'
      Required = True
      Size = 13
    end
    object CDSAplicacionDeNCIMPORTE: TFloatField
      FieldName = 'IMPORTE'
    end
    object CDSAplicacionDeNCAPLICA_ID_CPBTE: TIntegerField
      FieldName = 'APLICA_ID_CPBTE'
      Required = True
    end
    object CDSAplicacionDeNCAPLICA_TIPOCPBTE: TStringField
      FieldName = 'APLICA_TIPOCPBTE'
      Size = 2
    end
    object CDSAplicacionDeNCAPLICA_CLASECPBTE: TStringField
      FieldName = 'APLICA_CLASECPBTE'
      Size = 2
    end
    object CDSAplicacionDeNCAPLICA_NROCPBTE: TStringField
      FieldName = 'APLICA_NROCPBTE'
      Size = 13
    end
    object CDSAplicacionDeNCDETALLE: TStringField
      FieldName = 'DETALLE'
    end
    object CDSAplicacionDeNCCONCILIADO: TStringField
      FieldName = 'CONCILIADO'
      Required = True
      FixedChar = True
      Size = 1
    end
  end
  object DSPAplicacionDeNC: TDataSetProvider
    DataSet = QAplicacionDeNC
    Left = 744
    Top = 640
  end
  object QAplicacionDeNC: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'select * from movaplicaccvta where id_cpbte=:id and tipocpbte=:t' +
        'ipo and clasecpbte=:clase')
    Left = 840
    Top = 640
    ParamData = <
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'tipo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'clase'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QBorraAplicacionNC: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'delete from movaplicaccvta '
      'where id_cpbte=:id and tipocpbte=:tipo and clasecpbte=:clase')
    Left = 648
    Top = 688
    ParamData = <
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'tipo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'clase'
        DataType = ftString
        ParamType = ptInput
      end>
  end
  object QTipoComp: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      'select c.clase_comprob from comprobantes c'
      'where ((c.tipo_comprob = :tipo) or (:tipo = '#39'**'#39' ))  and'
      '      ((c.sucursal = :sucursal ) or ( :sucursal = -1 )) and'
      '      c.compra_venta='#39'V'#39
      '')
    Left = 680
    Top = 136
    ParamData = <
      item
        Name = 'tipo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'tipo'
        DataType = ftString
        ParamType = ptInput
      end
      item
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end
      item
        Name = 'sucursal'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object CDSTipoComp: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftString
        Name = 'tipo'
        ParamType = ptInput
      end
      item
        DataType = ftString
        Name = 'tipo'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end
      item
        DataType = ftInteger
        Name = 'sucursal'
        ParamType = ptInput
      end>
    ProviderName = 'DSPTipoComp'
    Left = 776
    Top = 136
    object CDSTipoCompCLASE_COMPROB: TStringField
      FieldName = 'CLASE_COMPROB'
      Required = True
      Size = 2
    end
  end
  object DSPTipoComp: TDataSetProvider
    DataSet = QTipoComp
    Options = []
    Left = 728
    Top = 136
  end
  object DSTipoComp: TDataSource
    DataSet = CDSTipoComp
    Left = 840
    Top = 144
  end
  object QActulizacionTotal: TFDQuery
    Connection = DMMain_FD.fdcGestion
    SQL.Strings = (
      
        'update fcvtacab f set f.total=:total, f.debe=:total where f.id_f' +
        'c=:id')
    Left = 904
    Top = 80
    ParamData = <
      item
        Name = 'total'
        DataType = ftFloat
        ParamType = ptInput
      end
      item
        Name = 'total'
        DataType = ftFloat
        ParamType = ptInput
      end
      item
        Name = 'id'
        DataType = ftInteger
        ParamType = ptInput
      end>
  end
  object PopupMenu1: TPopupMenu
    Left = 560
    Top = 112
    object ActualizaTotal1: TMenuItem
      Action = ActualizaTotal
    end
  end
end
